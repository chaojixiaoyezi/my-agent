...
def declaration() -> dict:
    return json.loads(files("workspace_peek").joinpath("declaration.json").read_text(encoding="utf-8"))
...
def fields(value: object, schema: dict) -> dict:
    ...
    if (not isinstance(value, dict) or set(value) - set(properties)
        ...
        raise ReadError("INVALID_ARGUMENTS", "参数缺失或包含未声明字段。")
    result = dict(value)
    for name, spec in properties.items():
        ...
        if spec["type"] == "integer":
            ...
            raise ReadError("INVALID_DECLARATION", "插件声明使用了未实现的字段类型。")
        if not valid:
            raise ReadError("INVALID_ARGUMENTS", "参数类型或取值范围无效。")
    return result
