...
#   握手同时声明读取与写入上下文扩展；宿主只给 requested_effect=mutating 的 export 附带写入上下文。
# 模块用途: 将同源工具声明和表格渲染/导出业务接到原 MCP 传输，输入错误不影响后续正常请求。
...
from .operations import export_html, render_table
from .tables import GenuiError


# LLM: 对象只持有静态声明与已验证设置；不缓存调用权限，也不拥有宿主安装或激活状态。
# 类用途: 处理标准 MCP 初始化、工具目录及逐次渲染/导出调用。
class GenuiServer:
    ...
    def handle(self, request: object) -> dict | None:
        ...
        if method == "initialize":
            ...
            result = {"protocolVersion": "2024-11-05", "serverInfo": {
                "name": "genui-lite", "version": version("my-agent-genui-lite")},
    ...
    # LLM: 权限只从 _meta 取得，arguments 不能冒充上下文；每次调用新建上下文。export 缺写入上下文直接失败，不降级。
    # 函数用途: 验证参数和上下文，再执行对应动作，所有失败都转成中文错误结果。
    def call(self, params: dict) -> dict:
        try:
            name = params.get("name")
            if not isinstance(name, str) or name not in self.tools:
                raise GenuiError("UNKNOWN_TOOL", "工具不存在。")
            ...
            if not isinstance(metadata, dict) or WORKSPACE_READ_EXTENSION not in metadata:
                raise GenuiError("MISSING_CONTEXT", "缺少宿主逐次工作区读取上下文。")
            if name == "export" and WORKSPACE_WRITE_EXTENSION not in metadata:
                raise GenuiError("MISSING_WRITE_CONTEXT", "缺少宿主工作区写入上下文，不能导出文件。")
            context = WorkspaceReadContext.from_payload(metadata[WORKSPACE_READ_EXTENSION])
            if name == "table":
                value = render_table(context, arguments["path"], arguments.get("chart"), self.settings)
            else:
                write = WorkspaceWriteContext.from_payload(metadata[WORKSPACE_WRITE_EXTENSION])
                value = export_html(context, write, arguments, self.settings)
            return self.result(value, False)
        except GenuiError as exc:
            ...
            return self.result({"code": "IO_FAILED", "message": "文件读写失败，请稍后重试。"}, True)
        except (ValueError, TypeError):
            return self.result({"code": "INVALID_CONTEXT", "message": "宿主工作区上下文无效。"}, True)

    # LLM: 结果使用原 MCP 文本结构，正文为结构化 JSON（保留中文，外层协议行仍按 ASCII 转义输出）。
    # 函数用途: 编码正常或错误的工具返回值。
    @staticmethod
