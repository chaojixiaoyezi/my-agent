# LLM: 声明是命令、工具及默认配置的唯一来源；这里只支持本包实际使用的平面 string/integer 字段，不实现另一套宿主 schema 引擎。
...
from .snapshots import SavepointError


# LLM: 仅加载本包固定资源，不读 cwd、用户路径或宿主模块；返回调用方私有对象。
# 函数用途: 取得与安装清单构建同源的静态声明。
def declaration() -> dict:
    return json.loads(files("savepoint_lite").joinpath("declaration.json").read_text(encoding="utf-8"))
...
def fields(value: object, schema: dict) -> dict:
    ...
    if (not isinstance(value, dict) or set(value) - set(properties)
        ...
        raise SavepointError("INVALID_ARGUMENTS", "参数缺失或包含未声明字段。")
    result = dict(value)
    for name, spec in properties.items():
        ...
        if spec["type"] == "integer":
            ...
            raise SavepointError("INVALID_DECLARATION", "插件声明使用了未实现的字段类型。")
        if not valid:
            raise SavepointError("INVALID_ARGUMENTS", f"参数 {name} 的类型或取值范围无效。")
    return result
