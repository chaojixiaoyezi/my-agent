...
- 对照 会话运行时 `base_instructions/default.md` 的“不要计划无法执行的测试”、针对性验证和无法运行时提供复核步骤，
  当前不恢复机器验收，也不写 HTTP/防火墙专项分支。`PromptBuilder` 在 System 后统一注入一次通用边界：
  本机、替代环境、模拟器、局部入口或单一身份只证明自身；不能观察目标边界时必须写未验证、已验证范围和
  剩余具体复核步骤。
- 该段不读用户正文、不改变权限、状态或完成裁决；`system_prompt_override` 也不能绕过，因此主代理、普通
  child、专注 child 和 Compact 后续模型回合共用一个事实源。Prompt/子代理相关 203 项收集为 196 passed、
  7 个既有 xfail，语法和本地严格 gate 也通过；待推送、单 Gateway 部署后，用同一条普通中文“另一台电脑
  到底能不能用”复测。
