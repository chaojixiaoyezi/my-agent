...
OpenAI 工具准备进度和协调职责已实现；Read 脱敏/显示正在合入。本机尚为 R229d，部署后再记实际请求，不提前宣称通过。

## R229 现场紧急故障（分批部署，旧会话故障仍在复现）

已改前台 attach 重复展示、非工具轮 reasoning 回放缺失、请求拒绝误报配置错误，463 相关定向通过。
原 canonical 消息只记一次，不按文本去重；保留真实思考，不伪造丢失历史。
超长重复、Read 投影、F6 历史、长等待及 Todo/自主行为仍开放，见 [逐项台账](docs/audits/R229_LIVE_FAILURES.md)。
`.10` OpenCode Go 两 child 完成、工作中插话及主汇总通过；本机原会话 R229a/b 仍 HTTP 400，继续精确诊断。
追加修复 Ctrl+A 全选入口、超大复制及 typed 错误被文本误判重试；原 TUI 166215 字节 native/tmux 一致。
用户授权本机更新并复用其原 TUI，会话/记忆/模型配置保留，当前入口 runtime-r229d，单 Gateway。
原会话新请求仍拒绝，真实界面与终态已显示 HTTP 400/provider 来源；物理右键、复读原因与旧路由恢复未关闭。
不把新会话通过算成旧会话恢复，不把真实任务编译错误等同工具失败；本地严格 gate 已通过，提交/推送以 Git 记录为准。

## R228 收尾提交批次（工程修复已验，开放问题保留）

以 main/510d9f15 为基线继续 R227 误判修复；当前状态优先于下方历史记录。
Context 同代持久显示、400 分类、诊断角色与验证指导、分段摘要执行指令冲突已修，原长会话已成功
压缩到 generation 3 并续跑；后续补修用户原话被长汇报挤掉的原文锚点预算。
发布材料补齐 bubblewrap 同版本源码/许可、同名源码内容一致性及越界验包守卫；开发指南 638 → 334 行。
最终 35 文件联合 539 passed，无 skip/xfail；全仓收集 15967 项但没有全部执行。全部本地严格 gate 通过，
最终独立 wheel 已安装本机和 `.10`，两台各一个 Gateway；真实 `.10` 三 child 审查和手动 compact 分项通过。
模型仍错误归因沙箱、工具运行时 400 上游拒绝原因、Todo 原现场代次复核未关闭。
按用户“收尾”要求纳入正式 my-agent/main 修复提交；不改版本号、不创建 Release，不将开放问题标成已修。
提交/推送结果以 Git 记录为准；下面历史段落的“未提交”只描述当时状态。
全面检查是全仓静态/导入/语法/测试收集/链接/包边界，加相关功能测试，不代表全部 1.5 万项已执行。
具体已修、未修、影响与真实请求证据见 [R228 发布候选台账](docs/audits/R228_RELEASE_CANDIDATE.md)。
...
- 已确认origin为正式chaojixiaoyezi/my-agent，远端main为63422888；与当前实现没有共同祖先。
  当前没有合流或远端写入，不force push、不改另一份检出；后续须保留双方提交历史。
- 最近待提交代码/测试累计增删13583行，按规则补跑全量。377项定向通过，完整Python3.12环境全量
  15765项：15667通过、37失败、61跳过/预期失败，耗时657.824秒；不是新增真实TUI验收。
- 失败包含旧task目录/路径改写断言、3项结构守卫，以及消息恢复/IM/scale等待定位样本；不等于37个线上故障。
  Ruff、doc-sync、strict尺寸hard0、diff、clean-package及wheel边界通过，但额外import-boundary检查失败。
- 严格gate未通过，故尚未推送、未创建Release或改版本号。完整分类见TUI问题账本最新“正式库提交gate”。
  下一步先明确修复范围再推送；当前用户请求是代码提交，不借机扩展到IM/Audit重构。
...
老客户端通过显式能力协商保留原行为；后台续片、冷 owner、稳定 message ID 及模型历史边界继续保留。
131 focused 与严格 gate 已通过，未跑全仓；随后同 session 两页真 TUI 复验；实时思考/工具流仍未接通，不能关闭 BUG-146。
...
- BUG-147：共享 main 漏读审批事件，另一个窗口误显示 Working；现按精确 permission ID 维护等待显示，
  最后一个审批回执后恢复，等待时的上下文仍更新。观察页显示静态“等待审批”，不创建权限 overlay。
- 两个产品红测场景复现并修复，Gateway main/TUI renderer/会话活动共 87 focused 通过。
- 原 Go 长任务及最后两名 child 已结束，但模型报告仍列未实现功能，不标完整复刻通过。
- BUG-146 完整前台过程跨窗口、原 P0/P1 及跨模型 child 真验仍开放。未部署，未推远端。
...
- BUG-149 本地已修：当前选择由配置成功回执进入 typed 显示状态；冷启动/恢复读取已保存选择，欢迎卡不再覆盖成旧默认。
- 117 focused 通过，含实际 prompt_toolkit 菜单与宽窄冻结渲染；尚待新版真实 TUI 验证。
- 用户要求保留主 A / 子 B；缺省继承、创建前显式选当前 owner 的已保存配置，入口尚在实现。
...
- 唯一 Gateway 3526903，独立非 editable R211 环境；新增 my-agent 包与旧环境分离，未改旧运行包。
  依赖只硬链接复用，避免近满磁盘重复安装；完整回滚记录位于 deploy/r211-f373b2b。
- ma-r211-110-models / owner p1-r205-truncation 恢复原家庭工具会话，完成菜单/Auth/非法窗口/保存/选择，
  密钥不进入聊天，菜单本身没有模型调用。普通续作已 8 次 MiniMax-M2.7 成功、0失败。
- 主窗口 45.3k/96k、compact 1，两名新 child 工作中。业务交付仍待验，不把此项当原 P0/P1 全部通过。
- 新增先红配置组合用例后补修：旧 task overlay 不能拆开覆盖模型字段，profile ID 保留至孙代理；
  57 focused 通过，尚未部署。当前部署的 /model 不宣称已覆盖全部递归/恢复组合。
- 两次部署校验问题已保留证据：lib64 是 lib 符号链接，改为解析后比较；首次客户端误用 Gateway 配置
  被 owner 会话检查拒绝，保留原客户端配置后正确恢复，没有新建空会话或修改权限门。
...
- BUG-143：轮询失败仅退避，旧 Working 会假装仍实时。新增客户端 typed 刷新健康，主/子页面与展开模式
  都提示正在重试，后台主行显示静态“状态未同步 / 上次”。恢复读取即清除，不判任务失败或停止工作。
- 健康事件不推进模型活动计时，不清空历史、排队输入或计划；沿原 0.5/1/2/4/8 秒退避，无新增请求。
- 166 focused 通过，尚未部署。Go 同步父/子采样均 99.1k / compact 9；新测试结果 5/22 通过，仍在返工，
  不是完全无执行，也不能算完整交付。数字差异尚未同步复现，保留观察项。
...
- 本地/Gateway TUI 共用 typed max-tokens 提示；Gateway ok=True 不再掩盖长度限制，实际半截正文保留。
- 主代理 final/延迟补交、后台 final 和 child 共用 typed turn-end；历史提示使用原 message ID 单独显示，
  不进正文/模型上下文。后台完整快照标记不再依赖最后一条事件位置；child 的实时流也保留长度说明。
- 3 个历史定位用例先红；11 个相关文件 431 例中 429 passed / 2 原有 xfailed。有界同轮恢复和真实 TUI 验收仍未完成，
  不关闭 BUG-140；没有增加模型调用、提高 token 上限或恢复普通自动续跑。
- R204 唯一 Gateway 已运行，PID 3305553 / 8420；原 K/L 精确 session 升级客户端 3306994/3306997，
  已经通过普通 TUI 分别继续 Go 复刻和玛丽。F/E/I 也继续局域网、月末账单和资料整理；五路新用量均有
  MiniMax-M2.7 记录。证据 deploy/r204-a3420b0/five-tui-progress-1788830987.json。
- K 原 worker 仍运行，错目标修复还不能据此关闭；L 已 final，但没有按本轮要求创建协调员/子代理。
  F 8504 本机外部探测仍为 HTTP 000；它把服务器内 curl 当外部可达的报告没有验收效力，LAN 未通过。
- 回收零引用 R194 约 651 MB，原 I 精确会话已用 R203 恢复；R194/R201 有包和依赖备份，用户项目未删。
...
- R203 客户端已在原 F/E 恢复并用 MiniMax 继续任务；F 恢复时 Todo 显示“待继续”且静止，完整忙闲组合待验。
  原 R202 / 3271274 已停止，当前 R204 / 3305553；此次无活跃工具切换是部署，不算新的 P0 故障恢复通过。
...
- 部署提交 `510849e`，独立 R198 环境；wheel SHA
  `9f0985f2ad7b1ba02abf37b2bdcba85835aa8960345097dbf0b98cce294c689e`。已撤除 R197 的 8895 定向断流夹具，恢复真实上游直连。
- `ma-r198-110-handoff` 真实玛丽后续排查请求 `gwreq-1788793511-f9900e78f3fc4925bcf1a58143014c39`：
  1788793853.937 停止 PID 3166408，1788793859.698 启动唯一 Gateway PID 3199680；旧执行代
  `attempt-1788793513-c220578b` recovered，同 agent/run 新执行代 `attempt-1788793894-3788374b` 继续工具并 done。
  请求 attempts=2 / processing_failure_count=0；观察到最终回复和 Working 收起，所有副作用去重仍逐项核对。
- 玛丽原外部访问失败，经原 TUI 普通追问由被测模型自行处理，随后本机 HTTP 200；浏览器工具两次超时，
  只确认 LAN 连通，不把它当全部游戏玩法通过。调研 K 的 8 child 已完成，主代理给出报告并列出多个未验证缺口。
- 部署首次因重写 YAML 改变空值格式导致启动失败，改为保留原文件格式的字段替换后通过；这段管理操作时间不算产品启动基准。
  已清理无进程引用的 R193 环境约 712 MiB、pip/dnf 下载缓存约 354 MiB，可重装恢复；项目和证据未删。
...
- 318 focused 通过、2 项原有 xfailed；没有全仓 pytest。主代理恢复仅上述切片通过，完整递归矩阵未通过，旧 A/G 失败记录保留。
...
- 真实 C 修改画像的不存在条目、A 子代理无 goal 的更新均被误判为未知副作用，整轮停止。按 会话运行时 目标工具、
  长期助手 记忆替换的前置拒绝语义，handler 明确返回 not_started/validation；真实写入中断仍保持 UNKNOWN。
- 补登记 Persona CAS 错误码，未扩大重放白名单、权限或记忆写入范围。99 focused 通过，原 TUI 复验待做。
- 另发现 Gateway 在第二次重启后把活跃请求判成“900 秒超时”；恢复计数与进程代次混用，正在单独修复。

## R191 动态状态按来源增量追加（已部署，真实样本通过，公平费用对比未封板）

- PromptBuilder 按真实字段提供来源分段；未变化状态留在原 IR，状态返回旧值仍按顺序追加。删除全局 ever-seen
  集合，避免回滚后误跳过状态；不改已有普通消息前缀。Compact 保留每类最新状态和全部真实用户输入。
- 预检查与模型发送共用无副作用的 prompt/IR 投影，动态包不重复估算，也不污染稳定表面指纹。
- 260 focused 通过；来源隔离、整包重复、A→B→A、渲染一致、预检查不写历史、两代摘要与取消/CAS 回滚覆盖。
  `.10` 单 Gateway PID 2958237，独立 R191 wheel；真实请求未变记忆段不再重复注入。
- 同提示词新资料整理任务主代理 Compact 0 后 final；完整批量续作自动到 4 代后 final。第一次实际只跑 13 份
  E2E，因此虽然输入账显著下降，不能把费用差全归因本修复或宣称质量相同；详细数据见 FT-181～184。

## R190 旧运行状态快照回收（已部署，原 main/child 真实压缩后继续）

- BUG-120：普通工具轮追加的旧 RuntimeFactsTurn 没有随摘要覆盖的工具前缀退休，最终把 native Compact
  最低水位抬高，进入频繁 archive Compact。原 B 主到 68 代才自然 final；A child 已到 23 代仍运行。
- 对照 会话运行时 历史替换，整对回收现在只在完整摘要存在时同时删除连续退休区的旧快照；最新快照、真实用户
  输入、保留工具尾部和 carried handoff 不变。普通请求不裁旧前缀，原始工具/副作用账本不动。
- 141 focused 通过：先复现 5 个旧实现失败，修后覆盖两代长历史、非连续删除、并行工具、摘要输入完整性、
  取消/CAS 回滚及正常请求稳定性。唯一 Gateway PID 2561860、独立 R190 wheel 已验证；原 child 第 96 代
  119,356→48,731 后自然 DONE。B main 第 84 代 120,587→54,405，仍有频繁重复状态包，继续 R191。
  功能回收通过不等于频率、价格或任务质量通过；完整 TUI 证据与保留问题见 FT-176～180。
...
- 98 项定向验证通过，含 9 次成功压缩后结束、工具显示 ID 不碰撞、无进展仍失败、父级失败记录保留、
  Compact 中断回滚和直属子级生命周期。Ruff、doc sync、strict size、diff、clean-package 通过。
- BUG-116 保持待真 TUI 超过旧上限复验；测试中另见后台相对路径落点异常，正在读精确工具执行位置，
  未以模型解释直接判定，也不把删除次数上限当作高频 Compact 成本问题已修好。
...
写入/读取主链，`message_stream` 从唯一消息库按完整行游标分页；历史快照与实时显示交接同一 message_id。
恢复期间新增回复、发布失败、半行和损坏游标有 363 focused / 2 原有 xfailed 覆盖。`.10` 唯一 Gateway
PID 2270581，wheel `ad3325e4575de961faa624cf696c5019734d16f12ed9c9d638d151bb3c623285`。B 真实多子代理
续作已产生 canonical final，正文自动显示且 Working 收起；退出再恢复时，旧过程环又从 event_after=0 重放，
追加到最终汇报之后，因而完整恢复验收失败。不能把“最终正文不重复”当成“整段历史排序已修好”。
BUG-111 继续收口完整过程/恢复快照/实时缓冲；BUG-115 已查到第 5/6 代摘要保留误改事实，错误首先由正式
汇报产生，第 7 代再继承，不是简单丢失要求。A 原长任务仍在运行，未因 B 完成而停止或代写产物。
...
- r38 超级玛丽任务的产物在测试机本机 `0.0.0.0:8765` 返回 200，但 `.7` firewalld 未开放该端口，从开发机
  ...
  “局域网能用、无需改防火墙”，证明问题不是用户要求不清，而是模型缺少稳定的证据边界纪律。
- 首版 `57e91cb` 已部署 `.7` 唯一 Gateway。错误 cwd 的 r39 不算产品证据；从原任务目录启动的 fresh
  `ma-evidence-r40-correct-cwd` 显示 MiniMax-M2.7 并真实调用。模型先正确想到“不能从另一台机器验证”，
  但看到 `0.0.0.0 + localhost 200` 后仍写出“现在能用/局域网可访问”，最终又说需要外部电脑复核，正文
  自相矛盾；它还在只要求核对时擅自重启服务。因此首版抽象提示不算通过。
- 第二候选仍不恢复机器验收，也不解析用户或模型正文。唯一 PromptBuilder 规则改在完整 Prompt 最末重新
  提醒，并加入跨边界通用反例：本机监听、`0.0.0.0`、localhost/本机地址都不能证明另一机器可达；没有目标
  观察点结果必须明确写“未验证”，不能写“能用/应该可用”。同时按 会话运行时 请求授权边界补齐：用户只要求
...
- 按键层不解析 `N new messages` 文案或未读计数；普通/详情视图经唯一 `is_following()` 读取当前视口事实。
  6 个相关 TUI 文件 145 项与本地严格 gate 通过；`814cc3b` 已推送、通过增量 Git bundle 快进部署 `.7`，
  唯一 Gateway 为 `ma-gateway-814cc3b`，PID `1366460`，启动约 2 秒且队列为空。
- fresh `ma-scroll-r38-resume` 在 3.5 秒内恢复 `sess_1787741717_0852dc61` 的完整复杂历史并显示
  `MiniMax-M2.7 · API`。主视图 `Ctrl+Home` 后第一次 `↓` 清掉 `Jump to bottom` 且不移动 child，高亮
  第二次 `↓` 才选择 `EngineDev-1`；进入终态 child 后，同样以 `Ctrl+Home → ↓` 回到自己的最终回复，未
  切换 main 或污染另一视口。随后真实中文 follow-up 立即入屏并进入流式 Thinking，证明新 Gateway 真调用。
- 同场超级玛丽任务本身只能判部分成功：13 个 JS 模块语法和 JSDOM 14/14 独立复测通过，7,456 行产物及
  `0.0.0.0:8765` 本机 200 均存在；但 `.7` firewalld public 区只开放 ssh/mdns，本机从局域网地址连接被拒。
  模型仍把本机 200 推成“局域网能用、无需改防火墙”，这是下一条通用证据边界问题，不能算交付完成。
...
- 根因是 `_native_provider_messages()` 把 native 首轮空历史 `[]` 压成 `None`，backend 随即按普通 text
  请求跳过缓存。`df95d27` 保留结构化区别：`None` 只代表 text，native 首轮明确传 `[]`，从第一次
...
- 9 个直接相关测试文件 193 项、本地严格 gate 全部通过；`df95d27` 已推送并快进部署 `.7`，旧任务安全
  终态后只重启唯一 Gateway。`/status` 返回 running，模型为 MiniMax-M2.7，进程精确为 1。
- fresh TUI `ma-cache-firstturn-r34` 的首个真实请求包含 3 次物理模型调用：7,875 accounted input、
  25,999 cache-creation、12,313 cache-read。随后同 thread 的两个普通后续任务又分别得到
  46,047/37,234 与 21,878/46,972 cache-creation/cache-read，证明首轮建缓存和跨回合复用均已进入
  Agent 主链。canonical `compact_generation=0`；Context 69.1k→46.3k 是终态工具折叠，不是漏记 Compact。
