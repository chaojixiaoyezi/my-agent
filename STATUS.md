...
- `system_prompt_override` 不能绕过，因此主代理、普通 child、专注 child 和 Compact 后续回合共用一个事实
  源。203 项相关回归为 196 passed、7 个既有 xfail；语法、全项目 Ruff、doc-sync、strict code-size、diff
  和 clean-package 全部通过。本轮远低于 10,000 行，未跑全仓 pytest；待部署后再次真 TUI。
