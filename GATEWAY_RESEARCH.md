...
通道运行时 把 gateway 定义为 24/7 后台服务，负责通道连接、消息路由、定时任务和工具执行，并提供 start/stop/restart/status/logs。长期助手 的 gateway 是长运行进程，用 PID 文件做 profile 级跟踪，还处理 cron、session expiry、memory flush、模型/provider 状态刷新。

优点：
- 和我们的最终用户体验最接近：TUI/外部渠道只是客户端，gateway 是本体。
- 适合把聊天、定时任务、后台维护、工具执行放到一个控制面。
- 长期助手 的“每条消息创建 agent + session id + memory flush”给多用户/多会话隔离提供了参考。
...
参考：
- 通道运行时 gateway: （外部资料链接已移出发布文档）
- 通道运行时 runbook: （外部资料链接已移出发布文档）
- 长期助手 gateway internals: （外部资料链接已移出发布文档）
- 长期助手 messaging gateway: （外部资料链接已移出发布文档）

### 3. Detached UI + persistent kernel/session

代表：JupyterLab/Jupyter Server、模型助手 Code session。

Jupyter 的浏览器页面可以关闭，底层 kernel/terminal 仍在 server 上继续跑；Jupyter Server 通过 Kernel Manager 管理 kernel 生命周期。模型助手 Code / Agent SDK 持久化 conversation history，可以 resume/continue/fork，但官方也强调 session 持久化的是 conversation，不是文件系统状态。
...
- 模型助手 Code sessions: （外部资料链接已移出发布文档）
...
代表：OpenAI 会话运行时 云端任务。

会话运行时 把每个 coding task 放在独立云 sandbox，并行执行，完成后提供 terminal log、测试输出和可审查结果。它不是本地 gateway，但它把“任务隔离、并行、证据链”做成了一等公民。
