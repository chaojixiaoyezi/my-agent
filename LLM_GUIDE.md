...
transcript Compact 的触发和候选接受已复用普通请求的已知输出预留；恢复目标不能绕过容量门，未知cap不补猜。完整宿主恢复输入仍未接入旧会话估算，不能据此宣称12.4/12.5整项已验。
原 PromptRenderInput 现保留不可变注入片段，合并文本仅由片段派生；宿主候选须按准备时已知位置替换，不得搜索正文猜来源。此片保持原模型输入与缓存布局，完整 Compact 恢复接线仍待做。
