...
第 7.7 的当前默认双机路径验收已核对，新增 7.8 正在定位：TUI212 原生已有完整 completed final、会话任务已 completed，但能力请求唤醒分支抑制公开最终回复；BLOCKED 孩子的 AgentRun／TaskRun 保留开放是现行合同，不表示仍在执行，也不改成 done。默认目录墙外的测试工作区属于准备失误，不能掩盖随后发现的完成交付问题。第 7 步仍未收口，暂不进入第 8 步；详见唯一 TODO 和 TESTS。

第 7.7 步前台普通退出清理源码 `9330ee385` 已随 `ed20fd061` 推送远端 main；215 项定向与本地严格 gate 通过。同一 wheel 双机各 1,274 文件核对一致，默认入口和唯一 Gateway 已切换，原生 TUI210／213 已核对本机回收与直接非零退出，211 未满足场景覆盖，212／214 继续复验；未完成验收，不勾第 7 步。线上 CI 无本次提交的运行记录，未作为验收来源。详见 TESTS。

当前第 7 步原生递归、通知发布恢复、明确孩子失败交接及多子代理长任务已分别留证，业务交付缺陷保留。新发现前台普通退出的后代清理缺口，按唯一 TODO 7.7 修复；原生 `/stop` 未清理不能改报通过，人工准确资源清理与产品验收分账。修复前不进入第 8 步，详见 STATUS／TESTS。
...
第 7 步首组实际验收已读回：TUI195／196 基础父子交接、四条完成通知消费及产物正确；197 任务完成但原页面超时，198 原生重连能显示历史和最终结果。主机停顿不算有效长任务，完整递归／取消／恢复矩阵仍待完成，详见 TESTS。

当前十步 Goal 的第 1—6 步框架必测项已收口，第 7 步子代理结果链前四片已从独立工作树应用到原 checkout，组合定向回归、严格 gate 与 wheel 边界检查通过，已推送远端 main，测试机默认入口与唯一 Gateway 已切换，TUI195—197 已发起但验收未收口，本机切换和完整真实 TUI 矩阵仍待完成；按[唯一 TODO](docs/tasks/REFACTOR_PLUGIN_GOAL.md#当前-todo唯一执行清单) 继续。第 5 步源码 `689e83e85` 已推送 main、同一 wheel 双机部署；第 6 步沿同一运行包完成插件故障与长任务的多 TUI 验收，未改产品代码。原生 TUI192 的 600 页插件原返回全部正确，但模型最终文件有 306 页金额错误，框架与内容结论须分开读 TESTS。并行的 TUI／历史修复由官方 MiniMax-M2.7 原生 TUI 验过阅读与插话边界，并已随本轮快进推送远端 main，已随组合包切换测试机默认 Gateway，本机尚未切换；旧 TUI164 的表情和行尾失真及其它报告错误仍保留给后续通用交付核验。

以下首段是前版开发阶段记录，当前状态以上述段落为准。

当时 4.6d 三项修复已本地实施、待发布与真实 TUI 复验：MCP 完整失败回执结算、显式连接释放纳入原执行区间、重复启用不构造非法空资源声明。查询保留收尾提示；完整失败不证明副作用回滚，未知资源门和旧 UNKNOWN 保持。
