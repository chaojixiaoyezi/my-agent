...
当前十步 Goal 的第 4 步框架必测项已收口，5.1／5.2 的本地代码和定向验证、5.3a 严格发布 gate 已完成，正在做 5.3b 发布部署；按[唯一 TODO](docs/tasks/REFACTOR_PLUGIN_GOAL.md#当前-todo唯一执行清单) 的工作队列推进。三项既有修复的 `fb3857e46` 已推送 main、同一 wheel 部署双机，旧版 TUI163—167 的原生证据见 TESTS；本片新源码未部署。164 的插件工具链成功、模型产物表情和行尾失真，交付失败保留并归第 8 步通用核验；第 5—10 步尚未完成。

以下首段是前版开发阶段记录，当前状态以上述段落为准。

当前 4.6d 三项修复已本地实施、待发布与真实 TUI 复验：MCP 完整失败回执结算、显式连接释放纳入原执行区间、重复启用不构造非法空资源声明。查询保留收尾提示；完整失败不证明副作用回滚，未知资源门和旧 UNKNOWN 保持。
...
当前 4.6 已细分为七项可验收交付，4.6b／4.6c 启动观察修复已发布部署并由新版 TUI 复验，当前做 4.6d 剩余装卸用例；按实际依赖并行测试，不重复审阅或无依据重跑全仓。
