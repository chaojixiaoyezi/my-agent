...
后台供应退避切片已验；唤醒/Goal 路由已分离并同版部署，353 项相关回归及 TUI 120—123 所测框架链路通过。长 Goal 暂停续采/Compact、并行普通任务、三子代理及定时投递已验，模型交付失败与未覆盖分路单列；租约/恢复源码已迁移，324 项回归和独立复核通过，新版实际验收待做。插件详细合同读 [可装卸插件方案](docs/design/PLUGIN_LIFECYCLE.md)。
...
拟议入口统一为 `/plugins` 管理与 `/plugins@插件ID` 调用；首 token 的 `@` 需公共解析和动态补全支持，尚未实施，不能沿旧 slash 正则静默进入模型。
...
后台执行 claim 的领取、最终准入与结算现读 `background_claim.py`，每次权威恢复检查读 `background_recovery.py`；runtime 绑定原状态查询、来源退休和失败记账能力。共享心跳仍在 `run_claim.py`，三种租约不合并，源码片尚待安装版真实验收。
这些计划尚未实施，不替代已完成四批的验收记录；现有启动插件不代表已经支持 TUI 热装卸。
