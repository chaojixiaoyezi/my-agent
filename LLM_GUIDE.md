...
当前十步 Goal 的第 4 步框架必测项已收口，下一交付为第 5 步 `5.1` 只读摸底：三项修复的 `fb3857e46` 已推送 main、同一 wheel 部署双机，新版 TUI163—167 的原生证据见 TESTS 与[唯一 TODO](docs/tasks/REFACTOR_PLUGIN_GOAL.md#当前-todo唯一执行清单)。164 的插件工具链成功、模型产物表情和行尾失真，交付失败保留并归第 8 步通用核验；第 5—10 步仍待执行。
