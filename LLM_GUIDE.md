...
宿主目录与提交版本已接源码：`plugin_command_catalog.v1` 只是只读声明，原 owner 解析仍唯一；无冷用户初始化。
...
插件入口统一为 `/plugins` 管理与 `/plugins@插件ID` 调用；公共 `command_catalog.py` 已统一名称、别名和核心尾部语法，参数声明驱动绑定、帮助与补全。帮助及错误在公共入口结束，不进入模型或旧停止分支；动态 owner 目录、版本绑定和装卸尚未实现，发布及实际验收看 STATUS。
...
第 3 步公共命令、参数次片与资源停止修复已发布部署，实际分项复验见 TESTS；接着核对宿主目录及身份接线，第 4—10 步待做。现有启动插件不代表已经支持 TUI 热装卸。
