...
- `agent_py_agent/agent/subagents/manager_parent_acceptance.py`: 新增 manager 父级验收桥接函数，把 plan/write/apply/next-action 流程从 `manager_acceptance.py` 类体拆出，保持 manager facade 轻量。
...
- `agent_py_agent/cli/_acceptance_plan.py`: 新增 `subagents-acceptance-plan` 命令渲染；默认只展示父级 dry-run 决策和 refs，`--write` 写入 `parent_acceptance_decision.json`，`--apply` 只允许 inspect_only 进入普通验收 apply，`--next-action` 只展示上级动作建议；不执行 tests、不读取 artifact 正文。
...
- `agent_py_agent/cli/subcommands_agents.py`: 注册 `subagents-tests <run_id> [--re-run] [--timeout]` 和 `subagents-acceptance-plan <run_id> [--json] [--write] [--apply] [--next-action]`，并给 `subagents-acceptance` 增加 `--execute-tests` / `--no-execute-tests` / `--test-timeout`。
...
- `agent_py_agent/tests/test_parent_acceptance_controller.py`: 覆盖父级验收 dry-run 决策、refs-only 审计落盘、显式 apply 边界和 next-action 建议，包括缺少真实测试报告时建议执行、危险命令要求人工确认、已有通过报告时只需 inspect，以及非 inspect_only apply 不改任务状态。
...
- `agent_py_agent/tests/test_subagents_tests_command.py`: 覆盖 `subagents-tests` 查看已有报告、显式 `--re-run` 写回报告、`subagents-acceptance-plan` 展示/写入/显式 apply/next-action 父级决策，以及 `subagents-acceptance` 从配置读取真实执行默认值并被 CLI 覆盖。
