...
|   |-- commands/                             # 命令行分组入口和小型 command helpers；含 background-main-agent 注册入口
...
|   |-- background_main_agent.py              # 长期主代理本地 message/bind-task/observe/tick/service 命令实现
|   |-- collaboration.py                      # 通用协作 case/request 的 overview/list/status/update 可视化和状态推进命令实现
...
|   |-- collaboration/                        # 通用多代理协作控制面：case/request 生命周期、开放世界线索包、多目标 responder 逐个闭环、list_collaboration_requests 待办发现、internal thread 兜底、结构化换路、request timeout、evidence、decision、overview 和 rework 账本工具
|   |-- concurrency/                          # 乐观锁、任务锁和透明重试
|   |-- contracts/                            # 主代理合同、离线矩阵、产物验收、runtime gate 和恢复 replay 合同
|   |-- conversation/                         # 长期主代理 thread、消息、渠道绑定、observation/wake queue、上下文预算裁剪、后台运行 claim/heartbeat、定时汇报和后台唤醒 runtime
...
|   |-- subagents/                            # 子代理 manager facade、服务、验收、patch、runner、persistence；runner context 负责读/写边界分离
|   |   |-- capability_request_identity.py      # 能力申请 scope 签名和去重
|   |   |-- root_task_policy.py                 # 区分 self-authorized root seed 和主代理直接创建的一层 worker
...
|   |       |-- hierarchy_qa_ready_refs.py        # QA 波次输入 refs 收集，给 tester/acceptor 明确 ready worker 来源
...
|       |-- main-agent-runtime-gates.md        # 主代理运行门、验收触发、次数门配置和 bootstrap 硬门删除记录
...
|   |   |   |-- orchestration_quality_advice_payload.py # schedule/dispatch 共享 QA advice refs-only JSON 渲染
...
它负责承载会长期扩展的设计细节，例如模块背景、痛点、schema、配置开关、分阶段开发计划和验收策略。当前已有 subagent 质量契约设计、log analysis 模块设计和主代理运行门文档，后续 memory、gateway 等模块如果设计内容继续膨胀，也应按同样方式拆出独立文档。
...
- `agent_py_agent/agent/agent_core/runtime/capabilities.py`: infers runtime capabilities from normal user prompts, currently focused on explicit or obvious `logs/security` tasks.
...
- `agent_py_agent/agent/agent_core/runner_dispatch.py`: runner 候选选择和重试策略；候选只按真实可执行状态、通道、能力请求和重试预算过滤，不再按角色阶段或 workflow 依赖隐藏挡住 runner。需要“先 A 后 B”时，由父代理显式先派 A、看 tree/board/closeout 结果，再派 B；dispatch record 仍会保留 runner 创建的 child 状态摘要、未完成 child ids 和 partial-success 标记。
...
- `agent_py_agent/agent/contracts/tool_protocol_v2.py`: 工具调用/结果协议 v2 归一化层；兼容旧 flat tool call，并保持工具 input 字段开放世界，避免把业务参数 `status` 等字段误吞成协议状态枚举。
...
- `agent_py_agent/agent/agent_core/tool_context/reducer.py`: 大工具输出进入下一轮 live prompt 前只注入 artifact 摘要和 checkpoint refs，小输出仍保留原工具结果；调度类输出会交给 orchestration summary 只保留 next_action/run refs。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/refs.py`: 汇总 dispatch 触达 run 的 artifact/evidence refs，并生成 `result_refs_by_run`；它会从 child `output.json.artifacts[]` 提取主产物路径和短摘要，父级先读 summary/refs 再按需读正文。
- `agent_py_agent/agent/agent_core/orchestration/scope_resolution.py`: 编排身份裁决 helper；当显式 run/root/parent/sender/source/requester 参数和当前 runner 上下文冲突时，给 tree/dispatch、消息、协作、证据和能力申请输出 `scope_resolution` 和 `scope_warnings`，避免线程级 current 静默覆盖真实身份或模型冒充其它 run。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/state_contract.py`: 当前轮状态合同层；create/schedule/dispatch 共用，仅读取 remembered run ids，输出 `current_turn_run_state` 的 status buckets、dispatchable/awaiting_acceptance/running/blocked/verified ids 和下一步建议；待验收 run 的建议会显式开启 `execute_acceptance_tests` 和 `auto_apply_acceptance_followup`，避免 root 读大 records、重复调度或把待验收 run 当未知状态。
- `agent_py_agent/agent/agent_core/orchestration/context/live_summary.py`: externalized dispatch/schedule/read_artifact/case_status 等控制面输出的 live-prompt 摘要层，保留状态、协作 request/evidence/ready/rework 计数、建议工具调用、`result_refs_by_run`、artifact summaries 和 refs，不默认诱导父级读 artifact 正文。
...
- `agent_py_agent/agent/agent_core/tool_context/recovery_summary.py`: 渲染 recovery strategy 的 bounded preview，让 packet/checkpoint/takeover 批次进入 live prompt 时不展开完整恢复正文。
...
- `agent_py_agent/agent/agent_core/orchestration_tools.py`: 顶层 `create_subagents` 响应会附带同一 `subagent_schedule` typed envelope；顶层和多层派工走同一 refs 形状。顶层批量派工支持 长期助手 风格 `items/tasks`，不同子任务拥有独立 goal；系统默认名会补成 `小傻妞-角色-编号`，并在响应里给出下一步 `dispatch_subagents` 的真实 run_id 和 `current_turn_run_state`；typed envelope 会在状态合同生成后写入，避免恢复层丢掉 dispatchable refs；没有可调度 run 时会建议 `subagent_board`，避免空 dispatch。`raise_observation` / `raise_main_event` 让子/孙代理写入持久 observation 和 wake signal，主代理后台 tick 再判断是否分析、调度或汇报。
- `agent_py_agent/agent/agent_core/orchestration/create_items.py`: 解析 `create_subagents` 的 `items/tasks` 批量入口；继承顶层默认字段，让每个 child item 独立覆盖 goal/role/name/验收/写入边界；顶层全局 plan 不自动复制到每个 child。
- `agent_py_agent/agent/agent_core/orchestration/create_policy.py`: create_subagents role 纠偏层；只把本地 goal/name/thought/plan 的明确派工语义用于 worker->coordinator 纠偏，`allowed_tools` 只当能力授权，不再把普通 items worker 误升成 coordinator。
...
- `agent_py_agent/agent/agent_core/orchestration/create_idempotency.py`: create_subagents 幂等合同层；明确命名 child 按 parent/root/name/role 复用，默认名和 `小傻妞-角色-编号` 这类系统名按 parent/root/role/goal/owner/写入根精确合同复用；`count>1` sibling 会生成稳定序号名，重复 create 不再扩容。
- `agent_py_agent/agent/subagents/services/repair_contract_identity.py`: repair/execute/verify 同 run 身份键 helper；从 repair context pack 里提取 `subagent_repair_contract.v1` 的 kind、失败 run、目标产物和必读 refs，让 create/schedule 幂等层区分不同修复范围并复用同一修复 owner。
...
- `agent_py_agent/agent/agent_core/orchestration/dispatch/payload.py`: 承接 runner-context `dispatch_subagents` 工具返回 payload 的单条 record 构造，输出 test/follow-up refs 和摘要，不展开正文；错 run_id 时顶层 recovery 会给 `valid_run_ids`。
...
- `agent_py_agent/agent/agent_core/orchestration_board_payload.py`: 承接 `subagent_board` 的状态桶、完成状态、当前轮作用域和 kernel snapshot payload；看板只附加 refs-first 内核快照，不读取业务产物正文；状态工具只暴露 tree/status/refs，不再生成父级验收专项推进建议。
...
- `agent_py_agent/agent/subagents/parsing.py`: 旧 `[SUBAGENT_RESULT]` / planner 兼容解析入口；`parse_subagent_result_envelope()` 只保留旧导入路径，实际 typed 转换委托给 `parsing_envelope.py`；顶层 `evidence_refs/artifact_refs` 会补成 refs-only evidence packet，避免调查类 worker 的证据在 parser 到验收之间丢失。
...
- `agent_py_agent/agent/subagents/services/takeover_run.py`: 原 runner 挂死后的幂等接管 run 创建服务；新 run 保留旧 task_dir/artifacts/checkpoint/packet refs，并继承 `quality_contract`、`context_manifest`、`context_packs`、output refs、artifact/evidence refs 和写入根；复用旧 takeover 时会补齐缺失交接字段，同一个 source run 重复恢复不会无限创建接管者。
...
- `agent_py_agent/agent/tooling/registry_invoke.py`: 工具执行前的参数准备、写边界校验和临时工作根注入层；当父级明确授权外部 product/artifact 或 read roots 时，同一工具调用内会把这些目录加入文件工具 read/list/search/write 根，调用结束后恢复原工具状态。
...
- `agent_py_agent/agent/subagents/execution_executor.py`: 新增最小 `TestExecutor`，执行 command/file/content/static_site 四类检查并产出 `TestExecutionRecord`；`file_exists/path_exists/artifact_exists` 作为存在性别名归一到 `file_check`，当前不接 acceptance 自动写回。
...
- `agent_py_agent/agent/subagents/static_required_files.py`: 从 task goal/thought/description/acceptance_checks 里的结构化 `required_files` 和内部必需文件标签提取 `index.html`、`style.css`、`app.js` 等静态 Web 必需文件名；只从结构化 `required_dom_ids` 提取必需 DOM ids，不从普通正文猜测业务区块，不读取产物正文。
- `agent_py_agent/agent/subagents/required_content_lines.py`: 从 task goal/thought/description/acceptance_checks 里的结构化 `required_content_lines` / `required_content_texts` 和 per-file 内容合同提取父级普通文件 `content_check`；只读机器字段和字段续行，不读取产物正文，也不从普通句子里猜内容要求。
- `agent_py_agent/agent/subagents/required_file_terms.py`: 层级 handoff、context bundle 和静态站验收共用的文件契约提取器；只读取 `required_files`、`forbidden_files` 和内部精确标签字段，bullet 或纯文件续行也必须挂在这些字段下面。普通“必须生成/不要创建/禁止改名”等自然语言不再作为机器文件合同来源。
...
- `agent_py_agent/agent/subagents/parent_acceptance_empty_report.py`: 拆出可执行 test 筛选和空 `test_execution.json` 的 inspect-only 判定，只检查 evidence/artifact refs 元数据，不展开正文；非内部 evidence ref 可让无可执行测试的调查/查询任务进入 inspect-only，不再误报 `total=0 failed=0` 为失败。
...
- `agent_py_agent/agent/subagents/services/hierarchy_scheduler.py`: 新增层级调度器 v1，使用 `HierarchyScheduleRequest` / `HierarchyChildSpec` 显式预览或创建 child/grandchild run，并统一限制深度和 fan-out；当真实模型把 `role=child`、具体角色写进 `agent_name` 时，会先恢复 researcher/tester/acceptor/bug_finder/writer/worker 等角色再套权限策略；模型漏传 `acceptance_checks` 时会通过 `hierarchy_acceptance.py` 从 child goal/角色派生最小验收项；父任务明确点名 tester/bug_finder/acceptor 时，会返回 `quality_advice` 提示缺失角色和红线，但不替 LLM 固定创建 QA child；`apply=True` 时会先走调度幂等合同，同合同 direct child 已存在就复用。
- `agent_py_agent/agent/subagents/services/hierarchy_schedule_idempotency.py`: runner 内 schedule_child_subagents 幂等合同层；按 parent/root/role/goal/extra_write_roots/status 匹配当前 direct children，返回新建、复用和可 dispatch 的 run ids，避免重复派工无限扩容。
- `agent_py_agent/agent/subagents/services/hierarchy_agent_names.py`: 层级展示名统一生成器，负责 `小傻妞-*` / `小小傻妞-*` / `小小小傻妞-*` 前缀；真实模型只写 `小小傻妞` 这种无后缀名字或把 `*` 占位符原样传入时，会回退到 role 后缀并补 sibling 编号，例如 `小小傻妞-tester-2`；已有语义后缀如 `小小傻妞-product-worker` 会保留，避免把辨识信息吞掉。
- `agent_py_agent/agent/subagents/services/hierarchy_child_context.py`: runner 内层级 child context 选择 helper；child spec 携带 repair/context refs 时优先写入 child task，否则沿用父级 context。
- `agent_py_agent/agent/subagents/services/base.py`: 子代理基础创建服务；不再从目标自然语言里提取本地写入根。额外写入根必须来自 `extra_write_roots`、workspace/product root 或结构化 refs，避免图片/API 地址被误当成可写目录；创建完成后会登记开放世界 `AgentCapability` 快照，让协作请求可以按结构化能力路由到已有子代理。
...
- `agent_py_agent/agent/subagents/services/hierarchy_tool_policy.py`: 从 scheduler 拆出的工具策略，统一处理 coordinator/leaf 的工具继承、写文件工具补齐和 `write`/`read` 等模型工具名别名修正；coordinator 显式 allowed_tools 会补回内置编排工具和 `raise_observation` / `raise_main_event`，避免模型漏传后失去派工或上报主代理能力。
- `agent_py_agent/agent/subagents/execution_test_items.py`: 预处理父级验收 tests，推断 workspace 内 `working_dir`，拆安全 `cd <dir> && pytest`，并把带明确期望内容的 `cat <workspace文件>` 改成受控 `content_check`，避免为真实模型输出放开 `cat` 命令；多页 HTML artifacts 会自动补 `static_site_check`，普通文件可从 `required_content_lines` 自动补逐行 `content_check`。
...
- `agent_py_agent/agent/agent_core/_tool_loop_service.py`: 主代理和 subagent 共用的工具循环；到达 `max_tool_rounds` 后给模型一次收口机会，如果模型仍吐工具调用，返回确定性停止说明而不是把新 `[TOOL_CALL]` 当最终回答；执行真实工具前会检查 per-run 工具预算，预算触发时只拦截当前 run 的工具并给模型自检/上报提示；工具归档后会刷新父级小型读取 brief 缓存，供后续 `create_subagents` 共享给子代理。
- 已删除旧读正文硬门：`orchestration_body_read_guard.py`、`orchestration_predelegation_read_guard.py`、`orchestration_direct_write_guard.py`、`tool_body_read_guard_stage.py`、`tool_direct_write_guard_stage.py` 不再存在。父代理、子代理是否读取正文或写产物由 prompt、工具权限、路径边界和统一 closeout 验收控制，不再用“委托期/派工前/直接写”额外卡点阻断普通协作流程。
- `agent_py_agent/agent/agent_core/orchestration_tools.py`: 顶层 `create_subagents` / `subagent_board` / `inspect_agent_tree` / `raise_observation` / `raise_main_event` 工具入口；显式 root/coordinator seed 只继承结构化 required/forbidden 文件合同、refs、context packs 和写入根，不再从当前原始用户 prompt 的普通句子里补机器事实。`create_subagents` 同时支持 `goal + count` 单任务兼容模式和 `items/tasks` 多任务结构化模式；创建子代理前会追加父级最近小型读取 brief 到 `context_packs`，让协作子代理共享父级已经读到的短线索；系统默认名会补成 `小傻妞-角色-编号`，例如 `小傻妞-worker-1` / `小傻妞-tester-2`；默认创建后会立即启动新 run，只有显式 `defer_start=true` 才只建任务记录；`dispatch_subagents` 保留为运行中提示注入、人工催办、推进卡住项、重跑指定 run、查状态并尝试恢复的工具；`raise_*` 只上报事件和叫醒主代理，不直接调度或验收。
- `agent_py_agent/agent/agent_core/orchestration/shared_context.py`: 父级共享 brief helper；从本次成功工具记录、`archive_tool_calls` 或缓存里提取最近小型成功读取结果，生成 bounded `parent_recent_read` context pack，只传摘要、来源路径和工具 ref，不复制大正文，也不作为验收门。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/refs.py`: dispatch/subagent board 的 child 结果索引 helper；把 child `summary/output_json/artifact_refs` 放到父级最前面，避免长 `records` 截断后父级漏掉已产出的发现。
- `agent_py_agent/agent/agent_core/orchestration/sibling_roster.py`: 同批子代理 roster helper；批量创建后给每个 child 附加同批 peers 的 run_id/name/role/目标摘要，供 coordinator 点名协作和结果核对。
...
- `agent_py_agent/agent/agent_core/tool_agent_budget.py` / `tool_agent_budget_stage.py`: 单个代理滚动工具预算 helper 和工具循环集成层；默认按 `run_id` 读取 `runtime_guard_config.yaml` 里的滚动窗口和次数限制，当前默认是 600 秒 / 200 次；没有 `run_id` 的主代理普通聊天不受限，且不做任务树或单次对话的全局预算。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/progress_payload.py`: runner-context `dispatch_subagents` 的直接 child 进度摘要；只暴露继续调度、恢复、QA/产物建议和 refs-first 状态，不创建父级验收状态，也不替父级改写完成结论。
- `agent_py_agent/agent/agent_core/tool_loop/round_execution.py`: 单轮工具执行 helper；负责记录 assistant tool round、执行/记录工具调用、检测子代理 `output.json` 收口，并把同轮 `schedule_child_subagents` 后依赖真实 run id 的 `dispatch_subagents` 等编排调用延后到下一轮，避免模型使用脑补 run id；`output.json` 收口检测支持 flat `path` 和 bundle `filesystem.path`。
...
- `agent_py_agent/agent/subagents/services/hierarchy/facade.py`: 新增 `SubAgentManager.schedule_child_runs(params=...)` facade，保持层级创建只走 bundle 入口和既有 `create_run` 持久化路径。
...
- `agent_py_agent/agent/subagents/root_task_policy.py`: 判断 subagent run 是否是 self-authorized root/coordinator/lead seed；普通一层 worker/researcher/writer 即使没有 subagent parent_id，也按有主代理上级处理，保留 capability request 通道。
...
- `agent_py_agent/agent/agent_core/tool_context/call_reducer.py`: 大段 assistant tool-call 参数摘要层，避免 `write_file(content=<large html>)` 原文反复进入下一轮 prompt；保留工具名、路径、字段大小、hash 和短预览。小 dict payload 也渲染为摘要行，避免模型把历史 dict 复制成新工具调用。
...
- `agent_py_agent/agent/agent_core/spawn_role_seed.py`: CLI 显式 role seed 入口；root/coordinator seed 不再从 goal 自然语言抽产品路径，自身 allowed write roots 只保留 task-local 协调目录和调用方显式传入的结构化根。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/progress_payload.py`: runner-context dispatch 的直接 child 进度摘要；含状态计数、unfinished ids、recovery ids、`needs_more_dispatch` / `needs_recovery` 和带 `run_ids` 的建议继续调度、修复或恢复工具调用；不再输出父级验收专项 repair advice。
...
- `agent_py_agent/agent/agent_core/orchestration/dispatch/tool.py`: `dispatch_subagents` 模型工具类；把模型参数收敛成 `DispatchParams`，返回 refs-first 调度报告、typed dispatch envelope、当前轮状态合同和错误 run id 恢复提示。真实模型常见的 `orchestration:{run_ids, concurrency, mode}` wrapper 会在入口归一成顶层 `run_ids/max_runners/apply/execute_runners/workflow_mode`，避免已创建子代理因参数包形态不同而一直停在 PLANNING。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/scope.py`: 集中维护 dispatch_subagents 的 apply/execute 默认、parent scope、self-exclude 和 workflow-off 策略；顶层 active root/coordinator 在 `apply=true` 时强制 workflow off，避免全局 auto workflow 先生成 producer/critic/repair 子工单并绕过 root 自己派工；父级验收专项收口已移除，最终质量回到统一 closeout。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/scope.py`: 显式 `run_ids/include_run_ids/subagent_ids/target_subagent_ids/target_run_ids/agent_ids/child_run_ids/children/items[].run_id` 会归一为同一批目标；`direct_children=true` 是作用域意图，不会被当成 `"True"` run_id；顶层给出显式目标 ID 时省略 `apply/execute_runners` 会默认真实推进 runner，显式 `apply=false` 才 dry-run；真实 dispatch 若漏写 `max_runners`，默认按目标 ID 数量推进，避免模型点名多个孩子却只跑第一个。
...
- `agent_py_agent/agent/agent_core/orchestration/create_constraints.py`: create/schedule 写入根和任务约束推导层；保留安全/边界类约束，例如委托约束冲突和缺写入根提示；不再因为多个子代理共享输出路径或 count 复制具体文件目标而在 create 阶段硬拒绝。
- `agent_py_agent/agent/agent_core/orchestration/create_target_roots.py`: create/schedule 的目标写入根解析层；会从 context/read/repair target refs 推导产品写入根，并把目标文件路径归一为父目录。若同一个结构化任务包里的输出 ref 与真实输入 ref 共享窄任务目录，会把输出父目录授权给 child，防止临时任务目录被误判为仓库外非法写入。
- `agent_py_agent/agent/agent_core/orchestration/dispatch/payload.py`: dispatch 工具返回 payload 压缩层；单条 record 只保留 refs 和关键字段，错 run_id 时把 `runner_selection_recovery.valid_run_ids` 放到顶层；顶层父级验收 REJECT 也会返回 `parent_acceptance_repair_advice` 和 `create_subagents` 修复建议，并把原始 task refs / goal / acceptance checks 传给 repair child，避免修复目标缩水。
...
- `agent_py_agent/agent/agent_core/dispatch_runner_candidates.py`: runner 候选策略层；显式 run_ids、状态、scope、packet/checkpoint 恢复候选都在这里统一判断。它不再用输入文件依赖或 workflow sibling 依赖静默过滤候选。
- `agent_py_agent/agent/agent_core/dispatch_runner_batches.py`: runner 候选收集和执行批处理；调用 selection 精确推进父节点给定的直接孩子。缺文件、缺资料、缺权限现在交给真实 runner 工具调用和父级 tree/board 反馈处理，不在启动前提前卡死。
- 已删除旧 sibling workflow 推断层：`orchestration_item_dependencies.py` 和 `runner_workflow_dependencies.py` 不再存在。真要流水线，由父代理按“派 A、看 A 完成、再派 B”的方式显式控制；批量 items 不再自动把 `dependencies` / `input_refs` 转成等待关系。
...
- `agent_py_agent/agent/subagents/services/acceptance_findings.py`: 普通验收 finding 汇总层；已有 `reports/test_execution.json` 时优先以机器执行报告判断 tests_passed；pending capability finding 防止半成品被验收；`required_child_spawned` 会在目标要求创建下级时核对真实 `task.child_ids`，`descendant_health` 会阻断仍有未完成/失败后代的父级验收。
- `agent_py_agent/agent/subagents/services/acceptance_product_findings.py`: 业务产物验收 finding；只核对 product write roots 下的 required files，避免 agent-run 内部报告冒充用户交付物。
- `agent_py_agent/agent/subagents/services/acceptance_patch_findings.py`: patch 协议验收 finding；检查 planned/blocked、非法 status 和 applied patch 审核状态。
...
- `agent_py_agent/agent/settings/config.py`: 新增 `acceptance_execute_tests`、`acceptance_test_timeout_seconds`、`subagent_allowed_tools`、`subagent_role_template_dirs` 和 `tool_artifact_read_budget_*`，真实测试执行默认关闭，子代理工具默认自动判断，artifact 正文读取预算默认 10 分钟 240000 字符。工具轮、runner 重试和单代理工具预算的默认值集中到 `agent_py_agent/config/runtime_guard_config.yaml`，避免 `AgentConfig` 和运行门 YAML 双写同一数字。
- `agent_py_agent/agent/settings/runtime_guard_config.py`: 运行门配置低层 reader；`tooling` 和 `agent_core` 都从这里读 `runtime_guard_config.yaml`，避免循环导入和隐藏默认值。
...
- `agent_py_agent/tests/test_subagent_required_content_contracts.py`: 覆盖普通文件结构化内容合同提取、字段续行/fenced block、per-file 内容映射，以及由内容合同推断 `content_check`。
...
- `agent_py_agent/tests/test_tool_loop/round_execution.py`: 覆盖单轮工具执行 guard，确保 `schedule_child_subagents` 后的同轮 `dispatch_subagents` 会延后到下一轮读取真实 run ids。
...
- `agent_py_agent/tests/test_orchestration/write_guard.py`: 覆盖派工写入预检，确保 UI 文案、HTML 标签和图片 URL 不会被误判为外部绝对路径，并验证疑似工作区路径拼写错误会返回可重试的 `suggested_target`。
