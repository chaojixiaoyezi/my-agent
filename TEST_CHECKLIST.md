...
- [ ] 原 D 精确 resume 后读到首轮输入，主/子切换再回看、中文选取/复制；用量账和 Compact 零变化。
- [ ] F 全部执行代 done 但仍等待协调员的父层唤醒问题，独立查因，不归到历史展示修复。
...
- [ ] 新版真实 TUI 长项目超时、排查、继续与单 Gateway 恢复闭环；旧 UNKNOWN 保留，不把旧截图算新修复失败或通过。
- [ ] 独立核对 B 的 IncompleteRead 与 provider 退避，不把本次 shell 修复当成全部网络错误修复。
- [ ] 长历史 backward paging：已定位默认只读最近 80 条，没有更早游标；对照 终端交互 已读，尚未修复。
...
- [ ] 240 秒 shell 超时的进程终止证明、部分输出、未知结果与安全继续边界；不先修改旧 UNKNOWN 账本。
