...
- [x] 原 handler 退出与取消分离，坏 executor 字段不授权环境删除。
- [x] 原资源完整证明、固定环境安全删除、同表 release CAS 与重新启用。
- [x] 旧快照及原 disable 重送不能影响新代；保留原 enable UNKNOWN。
- [x] 原结果提交失败/损坏不消费，部分删除失败重送，不重跑 handler；查询只读。
- [x] 固定引用身份/版本/完整证明、同 ID 换代拒绝，普通资源不受影响。
- [x] session v4 准备记录保留，普通历史裁剪不变，旧 v2/v3 原版本更新。
- [x] 最终固定源码定向回归：50 文件、1,043 项通过，源码无漂移。
- [x] 本地严格 gate 收口，尺寸基线不变，线上 CI 未作为验收来源。
- [ ] 卸载、显式业务命令、发布部署及实际多 TUI 完整装卸。


## 实际启用与新运行工具组合（第 4 步本地开发）

- [x] 实际本地包、独立环境、完整目录验证和候选退出后发布；原请求重放不重复启动。
- [x] 原执行器实际调用、停用后旧快照拒绝、同 owner 不同权限视图工具同步。
- [x] 坏目录、启动失败、权限和开关拒绝，私有设置不进入公开结果。
- [x] 实际 scoped Agent 可信 owner 注入、构造无插件进程；迟到登记与永久关闭交错。
- [x] 完整 session 清理证明持久保留，原终态不变；UNKNOWN 保留与原 redo 恢复分开验证。
- [x] 未发送拒绝与发送后未知在原工具账区分，不按错误文案猜结果。
- [x] 释放/消费与重新启用的组件接线，详见首段。
- [ ] 卸载、显式插件业务命令及完整实际多 TUI。

## 管理停用与双归属清理（第 4 步本地开发）

- [x] 原管理权限/目录版本/原请求重放；空激活停用也核对安装版本。
- [x] 准备中和已发布、原 handler 在途和已结束，均关闭原准备权限并清理两类精确资源。
- [x] 业务调用阻塞时管理停用独立完成，另一插件和普通任务保持运行。
- [x] 独立 host 在实际准入前后与 revoke/freeze 交错：未准入拒绝，已准入的新 child 纳入精确清理。
- [x] 坏原请求链、资源声明失配与未知清理保留，终态准备资源明确纳入。
- [x] 资源证明后的释放、原结果落盘后的记录消费和重新启用组件。
- [ ] remove 和真实多 TUI。

## 托管 MCP 与鲜活准入（第 4 步本地开发）

- [x] 临时真实 MCP 的字节握手、Unicode/大帧、独立 stderr 及原 host/child 回执。
- [x] 准备阶段只发现，发布后同代调用；旧队列撤销、旧代理不追随重连及单次拒绝隔离。
- [x] 原目录锁的线程/进程排队可取消，原清理提交异常和未接管启动 UNKNOWN 保留。
- [x] 原 executor 的未发送/UNKNOWN 归档与 28 文件 681 项最终联测，本地严格 gate 通过。
- [x] 实际 enable、静态目录匹配与新运行贡献已做组件验证，见本页首节。
- [ ] 撤销后资源释放与卸载、实际多 TUI。

## 托管 stdio 与激活资源（第 4 步本地开发）

- [x] 原管道大字节、stderr 分离、EOF、自然退出、启动失败与 launcher 消失的组件验证。
- [x] v3 激活归属、旧 v2 原版本恢复、任务停止隔离和共享退出证据保留。
- [ ] 完整启用/MCP 准入、清理后收口、卸载及实际多 TUI 验收。

## 唯一安装表的激活权威（第 4 步本地开发）

- [x] 同代准备/发布/撤销、原回执重放、旧快照拒绝、配置与新代准备隔离。
- [x] 线程发布/撤销竞争、独立进程预留 CAS、撤销不等待环境准备的 quota。
- [x] 显式 v1/v2→v3 迁移保留来源，坏表拒绝及各阶段提交异常分类。
- [ ] 激活与托管资源串通、完整启停/卸载和实际多 TUI 验收；内部 stdio 与归属验证见上。

## MCP 连接生命周期（第 4 步本地开发）

- [x] 固定连接队列、旧线程、初始化中关闭及发布前关闭的确定性交错。
- [x] 暂时断连恢复、未知清理禁止重连、未回收组长的同组子进程和管道退出。
- [x] 原注册/工具链、共享权限视图及关闭打断发现的定向回归。
- [ ] 持久插件激活与完整装卸接通后，实际多 TUI 验收；组件不代替产品验收。

## 插件环境的原操作与进程归属（第 4 步本地开发）

- [x] 原 claim 先持久化完整计划；不同 owner/attempt/holder/代数/epoch/锁均拒绝新准备。
- [x] 原执行器内完成三个真实临时环境命令，查询/重送不重启，安装表不因此启用。
- [x] 托管交接后取消、启动方崩溃、宿主期限、丢失进程账和未知退出；普通后台回归。
- [ ] 完整启停调用链及实际多 TUI 验收，本片组件不代替产品验收。

## 插件私有配置（第 4 步本地开发）

- [x] 配置走原管理员/路径权限/工具执行链，完整替换前严格验证；值不进入公开目录、参数或工具回包。
- [x] 安装表 v2 原子保存配置、版本和回执；v1 查询不写，首次变更记录迁移来源；坏账不补默认值。
- [x] 原请求重送、来源消失、并发 CAS、目录过期、UNKNOWN 不重跑及 HTTP/direct 原入口开发验证。
- [ ] 启用、实际调用、精确撤销与卸载接通后，多实际 TUI 验收；本片未发布部署。

## 资源身份与本地中断首片

- [x] 访问身份与执行归属独立，访问回退不补齐任务，全部停止维度精确匹配；原 PTY 预留与恢复轮隔离回归保持。
- [x] direct/local `/interrupt` 只中断精确回合，保留插话和独立资源入口；从正式入口中断已运行的同步 Shell，回执为 `CANCELLED`。
- [x] 10 个相关文件 302 项源码定向通过；新增两项中断测试在旧实现失败，原 `/stop` 分派回归保持。
- [ ] 本片发布部署与新增实际多 TUI 验收；普通后台持久归属、启动/停止竞态和明确交接仍待实现，不能关闭 TUI 137。

## 长时间运行增量

- [x] 长等待可取消且不持注册表锁；届满返回运行中，不杀进程。
- [x] 完成通知去重、入队后崩溃补发、陈旧回写、停止不重启、owner 重发现。
- [x] 出站摘要不存正文/凭据，不同线程不比较；追加、缩短、改前缀与部分比较区分。
- [x] 自然结束后通知经真实 scheduler/claim/模型/持久回复；回执发布失败期间保留事件，不伪造处理成功。
- [x] 新包正常 TUI：90 秒后台采样在首轮回复结束后自动唤醒、读取日志、发最终统计；没有补发催促。
- [x] 多正常 TUI：长后台与前台并行、子代理插话、连续 Goal、Esc 续接、手动/自动 Compact；三个产物独立 18/21/51 项测试通过。
- [x] 旧 child phase 抑制收尾回放与未读信封保护；新版双子代理 TUI 最终回复可见，主任务确为 completed。
- [x] 精确编辑冲突返回状态类代码、未写入事实、有界当前片段和明确读取参数（定向）。
- [x] 一路慢模型无子代理：长预填充及慢流持续超过一小时，输入导航和中断可用，缓存诊断持久值可读。
- [ ] 慢模型任务自然完成与循环恢复未通过：两段长跑分别陷入编辑冲突循环、同一 Shell 观测 24 次；失败现场保留。
- [ ] 真实账号 OAuth、IM 投递、跨主机缓存争用、所有断网/磁盘故障组合仍未由本轮覆盖。

本增量 951 passed、2 项既有 xfail。下方历史轮次保留原证据边界，不与此数累加，也不替代当前开放问题。

## 客户端计时增量

- [x] 系统时间前跳不提前结束等待；回拨不延长真实无活动等待，租约续期仍有效。
- [x] 正常模型真实 TUI：客户端一小时校时注入后完成七轮任务并收到终态。
- [ ] 慢模型重复等待受管进程：显示耗时变化不是业务进展，目前仍未关闭。

## 成功复读与后台模型争用增量

- [x] 400 次成功调用持续提醒；完整输出摘要区分归档引用变化与真实内容变化。
- [x] 同端点前台期间 Curator busy 且 pending/游标不变；释放后继续，pre_compact 不互等。
- [x] 后台预算进入 HTTP 且不污染其它线程；超时取消连接，旧线程未退出不叠加请求。
- [x] 本机新版官网模型 TUI 单文件动画真实落盘、自然最终回复、无连续重复；不代替视觉质量验收。
- [ ] 新版慢模型单路单代理动画已写入文件；测试机 SSH 中断，最终结果暂未核对。
  未触发循环的正常任务不当作循环恢复通过。

## 账号认证与用量增量

- [x] 成功/失败/取消统一用量结算，前后台 source 去重与累计容器重建换代的定向回归。
- [x] 真实 TUI 正常模型中断后追加，已报告输入/输出与账本一致；两孩子 Goal 插话、后台续接与 Compact 分项对账。
- [x] OAuth owner 隔离、秘密不回显、刷新串行、取消/退出迟到回调、参数编辑、重定向拒绝、禁止跨用户共享（定向）。
- [x] 新认证入口在真实 TUI 新增/保存/取消/编辑/选择/退出；回环协议替身短调用和自动刷新完成，普通任务因缺少工具能力被拒绝。
- [ ] 认证启动失败清理、退出后模型可用性刷新补修的新版真实 TUI 复验；不能把旧客户端的结果当新版通过。
- [ ] ChatGPT 实际账号在官方网页授权后短调用，不以替身冒充 Plus/Pro 账号验收。
- [x] 每模型排队预算配置与首事件估算（定向）；默认 0，不改变流静默与输出预算。
- [x] 慢模型超过九分钟预填充后继续流式调用，工具参数按秒更新；单路、不派子代理，期间输入被消费。
- [ ] 慢模型长任务完整产物仍未通过：重复覆盖未增长；上游 grammar 400 仍开放。缓存 0% 的服务端全量预填充与 2 GiB 状态缓存上限已记录。

## 六路真实 TUI 增量验收

- [x] 四路官方正常模型、两路本地慢模型并行使用一个 Gateway；每路独立 owner、会话和配置，凭据未写入仓库。
- [x] 六路输入空格、全角空白及 Tab 不出现补全越界；正常路径补全仍有定向回归。
- [x] Goal 草稿保存、放弃、版本冲突保留；明确暂停后问答仍保持暂停，恢复后完成，TaskRun 与 attempt 正常终态。
- [x] 新发现 Goal task/request 混淆已修；原五孩子现场恢复以及同会话第二个五孩子目标自动收口。
- [x] 普通追问保留消息 ID、绑定真实 run/attempt；工具权威门通过，错任务和旧 attempt 仍拒绝，Compact 再入凭据不漂移（定向）。
- [x] 上述追问修补的新包原会话恢复、Read/Shell/修改及 14 项测试通过；另一会话等待两名孩子时追问，真实命令执行成功且孩子继续运行。
- [x] 追问后两名孩子自然完成，主代理自动接回、Compact 后继续整合；Goal/attempt/TaskRun 完成状态一致，没有追加“继续”推动收尾。
- [x] 日志工具连续追加功能，自动 Compact 后手动 Compact 代次再加一；后续能复述原需求并实际重跑测试。
- [x] 子代理视角看到原始派工、工具和思考；完整原文有 21 页、退出后主历史保留；输入在运行中 child 页面进入历史。
- [x] 慢流约 45 分钟仍更新工具参数进度；输入草稿、翻页及中断可操作。中断后的未确认插话如实标记，不自动重发。
- [ ] 慢模型端到端产物未通过：生成文件缺少脚本及闭合标签，超长命令校验失败，后续同一成功 Shell 重复 9 次。
- [ ] 慢模型 Goal 未通过：Messages/Chat 两种接口均有 grammar complexity HTTP 400；单槽竞争另有首事件超时。
- [ ] 用量未通过：暂停路径漏结算，以及同 request/run 的前后台重复入账；已记录原始账，不以显示值或模型估算代替。
- [ ] 真实鼠标拖选到本机剪贴板、强杀/断电、小时级全组合、IM 和孙级控制未由本轮六路替代。
- [x] 第二轮仓库子代理收到插话、跨 3 次 Compact 后继续；两名孩子约 68 分钟未收尾，显式暂停后部署补修，主代理恢复整合并运行 14 项测试通过。暂停不计孩子自然完成。

## 空白输入与 Goal 中断

- [x] 空串、空格、Tab、换行、Unicode 空白、光标前空白和引用后空格不越界；显式路径补全继续可用。
- [x] 本机独立 TUI 连续空格与 Tab 无异常；旧已启动窗口仍需重新打开，不把磁盘热修当作旧进程已更新。
- [x] 真实 TUI：Esc 后无新增消息自动续接，task 不换；明确暂停后普通问答保持暂停，显式恢复最终 complete。
- [x] 后台普通输入的 running claim、结束/暂停/错任务/过期目标、稳定消息去重与原输入回执定向覆盖。
- [x] TaskRun 合法换代重开、保留旧关闭事件、再次正确关闭；准入拒绝不修改总账（定向）。
- [x] 补修包真实后台插话：模型 consumed、用户历史、公开回答均核对；未再误报后台回合已结束。
- [x] 主 Goal 编辑放弃/保存、编辑中 Esc 不保存并自动续跑；显式暂停后普通问答不恢复目标。
- [x] 同项目顺序目标恢复：已完成历史不算冲突；旧 cancelled 总账重开，最终 Goal complete / attempt done / TaskRun done。
- [ ] 小时级慢模型、孙代理与 IM 的组合不由本轮主代理验收替代。

## 重复工具恢复

- [x] 300 次门自身拒绝不清计数、不挤掉有界观测；真实 handler 失败和其它权限拒绝仍保留。
- [x] 真实 Shell 经 Registry/Executor 连续同参数检查，3 次执行后 12 次拒绝；真正写文件后原检查可执行。
- [x] 原始拒绝、计数和换路说明经 canonical 归档进入 native 消息；不是仅在 UI 加字。
- [x] 两种文本分页的二进制失败提示一致，不写转换文件或调用视觉模型。
- [x] 新包真实 TUI 动画任务及同会话追加控制正常结束；一次补丁参数错误后自行恢复，空格不再抛异常。
- [ ] 本地慢模型原重复场景的新版长跑复验：当前用户任务未中断，本机 Gateway 未更换。
- [ ] 动画内容质量：实际产物非 SVG，未做浏览器目视验证；不计成品验收通过，不通过修改用户产物代测。

## 活动思考折叠计数

- [x] 超出预览后增量到达，折叠原文行数和字符数继续刷新；单行 100K 内容不全量排版。
- [x] 末尾换行、窄终端、预览二次折叠分别计数；终态自动收起，原文及分页不受影响（81 项相关定向）。
- [x] 新包真实 TUI 连续任务与 40 列窗口：已接收 1,714→6,445 字符，先有排版折叠，超过 4,000 字符后原文折叠计数仍增长。

## 父子并行与活动诊断

- [x] 模型状态查询不暴露恢复路径；未知状态不猜终态，已存在终态报告与实际产物可达；大树省略范围明确（定向）。
- [x] 新包双真实 TUI：中长任务及完成后查询；主请求根查到两名孩子，会话查询查到协调者和两名孙代理，实际文件和终态报告可读。运行中列表的大量变更/故障组合另验。
- [x] 派工状态只陈述事实，不同时发继续/等待指令；快慢混合保留各自身份与故障信息（定向）。
- [x] 协调者仅加载自身冻结角色与候选索引；不注入全部角色正文，不强迫亲手重写下级成果（定向）。
- [x] 新包真实父级自主工作、递归角色、实际依赖等待的主链验收；首次分层被扁平化记为偏差，后续追加才有深度二身份。
- [ ] 分工效率复验：避免转交父级本来可独立做的部分、反复查状态和猜内部路径；主链结束不算本项通过。
- [x] 成功通知去掉全树屏障，原合批期限不被慢兄弟延期；只确认本批消息（定向）。
- [x] 递归创建不强制让出，任一新 DONE 可释放直属父级；两名孩子并发完成仅消费一次等待（定向）。
- [x] 模型回答到登记等待间的迟到结果不被算已读；忙父级得到完整有界交接，不读取根邮箱（定向）。
- [x] 首 token、流静默、长工具、已知审批、退避与阶段间隔分类，慢流数小时不按总时长强杀（模拟时钟）。
- [x] 新旧 attempt 隔离，进度快照不覆盖诊断，写回失败原键重投，诊断错误不停止心跳（定向）。
- [x] 单 Gateway 双真实 TUI 中长任务：Markdown 检查器与日志分析器完成；父级先消费快孩子，慢孩子继续。
- [x] 新活动诊断包真实 TUI：资源巡检和追加连续采样完成；低阈值提醒回执为 handled，同一 attempt 自然 DONE，无重派。测试阈值已移除。
- [ ] 递归孙级、父级忙时插话完整组合；两路要求分层协作的真实模型均扁平派工，不能算孙级通过。
- [ ] 新路径下重启/存储故障与小时级极慢模型组合；不能由模拟时钟或上述两路成功推断通过。

## 每代理单 Goal 与草稿编辑

- [x] 主子各有一个未结束目标；第二个名字不创建额外执行器，普通派工不自动继承父目标（定向与真实 TUI）。
- [x] 主子目标保存后在各自模型安全边界消费；不串读父级邮箱（真实 TUI 与提交/消费链回归）。
- [x] Enter 编辑、Ctrl+S 保存、Ctrl+G 放弃、子代理 Esc 停止；未保存内容不落盘，状态变化后旧版本不能覆盖（真实 TUI）。主代理新中断语义另列。
- [x] 独立 Todo、Goal 用量隔离、暂停编辑不恢复、内容版本 CAS、同执行器跨逻辑轮与 Compact（定向）。
- [ ] 孙级编辑、小时级慢模型、IM 入口与旧多目标迁移组合另验。下方多目标成功/失败项为历史证据，不代表现行支持多个未结束目标。

## 模型统计与目标展示

- [x] 确定性回归：逻辑模型轮与重试分开；当轮工具未知与零分开；缓存总输入不重复相加。
- [x] 累计与历史窗口分离，相同 request/run 的已落账快照不重复加；主子 exact thread 隔离，显示字段不进入模型上下文。
- [x] 空闲、重连、迟到轮询和窄终端数值投影；缺报缓存显示未知，估算消耗明确标记。
- [x] 新包单 Gateway 双 TUI 真实模型验收：两段连续任务累计等于两份请求账之和；子代理切页、主代理接续整合和实际用量分别核对。
- [x] 独立压缩漏账已定位并补定向回归：成功/失败均保存本范围用量，幂等重放不重复，绑定工作片的不另算。
- [x] 修复后真实 TUI 手动压缩入账 28,722 tokens，压缩后继续任务；四笔持久用量合计 1,053,555，与 TUI 原始统计完全一致。
- [ ] 长会话多次自动压缩、断线及极慢模型的统计组合另验；旧版本已漏记消耗不能从上下文长度倒推补账。
- [x] Goal 默认直接进入工具 schema；自然语言请求在真实 TUI 中创建持久 Goal、显示并通过工具正式完成，不把 task_progress 或口头承诺当作创建。
- [x] Goal 零工具和无 Todo 的正常结束轮仍可续跑；取消、审批、错误、未知副作用不绕过。完整 native 请求装配覆盖回合内创建及状态变更，不只测提示 helper。
- [x] 普通聊天对照实际读取使用文档后正常结束，四轮模型调用，无 Goal 文件、无 active task；目标续跑修复没有激活普通聊天。
- [x] 同 Gateway 的两个命名目标有独立 task，分别调用 update_goal 完成；无预设预算时两个 duration/token 上限均为空，耗时未再次倍增。该条只验运行与记账，不代表质量或去重通过。
- [ ] 多目标真实试跑仍有前台完成自身目标后重复核验兄弟工作、提前说全部完成；完成回执补上兄弟状态与本轮汇报动作后须重新验证，旧失败保留。
- [ ] 旧双 active Goal 绑定 completed task 的显式恢复迁移已通过定向测试；真实用户旧记录未自动改动，仍待现场显式恢复核验。
- [ ] 模型在未被要求时自行填写 Goal 时限的样本仍开放；明确说时间不限的成功样本不能证明该行为已根除。
- [x] Goal 完成不提前关闭 TaskLink 或注册表；PLANNING/RUNNING/未知子树保持开放，真正回合结束且无未决子树才关闭任务（定向）。
- [x] 提前终态修复的真实 TUI 回归：Goal complete 后两名子代理保持运行，随后均 DONE，主代理汇总并关闭任务。
- [x] 当前任务命名 Goal 保留执行范围；额外 Goal 才独立后台；Goal 完成前后、并存独立目标时，后续消息仍可按原父级给 child 入队（定向）。
- [x] 上述插话修复的新包 TUI 回归：两个消息成功入队并有消费回执；两个 child 正常 DONE，主代理汇总与 active task 清空一致。
- [x] 旧冲突恢复的四组定向安全回归：前台/后台 claim × 有效/过期；未释放 running 不迁移、不覆盖原执行、不新增 wake。
- [ ] 多目标重复工作和重复文本达到长度限制仍开放，未混记为通过；业务报告仍列有未验证项目，不把底座交接通过当成业务无缺陷。
- [x] 轮限交接只按真实 active Goal 承诺续跑；保留旧编号但已 complete/paused/blocked/受限/删除，以及外任务绑定均不承诺（定向）。
- [ ] 修正系统持续执行归属后的两组真实多目标分工回归仍失败；已观察到重复核验和重复派工，继续保留失败结论。

## 无预选模型

- [x] dataclass 和随包 YAML 的模型协议、名称、地址均为空；空配置不隐式使用真实模型或离线模拟。
- [x] 配置菜单不制造默认模型项；保留用户显式选择、会话独立选择、共享目录和模型配置原子写。
- [x] Gateway 模型状态只投影当前调用配置；后台记忆整理的失效模型引用不能回退到启动配置。
- [x] 单 Gateway 真实 TUI：新会话显示未配置，菜单可进入，发消息返回 `MODEL_NOT_CONFIGURED`；另一用户显式选择的模型仍可真实调用，不自动选择共享目录里的模型。
- [x] 测试机旧工作目录遮蔽新版包已定位；这一轮误载旧代码的试跑作废，纠正启动目录后重验。

## 执行器退出与积压恢复

- [x] 无 session 的原进程退出、同宿主执行器返回、提示构造阶段异常均可识别。
- [x] 健康执行即使没有输出且心跳很旧也不回收；旧 attempt 不覆盖新轮。
- [x] 有未决工具保留 UNKNOWN、显示 BLOCKED 并通知父级；没有结果不能推断业务完成。
- [x] 121 条积压跨页及重建仓储后继续，超过 100 条消费回执不重复恢复，坏的前页不饿死后项。
- [ ] 新包单 Gateway 真实多子代理 TUI、退出注入、父级收到通知和重复恢复检查。
- [ ] 结果文件已写而任务状态尚未保存的退出窗口：需确认恢复不会以失败覆盖已产生的结果；未验证前不能宣布整项关闭。

## R228 发布候选

- [x] Context 实际容量与持久同代快照一致，遥测失败不阻断压缩，旧代不能回写。
- [x] HTTP 400 不猜瞬时异常；429/5xx 保留原重试，真实 工具运行时 拒绝原因仍单列。
- [x] 分段完整覆盖、空/工具/截断摘要失败不提交；可容纳请求保留原缓存面。
- [x] 原长 TUI 压缩后继续工具调用；不是新开空会话短问候替代。
- [x] bubblewrap 二进制与发行原包 hash 一致，补齐对应源码/许可并增加漏包回归。
- [x] 最终 35 文件联合 539 passed、全部本地严格 gate、完整第三方材料及源码字节一致性验包。
- [x] `.10` 三位 bug-finder 审查，原源码 hash 不变；新增临时复现脚本/报告单列，不称严格不落盘。
- [x] `.10` `/compact` 41,006 → 23,965，generation 5 → 6，保留 6 条用户原话和本轮审查原要求。
- [x] 清除失效 xfail：重复 Compact 深度已为 2，改验 owner home 的真实产物并验证外部旧目录没有产物。
...
- [x] final 后第三页恢复，消息身份/正文一致；观察页无额外审批入口或模型调用（R216真实）。
- [x] 旧客户端未声明 foreground_messages 时保持原后台语义（focused；旧版真实交互仍待补）。
- [ ] BUG-146 余项：前台执行中思考/工具流实时同步，不以本片替代。

## R215 同会话审批等待

- [x] 先红复现共享投影漏事件、普通/展开视图错误转 Working；修后 87 focused 通过。
- [x] 精确多个审批、无关回执、上下文刷新、迟到事件、不同 thread、静态非授权提示。
- [x] MiniMax 真 TUI 发起具体工具审批，同会话观察页等待一致，主页允许一次后继续并最终停止 Working。
- [x] 精确审批仅一条 requested/resolved；21个真实模型调用无重试，PTY退出0、147行教程在本owner家内。
- [ ] 真实拒绝/取消、多并发审批和Ctrl+O冻结中阶段切换组合；定向通过不代替这些TUI分支。
- [ ] BUG-146 完整前台正文与最终回复同源同步；不能被本片替代。
...
- [ ] `/goal` 未命名目标不含糊并发，多个命名目标有独立任务；按名称或编号 pause/resume/clear 保留归属。零工具、无 Todo 的安全结束轮仍续跑；审批、暂停、错误不续跑。旧共享任务只在显式 resume 时安全迁移，保留原记录。active goal 的 `/stop` 只暂停，complete/blocked 仅由精确 scoped 工具写入。
...
- [x] 后台进程 33 次变化时长的重复等待回放、日志字节增长和退出观测变化；原始审计结果不裁剪。
- [x] 新包正常模型 22 轮后台/前台交错任务与慢模型 4 轮小任务均自然收尾；独立 11 测试通过和金额复算一致。
- [ ] 新包真实 TUI 的重复等待恢复与模型采纳；软提醒通过不等于模型必定换策略。

- [x] R214 本地：选择确认后欢迎区更新；仅新增/失败不变；启动/恢复读取已保存选择；展开视图、58/120 列通过。
- [x] R214 真实 TUI：新开读取已保存选择，Qwen→MiniMax→Qwen 顶部立即刷新；Qwen 一句问候正常回复。
- [ ] R214 真实 TUI：/sessions 精确恢复旧会话后标签与不同 owner 并发切换的完整组合。
- [ ] 主 A / 子 B 显式选择：owner 边界、批次零部分创建、父模型不变、孙代理继承与重启恢复。
...
- [x] 本地 writer/reducer 的 live/final/缺帧/恢复、精确请求去重、旧能力过滤。
- [x] 本地候选接替、typed 插话丢弃、关闭不伪造成功；用户输入先于过程。
- [ ] 新包真实 TUI 的原页/观察页/恢复页完整过程和零重复。
- [ ] 真实取消、Compact、跨 owner、旧客户端并存与提交前断流恢复组合。
