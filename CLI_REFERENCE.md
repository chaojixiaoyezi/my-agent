...
| `subagents-acceptance-plan` | 查看、审计或显式应用单个 subagent 的父级验收决策 | `--write` 写 dry-run 决策；`--apply` 只允许 `inspect_only` 进入普通验收 apply；`--next-action` 只给上级动作建议 | 否 |
...
显示当前本地工作台总览：gateway 存活状态、gateway 队列数量、LocalStore 记录/事件数量、subagent summary、红灯任务、Shared Progress、Takeover View、Acceptance Plan、最近事件和建议下一步动作。它只读现有账本，不调用模型。
...
输出会包含 `Shared Progress`、`Takeover View` 和 `Acceptance Plan`：前者显示 root task 聚合计数，接管视图显示恢复入口 refs 和推荐读取顺序，验收计划显示父级 dry-run 下一步。完整 artifact 正文不会自动进入看板，也不会因为展示验收计划而执行 tests。
