...
def _add_update_status_command(sub) -> None:
    ...
    update_cmd.add_argument("--status", required=True, help="新状态：open 或 closed")
    update_cmd.add_argument("--actor-agent-id", default="", help="执行状态推进的代理 ID")
    update_cmd.add_argument("--summary", default="", help="状态推进摘要；closed 状态必须提供")
...
def _add_update_request_command(sub) -> None:
    ...
    update_cmd.add_argument("--status", required=True, help="请求状态：open/pending/completed/blocked/timeout/declined")
