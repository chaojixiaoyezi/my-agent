...
# LLM: 命令上下文只携带当前 owner 的提案服务与展示用事实；开关状态只用于展示，不决定能否确认。
# 类用途: 保存一次 skills proposals 命令解析出的 owner 服务和开关状态。
...
def _owner_context(args: Any) -> _ProposalCommandContext:
    ...
    return _ProposalCommandContext(
        service=SkillProposalService(home_paths_with_owner(base, owner), config=config),
        owner_id=owner.owner_id,
        self_learning_enabled=bool(config.enable_self_learning),
    )


# LLM: 列表返回完整提案记录，供用户确认前核对；开关状态只作展示。
# 函数用途: 构造 list 的输出。
...
# LLM: 每条提案一行摘要加一行描述，完整内容用 show 查看。
# 函数用途: 生成提案列表的中文展示。
