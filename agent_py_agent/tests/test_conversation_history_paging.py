...
def _conversation(tmp_path, turns=7, rows_per_turn=3, long=False):
    ...
    for turn in range(turns):
        for index in range(rows_per_turn):
            rows.append(store.append_message({
...
def test_backward_pages_keep_complete_turns_and_exact_utf8_offsets(tmp_path, long):
    ...
    while True:
        page = store.history_page_report(thread, before=before, limit=4)
        assert not page.errors
        assert page.after == (before if before is not None else store._message_path(thread).stat().st_size)
...
def test_partial_tail_and_late_append_do_not_advance_live_cursor(tmp_path):
    ...
    path = store._message_path(thread)
    ...
    page = store.history_page_report(thread, limit=2)
    assert page.after == complete_end and not page.errors
    with path.open("ab") as handle:
        handle.write(encoded[-3:])
    messages, _, errors = store.message_page_after_offset_report(thread, after=page.after)
...
def test_bad_cursor_or_message_is_explicit_failure(tmp_path, invalid):
    ...
    result = store.history_page_report(thread, before=before)
...
def test_latest_page_does_not_decode_unselected_old_prefix(tmp_path):
    ...
    with store._message_path(thread).open("r+b") as handle:
        handle.write(b"!")
    page = store.history_page_report(thread, limit=4)
...
def test_zero_cursor_means_beginning_not_latest(tmp_path):
    ...
    page = store.history_page_report(thread, before=0)
