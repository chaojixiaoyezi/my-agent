...
# 类用途: 承载真实 execute_tool_loop 跑完后的交付/账本/时序证据。
@dataclass
class _LoopRun:
...
def _drive_loop(monkeypatch, tmp_path: Path, script: list[object], *, run_id: str) -> _LoopRun:
    ...
    with monkeypatch.context() as loop_patch:
        ...
        prompt, response, rounds = loop_result.final_prompt, loop_result.final_response, loop_result.tool_rounds
    return _LoopRun(
        backend=backend,
        tool=tool,
        params=params,
...
def test_prior_segment_is_kept_across_a_truncated_probe_shot(monkeypatch, tmp_path) -> None:
    ...
    # LLM: 保留真实工具轮、产品记账与当前宿主，不把截断交付测试变成预置成功结果。
    # 函数用途: 核对请求宿主后继续原工具轮执行。
    def counting_tool_round(loop_agent, request):
        assert loop_agent is agent and request.agent is agent
        return real_run_tool_round(loop_agent, request)

    # LLM: 只替换具体测试工具，canonical ToolExecutor 保持实际执行。
    # 函数用途: 核对宿主后在隔离工作区执行原测试工具。
    def counting_execute_one(loop_agent, request):
        assert loop_agent is agent
        return execute_canonical_test_call(
            tmp_path,
    ...
    with monkeypatch.context() as loop_patch:
        ...
        loop_patch.setattr(tls, "generate_model_response", truncating_generate)
        loop_result = tls.execute_tool_loop(agent, params)
        prompt, response, _rounds = loop_result.final_prompt, loop_result.final_response, loop_result.tool_rounds
