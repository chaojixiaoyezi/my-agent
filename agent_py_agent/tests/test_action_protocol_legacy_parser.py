...
def test_legacy_tool_call_parser_returns_typed_envelopes():
    ...
    assert envelopes[0].source == "legacy_text_protocol"
    assert envelopes[0].call_id == "legacy-tool-call-1"
    assert envelopes[0].tool == "read_file"
    assert envelopes[0].args == {"path": "README.md"}
    assert envelopes[0].scope.task_id == "task-1"
    assert envelopes[0].reserved["legacy_index"] == 1


def test_legacy_tool_call_parser_masks_subagent_result_text():
    text = (
        "[SUBAGENT_RESULT]\n"
...
def test_legacy_xmlish_parser_returns_typed_envelopes():
    envelopes = parse_registry_tool_call_envelopes(
