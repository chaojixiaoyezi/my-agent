...
def _runtime_parts(tmp_path, *, enable_tools: bool, backend=None):
    ...
    scheduler = BackgroundMainAgentScheduler(
        {"runtime": runtime, "store": store, "collaboration_store": agent.collaboration_store}
    )
...
def _blocked_collaboration(agent: SimpleAgent, thread, *, source: str, target: str):
    ...
    agent.collaboration_store.register_agent(
        AgentCapability(agent_id=target, capabilities=("query",))
    )
    case = agent.collaboration_store.open_case(
        {
            "thread_id": thread.thread_id,
            "task_id": "task-1",
            "title": "普通任务里的协作阻塞",
    ...
    request = agent.collaboration_store.request_collaboration(
        {
            "case_id": case.case_id,
            "requester_agent_id": source,
            "target_agent_ids": (target,),
            "question": "请补充证据。",
            "now": 4.0,
        }
    )
    agent.collaboration_store.update_request_status(
        {
            "case_id": case.case_id,
...
def test_scheduler_processes_collaboration_cases_before_waking_agent(tmp_path) -> None:
    ...
    agent, _store, _channels, scheduler = _runtime_parts(
        tmp_path, enable_tools=False, backend=backend
    )
    thread = _thread_with_task(_store, title="协作任务", goal="多代理协作任务")
    agent.collaboration_store.register_agent(
        AgentCapability(agent_id="source-a", capabilities=("query",))
    )
    case = agent.collaboration_store.open_case(
        {
            "thread_id": thread.thread_id,
    ...
    agent.collaboration_store.submit_evidence(
        {
            "case_id": case.case_id,
            "source_agent_id": "source-a",
            "matched": True,
            "summary": "证据到位。",
            "evidence_refs": ("artifact://source-a/e1",),
            "now": 4.0,
        }
    )
    ...
    assert (
        reports[0].reason == "collaboration_case_closed"
    )  # 修 reason 透传后:报告带 signal 真实 reason(非泛泛 urgent_wake_signal)
...
def test_offline_collaboration_rehearsal_uses_inspect_collaboration_and_tree_tools(
    tmp_path,
) -> None:
    from agent_py_agent.agent.collaboration import AgentCapability
    ...
    for capability in (
        AgentCapability(
            agent_id="source-a", capabilities=("query", "summarize"), status="available"
        ),
        AgentCapability(
            agent_id="source-b", capabilities=("query", "correlate"), status="available"
        ),
    ):
    ...
    case = agent.collaboration_store.open_case(
        {
            ...
            "task_id": "task-1",
            "title": "需要跨源协作判断",
            "summary": "两个来源都可能有相关证据，需要主代理读取 case 后判断。",
            "priority": "urgent",
            "created_by": "source-a",
            "required_capabilities": ("query",),
            "entities": {"entity": "open-world-signal"},
            "now": 3.0,
        }
    )
    request = agent.collaboration_store.request_collaboration(
        ...
        agent.collaboration_store.submit_evidence(
            {
                ...
                "matched": True,
                "summary": f"来源 {source[-1].upper()} 找到补充证据。",
                "evidence_refs": (ref,),
...
def test_long_running_watcher_wakes_main_agent_when_responder_blocks(tmp_path) -> None:
    ...
    assert (
        reports[0].reason == "collaboration_case_closed"
    )  # 修 reason 透传后:报告带 signal 真实 reason(非泛泛 wake_signal)
...
def _watcher_blocked_case(agent: SimpleAgent, thread):
    ...
    agent.collaboration_store.register_agent(
        AgentCapability(
            agent_id="watcher-b", capabilities=("query", "correlate"), status="available"
        )
    )
    case = agent.collaboration_store.open_case(
        {
            ...
            "task_id": "task-1",
            "title": "常驻观察发现阻塞",
            "summary": "一个响应者无法访问来源，需要主代理决定换来源或降级处理。",
            "priority": "normal",
            "created_by": "watcher-a",
            "required_capabilities": ("query",),
            "now": 3.0,
        }
    )
    request = agent.collaboration_store.request_collaboration(
        {
            "case_id": case.case_id,
            "requester_agent_id": "watcher-a",
            "target_agent_ids": ("watcher-b",),
            "required_capabilities": ("query",),
            "question": "请围绕同一实体补充来源。",
            "now": 4.0,
        }
    )
    agent.collaboration_store.update_request_status(
        {
            ...
            "request_id": request.request_id,
            "status": "blocked",
            "actor_agent_id": "watcher-b",
            "summary": "来源连接失败，需要主代理决定是否改查其他来源。",
            "now": 5.0,
        }
    )
    return case
...
def test_scheduler_batches_same_task_wakes_without_sweeping_unrelated_events(tmp_path) -> None:
    ...
    backend = _CapturingBackend()
    agent, store, _channels, scheduler = _runtime_parts(
        tmp_path, enable_tools=False, backend=backend
    )
    thread = _thread_with_task(store, title="多事件同线程", goal="多 case 协作")
    agent.collaboration_store.register_agent(
        AgentCapability(agent_id="source-b", capabilities=("query",))
    )
...
def test_plain_language_background_scenario_can_rework_blocked_collaboration(tmp_path) -> None:
    ...
    assert [(item["tool"], item["status"]) for item in public_verification["groups"]] == [
