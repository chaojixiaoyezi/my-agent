...
由子代理记的,run_started_at-1s 的默认窗口抓不到 → delta 空 → 主代理响应被收尾门打回成
[MAIN_AGENT_DELIVERY_REWORK_REQUIRED] 内部信号时,整条被投递枢纽拦下、发不到用户飞书。
...
def test_delta_captures_earlier_subagent_finding_via_findings_since(tmp_path):
    ...
    rework = "[MAIN_AGENT_DELIVERY_REWORK_REQUIRED]\n{...}\n[/MAIN_AGENT_DELIVERY_REWORK_REQUIRED]"
    ...
    assert "MAIN_AGENT_DELIVERY_REWORK_REQUIRED" not in send_fixed  # 内部记号只进内部账,不出站
