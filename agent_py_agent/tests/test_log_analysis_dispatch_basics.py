"""Basic dispatch engine tests for log analysis.
日志分析派发引擎基础测试。"""
...
def test_security_prompt_disabled_leaves_base_prompt_unchanged():
    """LLM: Tests that disabling security prompt leaves the base prompt unchanged."""
...
def test_security_prompt_analyst_mode_is_scoped_and_evidence_driven():
    """LLM: Tests that analyst mode security prompt is scoped to evidence-driven queries only."""
    rendered = build_security_prompt(
        "base",
...
def test_analyst_contract_requires_evidence_refs():
    """LLM: Tests that analyst input validation rejects empty evidence_refs."""
    try:
        validate_analyst_input(
            {
                "case_id": "case-001",
...
def test_reviewer_rejects_report_without_evidence():
    """LLM: Tests that reviewer rejects a report that lacks evidence references."""
...
def test_reviewer_accepts_evidence_backed_report():
    """LLM: Tests that reviewer approves a properly evidence-backed analyst report."""
...
def test_evidence_ref_dataclass_normalizes_to_id_in_analyst_input_and_summary():
    """LLM: Tests that EvidenceRef dataclass normalizes to its ID string in analyst input and summary."""
...
def test_reviewer_known_refs_accepts_evidence_ref_dataclass_boundary():
    """LLM: Tests that reviewer accepts EvidenceRef dataclass instances as known refs."""
    report = AnalystReport(
        case_id="case-001",
...
def test_dispatch_engine_public_protocol_contract_is_minimally_usable():
    """LLM: Tests that DispatchEngine satisfies the public protocol contract and returns usable results."""
    engine = DispatchEngine()
...
def test_dispatch_engine_respects_enabled_budget():
    """LLM: Tests that dispatch engine dispatches agents when budget is enabled."""
    engine = DispatchEngine(
...
def test_case_summary_is_compact_and_excludes_raw_events_and_transcript():
    """LLM: Tests that case summary excludes raw events and transcript, keeping only compact fields."""
