...
class TestControlledExecAcceptanceFindings(_FindingSetupMixin, _FindingAssertMixin, _FindingReportMixin):
    ...
    def test_controlled_exec_goal_requires_actual_tool_and_refs(self, tmp_path: Path):
        """要求 controlled_exec 的任务不能只用 write_file 伪造完成。"""
        ...
        task = self._make_findings_task(
            tmp_path,
            goal="必须用 controlled_exec 执行 pwd，并报告 stdout_ref、audit_ref、trash_manifest_ref。",
    ...
    def test_controlled_exec_contract_reads_deliverable_refs(self, tmp_path: Path):
        ...
        task = self._make_findings_task(
            tmp_path,
            goal="必须用 controlled_exec 执行 pwd/rm，并报告 stdout_ref、audit_ref、trash_manifest_ref。",
    ...
    def test_controlled_exec_contract_accepts_descendant_tool_evidence(self, tmp_path: Path):
        ...
        task = self._make_findings_task(
            tmp_path,
            task_dir=parent_dir,
            goal="创建下级完成 controlled_exec，并报告 stdout_ref、audit_ref、trash_manifest_ref。",
            role="coordinator",
            child_ids=["child"],
            allowed_write_roots=[str(deliverables)],
        )
    ...
    def test_controlled_exec_contract_follows_small_tool_output_artifact_refs(self, tmp_path: Path):
        """refs 文件只写 artifact_ref 时，验收可读取受控小 artifact 里的 stdout/audit refs。"""
        from agent_py_agent.agent.subagents.manager_acceptance_findings import (
            SubAgentAcceptanceFindingMixin,
        )
