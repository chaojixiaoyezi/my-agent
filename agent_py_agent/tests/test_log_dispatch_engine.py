...
class TestSubmitCase:
    ...
    def test_enqueue_case_not_dispatched_when_budget_exhausted(self):
        """测试预算耗尽时不调度"""
    ...
    def test_enqueue_case_adds_to_queue(self):
        """测试提交案例添加到队列"""
        engine = DispatchEngine(budget=DispatchBudget(
            case_auto_dispatch_enabled=True,
            max_parallel_analyst_agents=5,
        ...
        engine.enqueue_case(case)
        # 调度允许时请求被标记为 DISPATCHED，所以检查队列长度
        assert len(engine.queue) >= 1

    def test_enqueue_case_with_evidence_refs(self):
        """测试带证据引用的案例"""
        engine = DispatchEngine(budget=DispatchBudget(
            case_auto_dispatch_enabled=True,
            max_parallel_analyst_agents=5,
    ...
    def test_enqueue_case_priority_extracted(self):
        """测试优先级提取"""
        engine = DispatchEngine(budget=DispatchBudget(
            case_auto_dispatch_enabled=True,
            max_parallel_analyst_agents=5,
...
class TestEnqueueCase:
    ...
    def test_enqueue_case_is_deterministic_for_same_case(self):
        """测试 enqueue_case 对同一 case 产生一致结果"""
        engine = DispatchEngine(budget=DispatchBudget(
            case_auto_dispatch_enabled=True,
            max_parallel_analyst_agents=5,
            analyst_agent_budget_per_hour=10,
        ...
        result1 = engine.enqueue_case(case)
        # 重置引擎
...
class TestHealth:
    ...
    def test_health_reflects_queue_state(self):
        ...
        engine.enqueue_case({"case_id": "health-case-001"})
        h = engine.health()
        # queue 有 1 个请求
...
class TestBoundaryCases:
    ...
    def test_enqueue_case_with_empty_case_id(self):
        """测试空 case_id"""
        engine = DispatchEngine(budget=DispatchBudget(
            case_auto_dispatch_enabled=True,
            max_parallel_analyst_agents=5,
    ...
    def test_enqueue_case_with_none_values(self):
        """测试 None 值"""
        engine = DispatchEngine(budget=DispatchBudget(
            case_auto_dispatch_enabled=True,
            max_parallel_analyst_agents=5,
    ...
    def test_concurrent_submissions(self):
        ...
        for i in range(5):
            engine.enqueue_case({"case_id": f"concurrent-case-{i}"})
        # 队列中应该有 5 个请求
