...
def test_active_goal_continues_without_ordinary_resume_policy(tmp_path):
    """显式 Goal 在进程内按自身护栏续跑，不创建普通任务 policy。"""
    from agent_py_agent.agent.agent_core.runtime.run_params import (
        run_params_with_request_id,
    )
    from agent_py_agent.cli.resume_loop import run_with_resume
...
def test_gateway_retires_removed_ordinary_resume_policy(tmp_path):
    """旧普通续跑 policy 无论历史 claim 字段如何都必须直接退休。"""
...
def test_unknown_halt_reason_aligns_with_taxonomy(monkeypatch):
    ...
    resp = svc._final_response_after_halt(None, _P(), 5, reason="unknown_outcome")
    final = resp[1]
    ...
    resp1b = svc._final_response_after_halt(None, p1b, 5, reason="unknown_outcome")
    final1b = resp1b[1]
    ...
    resp2 = svc._final_response_after_halt(None, p2, 5, reason="unknown_outcome")
    final2 = resp2[1]
