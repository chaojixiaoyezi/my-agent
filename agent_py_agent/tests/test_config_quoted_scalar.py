...
原问题(真机 QQ 接入暴露):qq_app_id: "1900000000" 在 parse_scalar 里被先无条件剥引号、再 int() 成
整数 1900000000,随后 string 字段归一 _string_config_value(int) 把它丢成空字符串 → QQ app_id 加载为
...
def test_parse_scalar_quoted_digits_stay_string() -> None:
    ...
    assert parse_scalar('"1900000000"') == "1900000000"
    assert parse_scalar("'1900000000'") == "1900000000"
    assert isinstance(parse_scalar('"1900000000"'), str)
...
def test_string_config_value_coerces_number_not_drop() -> None:
    ...
    assert _string_config_value(1900000000) == "1900000000"  # int -> str,不丢空
...
def test_load_config_quoted_numeric_id_survives() -> None:
    ...
    try:
        c = load_config(p)
        assert c.qq_app_id == "1900000000"  # 原 bug:加载成空字符串
        assert c.qq_app_secret == "fake-secret-abcdefghijklmnopqrstuv"
    finally:
        Path(p).unlink()
...
def test_load_config_bare_numeric_id_coerced_by_defense() -> None:
    ...
    with tempfile.NamedTemporaryFile("w", suffix=".yaml", delete=False, encoding="utf-8") as f:
        ...
        p = f.name
    try:
        assert load_config(p).qq_app_id == "1900000000"
    finally:
        Path(p).unlink()
