...
IMAGE = {"type": "image", "source": {"type": "base64", "media_type": "image/png", "data": "cGl4ZWw="}}
WINDOW = 60_000


# LLM: 与原 preflight 测试同形：真实配置与策略，只替换文字估算；媒体只以原生历史里的图片块表达。
# 函数用途: 构造一次 preflight 请求，按参数决定是否带图片、是否持久保存。
def _preflight(monkeypatch, *, media: bool, save: bool, tokens: int):
    agent = SimpleNamespace(
        config=AgentConfig(auto_save_memory=True, memory_compact_auto_trigger_percent=50),
...
# (是否带图, save, 估算 token, 是否拦截)
@pytest.mark.parametrize("case", [
    (False, True, 499, False), (False, True, 500, True),     # 纯文字：原压缩点 50% 不变
    (True, True, 500, False), (True, True, 999, False),      # 媒体：压缩链不可用，压缩点不再拦
    (True, True, 1_000, True),                               # 媒体：窗口硬上限照常拦
    (False, False, 999, False), (False, False, 1_000, True), # save=false 纯文字：原窗口门不变
    (True, False, 999, False), (True, False, 1_000, True),   # save=false 媒体：与纯文字相同
])
def test_preflight_threshold_follows_compact_capacity(monkeypatch, case):
    media, save, tokens, fires = case
    response = _preflight(monkeypatch, media=media, save=save, tokens=tokens)
    assert (response is not None) is fires
    if fires:
        assert response.runtime_status == "context_overflow"
        assert ("compact_capacity=non_text" in response.text) is media
...
# (是否带图, 长文字行数, 预期结果)
@pytest.mark.parametrize("case", [
    (True, 6, "sent"),      # 压缩点以下：原样带图发送
    (True, 16, "sent"),     # 越过压缩点、低于窗口：修复前在压缩点处整轮失败
    (True, 32, "refused"),  # 越过窗口：结构化非文本拒绝，不发业务请求
    (False, 16, "compacted"),  # 纯文字对照：原自动压缩后发送
])
def test_gateway_media_history_between_trigger_and_window(tmp_path, monkeypatch, case):
    outcome = case[2]
