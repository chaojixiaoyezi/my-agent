...
from agent_py_agent.agent.conversation.background_supply_backoff import (
    ProviderSupplyBackoff,
    consume_with_supply_guard,
)
...
def test_supply_backoff_throttles_retry_then_auto_resumes_after_recovery(tmp_path, capsys) -> None:
    ...
    assert scheduler.ready_thread_ids(now=101.0) == ()  # 就绪扫描必须共享消费时登记的冷却
    assert scheduler.tick(now=101.0) == []  # 退避窗内(30s):不打模型
    assert backend.calls == 1
    assert store.wakes.pending()  # 信号还在等

    assert scheduler.ready_thread_ids(now=131.0) == (thread_id,)
...
def test_backoff_anchors_at_failure_time_not_tick_start(monkeypatch) -> None:
    ...
    import agent_py_agent.agent.conversation.background_supply_backoff as supply_module

    clock = {"v": 1000.0}
    monkeypatch.setattr(supply_module.time, "monotonic", lambda: clock["v"])
    backoff = ProviderSupplyBackoff(base_seconds=30.0, max_seconds=900.0)
    ...
    assert consume_with_supply_guard(backoff, "t", 50.0, slow_failing_turn) is None
...
def test_no_report_preserves_backoff_but_false_report_clears_it() -> None:
    backoff = ProviderSupplyBackoff()
    backoff.record_failure("t", now=0.0)

    assert consume_with_supply_guard(backoff, "t", 30.0, lambda: None) is None
    assert backoff.record_failure("t", now=30.0)["consecutive_failures"] == 2

    assert consume_with_supply_guard(backoff, "t", 90.0, lambda: False) is False
    assert backoff.record_failure("t", now=90.0)["consecutive_failures"] == 1
