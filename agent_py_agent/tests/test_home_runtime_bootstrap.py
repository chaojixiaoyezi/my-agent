...
def test_simple_agent_initializes_my_agent_home(tmp_path: Path):
    ...
    agent = SimpleAgent(
        AgentConfig(tool_protocol="native", my_agent_home=str(home), prompt_files=[]), repo
    )
...
def test_saved_run_creates_home_task_workspace(tmp_path: Path):
    ...
    home = tmp_path / "home"
    cfg = AgentConfig(
        tool_protocol="native", my_agent_home=str(home), memory_path="memory.jsonl", prompt_files=[]
    )
...
def test_prompt_builder_reads_home_entry_files_every_round(tmp_path: Path):
    repo = tmp_path / "repo"
    home = tmp_path / "home"
    cfg = AgentConfig(tool_protocol="native", my_agent_home=str(home), prompt_files=[])
    agent = SimpleAgent(cfg, repo)
