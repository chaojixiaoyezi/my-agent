...
上下文窗口、compact 触发点全部单一权威）；子代理不允许另设 compact 阈值，
默认 prompt 模板也只有一个权威位置。
"""
...
def test_compact_auto_cycle_fields_reads_finalize_context_scope(tmp_path):
    ...
    agent = _Agent()
    agent.root = tmp_path
    run_home = tmp_path / "tasks" / "root-1" / "work" / "agents" / "run-1"
