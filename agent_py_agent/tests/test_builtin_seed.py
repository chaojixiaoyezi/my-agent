...
fingerprint 幂等,用户自定义 shared/skills 不受影响。
"""
...
def test_sync_mirrors_builtin_to_home(tmp_path, monkeypatch):
    ...
    home_builtin = tmp_path / "home" / "shared" / "builtin"
    index = tmp_path / "home" / "shared" / "indexes" / "skills.jsonl"
    count = sync_builtin_skills_to_home(home_builtin, index)
...
def test_sync_idempotent_skips_when_unchanged(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(builtin_seed, "_BUILTIN_SRC", src)
    home_builtin = tmp_path / "home" / "shared" / "builtin"
    index = tmp_path / "idx" / "skills.jsonl"

    sync_builtin_skills_to_home(home_builtin, index)
    marker = home_builtin / ".builtin_fingerprint"
    fingerprint = marker.read_text(encoding="utf-8")
    mtime = index.stat().st_mtime_ns

    count = sync_builtin_skills_to_home(home_builtin, index)  # 内容没变 → 跳过
    assert count == 1
...
def test_sync_full_resync_removes_deleted_skill(tmp_path, monkeypatch):
    ...
    home_builtin = tmp_path / "home" / "shared" / "builtin"
    index = tmp_path / "idx" / "skills.jsonl"
    assert sync_builtin_skills_to_home(home_builtin, index) == 2
...
def test_sync_does_not_touch_user_custom_skills(tmp_path, monkeypatch):
    ...
    user_skills = shared / "skills"
    user_skills.mkdir(parents=True)
    (user_skills / "my-own.md").write_text("我的自定义", encoding="utf-8")

    sync_builtin_skills_to_home(shared / "builtin", shared / "indexes" / "skills.jsonl")
...
def test_ensure_home_seeds_builtin_skills(tmp_path):
    """端到端:ensure_my_agent_home 跑完,home/shared/builtin 有内置 skill、索引非空。"""
