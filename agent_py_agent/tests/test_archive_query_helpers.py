"""memory archive query helper tests.

测试字段推导、内容预览、错误记录等 archive_helpers 行为。
"""
...
class TestCreatedAtSort:
    ...
    def test_sort_iso_format(self):
        ...
        result = _created_at_sort("2026-05-03T10:00:00Z", default=0.0)
    ...
    def test_sort_date_only(self):
        ...
        result = _created_at_sort("2026-05-03", default=0.0)

        assert result > 0

    def test_sort_empty_returns_default(self):
        """测试空字符串返回默认排序值。"""
        from agent_py_agent.agent.memory_archive.query.archive_helpers import _created_at_sort

        result = _created_at_sort("", default=123.45)

        assert result == 123.45

    def test_sort_invalid_returns_default(self):
        """测试无效时间格式返回默认排序值。"""
        from agent_py_agent.agent.memory_archive.query.archive_helpers import _created_at_sort

        result = _created_at_sort("not a date", default=999.0)
