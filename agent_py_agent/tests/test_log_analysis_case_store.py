...
class MakeFindingParams:
    """Parameter bundle for _make_finding."""
...
def _make_finding(params: MakeFindingParams) -> Finding:
    return Finding(
        finding_id=params.finding_id,
        detector_id=params.detector_id,
        risk_score=params.risk_score,
        entities=params.entities if params.entities is not None else {"attacker_ip": ["1.2.3.4"], "victim_ip": ["10.0.0.1"]},
        window=params.window if params.window is not None else ["2025-01-01T00:00:00Z", "2025-01-01T01:00:00Z"],
        confidence=params.confidence,
        hypothesis=params.hypothesis,
        evidence_refs=params.evidence_refs if params.evidence_refs is not None else [EvidenceRef(evidence_id="ev-1")],
        gaps=params.gaps if params.gaps is not None else [],
        next_queries=params.next_queries if params.next_queries is not None else [],
    )
...
class TestDedupKeyForFinding:
    def test_basic_dedup_key(self) -> None:
        finding = _make_finding(MakeFindingParams(detector_id="waf_attack_success_candidate"))
...
class TestCaseStore:
    ...
    def test_record_finding_creates_case(self, tmp_path: Path) -> None:
        ...
        finding = _make_finding(MakeFindingParams(risk_score=0.9))
    ...
    def test_record_finding_low_confidence_returns_none(self, tmp_path: Path) -> None:
        ...
        finding = _make_finding(MakeFindingParams(risk_score=0.5, confidence=0.3))
        case = store.record_finding(finding)
        assert case is None

    def test_record_finding_uses_confidence_over_risk_score(self, tmp_path: Path) -> None:
        store = CaseStore(tmp_path, min_case_confidence=0.5)
        finding = _make_finding(MakeFindingParams(risk_score=0.9, confidence=0.4))
        case = store.record_finding(finding)
        assert case is None  # confidence 0.4 < 0.5 threshold

    def test_record_finding_mapping_input(self, tmp_path: Path) -> None:
        store = CaseStore(tmp_path, min_case_confidence=0.5)
        data = _make_finding(MakeFindingParams(risk_score=0.9)).to_dict()
        case = store.record_finding(data)
        assert case is not None

    def test_record_findings_deduplicates_cases(self, tmp_path: Path) -> None:
        store = CaseStore(tmp_path, min_case_confidence=0.5)
        f1 = _make_finding(MakeFindingParams(finding_id="f-1", risk_score=0.9))
        f2 = _make_finding(MakeFindingParams(finding_id="f-2", risk_score=0.95))
    ...
    def test_list_cases_sorted_by_priority(self, tmp_path: Path) -> None:
        ...
        store.record_finding(_make_finding(MakeFindingParams(finding_id="f-1", risk_score=0.5)))
        store.record_finding(_make_finding(MakeFindingParams(finding_id="f-2", risk_score=0.95, entities={"attacker_ip": ["9.9.9.9"], "victim_ip": ["10.0.0.99"]})))
    ...
    def test_load_findings(self, tmp_path: Path) -> None:
        store = CaseStore(tmp_path, min_case_confidence=0.5)
        store.record_finding(_make_finding(MakeFindingParams(finding_id="f-1")))
    ...
    def test_merge_finding_updates_risk_score(self, tmp_path: Path) -> None:
        store = CaseStore(tmp_path, min_case_confidence=0.5)
        f1 = _make_finding(MakeFindingParams(finding_id="f-1", risk_score=0.7))
        f2 = _make_finding(MakeFindingParams(finding_id="f-2", risk_score=0.95))
    ...
    def test_merge_finding_adds_hypothesis(self, tmp_path: Path) -> None:
        store = CaseStore(tmp_path, min_case_confidence=0.5)
        f1 = _make_finding(MakeFindingParams(finding_id="f-1", hypothesis="possible breach"))
        case = store.record_finding(f1)
        assert case is not None
    ...
    def test_search_store_integration(self, tmp_path: Path) -> None:
        ...
        case = store.record_finding(_make_finding(MakeFindingParams(risk_score=0.9)))
        assert case is not None
