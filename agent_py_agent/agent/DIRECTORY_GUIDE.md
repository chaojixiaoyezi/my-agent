...
主代理应用服务层。放 `SimpleAgent` 的主循环、子代理 runner、父代理 planner、宿主内部自动 dispatch/watch、编排工具和 runner 重试规则。模型侧只保留统一创建和直属下级的插话、打断、权限裁决，不暴露查树、等待、手动 dispatch/schedule 或自报进展工具；活动与终态由宿主事件写入。
