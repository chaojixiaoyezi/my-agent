...
主代理应用服务层。放 `SimpleAgent` 的主循环、子代理 runner、父代理 planner、宿主内部自动 dispatch/watch、编排工具和 runner 重试规则。`orchestration/planned_delegation.py` 负责已有 canonical Todo 时的创建前 exact covers、编码写入集合与父 workspace 原子预检，不承担质量或完成判定。模型侧只保留统一创建和直属下级的插话、打断、权限裁决，不暴露查树、等待、手动 dispatch/schedule 或自报进展工具；活动与终态由宿主事件写入。
...
允许：子代理状态机、工单文件、父子关系、结构化生命周期、子代理报告渲染。
`direct_parent_lifecycle.py` 是递归直属等待/唤醒与孩子结果上下文的权威位置；不得在 runner、prompt 或工具层再造第二份。
历史验收字段只能做旧账本兼容，不得恢复为完成门。
