...
class ProjectedHistoryRows(Sequence[Any]):
    ...
    def __getitem__(self, key):
        ...
        return self.project_row(self.rows[key])
...
# 相等比较按值：同一规则且逐行内容相等才相等（地址视图比较会重读行，只适合测试与校验，不能放进热路径）。
...
class ConversationHistorySource:
    ...
    # 函数用途: 取得按原规则投影后的历史行序列。
    def projected_rows(self) -> ProjectedHistoryRows:
        return ProjectedHistoryRows(self.rows, self.project_row)


# LLM: select 只能是宿主原有的单行选择规则；地址视图去掉准备期的取消回调，重放取消由运行时自己的边界负责。
# 函数用途: 在准备阶段冻结选择结果，只保留地址或原内存行引用，不物化正文。
def freeze_history_source(
    ...
    *,
    ...
    return ConversationHistorySource(selected, project_row)


# LLM: 与原具体种子的 messages 字段同规则：只保留 user/assistant 且正文非空的投影行，保持原顺序。
