...
# LLM: 插话持久操作经 guidance 领域组件； Goal revision CAS supplies idempotency. Definition updates use durable guidance, not a receipt pinned to an already active input turn.
...
def save_goal_objective(agent, store, goal, *, objective: str, expected_revision: int, child: bool = False):
    ...
    with store.goals.transition_guard(goal.thread_id):
