...
# LLM: Only typed normal completion may continue an exact active Goal. A stopped/limited Goal normalizes the result status; errors are never retried here.
# 函数用途: 子代理一轮自然回复后重新读目标；没完成则在原执行器继续，出错和停止不被目标掩盖。
def active_delegated_goal_after_turn(agent: object, task: object, result: object) -> object | None:
    if is_interrupted() or result_turn_end_reason(result) != "completed":
        return None
    goal = agent.conversation_store.load_goal(task.agent_thread_id, task_id=task.id)
    if goal is not None and goal.task_id == task.id and goal.status not in {"active", "complete"}:
        result.runtime_status = "interrupted" if goal.status == "paused" else "blocked"
...
def transition_delegated_goal(manager: object, task: object, *, expected_status: str, status: str) -> None:
    ...
    with store.goal_transition_guard(thread_id):
        goal = store.load_goal(thread_id, task_id=task.id)
        if goal is None or goal.task_id != task.id or goal.status != expected_status:
            return
        store.update_goal({"thread_id": thread_id, "goal_id": goal.goal_id,
