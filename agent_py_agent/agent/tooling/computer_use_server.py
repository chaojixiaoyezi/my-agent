...
# contract. Raising preserves MCP's structured isError path; never convert failures to prose here.
# 函数用途: 在指定屏幕坐标执行竖向滚轮；不填坐标时在当前鼠标位置滚动。
@mcp.tool()
def scroll_screen(clicks: int, x: int | None = None, y: int | None = None) -> dict[str, object]:
