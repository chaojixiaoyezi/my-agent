...
多动作,这里拆成三个独立工具,因为 my-agent 走 native tool_use + 精确 parameter_schema,
...
class ProcessStatusTool(BaseTool):
    ...
    spec = ToolSpec(
        ...
        avoid_when=[
            "想看所有后台进程总览时用 list_processes",
            "要终止进程时用 kill_process",
        ],
        keywords=["进程", "状态", "process", "status", "poll", "后台", "输出", "进度", "日志"],
        parameters={
            "session_id": "要查的后台进程 session_id(来自 run_command 后台返回或 list_processes)。",
        },
        parameter_details={
            "session_id": "必填字符串。run_command(run_in_background=true) 返回的 session_id,或 list_processes 列出的 session_id。",
        },
        parameter_schema={
    ...
    def execute(self, params: dict[str, Any]) -> ToolExecutionResult:
        ...
        if status is None:
            return _process_not_found(self.spec.name, session_id)
        return ToolExecutionResult(self.spec.name, True, json.dumps(status, ensure_ascii=False))
...
class KillProcessTool(BaseTool):
    ...
    spec = ToolSpec(
        ...
        parameters={
            "session_id": "要终止的后台进程 session_id(来自 run_command 后台返回或 list_processes)。",
        },
        parameter_details={
            "session_id": "必填字符串。run_command(run_in_background=true) 返回的 session_id,或 list_processes 列出的 session_id。",
        },
    ...
    def execute(self, params: dict[str, Any]) -> ToolExecutionResult:
        ...
        if result is None:
            return _process_not_found(self.spec.name, session_id)
        return ToolExecutionResult(self.spec.name, True, json.dumps(result, ensure_ascii=False))


def _process_not_found(tool_name: str, session_id: str) -> ToolExecutionResult:
    payload = {
        "ok": False,
