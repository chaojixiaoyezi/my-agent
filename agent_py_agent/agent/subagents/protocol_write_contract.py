...
def build_write_contract(task: SubAgentTask) -> dict[str, object]:
    ...
    return {
        "internal_task_root": current_model_ref(_task_text(task, "task_dir")),
        "product_write_roots": _product_write_roots(task, allowed_roots),
        "allowed_write_roots": allowed_roots,
        "forbidden_write_roots": _model_ref_list(_task_list(task, "forbidden_write_roots")),
        "locked_files": _model_ref_list(_task_list(task, "locked_files")),
    }
...
def _effective_allowed_write_roots(task: SubAgentTask) -> list[str]:
    roots = _model_ref_list(_task_list(task, "allowed_write_roots"))
    for grant in _task_list(task, "capability_grants"):
        if _grant_allows_filesystem_write(grant):
            roots = _merge_unique([*roots, *_model_ref_list(_grant_path_scope(grant))])
    return roots
...
def _model_ref_list(values: list[object]) -> list[str]:
    return _merge_unique([ref for value in values if (ref := current_model_ref(value))])
