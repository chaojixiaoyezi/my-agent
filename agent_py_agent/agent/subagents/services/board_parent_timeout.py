# LLM: Parent-timeout due-check predicates live here to keep generic due-check rules small.
# 模块用途: 检查父节点超时后是否仍有未完成子任务，并生成 refs-only 恢复 issue。
...
# LLM: check_parent_timeout_child_issues reports leader loss when a timed-out parent still owns children.
# 函数用途: 父节点超时后检查是否还有未完成子任务；只生成恢复建议，不自动接管或改写子任务。
...
# LLM: _unfinished_child_refs keeps parent timeout recovery metadata tiny and refs-only.
# 函数用途: 从 task_index 里找出父超时后仍需收口的子任务，只返回 id/status 摘要。
...
# LLM: _child_needs_parent_recovery defines when a child remains unsafe after parent timeout.
# 函数用途: 判断子任务是否还需要父级恢复；只有已验证 DONE、已接管或放弃才算收口。
...
# LLM: _child_ref_summary limits issue messages while preserving exact child refs.
# 函数用途: 压缩子任务 id/status 列表，避免 due-check 报告因为大树过长。
