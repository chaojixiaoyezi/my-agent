...
"""LLM: rule file loading and caching for log analysis detectors.
...
def load_rule(detector_id: str) -> Any:
    """LLM: load a single rule by detector ID with caching.
...
def clear_rule_cache() -> None:
    """LLM: clear the rule cache."""
    _RULE_CACHE.clear()


def preload_rules(detector_ids: list[str]) -> None:
    """LLM: preload multiple rules into cache.
