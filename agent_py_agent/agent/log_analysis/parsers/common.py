...
EXTRA_FIELD_NAME_MAP: dict[str, str] = {
    "告警ID": "alert_id",
    "告警编号": "alert_id",
    "事件ID": "alert_id",
    "事件去重键": "dedup_key",
...
ENGLISH_FIELD_NAME_MAP: dict[str, str] = {
...
def build_field_names() -> dict[str, str]:
    field_names: dict[str, str] = {}
    for stable_key in SECURITY_ALERT_V1_KEYS:
        field_names[stable_key] = stable_key
        field_names[header_token(stable_key)] = stable_key
    for mapping in (CHINESE_SECURITY_ALERT_FIELD_MAP, EXTRA_FIELD_NAME_MAP, ENGLISH_FIELD_NAME_MAP):
        for raw_key, stable_key in mapping.items():
            field_names[raw_key] = stable_key
            field_names[header_token(raw_key)] = stable_key
    return field_names
...
FIELD_NAME_MAP = build_field_names()
...
def stable_field_key(raw_key: str) -> str:
    ...
    return FIELD_NAME_MAP.get(key) or FIELD_NAME_MAP.get(header_token(key)) or _to_snake(key)
