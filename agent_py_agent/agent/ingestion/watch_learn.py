...
SAMPLE_GUIDANCE = (
    ...
    "触发/输入端字段不算);2) 取值判据(可组合)——样本里往往没有目标事件,优先学常态、"
    "盯常态之外:normal_values=结果端是干净枚举/布尔时把常态取值列全,取值不在集合内即抬;"
    "normal_value_contains=结果端是混了变化尾巴的文本时列常态【结论记号子串】(每条消息"
    "尾巴都不同,精确集合会失效),不含任何常态记号即抬;target_values/target_value_contains="
    "目标取值/记号明确已知时直接点名;3) ignore_fields=高基数噪声字段(distinct 接近样本数、"
...
CONFIGURE_GUIDANCE = (
    ...
    "画像并将重新预热。现在开始 pull 长轮询盯守;若后续格式漂移(长期零候选/候选明显不对),"
    "重新 action=sample 看分布、再 configure 覆盖旧判据。"
)
