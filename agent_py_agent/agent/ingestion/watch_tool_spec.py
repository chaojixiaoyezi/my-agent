...
_DESCRIPTION = (
    ...
    "按「引擎宽筛→你逐条重判」干活:可先 sample 抓样本、configure 提交结构化判据 spec"
    "(判据只是宽筛器,决定引擎多抬什么);pull 在代码层持续消费全部事件、宽抬候选"
    "(宁多勿漏),把每秒上百条压成每批几条给你研判——比逐条 web_fetch 快几十倍且"
    "游标不掉队。代码只做结构化匹配/计数,绝不定性;【候选是否真命中由你逐条重判"
    "说了算,不由判据说了算】(同时看触发端和结果端字段)。游标/统计/spec 跨轮持久,"
    "断线/补岗自动续。盯守数据流必须用本工具,勿自写轮询脚本(实测误报泛滥)。"
)
