...
_PARAMETERS = {
    ...
    "落盘和 checkpoint 提交仍由宿主完成。宿主不会传命令行参数；脚本每次只从 stdin"
    "读取一个 JSON 对象，其中 phase 字段决定本次动作，并只向 stdout 输出一个 JSON 对象。"
    "phase=plan 时输入 checkpoint/now_unix/page_limit/base_request，"
