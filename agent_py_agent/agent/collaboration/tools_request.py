# LLM: Request-level collaboration helpers back the consolidated collaboration tools.
# 模块用途: 为 raise/inspect/update_collaboration 提供请求创建、查询、更新逻辑。
...
def list_pending_requests(agent: SimpleAgent, params: dict[str, object]) -> ToolExecutionResult:
    identity = request_identity(agent, params)
    ...
    requests = agent.collaboration_store.pending_requests_for_agent(agent_id=identity["agent_id"], agent_name=identity["agent_name"], agent_role=identity["agent_role"], limit=limit_param(params.get("limit"), default=10))
