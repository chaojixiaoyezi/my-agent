# LLM: Evidence, participants, and decisions are append-only collaboration facts.
# 模块用途: 保存协作参与者、证据包和决策记录，不校验业务专项内容。
...
class CollaborationEvidenceStore(CollaborationBaseStore):
    ...
    def case_evidence(self, case_id: str) -> list[EvidencePacket]:
        return [
            EvidencePacket.from_dict(row)
            for row in read_jsonl(self._evidence_path(case_id))
            if isinstance(row, dict)
        ]

    def case_participants(self, case_id: str) -> list[CaseParticipant]:
        return [
            CaseParticipant.from_dict(row)
            for row in read_jsonl(self._participants_path(case_id))
            if isinstance(row, dict)
        ]
    ...
    def case_decisions(self, case_id: str) -> list[CaseDecision]:
        return [
            CaseDecision.from_dict(row)
            for row in read_jsonl(self._decisions_path(case_id))
            if isinstance(row, dict)
        ]
