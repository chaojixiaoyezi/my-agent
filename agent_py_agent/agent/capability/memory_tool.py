...
def build_remember_spec() -> ToolSpec:
    return ToolSpec(
        ...
        description=(
            "把【值得长期复用】的信息记进长期记忆,未来会话自动召回复用。"
            "**不必等用户说'记住'——对话中察觉到用户画像(身份/角色/称呼)、稳定偏好(风格/格式/语言)、"
            "长期约定、或踩过的坑/有效做法,就主动记下。** 用户明确要求记住时当然也记。"
        ),
        use_cases=[
            "对话中得知用户稳定的身份/角色/称呼/偏好,主动记下(不必等用户明说'记住')",
            "发现一个下次同类任务能直接复用的做法、或踩过的坑,主动沉淀",
            "用户明确要求'记住X/以后都Y'时",
        ],
        avoid_when=[
        ...
        keywords=["记住", "remember", "长期偏好", "以后都", "我喜欢", "保存偏好", "用户画像", "主动记", "用户是", "踩坑"],
        parameters={
