...
from ...common.json_io import read_json_object, read_jsonl_objects
...
def _candidate_payload(
    ...
    evidence_refs = dedupe_strings(_task_evidence_refs(task))
    artifact_refs = dedupe_strings(_task_artifact_refs(task))
    finding_ids = dedupe_strings(_finding_ids(task))
    scope = _scope(task)
...
def _lesson_texts(task: Any) -> list[str]:
    output = read_json_object(Path(str(getattr(task, "output_json", ""))))
    ...
    return dedupe_strings([str(item).strip() for item in lessons if str(item).strip()])
...
def _candidate_id(run_id: str, candidate_type: str, content: str) -> str:
    ...
    return f"memgate-{safe_path_segment(run_id, default='run', replacement='_')}-{digest}"
