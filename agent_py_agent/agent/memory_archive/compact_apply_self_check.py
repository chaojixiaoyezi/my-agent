# LLM: Compact apply self-check helpers; failure must block without rewriting source facts.
# 模块用途: 生成 memory compact apply 的自检报告和自检失败报告。
...
# LLM: build_self_check_payload validates apply artifacts and source continuity for manual compact apply.
# 函数用途: 确认 apply 产物写入路径、源内容保留策略、恢复引用和工作状态字段，返回机器可读报告。
...
# LLM: build_self_check_failure_payload is written only when apply artifacts fail validation.
...
# LLM: _self_checks keeps every check small and deterministic.
# 函数用途: 生成 hard/soft 检查列表；hard 失败阻断，soft 失败只记录缺口。
...
# LLM: _apply_ids_consistent checks filenames still point at the same concrete apply attempt.
# 函数用途: 确认关键 apply 产物文件名和 work_state 中的 apply_id 一致，避免恢复包串号。
...
# LLM: _artifact_refs_valid treats empty artifact refs as valid and checks path refs when present.
# 函数用途: 校验 work_state 中 artifact_refs 的路径是否仍存在；当前无 artifact 时不阻断手动 apply。
