# LLM: Parent shared context keeps collaboration briefs refs-first and bounded.
# 模块用途: 从父代理最近的小型工具读取结果里生成 context_packs，供新建子代理共享同一条简短线索。
...
from .runner_ref_fields import _file_refs_from_value, _normalize_file_ref
...
# LLM: append_parent_shared_context is a best-effort context handoff, not a validation gate.
# 函数用途: 给 create_subagents 参数追加父级近期小型读取 brief；不改变用户显式 context_packs。
...
# LLM: parent_task_directive_packs is prompt context only, not a hard gate or task template.
# 函数用途: 把父级原始用户任务作为有界上下文传给子代理，避免局部 goal 压缩后丢失协作、限制和交付要求。
...
# LLM: parent_shared_context_packs reads only current loop archive state.
# 函数用途: 从当前工具循环归档里抽取最近小型成功读取，避免 create_subagents 子任务丢失父级已知线索。
...
# LLM: refresh_parent_shared_context_cache bridges tool-record time to later orchestration tools.
# 函数用途: 工具归档后刷新父级共享 brief 缓存；后续 create_subagents 即使拿不到临时 params 也能继承。
...
# LLM: direct tool records are the strongest handoff source before archive/window reducers run.
# 函数用途: 工具刚成功返回时直接刷新父级共享 brief，避免小型读取结果还没被归档扫描到就创建子代理。
...
# LLM: shared_context_packs_from_archive is deterministic for one-off verification and replay.
# 函数用途: 将 archive_tool_calls 转成 bounded context_packs；只保留来源 ref、工具 ref 和短摘要。
