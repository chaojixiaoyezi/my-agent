# LLM: Delivery closeout group validation lives outside the main artifact module to keep size guard clean.
# 模块用途: 验收统一 artifact registry 里的文件组产物，并登记组级验收结果。
...
# LLM: ArtifactGroupValidationRequest bundles group validation inputs and avoids wide helper signatures.
# 类用途: 保存文件组验收所需的合同项、registry 记录、工作区、归档工具调用和 run id。
...
# LLM: GroupArtifactPayloadRequest keeps group payload rendering narrow for code-size guard.
# 类用途: 保存文件组产物 payload 所需字段；不改变验收语义。
@dataclass(frozen=True)
...
# LLM: validate_artifact_group_record validates every member before registering a group result.
# 函数用途: 将 registry 文件组记录转成 closeout artifact 报告，逻辑与单文件验收保持同一口径。
