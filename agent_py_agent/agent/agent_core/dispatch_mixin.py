        ...
        if planner:
            ...
            if planner_record and planner_record.step == "parent_planner":
                effective_runner_instruction = combine_runner_instruction(
                    runner_instruction,
                    getattr(planner_record, 'message', '').split('instruction:')[-1].strip() if 'instruction:' in planner_record.message else '',
                )
            ...
            if not ctx.apply:
                records.append(
                    self.subagents.make_dispatch_record(
                        ...
                        message=(
                            f"dry-run: 将重试 runner（{retry_reason}）。" if retry_reason
        ...
        if pending_runner_jobs:
            ...
            if pending_runner_jobs and runner_concurrency > 1 and execute_runners:
                completed = run_concurrent_runners(
                    self, pending_runner_jobs, runner_concurrency, runner_timeout_seconds,
                    effective_runner_instruction, execute_runners, max_cards, probe,
                )
                for run_id, before, retry_reason in pending_runner_jobs:
                    result, after = completed[run_id]
                    records.append(
                        _runner_dispatch_record(self, run_id=run_id, before=before, after=after,
                            result=result, retry_reason=retry_reason, execute_runners=execute_runners)
                    )
                ...
                for run_id, before, retry_reason in pending_runner_jobs:
                    ...
                    result = run_single_runner(
                        self, run_id, task_timeout, effective_runner_instruction,
                        execute_runners, max_cards, probe, retry_reason,
                    )
