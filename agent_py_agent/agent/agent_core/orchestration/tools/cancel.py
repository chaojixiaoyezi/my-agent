...
class CancelSubagentsTool(BaseTool):
    ...
    def execute(self, params: dict[str, object]) -> ToolExecutionResult:
        run_ids = _resolve_run_ids(self.agent, params)
        if not run_ids:
        ...
        for item in targets:
            ...
            if task is None:
                ...
                continue
            try:
                cancelled.append(_cancel_one(self.agent, task, params))
...
def _resolve_run_ids(agent: SimpleAgent, params: dict[str, object]) -> list[str]:
...
def _filter_existing_targets(agent: SimpleAgent, run_ids: list[str], status_filter: set[str]) -> list[dict[str, object]]:
    ...
    for run_id in run_ids:
        try:
            ...
            targets.append({"run_id": run_id, "error": runtime_error_report(exc, context="cancel_subagents.load")})
            continue
        if status_filter and str(task.status or "").upper() not in status_filter:
            continue
        targets.append({"run_id": run_id, "task": task})
    return targets
...
def _cancel_one(agent: SimpleAgent, task: SubAgentTask, params: dict[str, object]) -> dict[str, object]:
