...
from .orchestration_tool_specs import (
    build_raise_main_event_spec,
    build_raise_observation_spec,
)
...
class RaiseObservationTool(BaseTool):
    def __init__(self, agent: SimpleAgent):
        self.agent = agent
        self.spec = build_raise_observation_spec()

    def execute(self, params: dict[str, object]) -> ToolExecutionResult:
        resolved = _resolve_event_thread(self.agent, params, tool_name="raise_observation")
        if isinstance(resolved, ToolExecutionResult):
            return resolved
        thread_id, task_id = resolved
        lineage = _event_lineage_defaults(self.agent, task_id)
        observation = self.agent.conversation_store.append_observation({'thread_id': thread_id, 'event_type': str(params.get("event_type") or "observation"), 'summary': str(params.get("summary") or ""), 'urgency': str(params.get("urgency") or "normal"), 'severity': str(params.get("severity") or ""), 'source_agent_id': str(params.get("source_agent_id") or lineage["source_agent_id"]), 'parent_agent_id': str(params.get("parent_agent_id") or lineage["parent_agent_id"]), 'root_task_id': str(params.get("root_task_id") or lineage["root_task_id"]), 'evidence_refs': _string_values(params.get("evidence_refs")), 'requires_main_agent': bool(params.get("requires_main_agent", False)), 'requires_llm_report': bool(params.get("requires_llm_report", False)), 'metadata': _metadata_values(params.get("metadata"))})
        wake_signal_id = _maybe_raise_observation_wake(
            _ObservationWakeRequest(self.agent, params, thread_id, observation)
        )
        return _event_tool_result("raise_observation", _EventResultRefs(
            thread_id=thread_id,
            task_id=task_id,
...
class RaiseMainEventTool(BaseTool):
    def __init__(self, agent: SimpleAgent):
        self.agent = agent
        self.spec = build_raise_main_event_spec()

    def execute(self, params: dict[str, object]) -> ToolExecutionResult:
        resolved = _resolve_event_thread(self.agent, params, tool_name="raise_main_event")
        if isinstance(resolved, ToolExecutionResult):
            return resolved
        thread_id, task_id = resolved
        lineage = _event_lineage_defaults(self.agent, task_id)
        urgency = str(params.get("urgency") or "urgent")
        observation = self.agent.conversation_store.append_observation({'thread_id': thread_id, 'event_type': str(params.get("event_type") or "main_event"), 'summary': str(params.get("summary") or ""), 'urgency': urgency, 'severity': str(params.get("severity") or ""), 'source_agent_id': str(params.get("source_agent_id") or lineage["source_agent_id"]), 'parent_agent_id': str(params.get("parent_agent_id") or lineage["parent_agent_id"]), 'root_task_id': str(params.get("root_task_id") or lineage["root_task_id"]), 'evidence_refs': _string_values(params.get("evidence_refs")), 'requires_main_agent': True, 'requires_llm_report': bool(params.get("requires_llm_report", True)), 'metadata': _metadata_values(params.get("metadata"))})
        signal = self.agent.conversation_store.raise_wake_signal({'thread_id': thread_id, 'observation': observation, 'urgency': urgency, 'reason': str(params.get("event_type") or "main_event"), 'dedupe_key': str(params.get("dedupe_key") or "")})
        return _event_tool_result("raise_main_event", _EventResultRefs(
            thread_id=thread_id,
            task_id=task_id,
            observation_id=observation.observation_id,
...
def _maybe_raise_observation_wake(request: _ObservationWakeRequest) -> str:
    if str(request.params.get("urgency") or "").strip().lower() != "urgent":
        return ""
