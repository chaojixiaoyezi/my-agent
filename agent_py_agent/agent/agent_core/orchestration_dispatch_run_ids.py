# LLM: Dispatch run-id parsing stays structural and keeps one public id field.
# 模块用途: 解析 dispatch_subagents 的 run_ids 和 direct_children 范围。
...
# LLM: dispatch_include_run_ids_param accepts only the canonical model-facing id field.
# 函数用途: 只解析 run_ids 和 direct_children，避免多套 run id 字段长期并存。
