# LLM: Coordinator runner policy text lives outside runner_prompts so prompt edits do not bloat the builder.
# 模块用途: 提供 coordinator/lead 专属执行规则文本；角色偏好不剥夺基础读写和派工能力。

from __future__ import annotations


# LLM: coordinator_execution_policy_lines returns stable prompt lines for lead-style subagents.
# 函数用途: 生成 coordinator/lead 的派工、验收、refs-first 和通信规则，供 runner prompt 拼接使用。
def coordinator_execution_policy_lines() -> list[str]:
    return [
        "- coordinator/lead 节点拥有完整基础读写能力：可以写自己的计划、证据、协调报告，也可以在授权产物根里检查、修复或接管。",
        "- coordinator/lead 的第一目标是让团队动起来：先读取最小必要材料来理解目标、目录、评分和验收边界，"
        "不要在派工前把所有正文、数据表、长报告都自己读完。能拆给 child 的研究、实现、测试和验收，先创建并 dispatch child。",
