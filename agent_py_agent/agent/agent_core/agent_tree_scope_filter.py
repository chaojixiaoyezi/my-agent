# LLM: Tree scope filters keep main-run fallback from showing stale historical subagents.
# 模块用途: 按本轮 remembered run_ids 过滤 inspect_agent_tree 回退快照，并从节点生成状态桶。
...
# LLM: visible_nodes lets background wakeups render only the current thread's active task ids.
# 函数用途: 按结构化 run_id 白名单裁剪展示副本；不改变真实任务树和调度候选。
