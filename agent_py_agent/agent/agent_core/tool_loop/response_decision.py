...
#   门槛5 重试救回来了，但模型只回一句承诺就停」，且**本轮确实已执行过工具**（纯问答轮
...
#   ①「产出这条响应的那次物理模型调用所属的 model turn 序号被生成层登记为『本轮发生过被
#   重试救回的供应商超时』」（live_archive_state 上的结构化字段，与账本同源）；
#   ②「本轮确实做过工作——run 级工具账本 executed_tools 非空（收窄 R1：纯问答轮零工具执行，
#   被救回的那一枪就是完整终答，再追问一次会把用户本该看到的正文顶掉）」。
#   两条都不读模型正文——把承诺换成任意其它文本，行为完全一致。
