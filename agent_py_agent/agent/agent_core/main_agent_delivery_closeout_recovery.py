...
# 函数用途: 从结构化 finding value 和产物路径中提取可写目标文件，不让模型从自然语言日志里猜路径。
...
# LLM: _finding_file_targets extracts leading file refs like `index.html:...` from structured findings.
# 函数用途: 只接受产物目录内的相对文件名，避免路径越界或从说明文本任意取路径。
...
# LLM: _existing_text_targets gives directory artifacts a bounded set of files that can be patched.
# 函数用途: 当 finding 只给 DOM id 等非文件位置时，提供现有文本文件候选，避免 required_tool_calls 为空。
def _existing_text_targets(artifact_path: Path) -> list[Path]:
    suffixes = {".css", ".html", ".js", ".json", ".md", ".txt", ".yaml", ".yml"}
    try:
        files = [path for path in artifact_path.rglob("*") if path.is_file() and path.suffix.lower() in suffixes]
    except OSError:
        return []
    return sorted(files, key=lambda path: (len(path.parts), str(path)))[:8]


# LLM: _safe_artifact_child resolves a relative file target under the artifact root.
# 函数用途: 保护 repair_targets 不越出产物目录；绝对路径和 `..` 直接丢弃。
...
# LLM: _safe_artifact_related_path resolves validator file refs near the artifact without trusting prose.
# 函数用途: 支持 finding.location 里的 workspace-relative 文件 ref，同时拒绝绝对路径和 `..` 越界。
...
# LLM: _candidate_repair_roots walks from artifact folder outward so workspace-relative refs resolve correctly.
# 函数用途: 将 `outputs/.../file.md` 这类结构化 location 映射到真实工作区文件。
...
# LLM: _unique_paths deduplicates path candidates without changing their first-use priority.
# 函数用途: 输出稳定且有界的 repair target 列表。
