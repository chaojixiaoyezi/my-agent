# LLM: Child result index keeps parent orchestration from guessing child report paths.
# 模块用途: 从真实子代理任务账本生成小型结果索引，供 create_subagents 和 inspect_agent_tree 共用。
...
def _child_result_row(task: object) -> dict[str, object]:
    ...
    primary_artifact_refs = _primary_artifact_refs(task, artifacts, expected_outputs)
    return {
...
def _primary_artifact_refs(task: object, artifacts: list[dict[str, object]], expected_outputs: list[str]) -> list[str]:
    refs: list[str] = []
    for row in artifacts:
        path = current_model_ref(row.get("path"))
        if path:
            refs.append(path)
    refs.extend(_artifact_paths_from_output(_output_payload(task)))
...
def _output_payload(task: object) -> dict[str, object]:
...
# LLM: _current_registry_refs applies current-path projection to registry rows as a batch.
# 函数用途: 批量过滤 child result index 中 artifact registry 的旧路径字段。
...
# LLM: _current_registry_row removes stale path fields without losing artifact identity.
# 函数用途: artifact registry 行保留 artifact_id 等字段，旧 data/subagents path 不进入父级结果索引。
