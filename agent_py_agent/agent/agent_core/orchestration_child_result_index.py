...
def _child_result_node_row(node: dict[str, object]) -> dict[str, object]:
    ...
    primary_refs = [str(item.get("path") or "").strip() for item in registry_refs if str(item.get("path") or "").strip()]
    primary_refs.extend(artifact_refs)
    return {
...
def _expected_outputs(attrs: dict[str, object]) -> list[str]:
    ...
    for key in ("output_files", "output_refs", "artifact_refs"):
        ...
        if isinstance(value, list):
            result.extend(str(item).strip() for item in value if str(item or "").strip())
