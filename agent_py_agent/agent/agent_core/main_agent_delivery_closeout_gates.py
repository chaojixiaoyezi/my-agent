# LLM: Closeout gate attachment is the structured bridge between artifact reports and final task status.
# 模块用途: 统一写入 run/runtime/state/quality/fact/acceptance/final gate，不读取自然语言结论。
...
from .main_agent_delivery_closeout_gate_recovery import attach_contract_recovery
...
# LLM: CloseoutGateRequest bundles gate attachment inputs without importing the parent closeout module.
# 类用途: 避免 gate helper 参数膨胀，把 report/contract/workspace 放进一个结构化请求。
...
# LLM: attach_closeout_gates writes every mandatory gate payload into the closeout report.
# 函数用途: 统一添加 run/runtime/state/acceptance/final gate，主流程只看 gate 决策。
