...
# 模块用途: 组合静态目录、显式安装/配置及原请求查询，不初始化完整 Agent 或插件进程。
...
# 类用途: 只给安装/配置服务传所需身份、权限和存储依赖，不携带完整 Agent。
...
# LLM: 冷入口与完整代理共用 owner 权限裁决；安装与配置分别受工具禁用策略约束，查询仍绑定原身份。
...
class PluginManagement:
    ...
    def _catalog(self, entries):
        ...
        actions = tuple(replace(action, available=(
            ...
            or action.name in {"install", "configure"} and context.enabled and context.is_admin
            and self._allowed(PLUGIN_INSTALL_TOOL if action.name == "install" else PLUGIN_CONFIGURE_TOOL)
    ...
    def command(self, text: str, *, revision: str, request_id: str) -> dict:
        ...
        if parsed.action.name in {"install", "configure"}:
            tool_name = PLUGIN_INSTALL_TOOL if parsed.action.name == "install" else PLUGIN_CONFIGURE_TOOL
    ...
    # LLM: 两个明确动作共用原请求链；认证和开关先于新写入，原请求先读账，旧目录不引发换 ID 或来源重读。
    # 函数用途: 提交安装或完整配置请求，首次请求必须携带已见目录版本。
    ...
    # LLM: 两个 handler 共用原执行器；配置冻结同次目录/安装读，原锁内继续 CAS，模型不可见不豁免工具禁用。
    ...
    def _prepare(self, repo: RuntimeRepository, binding, arguments: dict) -> ToolExecutorRequest:
        ...
        if tool_name == PLUGIN_INSTALL_TOOL:
        ...
        elif tool_name == PLUGIN_CONFIGURE_TOOL:
            ...
            tool = PluginConfigureTool(self.installations, context.path_policy, context.workspace,
                                       binding.request.operation_id, existing, self._catalog(entries).revision)
        else:
    ...
    # 函数用途: 让安装和配置分别遵守当前禁用策略。
    def _allowed(self, tool_name: str) -> bool:
        return {PLUGIN_INSTALL_TOOL: self.context.tool_allowed,
                PLUGIN_CONFIGURE_TOOL: self.context.configure_allowed}.get(tool_name, False)
    ...
    def _reply(self, payload: dict, request_id: str = "") -> dict:
        ...
        if envelope:
            payload["details"] = envelope.get("plugin_install", envelope.get("plugin_configure", {}))
        state = payload.get("state", "")
        messages = {"succeeded": "插件管理请求已完成，插件当前停用。", "failed": "插件管理请求失败，请核对原因。",
