...
@dataclass(frozen=True)
class RuntimeStatusReadReport:
    payload: dict | None
    load_error: dict | None = None


def _base_runtime_payload(status_path: Path) -> dict:
    status_report = read_runtime_status_report(status_path)
    if status_report.payload:
        return status_report.payload
    payload = {
    ...
    if status_report.load_error is not None:
        payload["previous_status_load_error"] = status_report.load_error
    return payload
...
def read_runtime_status(status_path: Path) -> dict | None:
    return read_runtime_status_report(status_path).payload


def read_runtime_status_report(status_path: Path) -> RuntimeStatusReadReport:
    if not status_path.exists():
        return RuntimeStatusReadReport(None)
    report = read_json_file_report(status_path, context="gateway.runtime_status.read")
    if report.load_error is not None:
        return RuntimeStatusReadReport(None, report.load_error)
    return RuntimeStatusReadReport(report.payload)
