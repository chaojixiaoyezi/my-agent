...
def read_pid_file(pid_path: Path) -> int | None:
    ...
    try:
        ...
        if not content:
            return None
        # Try JSON format first (new Hermes-style record)
...
def write_pid_record(pid_path: Path) -> None:
    """Write current process PID and metadata to the PID file (Hermes-style)."""
...
def get_running_pid(pid_path: Path, *, cleanup_stale: bool = True) -> int | None:
    ...
    Uses start_time to detect PID reuse (Hermes pattern).
...
def remove_pid_file_if_owned(pid_path: Path) -> None:
    """Remove the PID file, but only if it belongs to this process (Hermes pattern).
...
def write_pid_file(pid_path: Path, pid: int) -> None:
    ...
    For Hermes-style PID records with metadata, use write_pid_record() instead.
    """
    pid_path.parent.mkdir(parents=True, exist_ok=True)
