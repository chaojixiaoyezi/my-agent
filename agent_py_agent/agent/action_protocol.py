...
class SubagentScheduleEnvelope:
    ...
    pending_start_run_ids: list[str] = field(default_factory=list)
    start_accepted_run_ids: list[str] = field(default_factory=list)
    ...
    def from_dict(cls, payload: dict[str, Any]) -> SubagentScheduleEnvelope:
        return cls(
            ...
            pending_start_run_ids=string_list(payload.get("pending_start_run_ids")),
            start_accepted_run_ids=string_list(payload.get("start_accepted_run_ids")),
            ...
            start_status=str(payload.get("start_status") or ""),
            lifecycle_counts={
...
def subagent_schedule_envelope_from_payload(
    ...
    *,
    ...
    return SubagentScheduleEnvelope(
        ...
        reused_run_ids=string_list(payload.get("reused_run_ids")),
        pending_start_run_ids=string_list(payload.get("pending_start_run_ids")),
        start_accepted_run_ids=string_list(lifecycle.get("start_accepted_run_ids")),
        ...
        start_status=str(lifecycle.get("start_status") or ""),
        lifecycle_counts={
            str(key): _int_or_zero(value)
...
# LLM: Decode only current durable envelope kinds; removed orchestration-tool kinds must fail closed.
# 函数用途: 把持久化动作字典还原成当前协议对象，遇到已删除或未知类型时明确报错。
