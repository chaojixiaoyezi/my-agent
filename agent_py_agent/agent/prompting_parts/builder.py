        ...
        *,
        ...
        if _tools.tool_catalog_section:
            system_prompt = (
                ...
                "\n"
                "# 收口审计纪律（宣告完成前必读）\n"
                "- 宣告任务完成前，必须逐条证明：从任务要求出发，用当前工作区、"
                "命令输出、测试结果的真实证据覆盖每一项要求（文件存在且内容正确、"
                "命令可执行、测试通过）。\n"
                "- 不采信意图、部分进展、对早期工作的记忆或「看起来合理」的最终答案"
                "——只有当前状态的证据能证明完成；证据不足、间接或缺失时继续工作，"
                "不得宣告完成。\n"
                "- 工具结果出现「副作用未知/超时/执行者死」的未知状态时，必须先核验"
                "实际状态（检查文件/进程/重跑命令）再决定是否完成；无法核验的，"
                "明确报告未知，不得宣告完成。\n"
            )
