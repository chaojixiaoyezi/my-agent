...
# LLM: TaskRunSubprocessRequest bundles the process contract for one case run.
# 类用途: 描述要启动的命令、工作目录、日志文件、任务工作区和超时边界。
@dataclass(frozen=True)
class TaskRunSubprocessRequest:
    command: list[str]
    cwd: Path
    stdout_path: Path
    stderr_path: Path
...
# LLM: TaskRunSubprocessResult is the structured process outcome consumed by execution reports.
# 类用途: 保存进程退出码、耗时和超时类型；日志正文只通过文件引用读取。
@dataclass(frozen=True)
class TaskRunSubprocessResult:
    exit_code: int
...
# LLM: run_task_subprocess executes without shell and keeps logs observable while the generic task runs.
# 函数用途: 启动真实任务子进程，把输出写入文件，并在总超时或长时间无活动时终止进程组。
def run_task_subprocess(request: TaskRunSubprocessRequest) -> TaskRunSubprocessResult:
...
def _wait_for_process(
    process: subprocess.Popen,
    request: TaskRunSubprocessRequest,
    started: float,
...
def _activity_marker(request: TaskRunSubprocessRequest) -> tuple[int, int, float]:
    return (
...
def _process_result(
    ...
    *,
    ...
    return TaskRunSubprocessResult(
        exit_code=exit_code,
