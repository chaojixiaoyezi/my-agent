...
ERROR_CONTRACTS: dict[str, ErrorContract] = {
    ...
    "GATEWAY_RESTART_NOT_HOSTED": ErrorContract(
        code="GATEWAY_RESTART_NOT_HOSTED",
        category="tool",
        retryable=False,
        recommended_action=RecoveryAction.REPORT_BLOCKER.value,
        recovery_hint="当前会话不在 Gateway 进程内，无法安排安全重启；告诉用户由管理员在终端执行 my-agent gateway restart，不要用 shell 自停或 kill。",
    ),
    "GATEWAY_RESTART_ADMIN_ONLY": ErrorContract(
        code="GATEWAY_RESTART_ADMIN_ONLY",
        category="permission",
        retryable=False,
        recommended_action=RecoveryAction.REPORT_BLOCKER.value,
        recovery_hint="只有本机管理员（或已绑定为管理员的 IM 身份）可以安排 Gateway 重启；请联系管理员处理。",
    ),
    "GATEWAY_RESTART_COOLDOWN": ErrorContract(
        code="GATEWAY_RESTART_COOLDOWN",
        category="state",
        retryable=False,
        recommended_action=RecoveryAction.REPORT_BLOCKER.value,
        recovery_hint="刚完成过一次安全重启，仍在冷却；先确认刚才的重启是否已解决问题，确需再重启时等 retry_after_seconds 后再调用，不要循环重启。",
    ),
    "GATEWAY_RESTART_LOOP_GUARD": ErrorContract(
        code="GATEWAY_RESTART_LOOP_GUARD",
        category="state",
        retryable=False,
        recommended_action=RecoveryAction.REPORT_BLOCKER.value,
        recovery_hint="同一会话短时间内已多次安排重启，已被防循环拦下；停止重启，把排查结果和怀疑原因汇报给用户，由用户决定下一步。",
    ),
