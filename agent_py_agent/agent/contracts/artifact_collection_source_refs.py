...
def source_ref_is_audited(item: dict[str, object]) -> bool:
    if str(item.get("artifact_ref") or "").strip():
        return True
    if _has_operation_binding(item):
        return True
    return _has_fetch_binding(item)


def _has_operation_binding(item: dict[str, object]) -> bool:
    return any(str(item.get(key) or "").strip() for key in ("tool_call_id", "operation_id", "tool_result_id"))


def _has_fetch_binding(item: dict[str, object]) -> bool:
    source_type = str(item.get("source_type") or "").strip()
    content_hash = str(item.get("content_sha256") or "").strip()
    has_http_status = item.get("http_status") is not None
