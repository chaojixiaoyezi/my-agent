# LLM: Document content quality gate validates artifact substance through structured contracts.
# 模块用途: 对 Markdown/DOCX/PDF/HTML/TXT 文档做通用内容厚度检查，不写任务专项规则。
...
from .document_content_extractors import (
...
# LLM: evaluate_document_content_quality_gate is the public gate API for document artifacts.
# 函数用途: 只根据 artifact_ref、workspace_root 和 document_quality_contract 判断文档是否足够充实。
    ...
    *,
    ...
    if path is None:
        return GateDecision.repair(
            ...
            recommended_action="repair_document_artifact",
        )
...
# LLM: document_content_quality_findings returns reusable GateFinding rows for artifact validators.
# 函数用途: 提取文档结构事实并按合同阈值生成稳定 finding code。
def document_content_quality_findings(
    ...
    *,
    ...
    try:
        ...
        return [_finding("DOCUMENT_READ_FAILED", artifact_ref=str(path), current_state={"error": str(exc)})]
    except (BadZipFile, ParseError) as exc:
        return [_finding("DOCUMENT_FORMAT_INVALID", artifact_ref=str(path), current_state={"error": str(exc)})]
    return [
...
def _required_section_findings(path: Path, facts: DocumentContentFacts, contract: dict[str, Any]) -> list[GateFinding]:
    required = _string_list(contract.get("required_sections"))
        ...
        if min_chars and chars < min_chars:
            findings.append(
                _finding(
                    ...
                    artifact_ref=str(path),
                    location={"section": section.heading, "start_line": section.start_line, "end_line": section.end_line},
                    current_state={"text_chars": chars},
                    required_state={"min_chars_per_section": min_chars},
                    message="Document section content is thinner than the declared contract.",
                )
            )
    return findings
...
def _placeholder_findings(path: Path, facts: DocumentContentFacts, contract: dict[str, Any]) -> list[GateFinding]:
    ...
    return [
        _finding(
            ...
            message="Document contains too much placeholder content.",
        )
    ]
