...
# LLM: _dangerous_pattern_findings keeps this contract helper structure-first and stable.
# 函数用途: 支撑本模块的机器字段校验、转换或汇总，不读取普通自然语言作为事实。
def _dangerous_pattern_findings(argv: tuple[str, ...]) -> tuple[list[CommandPolicyFinding], set[int]]:
    ...
    command_positions = _command_positions(argv)
    for position in command_positions:
        executable = command_name(argv[position])
        args = _command_args(argv, position)
        if executable == "rm" and _rm_recursive_force(args):
            covered_positions.add(position)
            findings.append(
                CommandPolicyFinding(
                    "COMMAND_DANGEROUS_PATTERN_BLOCKED",
                    {"pattern": "RM_RECURSIVE_FORCE", "executable": executable},
                )
            )
        if executable == "chmod" and _chmod_world_writable(args):
            covered_positions.add(position)
            findings.append(
                CommandPolicyFinding(
                    "COMMAND_DANGEROUS_PATTERN_BLOCKED",
                    {"pattern": "CHMOD_WORLD_WRITABLE", "executable": executable},
                )
            )
...
# LLM: _rm_recursive_force keeps this contract helper structure-first and stable.
# 函数用途: 支撑本模块的机器字段校验、转换或汇总，不读取普通自然语言作为事实。
def _rm_recursive_force(args: list[str]) -> bool:
    lowered_args = tuple(arg.lower() for arg in args)
    recursive = any(arg in {"--recursive", "--dir"} for arg in lowered_args)
    recursive = recursive or any(_has_short_flag(arg, "r") for arg in lowered_args)
    force = "--force" in lowered_args or any(_has_short_flag(arg, "f") for arg in lowered_args)
...
# LLM: _chmod_world_writable keeps this contract helper structure-first and stable.
# 函数用途: 支撑本模块的机器字段校验、转换或汇总，不读取普通自然语言作为事实。
def _chmod_world_writable(args: list[str]) -> bool:
    return any(arg.strip() in {"777", "0777"} for arg in args)
...
def _is_dangerous_executable(executable: str) -> bool:
    return executable in _DANGEROUS_EXECUTABLES or executable.startswith("mkfs.")
