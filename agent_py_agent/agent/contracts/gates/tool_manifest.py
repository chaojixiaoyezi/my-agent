# LLM: Tool manifest gates validate registered tool metadata before any tool can execute.
# 模块用途: 校验工具声明是否包含 effect、参数 schema、幂等策略等机器字段，避免执行入口靠提示词兜底。
...
# LLM: ToolManifestFacts is the stable metadata gate input for one registered tool.
# 类用途: 保存工具注册声明里的 effect、参数 schema、幂等要求和超时等机器事实。
...
# LLM: evaluate_tool_manifest_gate rejects incomplete tool metadata at the registry boundary.
# 函数用途: 校验工具注册合同本身完整；失败时阻止工具执行或进入可见工具清单。
...
# LLM: tool_manifest_from_spec converts ToolSpec-like objects without relying on rendered prompt text.
# 函数用途: 从注册表里的 spec 属性读取结构字段，生成 ToolManifestFacts 供执行门校验。
...
# LLM: _manifest_facts normalizes mapping input for tests and legacy adapters.
# 函数用途: 允许调用方传 dict，但立即转成统一 dataclass，后续只读结构字段。
...
# LLM: _int_or_zero keeps timeout handling deterministic for malformed manifest fields.
# 函数用途: 将 timeout_seconds 转成整数；不能转换时返回 -1 触发 manifest finding。
