...
# LLM: prompt_for_case appends machine delivery contracts to the human task prompt.
# 函数用途: 把 expected_artifacts/acceptance refs 作为 JSON 合同传给主代理，避免真实任务只在 stdout 输出。
def prompt_for_case(case: MainAgentRealTaskCasePlan, *, workspace: Path) -> str:
    user_prompt = (workspace / case.prompt_ref).read_text(encoding="utf-8")
