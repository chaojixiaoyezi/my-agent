...
# LLM: TaskRunRecoveryPacketRequest bundles everything needed to write one packet.
# 类用途: 描述失败任务的状态、原因、引用路径和验收报告，避免从 stdout 自然语言猜恢复点。
@dataclass(frozen=True)
class TaskRunRecoveryPacketRequest:
    case_id: str
    title: str
    status: str
    exit_code: int | None
...
def _acceptance_payload(acceptance: TaskRunAcceptanceReport) -> dict[str, object]:
    return {
