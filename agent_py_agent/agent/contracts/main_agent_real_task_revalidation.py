# LLM: Real-task revalidation is a thin wrapper over the shared runtime revalidator.
# 模块用途: 保留旧 real_task 复验入口，实际报告读取和产物复验走统一 runtime revalidation。

from __future__ import annotations

from pathlib import Path

from .main_agent_real_task_execution_models import MainAgentRealTaskExecutionReport
...
def revalidate_main_agent_real_task_execution(
    ...
    *,
    ...
    return revalidate_main_agent_task_runtime(
        report_path,
        workspace=workspace,
        runtime_adapter=real_task_runtime_adapter(),
    )
