# LLM: Main-agent task execution models stay separate from subprocess orchestration.
# 模块用途: 定义通用任务受控执行的请求、单项结果和总报告数据结构。
...
SCHEMA_VERSION = "main-agent-task-execution.v1"


# LLM: MainAgentTaskExecutionRequest bundles execution controls and avoids ad-hoc subprocess args.
# 类用途: 描述真实任务执行入口需要的工作区、并发、超时、配置和 case 过滤条件。
@dataclass(frozen=True)
class MainAgentTaskExecutionRequest:
    workspace: Path
    max_workers: int = 4
...
# LLM: MainAgentTaskExecutionCaseResult records one planned or executed subprocess.
# 类用途: 保存单个主代理真实任务的命令、配置、日志、退出码和问题引用。
@dataclass(frozen=True)
class MainAgentTaskExecutionCaseResult:
    case_id: str
    title: str
    status: str
    worker_slot: int
...
# LLM: MainAgentTaskExecutionReport summarizes controlled execution without hiding failures.
# 类用途: 保存真实任务执行总报告；后续 CLI/Card Runtime 可以直接读取。
@dataclass(frozen=True)
class MainAgentTaskExecutionReport:
    ok: bool
    schema_version: str
    execution_mode: str
