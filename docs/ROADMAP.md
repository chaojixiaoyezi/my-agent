...
状态：首版 `57e91cb` 失败；第二候选 `4247582` 只修正结论、未守住只读；第三候选本地待严格 gate/真 TUI

解决问题：真实超级玛丽任务中，模型把测试机本机 HTTP 200 和 `0.0.0.0` 监听错误说成另一台电脑已经能
访问；即使用户明确要求“不能就说真实原因、不要把本机访问说成局域网访问”，后续模型仍重复误报。

当前进展：第二候选真 TUI 已能写清“本机已验证、另一机器未验证”，但仍在只核对时启动服务。第三候选
参考 终端交互 的 Bash 工具说明，把同一动作授权段追加到可能有副作用的原生工具定义；判断只读取工具
已有的结构化 effect policy，不读任务文字，纯只读工具不加，运行时也不做语义硬拦。授权正文只维护一份，
同时供完整 Prompt 末尾和工具说明使用。部署后必须从原任务 cwd 用同一普通中文 follow-up 复测；若仍
擅自改变状态，就如实记录软提示在当前模型上的上限，不继续堆重复提示或恢复机器验收。
