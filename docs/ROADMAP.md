...
状态：首版 `57e91cb` 真 TUI 失败；第二候选 196 passed、7 个既有 xfail 与严格 gate 通过，待部署复验

解决问题：真实超级玛丽任务中，模型把测试机本机 HTTP 200 和 `0.0.0.0` 监听错误说成另一台电脑已经能
访问；即使用户明确要求“不能就说真实原因、不要把本机访问说成局域网访问”，后续模型仍重复误报。

当前进展：首版抽象边界虽让模型先想到“不能从另一机器验证”，仍在本机 200 后过度断言并擅自重启服务。
第二候选把唯一规则放到完整 Prompt 最末，明确本机监听/`0.0.0.0`/localhost 不是跨机证据，并补齐只检查
时不得修改的通用授权边界。规则不含项目分支、不解析正文、不改变任务终态，也不恢复用户已否决的机器
验收。部署后必须从原任务 cwd 用同一普通中文 follow-up 复测；若仍越界，只记录软提示不稳定，再与用户
讨论更强但仍非语义裁判的结构化事实展示。
