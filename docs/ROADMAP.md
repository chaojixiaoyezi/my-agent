...
- 状态：141 focused 通过；摘要覆盖的连续旧工具前缀与旧快照一起回收，用户输入/最新状态和真实账本保留。
  待 `.10` 原 TUI 部署复验，再用真实完整上下文与 provider 缓存账评估效果；不恢复次数硬上限。
...
- 解决问题：有实际进展的长 child 在第 8 次 Compact 后被判失败，后续清理又掩盖失败状态。
  ...
  自动父级清理不覆盖已有终态。状态：98 项定向验证通过，真实 TUI 超过原上限的复验仍待完成。
  原 child5 已到 Compact 10、保持相同 run/attempt 继续工具执行；最终完成仍未验收。
