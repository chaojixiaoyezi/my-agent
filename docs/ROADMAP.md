...
会话选择与用户默认已分离；管理员共享目录只保存显式引用、不复制凭证。
详见 [会话模型选择](design/SESSION_MODEL_SELECTION.md) 与 [共享模型目录](design/SHARED_MODEL_CATALOG.md)。
完整显示分页、回到最新、思考工具边界、错误终态和 Bash 原文采集已集成并做真实 TUI 验收。
模型原始流异常、极端慢模型性能、后台空闲扫描和用量口径仍需独立验证，详见 [验收表](audits/SESSION_UI_ACCEPTANCE.md)。

## R230 同轮整合与原会话验收

解决成功工具复读漏提醒、源码误脱敏、显示操作号、复制乱序、OpenAI 长参数等待不透明及永久协调角色冲突。
整合实现与真实现场分开记录，见 [R230](audits/R230_CONSOLIDATED_FIXES.md)；旧会话 400 和模型单响应复读不能以新会话通过关闭。

## R230 模型采样配置（独立切片，待主线真实 TUI 验收）

解决已知 工具运行时/DeepSeek V4 Flash 默认采样差异和用户无法显式配置 top_p 的缺口。
贯通 YAML、/model、旧扁平新增/迁移、工作片缓存及三协议出站；未知端点默认不加参数。
本切片不操作 Gateway/真实模型或会话身份，不把该差异写成旧 HTTP 400/复读的根因。
主线合并后再在单 Gateway 真 TUI 核验新增、保存、切换和实际请求，见模型配置设计。

## R229 现场故障（分批真 TUI 复验 / 本机已部署）

优先修复连续请求拒绝和重复输入展示；超长重复、读取投影、F6 历史、长等待与自主推进仍保留。
.10 新会话工具/两 child/插话通过；本机旧会话仍 400，先查真实出站协议，不跳过失败直接宣称全部修复。
证据以 [R229 台账](audits/R229_LIVE_FAILURES.md) 为准，不把协议修复等同所有长任务通过。

## R228 工程修复收尾（已验，行为与上游开放项另跟踪）

解决 R227 Context 旧容量回弹、诊断委派扩权、无解释 400 被误重试；原长会话又复现分段摘要失败，
现已修复并完成同会话 264.7K → 35.4K 的 TUI 续接。整理开发入口与过时路径/审批说明、修复失效链接，
补齐第三方二进制对应源码和许可。最终 539 项 focused、本地严格 gate 与独立包核验已通过；
按用户要求收尾提交，不追加长任务或创建 Release。工具运行时 上游 400 的具体拒绝原因、Todo 代次和
业务误诊仍在 [R228 台账](audits/R228_RELEASE_CANDIDATE.md#尚未关闭)，不得因工程门通过而标成已解决。

## R227 当前真实会话故障（开发中）

优先修复切模型后的探针拒绝、跨窗口 Compact 失败与误导错误提示；排查模型主动结束但计划未完。
同时修复正文误灰、默认 child PTY 缺口及 Full Access 下 Shell 备份边界误判。
用户后续授权本机部署重启，原任务文件和记忆保留；两台单 Gateway，官网 MiniMax-M2.7 为默认。
工具运行时 Flash 大窗口 → MiniMax 已完成压缩，但首轮目录续接失败，原样工具路径及 runs 误导已补修。
第二轮 438K → 34K 后定位与普通 Shell 已通过；误用 PTY 句柄导致零写入 UNKNOWN 的错误已补修，
原 TUI 继续到冒烟 19/19 和两 child 实现。系统/工具提示去重与七类冲突调整已落地，
固定子集约减少 5.3K，不冒充完整请求或账单节省。仍需关闭 Context 容量旧投影、Todo 代次、
工具运行时 运行中另一个 HTTP 400 和诊断委派扩大修复的行为问题，见 [R227 台账](audits/R227_MODEL_COMPACT_TUI_REPORT.md)。
不恢复 TODO 强制验收或普通模式自动续跑；实时统计条在紧急问题后推进。

## R226 当前模型与统一自主权限（.10 已验，本机已切换验收）

解决问题：状态工具把 Gateway 启动模型误报为当前模型；子代理审批让用户反复值守。
模型/部署事实分开，并行工具继承工作片 Context；/permissions 与 F4 保存 owner 的三档明确模式，
普通自主不越家目录，Full Access 需管理员，子代理同 owner 自主且保持权限墙。320 定向与 `.10` 单 Gateway
真 TUI 已验，包括主等待审批原地续跑、主/子同期工作和 child 7 次 PTY；错分工具样本另记，不自动抹去。
用户明确要求重启后，本机全局入口已切到 runtime-r226，唯一 Gateway 重启；旧等待通过精确 `/stop` 已终止。
ma-r226-local-deploy 真 TUI 的 F4 与 DeepSeek 短问答通过，配置、原 ask 模式和任务文件保留。
R225 名称软提示已进入本机独立 wheel；横向指标条暂只完成设计，工具错分与剩余长链路仍开放。
...
Provider+model v2、编辑留空保留/明确清空、模型容量、三种协议、目录发现和主动短问候已实现。
本机单 Gateway、`ma-r222-models` 已保存 Go provider 和 35 个模型，最终 26 正常、7 不可用、2 待训练条款同意。
Kimi 单模型温度 1 修正已真 TUI 通过；自定义服务商保存/删除、编辑、目录、退出、顶部模型名已验。
291 focused 通过；Responses 工具/密文回放与跨 owner 头隔离有定向，真实长任务与跨模型 child 不在本片范围。
下一步核对新模型计价目录及旧真实任务账；.10 SSH 已恢复但旧 tmux 不存在，不重新发送旧 prompt。
见 [R222 通用模型管理报告](audits/R222_MODEL_PROVIDER_REPORT.md)。

## R221 公开过程逐块保存（本地定向通过，真实环境暂不可达）

解决问题：main 已显示的工具和思考尚未 final 时只在内存，Gateway 退出后恢复页可能丢失。
按 会话运行时 完成项先持久再投递，复用同一 canonical 会话账本保存 typed display 记录；不混入模型/Compact/Memory。
先做定向存储、重开、去重与输入不变，再在原单Gateway做真实TUI；不能用离线通过代替重启验收。
288 focused已通过；主/子未final恢复、未知工具迟到补齐、冷owner与旧客户端、模型/Compact/Memory输入排除均覆盖。
未部署、未追加模型请求。下一步连接恢复后先核原R220请求，再对唯一Gateway做受控重启/恢复组合。
...
解决问题：R217 实时已完整，但 resume 丢掉 Bash 括号内的命令，原页/观察页/恢复页不一致。
已参考 会话运行时 完成项保留命令及 终端交互 按 tool_use_id 关联调用，统一显示 metadata 的公开 detail 投影；
三个红测复现后，209 focused及严格gate通过；82da8d1独立客户端恢复原记录，8条命令标题与展开头尾均一致。
未重启Gateway、未重写canonical或补模型调用。下一步定位空闲resume的Context缺失并补未提交过程的耐久边界，
继续原P0/P1真实验收；不要把错误的LAN排障报告视为通过，其他独立场景可以并行。
R217 三页 user/final/18个思考和零补调用已验；空闲恢复 Context 缺失另查，完整P0/P1与跨模型child仍开放。

## R217 当前片：同会话完整前台过程与持续任务

解决观察页缺失实时思考/工具，以及临时半句与最终回复重复的问题。已本地复用公开事件 mapper 和
canonical final 快照，加入精确候选替换、关闭显示边界及显式能力协商；349 focused及实时/正文恢复真 TUI分项通过。
原 R216 游戏七 child 与一次网络追问已返回，继续同会话第二个小任务和第二个大型任务；LAN 外部连接失败
作为未通过项保存，不能用模型口头“服务端正常”消除。完整 P0/P1、跨模型 child 与提交流丢失仍未封板。

## R216 当前片：前台已提交消息真 TUI 复验

解决观察页收不到新用户消息和最终回复。50cdfaf已部署，三页真实输入/最终回复、零重复、恢复通过。
下一片接通执行中的思考/工具流，沿现有公开映射、精确请求关联和已提交显示历史补帧；
旧客户端兼容真实交互仍待补，BUG-146 与原 P0/P1 保持开放，不因本片通过而宣称整体封板。
...
已本地同步 typed permission 生命周期与静态提示，87 focused 通过；下一步独立包单 Gateway 受控部署，
用自建 owner 的真实 TUI/精确同会话观察页验证等待及解除，不操作用户原来的确认框。
完整前台正文 BUG-146 留在下一切片，不能以状态条修复代替。

## R214 用户插入：模型显示与子代理单独选模型

解决问题：选择新模型后顶部仍显示旧模型，以及主 A / 子 B 缺少显式派工入口。
...
## 当前优先：BUG-150 模型选择贯穿传输线程

解决问题：新/model在UI与父线程生效，HTTP保护线程丢失上下文而调用默认模型，压缩却用新模型。
本地已复制工作片 Context，58 focused通过；冷owner配置菜单去重初始化同时修复。
下一步先部署受控独立包，真实TUI按用户本轮例外使用本地Qwen、262144、主代理独立执行，
以provider日志/真实请求确认归属，不重复把旧混用模型任务算成功；其余owner仍MiniMax。

## 正式名称与发布仓库归一

解决问题：实验后缀继续出现在新部署和产品说明中，容易误装旧版本。
当前标题、包元数据与新 VM 源码入口统一 my-agent；后续发布使用正式 my-agent 仓库。
首次正式仓库合流与发布尚未执行，需保留远端历史和另一检出的未提交改动，不能强推覆盖。
本次已核对origin与正式main/63422888，双方无共同祖先；尚未合流。377项定向通过，按累计增删
13583行补跑全量后37项失败；已逐组修复，514项定向零失败、本地严格gate通过，下一步保留历史合流。
代码同步不等于版本发行或P0/P1封板，不创建新Release、不借合流扩展IM/Audit重构。

## R211 用户优先插入：/model 菜单

解决问题：用户急需在 TUI 新增/选择接口、模型名、地址、私密密钥和上下文窗口。
实现主链与定向验证中；真实 TUI 和模型账本核对后再交付。Auth 仅预留。
模型选择按 owner 保存、工作片冻结；已有子代理保存创建时引用以恢复原模型。
此片完成后继续 BUG-146/147 和原 P0/P1，不能把原问题清单标为完成。

## R210 已部署并真 TUI 分项通过；下一片为等待授权/完整正文

同 Go 会话两页 51.1k、compact 12 和 read_file 阶段一致；另一 owner 的家庭工具两页也为 58.4k，
转后台后两页同为 92.9k / 等待 1 child。真实 MiniMax 续测已继续，唯一 Gateway 3494500。
当前 Go 停顿是原页等待取消工具授权，观察页却显示执行中；先补 typed permission 的等待显示，
再接通完整前台正文与最终消息的同 thread 增量/去重。子页初次开在顶部的行为也需与文档统一。
不擅自批准现有授权，不把任何展示问题改为自动重试/自动终态；其他独立测试可并行继续。

## 下一片：BUG-146 前台跨窗口实时过程

R210 第一片：main 数字/阶段共用投影已实现，115 focused 通过、待部署真实 TUI。
完整正文跨窗口增量尚未实现；Go 长任务仍在返工，家庭工具已返回整合结果但未完成独立产物验收。

解决问题：同一会话的原窗口正在执行，另一个窗口只收到用户消息，看不到前台工具/context，误以为停住。
R209 展开上翻/失联恢复已真 TUI 通过；接下来对照 会话运行时 thread/active-turn 事件投递，接通现有同会话显示流，
不增加第二来源或模型调用。Go/家庭工具长任务继续，不替被测对象补业务代码。

## R209 展开模式故障提示（真 TUI 发现后修复）

解决问题：R208 普通视图通过，但 Ctrl+O 的二次装饰盖掉了刷新警告；按实时 typed 健康保留底栏。
R207 同 Go 会话恢复清单已真 TUI 分项通过。展开组合补先红用例后再部署，不把局部通过当封板。

## R208 状态刷新失败提示（本地已修，待真实客户端）

解决问题：连接或快照读取失败后旧 Working 看似仍在推进。现有轮询发布 typed 健康，保留旧状态并明确
正在重试，主/子页面一致；不把网络恢复计为任务进展，166 focused 通过。与 R207 一起做真实 TUI 复验。

## R207 恢复旧 Todo（本地已修，待真 TUI）

解决问题：历史工具副本误驱动当前清单，旧代数据锁死新代；上翻还会覆盖同代最新计划。
已在历史导入边界剥离显示副本的实时 Todo 字段，原账本不改，89 focused 通过。下一步部署客户端恢复原 Go。

## R206 复制异常与过期工作状态（进行中）

- 解决问题：没有选中文字时右键会抛异常、挡住 TUI；失联/恢复后旧状态又可能让用户误以为任务卡死。
- 空选区正常返回且不写剪贴板，对照 终端交互，38 focused 通过；R206 已部署客户端，真实 PTY 无选区
  右键和中文整句复制通过。系统剪贴板未核验，不把 tmux buffer 作为全部复制边界通过。
- 已恢复收到 SIGINT 的唯一 Gateway；信号发送者未知。旧 Working 没有失联说明、同会话新窗口显示
  旧 Todo 清单仍待修。主/子上下文一次同步采样相同，用户报告的差异仍待实际捕捉。
...
- frame-provider 换 store 时同步重绑冻结来源，保留详细档位/清理搜索坐标；44 focused、严格 gate 以及
  原 child/孙代理展开逐层返回通过。下一步独立验证系统剪贴板，同时继续原多轮任务的交付质量和恢复矩阵。

## R200 普通协调员接续被第 4 片卡死（真实恢复边已通过）

- 解决问题：F 的最后一个孩子已完成，但孤儿入口以累计 runner_attempts 达到 4 为由拒绝正常父级接续。
- 删除重复的生命周期次数限制，沿用同一候选、真实失败预算、活跃执行和 UNKNOWN 边界；88 focused 通过。
- 唯一 R200 Gateway 发布后，原 F 和旧 R171 的 coordinator 均自动 4→5 并 DONE；旧 R171 root
  已最终汇报，F root 继续产物验证。没有催办/新 run/手工改状态，全部父子孙故障组合仍需继续验收。

## R199 历史分页与恢复后续（进行中）

- 解决问题：恢复窗口固定截到最近 80 条，旧正文不可达；补收过程还可能在 final 后出现。
- 186 focused 与严格 gate 通过；已部署，原 D 精确 resume 后可见首条输入，Ctrl+O 可回看。
  消息/用量 SHA 与 Compact 不变；待主子页切换、滚轮与系统复制的组合验收。
- R198 主代理活跃重启已保持同 request/run 并 done，完整父子孙与副作用矩阵未关闭。
- 图书 F 的正常接续被累计次数门拒绝，按 R200 独立修复，不归为历史显示问题。
...
- 解决问题：正常下级完成唤醒被 footer 当作失败重试。已删除 renderer 的 attempts-1 推断，focused 通过，待真 TUI。
- R194 递归复验：两名原孙代理跨单 Gateway 重启继续完成，coordinator 和 main 依次唤醒；完整副作用账继续核对。
- 新阻断：A shell 240 秒超时后一律 UNKNOWN，恢复仅给通用失败提示。先对照 会话运行时 exec 补客观终止/输出
  证据，不能放宽未知副作用门。Memory 同义重复与等质量费用比较保持未完成。

## R194 当前阻断项

- R193 真重启 A/C 的恢复身份错误与冷 owner 死亡调和缺口已修；R194 原 TUI 两次顺序重启通过，
  下一步补等待 child/孙代理的完整恢复矩阵。不能把两路主代理成功当全部恢复通过。
- 继续 B/D/E 的质量账、同会话后续和 owner 画像隔离；双通道重复写偏好与公平费用对比尚未闭环。

## R192/后续恢复修复优先

- R192：确定未写入的 Persona/Goal 拒绝由 handler 声明结构化事实，99 focused 通过，待原 TUI。
- R193 Gateway 恢复：第二次重启误报超时已在本地拆开执行代次/真实卡死次数，待活跃真 TUI 在线重启复验。

## R191 减少重复状态包与预检查失真

- 解决问题：执行事实少量变化仍重复追加整包 14K～19K 字符；预检查又按物化前的整包计数，和实际发送不一致。
- 状态：260 focused 通过，动态事实按宿主来源增量追加，预检查/发送共用一份无副作用投影；各类最新状态和
  用户输入在 Compact 后保留。已部署真实请求验证来源增量；费用账已采集，但首轮测试深度不等价，不能称公平省钱结论。

## R190 长回合的旧运行快照不能永久占据上下文

- 解决问题：工具已经摘要化，旧运行快照仍逐轮累积，导致 native Compact 的 floor 越来越高，
  主代理转入两分钟左右一次的工具片压缩。原长 B 到 68 代才 final，不能算经济。
- 状态：141 focused 及 `.10` 原 main/child 回收续跑通过；原 child 已到 96 代后 DONE，B main 恢复约半窗口
  余量后继续。重复整包仍导致频繁压缩，费用不闭环，继续 R191；不恢复次数硬上限。

## R188 后台唤醒保持用户工作目录

- 解决问题：后台整合把内部运行归档当成 cwd，导致用户家内相对路径失败。
  状态：184 focused / 2 原有 xfailed 及严格检查通过，原 TUI 29 次相对文件操作成功；运行归档引用保留，
  用户执行位置改回可信 thread cwd 或 canonical owner home，既有权限门不变。
...
- 解决问题：有实际进展的长 child 在第 8 次 Compact 后被判失败，后续清理又掩盖失败状态。
  对照 会话运行时 成功压缩继续同轮的语义，删除累计次数硬终止，保留实际压缩失败、取消和预算；
  自动父级清理不覆盖已有终态。状态：98 项定向验证通过；原 child5 已在相同 run/thread 下完成到第 96 代
  并 DONE，成功压缩不再触发旧 8 次硬终止。实际复刻仍有功能失败，不能把生命周期成功写成任务质量通过。

## R186 后台已提交工作片的完整展示恢复

- 解决问题：旧后台过程在恢复后排到 final 下面。给工作片分配不可复用 ID，终态块快照随 canonical final
  保存，恢复后按 exact 工作片 ID 重基旧缓冲，活跃工作片继续显示。状态：已提交工作片的原 B TUI 恢复通过；提交前中断的过程归档、
  前台 Compact 前归档与长历史分页仍开放。对照与边界见 DESIGN_LEDGER 的 R186 记录。
...
- 解决问题：退出再恢复后旧 final 被追加成新消息。改为唯一 canonical 消息流和精确恢复游标，删除 notices
  正文旁路；真实后台 final 自动可见，但再次恢复时旧过程事件追加到 final 之后。下一步把 canonical
  历史快照与后台过程缓冲共同重基，保留未完成事件；完整历史分页和 Compact 前过程归档继续开放。
