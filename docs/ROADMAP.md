...
来自 STATUS.md，当前先发布并真机验收子代理可进入视图与精确控制，再继续正式矩阵。

### 子代理详情与用户精确控制

状态：本地实现与 80 项 focused 回归已通过，待推送、单 Gateway 部署和原样长任务真 TUI 按键验收

解决问题：用户只能看见 child 摘要，无法进入了解其 thinking/工具/Todo，也不能向某个 child 插话或精确
停止；此前若用 Esc 退出详情，又会和停止当前代理的既有习惯冲突。当前候选以 `↓/Enter` 进入 exact child，
`Ctrl+G` 返回，`Esc` 停止当前代理；运行中普通输入成为幂等 guidance，终态只读。详情与未来 Web 共用
owner-scoped Gateway 投影/控制服务，不根据名称或文字猜目标。fresh `.7` TUI 必须实际验证多行选择、进入、
插话、停止、返回以及终态只读，且仍只运行一个 Gateway、使用 MiniMax-M2.7。
...
状态：`f07c647` 已推送、部署 `.7` 并通过隔离回归；多 TUI 传输候选待部署后重跑正式矩阵
...
部署后先把这批故障样本标为 diagnostic，再在新目录重跑同题，不混入正式评分。
