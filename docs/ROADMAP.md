...
状态：主链已收口；本轮双用户真测发现 `/btw` 双执行轮与 `/stop` 等待模型超时的底层缺口，
候选修复已通过 focused tests，完整门禁、发布与 1.10 复验进行中。
...
`/btw` 只引导原 live turn 且不再创建后台重复执行器，`/stop` 能立即打断模型流并在之后自然
“继续”重开原 workspace，以及前台聊天和后台长任务并行时不串 prompt/workspace。
