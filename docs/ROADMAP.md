...
状态：待验证（累计上下文、owner/thread compact、旧聊天检索与 `/verbose` 离线主链已收口，等待 1.10 双用户实聊）

解决问题：同一用户多轮忘记、旧任务污染闲聊、首条消息被密码卡吞掉，以及长任务超过同步等待窗后真实结果无法送达。

待做：部署当前 main 到 1.10 后，临时把 compact 阈值设为 50%，由两个 Feishu 用户用 MiniMax M2.7
分别测试整段长任务和自然语言分步任务；核验同 chat 累计、自动 compact 后续接、旧聊天检索、跨用户/
跨话题隔离、`/verbose`、人设、USER 偏好、SOUL 确认卡与长任务回送。结束后恢复 90%；特殊
`/audit`、`/goal` 模式后续单独完善。
