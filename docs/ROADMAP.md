...
状态：`047e24f7` 已推送并部署；本轮双用户真测先修复 `/btw` 双执行轮与 `/stop` 等待模型超时，随后又发现
持久 task guidance 消费 ID 错位、失败 select 可绕过工作区决策门、前台自做长任务占住聊天入口，以及
完成短轮丢失真实测试摘要。结构化前台安全让出和摘要保留已发布；1.10 复验确认入口能在 4 个工具轮后
释放、后台能自主派工并完成，但又暴露普通聊天可再次 select 正在运行任务。当前本地候选已加入执行占用
fail-closed、续跑 policy 去重和内部轮数字段移除，通过 focused tests，待完整门禁、发布与双用户复验。
