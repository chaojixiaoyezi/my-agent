...
状态：本地候选，196 passed、7 个既有 xfail 与本地严格 gate 通过；待部署和真 TUI 复验

解决问题：真实超级玛丽任务中，模型把测试机本机 HTTP 200 和 `0.0.0.0` 监听错误说成另一台电脑已经能
访问；即使用户明确要求“不能就说真实原因、不要把本机访问说成局域网访问”，后续模型仍重复误报。

当前进展：对照 会话运行时 的针对性验证、不要声称无法执行的测试和无法验证时给出复核步骤，在所有主/子代理
共用的 `PromptBuilder` 中只注入一份通用证据边界。规则不含项目、协议、端口或防火墙特判，不解析模型
正文、不改变任务终态，也不恢复用户已否决的机器验收。部署后必须用同一普通中文 follow-up 复测；若
MiniMax 仍越界，只如实记录软提示对该模型不够稳定，再讨论更强但仍非语义验收的结构化工具证据展示。
