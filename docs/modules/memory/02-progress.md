...
- 对标吸收：Hermes 的工具输出外置 + preview + 分段读，OpenClaw 的响应前缀限制/事件截断，Codex/Claude Code 的“工具结果正文不要自动回灌 prompt”。my-agent 选择 index-first、refs-first、按需窄读，不把大 artifact 当普通文件读。
- 本轮 focused 验收：`/Users/xiaoyezi/ai_claw/bin/python -m pytest agent_py_agent/tests/test_memory_artifact_read.py -q` -> `12 passed`。
...
- 真实模型隔离测试跑在 `/Users/xiaoyezi/my-claude-code/main-agent-foundation-20260518_010636/`，使用独立 `home/` 和 `workspace/`，禁用 subagent。主代理独立完成 HTML 产物、路径记错后的 CSV 整理、手动 compact/resume、带 resume context 继续写交接说明。
...
- 对标吸收：Codex 的结构化工具输出和输出截断测试、OpenClaw 的 E2E/live/docker/package acceptance、Hermes 的“live path 前必须 E2E”和动态 toolset 可用性过滤。my-agent 采用“模型自检只是说明，机器验收才是证据”的原则。
