...
|-- settings/memory.py                # memory 配置、默认值、warning 和安全归一化
|-- memory_store/                     # 长期记忆 JSONL 存储
|-- memory_routing/                   # route index、匹配、诊断、上下文读取
...
- `memory_store/jsonl.py`：读写长期记忆 JSONL，是最朴素的事实落盘层。
- `settings/memory.py`：解析配置，处理非法值回退和 warning。
- `memory_routing/loader.py`：读取 route index。
- `memory_routing/matcher.py`：根据用户输入匹配可能需要读取的长期规则。
...
2. 再看 `agent_py_agent/agent/settings/memory.py`，学习配置如何设置默认值和 warning。
3. 再看 `memory_routing/models.py`，认识 route 是什么数据形状。
4. 再看 `memory_routing/matcher.py`，理解关键词和别名如何命中规则。
5. 再看 `memory_archive/models.py` 和 `storage.py`，理解归档保存什么。
6. 最后看 `agent_py_agent/tests/test_memory_*.py`，用测试反推每一层必须保证的行为。

## 当前第一版索引 / 待补齐

本页先讲主结构和阅读路径。后续需要补真实 route index 样例、raw archive 样例、doctor 输出样例和恢复链路图。
