...
- 写入节流 `_ADMISSION_WAIT_REFRESH_SECONDS=30s`，且受总预算
  `gateway_admission_wait_budget_seconds`（默认 3600s）约束；超预算停写并只留一次
  `admission_wait_expired_at`，客户端按自己的空闲窗口如实收口，请求不被丢弃。
- 客户端零改动：它已按 chunk/inbox/processing 文件活动续期，因此这些写入就是"合法存活"信号；
  收口条件不变（停写超时 / 取消 / 终态 / 崩溃恢复后新网关继续按预算写）。
