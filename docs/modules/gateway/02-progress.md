...
- 2026-06-03 Gateway/CLI 上下文显示收敛：gateway request execution 会同时返回 `turn_token_estimate` 和 `cumulative_token_estimate`；CLI/TUI 的 `ctx_tokens` 显示改用累计上下文估算，避免长任务观察 compact 百分比时只看到单轮数字。
