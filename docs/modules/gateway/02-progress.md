...
N=10000 条 run（4.7MB store）下，调度只读路径都在锁内 `_load_store_unlocked()` + 全量逐条解析。
配对交替实测（两 worktree、同一夹具、交替 4 轮 × 5 次取中位）：

| 路径（频率） | 修复前 总/锁(ms) | 修复后 总/锁(ms) | 锁改善 |
|---|---|---|---|
| `queued_runs`（**每个调度 tick**） | 51.69 / 46.710 | 61.77 / **0.013** | 3593x |
| `active_runs_by_job`（scheduler 工具 list） | 106.24 / 44.123 | 126.35 / 0.013 | 3394x |
...
做法与 R267 同源：抽 `_store_snapshot()`（锁外读+解析、锁内一次 stat 复核、失败退回权威读取），
六个只读路径改用它；同时发现并去掉一处白付成本——单次读者本来只需要 stat 复核，却在探针里算了
blake2b 摘要（4-5MB 上几毫秒）没人比对，改为 `digest=False`（跨调用缓存仍保持 `digest=True`）。
**总耗时**在本次负载下 +10~20%（探针多一次 stat/读的固定开销，未做更精细分离），换到的是**锁持有
从几十毫秒降到 ~13µs**：每个调度 tick 不再独占 owner 账本锁。
写路径 `_mutation_scope`（每调度事件一次，锁内 ~21ms）作为**已测量残余**记录，未改。

## 2026-09-13 R272 合法排队等车道的结构化等待信号（慢模型下不再假超时）

R271 发现的缺口：请求因车道/配额占满留在 inbox 期间没有任何写入者，客户端续期没有信号，
超过初始窗口（默认 `gateway_request_timeout=300s`）就报"等待超时"——慢模型下"上一条长回合占着
车道、下一条排队"是常态，属于需求"慢模型也必须支持长期工作"内的底座缺口。

本轮按既有准入事实源补齐（不引入新服务、不改用户模型/权限/Compact）：
- worker 在**每次扫描被限流**（`global_inflight`/`user_inflight`/`conversation_busy`）与**恢复退避**
  时，在请求文件上写结构化等待事实 `admission_wait_at`/`admission_wait_reason`/`admission_wait_count`
  （首次记 `admission_wait_since`）。只写等待事实：**不写** `status`/lease/模型字段，不伪装成已认领。
- 写入只做节流 `_ADMISSION_WAIT_REFRESH_SECONDS=30s`：**没有累计等待上限**——健康网关持续证明合法
  等待时，慢模型排队再久也不会因为总时长被判失败；收口只来自权威事实（网关停写=真失活→客户端按自己的
  空闲窗口收口；取消→条目消失；权威终态→终态记录；崩溃恢复→新网关继续写，真死则停写）。第一版误加了
  1 小时总预算，验收判定那是把误超时从 5 分钟搬成 1 小时、并制造"前台报失败、后台照常执行"的隐式语义，
  已按要求撤掉该配置与过期分支。
- 客户端零改动：它已按 chunk/inbox/processing 文件活动续期，因此这些写入就是"合法存活"信号。
- 准入判定改成结构化：`try_acquire_report()` 返回原因码（空串=已获取），`try_acquire()` 是其布尔包装。
- 硬化：没有进队时间戳的老请求在首次写信号时把改写前 mtime 钉成 `created_at`，避免排序回退用 mtime
  导致它每轮被后移（自造饥饿）。
- 存活用例用**夹具回拨进队时刻**制造虚拟累计等待（不是真机等待那么久）：刚过 1 小时、半天、3 天、30 天时
  仍继续发新信号且不产生任何"等待过期"事实；另有网关停写、取消、终态三类反例。

验证（12 例）：结构化字段写入且不含 lease/status/模型字段、节流窗口内不重复写、超预算只写一次
过期事实、退避请求同样有信号、被正常认领的请求不带等待字段；客户端在只有等待信号时持续等到终态、
停写后窗口内收口、取消后窗口内收口；**真实 worker + 真实客户端**端到端用例，以及关掉信号后同一
场景必然超时的负向对照。
