...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code/third-party-eval`
  - Same task: B2B SaaS 东南亚市场进入策略，要求主代理组织多层小傻妞协作并输出中文报告。
  - Models: my-agent / Hermes / OpenClaw 都使用 MiniMax-M2.7。
  - Logs:
    - my-agent: `logs/my-agent-task17-20260516-043840.log`
    - Hermes: `logs/hermes-task17-20260516-043840.log`
    - OpenClaw: `logs/openclaw-task17-20260516-044741.log`
- Result summary:
  - Hermes 快速完成 `final_report.md`，报告结构完整，并用文字描述了三层代理协作；本轮黑盒日志未证明它真的创建了多层子代理。
  - OpenClaw 快速完成 `final_report.md`，工具调用稳定；报告中明确写了“模拟执行”，说明这轮隔离 CLI 环境里不是完整真实多层派工。
...
- 中文解释:
  - 这轮不是简单比谁快。Hermes/OpenClaw 这次更像“一个主代理自己快速完成报告”，my-agent 更像“真的拉了一支队伍干活”，但队伍规模失控，最后还有两个人没验收完。
...
- Test scene:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-070425.log`
  - Output: `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_17/agent_subagent_eval_suite/task_17_hierarchical_agents_sea_market/final_report.md`
- Good result:
...
- Test scene:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-072745.log`
- Symptom:
...
- Test scene:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-123517.log`
  - Workspace: `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_17/agent_subagent_eval_suite/task_17_hierarchical_agents_sea_market`
...
- Cross-project reference:
  - Hermes delegate 更强调 parent 接收 summary / refs / tool trace，不把 child raw transcript 全塞回父级。
  - OpenClaw 控制面更强调 session/run/status/logs 分离，让父级先看状态和文件引用。
  - Codex 风格的结构化工具协议也说明：机器事实应该走字段，不靠自然语言摘要猜。
- Fix:
...
- Verification:
  ...
  - Required files present in `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r71/build`:
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`
  - Clean deliverables root: `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r72/build`
  - Runtime root: `/Users/xiaoyezi/my-claude-code/.my_agent_runtime/stage7_shop_complete_20260512_r72`
  - Model backend: `anthropic_compatible`
...
- Borrowed lesson:
  - Hermes delegate returns compact child summaries, tool traces, token counts, and status instead of handing full child transcripts back to the parent.
  - OpenClaw separates context injection/cache/guard behavior so continuation turns can avoid reloading bootstrap context and can inspect prompt-cache health.
...
- Next:
  - R74 should restart from a clean `/Users/xiaoyezi/my-claude-code` task directory and verify root creates QA roles from dispatch advice instead of growing its own context with product body reads.
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`.
  - Prompt and config:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r81_prompt.md`
    - `/Users/xiaoyezi/my-claude-code/.my-agent-stage7-shop-complete-20260512-r81.yaml`
  - Run log:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r81_run.log`
...
- Borrowed lesson:
  - Hermes keeps internal history in structured message roles: assistant messages carry `tool_calls`, and real tool results are appended as `role=tool` with `tool_call_id`; this means model prose is never the authority for whether a tool actually ran.
  - Hermes also persists large tool results out of context and leaves a bounded preview plus path/ref, which matches our existing tool-output externalization direction.
  - OpenClaw has text sanitation for leaked tool tags and downgraded `[Tool Result ...]` text, and also marks some external events as untrusted. The useful part for my-agent is: if tool-looking syntax appears in assistant-visible prose, treat it as display/noise unless it came from the runtime tool channel.
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`.
  - Prompt and config:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r82_prompt.md`
    - `/Users/xiaoyezi/my-claude-code/.my-agent-stage7-shop-complete-20260512-r82.yaml`
  - Run log:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r82_run.log`
- Observed behavior:
...
- Borrowed lesson:
  - Hermes' structured message history treats model output as either assistant text or assistant tool-call objects, then appends real tool results separately as tool-role messages. A model cannot prove a tool result by writing prose.
  - OpenClaw sanitizes leaked tool tags and tool-result-looking text before it becomes assistant-visible text.
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`.
  - Prompt and config:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r83_prompt.md`
    - `/Users/xiaoyezi/my-claude-code/.my-agent-stage7-shop-complete-20260512-r83.yaml`
  - Run log:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r83_run.log`
- Observed behavior:
  ...
  - One runner wrote real files:
    - `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r83/build/index.html`
    - `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r83/build/data/products.json`
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`.
  - Prompt and config:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r84_prompt.md`
    - `/Users/xiaoyezi/my-claude-code/.my-agent-stage7-shop-complete-20260512-r84.yaml`
  - Run log:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r84_run.log`
...
- Observed behavior:
  ...
  - Real product files were written:
    - `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r84/build/index.html`
    - `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r84/build/css/styles.css`
    - `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r84/build/js/app.js`
...
- Borrowed lesson:
  - Hermes keeps subagents on independent iteration budgets and separates tool-call history from assistant prose; this supports our direction of per-run prompt identity plus per-run budget rather than one shared parent prompt/budget.
  - OpenClaw's tool-tag sanitation reinforces the same boundary: model-visible text should not become trusted runtime state, and child session identity should come from runtime context, not copied parent prose.
- Fix:
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`.
  - Prompt and config:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r85_prompt.md`
    - `/Users/xiaoyezi/my-claude-code/.my-agent-stage7-shop-complete-20260512-r85.yaml`
  - Run log:
    - `/Users/xiaoyezi/my-claude-code/stage7_shop_complete_20260512_r85_run.log`
...
- Observed behavior:
  ...
  - Real files were written under `/Users/xiaoyezi/my-claude-code/deliverables/stage7_shop_complete_20260512_r85/build/`, including `index.html`, `styles.css`, and `app.js`.
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code`.
  - Prompt and config:
    - `/Users/xiaoyezi/my-claude-code/stage7_r86_artifact_tool_prompt.md`
    - `/Users/xiaoyezi/my-claude-code/.my-agent-stage7-r86-artifact-tool.yaml`
  - Run log:
    - `/Users/xiaoyezi/my-claude-code/stage7_r86_artifact_tool_run.log`
...
- Observed behavior:
  ...
  - The leaf wrote `/Users/xiaoyezi/my-claude-code/deliverables/stage7_r86_artifact_tool/build/proof.txt`.
...
- Discovered at: 2026-05-13 while reviewing R85/R86 artifact-wrapper behavior and comparing Hermes/OpenClaw/Codex-style output handling.
- Symptom:
...
- Verification:
  ...
  - Focused run: `/Users/xiaoyezi/ai_claw/bin/python -m pytest agent_py_agent/tests/test_memory_artifact_read.py -q` -> `12 passed`.
- Remaining gap:
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code/recovery_cases/case12_subagent_multi_compact`.
  - Config: `/Users/xiaoyezi/my-claude-code/recovery_cases/case12_subagent_multi_compact/e2e_agent_config.yaml`.
  - Model backend: `anthropic_compatible`.
  - Model name: `MiniMax-M2.7`.
...
- Observed behavior:
  ...
  - Deliverable written:
    - `/Users/xiaoyezi/my-claude-code/recovery_cases/case12_subagent_multi_compact/算法测试方案.md`
...
- Reference comparison:
  - Codex 的 stream 侧会区分可重试错误，并用 retry budget + backoff 继续请求。
  - Hermes 对长任务更偏向 inactivity timeout，并把网络/timeout 这类 transient failure 放进恢复或重试路径。
  - OpenClaw 的 cron/retry 类型里也把 `timeout` 作为 transient retry 类别之一。
- Fix:
...
- Reference comparison:
  - Codex 风格更重视 typed action 的语义校验：不是只看某个自然语言字段，而是把结构化字段和意图一起归一化。
  - Hermes/OpenClaw 也会在工具边界做 schema/registry 归一，而不是完全相信模型一段自由文本。
- Fix:
...
- Verification:
  ...
  - Real Phase 3 fixed5: leaf 写入 `/Users/xiaoyezi/my-claude-code/phase3_hierarchy_e2e_user_style_fixed5/fixture_project/artifacts/小傻妞报告.md`。
- Status: fixed.
...
- Scenario:
  - Test directory: `/Users/xiaoyezi/my-claude-code/phase3_hierarchy_e2e_user_style_fixed5`.
...
- Symptom:
  ...
  - Because the CLI process was launched from the repo checkout, runtime memory was written under `/Users/xiaoyezi/my_agent/my-agent/.my_agent_home` instead of the isolated test directory.
...
- Fix:
  ...
  - R10 used `/Users/xiaoyezi/my-claude-code/phase4_natural_recovery_furniture_v10/.my_agent_home`, and the root no longer hallucinated the previous timeout before doing work.
...
- Symptom:
  - The repair leaf correctly edited the real product file at `/Users/xiaoyezi/my-claude-code/phase4_natural_recovery_furniture_v11/artifacts/index2.html`.
...
- Test scene:
  - Workspace: `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_17/agent_subagent_eval_suite/task_17_hierarchical_agents_sea_market`
  - Runner script: `/Users/xiaoyezi/my-claude-code/third-party-eval/bin/run-my-agent-task17.sh`
  - Model backend: `anthropic_compatible`
  - Model name: `MiniMax-M2.7`
...
- Latest run:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-035520.log`
  - Final deliverable: `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_17/agent_subagent_eval_suite/task_17_hierarchical_agents_sea_market/final_report.md`
- Observed behavior:
...
- Test scene:
  - Failed run: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-075215.log`
  - Passing run: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-080326.log`
- Symptom:
...
- Test scene:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-134034.log`
...
- Test scene:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-141952.log`
...
- Test scene:
  - Log: `/Users/xiaoyezi/my-claude-code/third-party-eval/logs/my-agent-task17-20260516-150236.log`
...
- Trigger:
  - Task18 short real run under `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_18_short/20260516-232743/task_18_short_github_star_growth_xlsx`.
  - Tool output artifact: `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_18_short/20260516-232743/task_18_short_github_star_growth_xlsx/memory_archive/artifacts/tool_outputs/create_subagents-2-1-735fef4a2b38.json`.
- Problem:
...
- OpenClaw/Hermes comparison:
  - OpenClaw 侧重 session/run handle 与 workspace 上下文，任务续接时靠当前 session/工作区事实，不让模型猜未来 run id。
  - Hermes delegate/handoff 更像 task-local refs，worker 拿到的是任务空间和交接 refs，而不是一个由模型拼出来的子代理目录。
...
- Status:
  - Fixed in focused tests.
  - Next live check: rerun Task18 short and compare with OpenClaw/Hermes behavior; if a child still loops after writing, investigate structured closeout rather than path guessing.

### Finding 159: Workspace product artifact refs must validate from the task workspace root

- Trigger:
  - Task18 short rerun under `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_18_short/20260516-234925/task_18_short_github_star_growth_xlsx`.
  - Child `subagent-1778946599-5869837d` wrote `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_18_short/20260516-234925/task_18_short_github_star_growth_xlsx/data/subagents/subagent_data_collection/star_data.md`.
- Problem:
...
- OpenClaw/Hermes comparison:
  - OpenClaw 的 session/workspace 事实会把当前工作区作为路径边界，不要求所有产物都塞进某个子 session 内部目录。
  - Hermes delegate 输出也更偏向 job/task output dir 和结构化 handoff refs，父级按任务输出空间找 refs，而不是只按 worker 私有目录猜。
...
- Trigger:
  - Task18 short rerun under `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_18_short/20260517-000719/task_18_short_github_star_growth_xlsx`.
...
- OpenClaw/Hermes comparison:
  - OpenClaw 的 session/run 控制面把当前活动 session 作为事实，不需要模型从自然语言回忆刚刚 spawn 了谁。
  - Hermes delegate/handoff 也是任务 handle 优先；重复操作应该回到已有 job/task refs，而不是让树继续膨胀。
...
- Trigger:
  - Task18 short live retest under `/Users/xiaoyezi/my-claude-code/third-party-eval/runs/my-agent/task_18_short/20260517-003526/task_18_short_github_star_growth_xlsx`.
...
- OpenClaw/Hermes comparison:
  - OpenClaw 的 session/task continuation 更像同一个会话继续处理当前故障，不会每个小动作都变成新 session。
  - Hermes delegate/handoff 更强调一个 job 的 output dir 和 failure context；修复任务应该带着失败上下文完成闭环，而不是只做半步。
...
- OpenClaw/Hermes/Codex comparison:
  - OpenClaw 和 Hermes 都更偏向 session/task workspace 事实：私有 run 目录和项目/任务工作区是两个已知边界，不让模型自己猜路径。
  - Codex 的工具/路径合同也更强调结构化 root，而不是从自然语言摘要里重新推断。
...
- OpenClaw/Hermes/Codex comparison:
  - Codex 的做法更像把 provider 协议差异收在 backend/protocol 边界，上层拿到的是统一结果。
  - OpenClaw/Hermes 也会把运行状态和模型调用失败分层记录；真实业务状态不应该和 provider transient issue 混成一个自然语言错误。
...
- Trigger:
  - Natural-language furniture-site E2E setup under `/Users/xiaoyezi/my-claude-code/real_e2e_next/scenario-20260517-072914-f418a6`.
...
- OpenClaw/Hermes/Codex comparison:
  - OpenClaw/Hermes 的后台 worker 更像按 tick/poll 运行，空闲时只维持状态，不把每个 no-op 都当成业务事件。
  - Codex 的工具/事件链也更强调“真实动作才成为持久事实”；心跳和 no-op 不应该淹没工具/任务记录。
...
- Verification:
  ...
  - Ran isolated gateway idle smoke under `/Users/xiaoyezi/my-claude-code/real_e2e_next/watch-idle-fix-20260517-074538`: after start/sleep/stop, watch log had 1 line, local store had 8 files, and `dispatch_records=0`.
- Status:
...
- Trigger:
  - Natural-language furniture-site E2E under `/Users/xiaoyezi/my-claude-code/real_e2e_next/scenario-20260517-075800-25c13e`.
...
- OpenClaw/Hermes/Codex comparison:
  - OpenClaw 更像让同一个 session 带着当前故障继续处理，而不是每个小动作拆成新 session。
  - Hermes delegate 更强调一个 job 的 output dir 和 failure context；修复任务应该继承失败上下文、目标产物和验收条件。
  - Codex 的结构化状态让上层更容易看见“已有修复负责人”和“当前 blocked artifact”，避免靠自然语言重开新任务。
- Fix:
...
- Verification:
  ...
  - Reran natural-language furniture-site E2E under `/Users/xiaoyezi/my-claude-code/real_e2e_next/scenario-20260517-082741-21614b`.
...
- OpenClaw/Hermes/Codex comparison:
  - Codex 的稳定点是把 run/tool identity 放进结构化字段，不靠普通英文片段猜任务类型。
  - OpenClaw/Hermes 的 session/job 也更强调显式任务身份；自然语言只辅助展示，不应该决定是否复用一个 run。
...
- OpenClaw/Hermes/Codex comparison:
  - Codex 风格是让 run id / operation id / task index 成为机器字段。
  - Hermes delegate 的 tasks[] 也天然把 sibling item 当成独立任务。
...
- OpenClaw/Hermes/Codex comparison:
  - Codex 的文件合同更偏结构化：输入路径和输出路径要分开。
  - Hermes delegate job 的 output dir/refs 也会把 source/input 和 output artifact 分层。
...
- OpenClaw/Hermes/Codex comparison:
  - Codex 的方向是工具结果和 artifact refs 机器可读，不让模型从自然语言摘要里猜。
  - Hermes delegate 父级拿 summary / refs / artifacts，不把 child 私有报告当最终产物。
  - OpenClaw 控制面看 session/run 状态；最终汇报不能只看“文件存在”。
- Fix:
...
- OpenClaw/Hermes/Codex comparison:
  - OpenClaw 的控制面会让父级看到 run 状态和下一步，而不是无限等。
  - Codex 的工具协议会把下一步放成结构化字段。
  - Hermes 的 delegate 思路是父级读 refs 和 summary，必要时再开新 task 修复，不把正文塞回父级。
- Fix:
