...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case02_takeover_run`
  - Config: `/Users/example/my-终端应用/recovery_cases/case02_takeover_run/e2e_agent_config.yaml`
...
- Observed behavior:
  ...
  - takeover worker 继承了原 worker 的 task refs、artifact refs 和业务写目录 `/Users/example/my-终端应用/recovery_cases/case02_takeover_run/deliverables/build`。
  - takeover worker 写出：
    - `/Users/example/my-终端应用/recovery_cases/case02_takeover_run/deliverables/build/README.md`
    - `/Users/example/my-终端应用/recovery_cases/case02_takeover_run/deliverables/build/demo.py`
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用`.
  - Config: `/Users/example/my-终端应用/.my-agent-stage7-shop-smoke-20260511-r37.yaml`.
  - Runtime root: `/Users/example/my-终端应用/.my_agent_runtime/stage7_shop_smoke_20260511_r37`.
  - User deliverables root: `/Users/example/my-终端应用/deliverables/stage7_shop_smoke_20260511_r37/build`.
- 中文说明：
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用`.
  - Config: `/Users/example/my-终端应用/.my-agent-stage7-shop-smoke-20260511-r38.yaml`.
  - Runtime root: `/Users/example/my-终端应用/.my_agent_runtime/stage7_shop_smoke_20260511_r38`.
  - User deliverables root: `/Users/example/my-终端应用/deliverables/stage7_shop_smoke_20260511_r38/build`.
- 中文说明：
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用`.
  - Config: `/Users/example/my-终端应用/.my-agent-stage7-shop-smoke-20260511-r39.yaml`.
  - Runtime root: `/Users/example/my-终端应用/.my_agent_runtime/stage7_shop_smoke_20260511_r39`.
  - User deliverables root: `/Users/example/my-终端应用/deliverables/stage7_shop_smoke_20260511_r39/build`.
- 中文说明：
  ...
  - 路径 typo guard 在真实链路里生效：leaf 把 `/Users/example` 误写成 `/Users/xiaoyuzi` 时，工具层拒绝并给出 suggested target。
...
- Observed behavior:
  ...
  - Product files eventually existed under `/Users/example/my-终端应用/deliverables/stage7_shop_complete_20260512_r75/build`.
...
- Borrowed lesson:
  - 长期助手 approval gates focus on dangerous commands, recursive removal, system paths, update/self-kill, and trusted skill/tool boundaries.
  - 通道运行时 execution policy focuses on stable cwd/script/env/approval binding and path authority.
...
- Borrowed lesson:
  - 通道运行时 keeps execution authority at the exec policy boundary (`security`, `ask`, `allowlist`, approval decisions).
  - 长期助手 exposes dangerous-command approval/yolo/user config as a top-level policy, not as a child asking a nonexistent parent.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用`.
  - Temporary config: `/Users/example/my-终端应用/e2e_agent_config.yaml`.
  - Runtime data was isolated under `/Users/example/my-终端应用/_agent_runtime/`.
  - Deliverables were written to `/Users/example/my-终端应用/deliverables/e2e_root_reply_20260514_003941/build`.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case01_packet_failure`.
...
- Observed behavior:
  ...
  - The takeover run inherited the same deliverables root and wrote `README.md` and `demo.py` under `/Users/example/my-终端应用/recovery_cases/case02_takeover_run/deliverables/build/`.
- 中文解释：
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case04_recovery_handoff`.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case05_batch_recovery`.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case06_packet_fallback`.
  - Config: `/Users/example/my-终端应用/recovery_cases/case06_packet_fallback/e2e_agent_config.yaml`.
...
- Observed behavior:
  ...
  - Worker wrote `/Users/example/my-终端应用/recovery_cases/case06_packet_fallback/deliverables/build/packet-fallback.proof`.
- 中文解释：
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case07_no_progress_fuse`.
  - Config: `/Users/example/my-终端应用/recovery_cases/case07_no_progress_fuse/e2e_agent_config.yaml`.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case08_four_layer_recovery`.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case09_shopping_recovery_e2e`.
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case09_shopping_recovery_e2e`.
  - Archived previous runtime/deliverables to `/Users/example/my-终端应用/recovery_cases/_archived_case09/20260514T125806-after-tester-semantic-fix`.
  - Outer observer only launched the top-level root command; root created and dispatched all subagents.
  - Product root: `/Users/example/my-终端应用/recovery_cases/case09_shopping_recovery_e2e/deliverables/shopping_site/build`.
...
- Test scene:
  ...
  - Runtime subagent workspace: `/Users/example/my-终端应用/recovery_cases/case09_shopping_recovery_e2e/_runtime/subagents`.
- Symptom:
...
- Test scene:
  - Workspace: `/Users/example/my-终端应用/recovery_cases/case11_main_multi_compact_clean4`.
...
- Reference comparison:
  - 长期助手 keeps tool calls and structured content behind schema/registry boundaries, and normalizes model mistakes at the boundary.
  - 通道运行时 uses schema/registry-style parsing and sandbox boundaries rather than relying on free-form natural-language reminders.
  - No local 会话运行时 source copy was present under `~/Downloads`; the applicable 会话运行时 lesson is still the same: isolate control-plane calls and parse typed payloads at the protocol boundary.
- Fix:
...
- Reference comparison:
  - 长期助手/通道运行时 both separate runtime execution from structured control/registry boundaries.
...
- Symptom:
  ...
  - Their `output.json` did not list artifacts, so auto static-site acceptance guessed `/Users/example/my-终端应用/deliverables`.
...
- Verification:
  ...
  - Deliverables exist:
    - `/Users/example/my-终端应用/subagent_hardening_e2e_20260515_group03n/deliverables/index1.html`
    - `/Users/example/my-终端应用/subagent_hardening_e2e_20260515_group03n/deliverables/index2.html`
    - `/Users/example/my-终端应用/subagent_hardening_e2e_20260515_group03n/deliverables/index3.html`
- Remaining notes:
...
- Discovered at: 2026-05-15 during real MiniMax furniture-page E2E under `/Users/example/my-终端应用`.
- Symptom:
...
- Discovered at: 2026-05-15 during real MiniMax repair-loop E2E.
- Symptom:
  - Parent `write_boundary.allowed_write_roots` correctly included `/Users/example/my-终端应用/.../deliverables/furniture-home`.
...
- Discovered at: 2026-05-15 during clean E2E under `/Users/example/my-终端应用/real-e2e-20260515-180300-repair-loop4`.
- Scenario:
...
- Result:
  ...
  - Worker wrote `/Users/example/my-终端应用/real-e2e-20260515-180300-repair-loop4/deliverables/furniture-home/index.html`.
...
- Discovered at: 2026-05-15 during real MiniMax read-root E2E under `/Users/example/my-终端应用/real-e2e-20260515-181352-readroot-check`.
- Symptom:
...
- Discovered at: 2026-05-15 during follow-up real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-182823-artifact-suffix-check`.
- Symptom:
...
- Discovered at: 2026-05-15 during follow-up real MiniMax repair-advice E2E under `/Users/example/my-终端应用/real-e2e-20260515-183735-repair-advice-check`.
- Symptom:
...
- Discovered at: 2026-05-15 during real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-184908-repair-alias-check`.
- Symptom:
...
- Discovered at: 2026-05-15 during repair-loop E2E under `/Users/example/my-终端应用/real-e2e-20260515-185933-artifact-evidence-check`.
- Symptom:
...
- Discovered at: 2026-05-15 during repair-loop E2E under `/Users/example/my-终端应用/real-e2e-20260515-192113-repair-loop6`.
- Symptom:
  - Worker wrote the product under `/Users/example/my-终端应用/.../deliverables/furniture-home/`.
...
- Discovered at: 2026-05-15 during repair-loop E2E under `/Users/example/my-终端应用/real-e2e-20260515-193529-repair-loop7`.
- Symptom:
...
- Discovered at: 2026-05-15 during fresh real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-195719-artifact-repair-lane`.
- Symptom:
  - Root used normal user wording and created one `小傻妞-worker`.
  - Worker wrote `/Users/example/my-终端应用/real-e2e-20260515-195719-artifact-repair-lane/deliverables/furniture-home/index.html` with about 20KB / 579 lines.
...
- Discovered at: 2026-05-15 during fresh real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-201434-progress-closeout`.
- Symptom:
...
- Discovered at: 2026-05-15 during fresh real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-204332-workflow-off-respect`.
- Symptom:
...
- Discovered at: 2026-05-15 during real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-210000-link-examples`.
- Symptom:
...
- Discovered at: 2026-05-15 during real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-211000-batch-link-repair`.
- Symptom:
...
- Discovered at: 2026-05-15 during real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-212000-delegated-repair-guard`.
- Symptom:
  ...
  - A follow-up run under `/Users/example/my-终端应用/real-e2e-20260515-214000-output-progress-guard` showed another gap: the worker repaired the HTML to `warning_codes=[]`, but its `output.json.artifacts=[]`, so parent/root had to rediscover the product file by reading the deliverables path.
- 中文解释：
...
- Discovered at: 2026-05-15 during real MiniMax recovery E2E under `/Users/example/my-终端应用/real-e2e-20260515-214800-artifact-ref-recovery`.
- Symptom:
...
- Discovered at: 2026-05-15 during clean real MiniMax E2E under `/Users/example/my-终端应用/real-e2e-20260515-223000-parent-repair-action`.
- Symptom:
...
- Test scene:
  - Log: `/Users/example/my-终端应用/third-party-eval/logs/my-agent-task17-20260516-085115.log`
...
- Follow-up from real rerun:
  - Log: `/Users/example/my-终端应用/third-party-eval/logs/my-agent-task17-20260516-092742.log`
...
- Test scene:
  - Log: `/Users/example/my-终端应用/third-party-eval/logs/my-agent-task17-20260516-172128.log`
...
- Next check:
  - Run the same Task17 prompt through 通道运行时 and 长期助手 in isolated task directories, then compare completion speed, delegation shape, final artifact quality, and whether their roots avoid unnecessary body reads.
...
### Finding 141: Same-task comparison against 通道运行时 and 长期助手

- Test scene:
  - Same Task17 SEA market entry task, MiniMax-M2.7.
  - my-agent log: `/Users/example/my-终端应用/third-party-eval/logs/my-agent-task17-20260516-172128.log`
  - 通道运行时 log: `/Users/example/my-终端应用/third-party-eval/logs/通道运行时-task17-20260516-174412.log`
  - 长期助手 log: `/Users/example/my-终端应用/third-party-eval/logs/长期助手-task17-20260516-174412.log`
- Result comparison:
  - my-agent: true multi-layer run completed; 23 subagent nodes all `DONE / VERIFIED`; root `final_report.md` was 25,637 bytes.
  - 通道运行时: produced a root `final_report.md` of 5,782 bytes, but subagent spawning failed in the isolated setup because the gateway token was missing.
  - 长期助手: produced a root `final_report.md` of 9,442 bytes and created seven isolated session files during the run.
- 中文解释:
  - my-agent 这次真的派出了小傻妞、小小傻妞、小小小傻妞，而且每个节点都有机器状态和验收结果。
  - 长期助手 也很干净：它用多个 session 做任务隔离，最后报告更短，但收口快。
  - 通道运行时 这次因为测试环境里 gateway token 没配，子代理没起来；不过它有一个值得学的点：子代理失败时，主代理会明确说明原因并兜底产出报告。
- Lessons:
  - Keep my-agent's machine-readable tree, task state, and acceptance records; they make real hierarchy auditable.
  - Borrow 长期助手' simplicity where possible: session/task isolation should remain easy to understand, not hidden behind too many policy layers.
  - Borrow 通道运行时's graceful degradation principle: when delegation infrastructure is unavailable, the root should report the blocker clearly and choose the safest allowed fallback instead of hanging.
- Status: comparative black-box run completed. No code fix was made from this comparison yet.
- Next check:
  - Re-run 通道运行时 only after a valid isolated gateway token is available, otherwise it cannot serve as a fair multi-agent comparison.
...
### Finding 142: 通道运行时 real environment completes after absolute task path prompt

- Test scene:
  - Log: `/Users/example/my-终端应用/third-party-eval/logs/通道运行时-real-task17-abs-20260516-175751.log`
  - Task dir: `/Users/example/my-终端应用/third-party-eval/runs/通道运行时-real/task_17_abs/agent_subagent_eval_suite/task_17_hierarchical_agents_sea_market`
  - Model: MiniMax-M2.7 through the user's real 通道运行时 config.
- Result:
  - 通道运行时 created three subagent sessions under the real `main` agent:
  ...
  - The real config file `~/.通道运行时/通道运行时.json` did not change; only runtime state such as `~/.通道运行时/subagents/runs.json` changed.
- 中文解释:
  - 第一轮真实 通道运行时 失败不是 MiniMax 问题，而是它默认只看 `~/.通道运行时/workspace`，没有把 shell 的当前目录当任务目录。
  - 第二轮把测试材料绝对路径写进 prompt 后，通道运行时 就能读到任务、派 3 个子代理，并写出最终报告。
  - 这说明 通道运行时 的强项是“子代理像独立会话一样工作”，但任务目录选择更依赖它自己的 workspace/session 机制。
- Observed gap:
  - During closeout, 通道运行时 printed a transient gateway timeout while announcing subagent completion:
...
- Lessons for my-agent:
  ...
  - Keep subagents as full-context independent sessions where possible; 通道运行时's subagent sessions all used the same large context window instead of artificial child limits.
- Status: comparative real-env 通道运行时 run completed. No my-agent code change yet.
- Next check:
...
- Trigger:
  ...
  - 通道运行时 real-env run finished the artifact but printed a transient subagent completion announce timeout.
- my-agent check:
...
- Next check:
  - Keep comparing 通道运行时/长期助手 notification and completion behavior in future E2E runs, especially where subagent completion succeeds but announcement delivery fails.

### Finding 144: Task18 star-growth xlsx comparison exposed pipeline-start and workspace defaults

- Test scene:
  - Same Task18 prompt across my-agent, 长期助手, and the user's real 通道运行时 environment.
  ...
  - my-agent log: `/Users/example/my-终端应用/third-party-eval/logs/my-agent-task18-20260516-182730.log`
  - 长期助手 log: `/Users/example/my-终端应用/third-party-eval/logs/长期助手-task18-20260516-182730.log`
  - 通道运行时 real log: `/Users/example/my-终端应用/third-party-eval/logs/通道运行时-real-task18-20260516-182731.log`
- Result comparison:
  - 通道运行时 real environment completed and wrote `github_weekly_star_growth_2026.xlsx`, `final_report.md`, `data_research.md`, and helper files under its workspace task directory.
  - 通道运行时 first tried subagents but its default subagent model was rejected (`model not allowed: openai-会话运行时/gpt-5.5`), then root clearly fell back and delivered the artifact itself.
  - 长期助手 exited with status 0 but only printed that it would decompose the task; it did not produce xlsx or final report in the isolated run directory.
...
- Trigger:
  ...
  - my-agent run directory: `/Users/example/my-终端应用/third-party-eval/runs/my-agent/task_18_short/20260516-195147/task_18_short_github_star_growth_xlsx`
  - my-agent log: `/Users/example/my-终端应用/third-party-eval/logs/my-agent-task18-short-20260516-195147.log`
- Problem:
...
- 通道运行时/长期助手 comparison:
  - 长期助手 has `kanban_create(parents=[...])` and `kanban_link`; child tasks stay waiting until parent tasks are done. It does not depend on prose like “use A's result” as the only scheduling fact.
  - 通道运行时's subagents are separate sessions with stable task handles and session/run metadata. Its QA scenarios assert real `sessions_spawn` calls through debug logs instead of trusting text that says “I delegated”.
...
- Trigger:
  - Plain Chinese dependency-pipeline E2E under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-201800/pipeline_dependency_check`.
...
- 中文解释:
  ...
  - 这就是“控制面视野太窄”，不是模型笨。
- 通道运行时/长期助手 comparison:
  - 长期助手 kanban dependency gating checks dependency state from the task board, not just the currently requested child subset.
  - 通道运行时 session/run metadata also treats child sessions as control-plane facts; waiting/yielding is based on session events and handles, not a narrow prompt-local list.
...
- Trigger:
  - Clean dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-202800/pipeline_dependency_check`.
...
- 通道运行时/长期助手 comparison:
  - 长期助手 delegate/kanban handoff keeps child results as refs/summaries for the parent to consume, not as guessed filenames.
  - 通道运行时 uses session handles/events and workspace/session metadata; the parent knows which subagent produced which result instead of relying on a sibling to infer a path.
...
- Trigger:
  - Clean dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-203400/pipeline_dependency_check`.
...
- Trigger:
  - Clean dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-205600/pipeline_dependency_check`.
...
- 通道运行时/长期助手 comparison:
  - 通道运行时's `sessions_spawn` keeps tool availability/sandbox policy separate from the spawned task's `taskName`, session, and runtime state.
  - 长期助手 `kanban_create` separates task body, assignee, parents, and available worker capabilities; dependency/role state is not inferred from a generic tool grant.
...
- 通道运行时/长期助手 comparison:
  - 通道运行时 treats subagent completion as a runtime/task event and warns parents not to poll in loops while waiting for completion.
  - 长期助手 kanban tasks have terminal state; orchestration should advance from the board state instead of reinterpreting prose.
...
- Trigger:
  - Clean dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-211725/pipeline_dependency_check`.
...
- 通道运行时/长期助手 comparison:
  - 通道运行时 marks active subagent context as runtime-generated state, not user-authored instructions, and says subagent outputs are evidence rather than policy-overriding commands.
  - 长期助手 worker context puts the current task title/body first, then bounded prior attempts, parent handoffs, and comments; history is context, not the task source of truth.
...
- Trigger:
  - Follow-up clean dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-212849/pipeline_dependency_check`.
...
- 通道运行时/长期助手 comparison:
  - 通道运行时 uses session/run handles and completion events instead of requiring a parent to rediscover child output paths from prose.
  - 长期助手 kanban/delegate tasks store task parents and structured handoff refs; workers consume parent results through task records, not guessed sibling filenames.
...
- Trigger:
  - Dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-214106/pipeline_dependency_check`.
...
- 通道运行时/长期助手 comparison:
  - 长期助手 同题会直接在任务目录生成 `final_report.md`，同时把中间数据写成独立 JSON 文件。
  - 通道运行时 的工具/会话输出会带 workspace task dir；内部 session 报告和用户 workspace 文件不是同一个事实源。
...
- Trigger:
  - Dependency-pipeline rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-220905/pipeline_dependency_check`.
...
- 通道运行时/长期助手 comparison:
  - 长期助手 同题完成后有 `data_collection.json`、`content_writeup.json`、`final_report.md` 三个明确文件；它用结构化 handoff/工作区文件做下游输入，没有把口语别名当唯一事实。
  - 通道运行时 同题主会话先写 `data.json` 并 spawn 下一步 session；它用 session/workspace refs 表达后续工作，没有在本轮出现“读错人名路径后 BLOCKED”。
...
- Verification:
  ...
  - Live my-agent rerun under `/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-222638/pipeline_dependency_check` completed without repair fallback; `final_report.md` exists at the task root and the report worker reached `DONE / VERIFIED`.
- Status:
...
- 通道运行时/长期助手 comparison:
  - 长期助手 同题用明确任务目录文件作为交付，不会把工作区目录名重复拼接到产物路径里。
  - 通道运行时 的 workspace task dir 是任务上下文事实；产物路径相对这个事实解析，避免父级从自然语言路径重新拼错。
...
- What happened:
  ...
  - The child wrote `/Users/example/my-终端应用/real_e2e_next/20260517-natural-html-01/fixture_project/lab_outputs/furniture-home/index.html`.
...
- 通道运行时/长期助手/会话运行时 comparison:
  - 会话运行时 的优点是工具结果和 artifact refs 是结构化事实；这次我们也只信文件和 refs，不信最终 prose。
  - 通道运行时 的 session/run 控制面强调当前任务是否真的完成；这次 dispatch/acceptance log 给出了明确 `DONE/VERIFIED`。
  - 长期助手 的 delegate 思路是父级拿 summary/refs，不搬正文；这次 root 返回 artifact refs 和 output refs，未把 50KB HTML 塞回最终回复。
- Fix:
...
- 通道运行时/长期助手/会话运行时 comparison:
  - 长期助手 release notes强调 inactivity-based timeout，活跃工具调用不应被墙上时间误杀。
  - 通道运行时/会话运行时 的稳定点也是把预算/熔断做成明确控制面，而不是让普通任务默认踩很小的测试阈值。
...
- 中文解释:
  ...
  - 这是路径合同不统一，不是提示词问题。
- 通道运行时/长期助手/会话运行时 comparison:
  - 会话运行时 的成熟点是工具和产物 refs 走同一套 workspace/run scope，而不是每个模块各猜路径。
  - 长期助手 delegate 的 task workspace / artifact refs 也要求父级按 refs 查产物，不从私有目录猜业务路径。
...
- 通道运行时/长期助手/会话运行时 comparison:
  - 通道运行时 控制面强调真实 session/run 状态；不能让自然语言或产物存在覆盖掉仍未完成的 job。
  - 会话运行时 的状态统计应从持久化任务状态出发，coverage 只能是明确机器事实。
...
- 通道运行时/长期助手/会话运行时 comparison:
  - 会话运行时 风格是区分 input refs 和 output refs；自然语言只辅助抽取，不能把两者混成一个。
  - 长期助手 delegate 也把输入资料和输出目录分开传递。
...
- 通道运行时/长期助手/会话运行时 comparison:
  - 会话运行时 的工具/产物结果靠结构化 fields、call id 和真实文件路径关联，不靠自然语言复述。
  - 长期助手 的 `kanban_complete` 会验证结构化引用是否真实存在；幻觉引用不能让任务完成。
  - 通道运行时 的 run/session 状态和工具记录会绑定真实 runId，而不是把摘要当事实源。
