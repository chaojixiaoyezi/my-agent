...
- `agent_py_agent/agent/subagents/execution_inferred_content_items.py`：根据单个普通文件 artifact 和显式 `required_content_lines` 生成逐行 `content_check`；多个普通文件 artifact 时不猜目标，避免把 CSV 行套到错误文件上。
...
- `agent_py_agent/agent/subagents/required_content_lines.py`：从 task goal/thought/description/acceptance_checks 里提取显式 `required_content_lines` / `required_content_texts` 合同；只接受结构化标记或项目符号，不从普通自然语言里猜，避免把说明文字误当必须匹配的文件内容。
