...
- `agent_py_agent/agent/agent_core/runner_dispatch.py`：runner 候选会先过滤可执行状态，再按角色阶段排序：coordinator/规划类优先，worker/产出类先于 tester/bug_finder/critic，acceptor 最后，排序发生在 `max_runners` 截断前，避免真实业务链路出现“先验收再干活”。runner dispatch record 会保存内部创建的 child ids/roles/status counts；runner 超时但已创建 child 时标记 `runner_partial_success` 和未完成 child ids。
