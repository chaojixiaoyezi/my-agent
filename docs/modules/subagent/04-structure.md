...
- `agent_core/orchestration_repair_contract.py` 是 repair/execute/verify 的共享合同层。parent-acceptance repair 和 artifact-integrity repair 都通过它给 suggested tool call 附加 `repair_contract`、`required_read_paths`、`context_manifest` 和 `context_packs`，要求同一个 repair run 完成读取失败 refs、修复、必要执行、产物验证和 refs 汇报；这条边界学习 会话运行时 的 schema 化工具返回、通道运行时 的 completion handoff 状态、长期助手 的 refs-only delegate handoff，避免 repair 链继续拆成“只修”“只执行”的漂移 child。
- `subagents/services/repair_goal_identity.py` 是 repair contract 的自然语言兜底：当模型忘记把 `repair_contract` 放进工具参数、但目标明显是“修复/repair/fix 某个文件”时，create/schedule 会按同父级、同 root、同目标文件复用已有 repair owner。`agent_core/orchestration_create_constraints.py` 也把 `修复 index.html` 识别为产品写入任务，默认给项目 workspace/product root，而不是只给私有 run 目录。
