...
- 子代理硬行为只能读机器字段、模板 id、任务状态和 refs，不读普通自然语言关键词。`required_files` / `forbidden_files` / `required_content_lines` / `required_qa_roles` / `qa_roles` / `required_read_paths` / `output_files` / `dependencies` / `workflow_depends_on` / `workflow_task_type` / `workflow_template_id` / `delegate_only=true` / `refs_only=true` 是合同；“必须生成”“不要创建”“需要测试”“小傻妞再找小小傻妞”等普通句子只交给 LLM 理解，代码层不得把它当机器事实。
...
- `agent_py_agent/agent/agent_core/orchestration_item_dependencies.py`：`create_subagents(items=...)` 的批次依赖推断层；只读取 `dependencies` / `depends_on` / `workflow_depends_on`、`required_read_paths` 和上游 `output_refs` 这类结构化字段来追加等待关系。大白话说：模型可以用自然语言向用户解释“B 用 A 的结果”，但真正让 B 等 A 的机器事实必须写进字段或 refs。
...
- `agent_py_agent/agent/subagents/dependency_artifact_refs.py`：workflow sibling 依赖的 artifact ref 解析层；当下游 `required_read_paths` 只有 `data_collection.md` 这种短名，而上游已完成并登记 `/.../subagent-id/data_collection.md` 时，调度门会认为输入已满足，执行上下文会把真实 artifact 路径补进下游 `Context Manifest`。这对齐 长期助手/通道运行时 的 refs-first 交接思想：父级/下游读机器 refs，不猜上游目录。
...
- `agent_py_agent/agent/agent_core/tool_round_execution.py` / `tool_loop_completion.py`：承接一轮工具调用执行、live context 回写、子代理 `output.json` 收口和顶层 dispatch 完成收口。当前 runner 成功写入自己的 `output.json` 后，会从该 JSON 合成 `SUBAGENT_RESULT` 并停止工具循环，避免完成态子代理继续请求模型；顶层 `dispatch_subagents` 后若 scoped tasks 全部 `DONE/VERIFIED` 且没有缺失的结构化 `required_qa_roles` / `qa_roles`，会直接用本地 task 状态、artifact refs 和 output refs 生成确定性收口，不再额外发起自由模型轮。收口检测同时支持 `path` 和 `filesystem.path` 两种 write_file 参数形态，适配真实模型常用的 bundle 写法；同一轮如果先执行了 `create_subagents` / `schedule_child_subagents`，后续依赖真实 run id 的编排工具会被延后到下一轮，防止模型用脑补 id 直接 dispatch。
...
- `agent_core/orchestration_item_dependencies.py` 在 `create_subagents(items=...)` 阶段把结构化 sibling 合同转成机器依赖：下游 input refs 匹配上游 output refs，或 `dependencies/depends_on/workflow_depends_on` 显式字段匹配上游 agent 名/产物名时，会写入真实 `workflow_depends_on`。普通自然语言里提到“基于某代理结果”不会产生硬依赖；需要确定依赖必须写成字段或 refs。
