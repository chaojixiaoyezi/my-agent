...
- `agent_py_agent/agent/agent_core/orchestration_write_guard.py`：派工前只做明显越界写入预检，真实写边界仍由工具执行层负责；当外部绝对路径疑似只是工作区前缀拼错时，会返回 `suspected_path_typo=true` 和 `suggested_target`，要求模型按建议路径重试调度，而不是申请扩大权限。
...
- `agent_py_agent/agent/tooling/_filesystem_read.py` / `agent_py_agent/agent/tooling/filesystem_artifact_guard.py` / `agent_py_agent/agent/tooling/artifact.py`：普通 `read_file` 只读工作区文本文件，不允许直接读取 `memory_archive/artifacts/tool_outputs/*.json` 外置工具输出包装；需要正文时必须走 `read_artifact`，通过已登记 `artifact_ref` / `call_id` 和 `max_chars` 分片读取，避免大工具输出重新灌回 live prompt。
