...
- `agent_py_agent/agent/subagents/parent_acceptance_auto_execution.py`：定义父级验收自动执行 Request/Result bundle，并生成 `parent_acceptance_auto_execution.json` dry-run 审计；当前只记录计划、policy ref、recommended command 和 blockers，不执行命令、不改状态。
- `agent_py_agent/agent/agent_core/dispatch_service.py`：在 acceptance 调度记录上附加 parent acceptance auto-policy 的 refs-only 摘要；包括 manual-only 和 preflight 字段，但该接入只调用 dry-run policy 审计，不执行 tests、不 apply acceptance、不触发 rescue。
- `agent_py_agent/agent/subagents/report_dispatch_models.py` / `services/dispatch_params.py`：`DispatchRecord` 和 `DispatchRecordParams` 持有 `parent_acceptance_policy_ref`、decision、action、would_execute、executed、execution_mode、automatic_execution_allowed、recommended_command、preflight_status、ready_for_automatic_execution 和 preflight_blockers 等机器摘要字段，供 JSON report、Markdown 和 watch 读取。
...
9. Parent Acceptance Auto Execution facade v1 当前只消费 auto-policy 输出并写 `parent_acceptance_auto_execution.json`；它把 future executor 需要的 request/result bundle 固定下来，但第一版 `executed=false`、`mutates_task_state=false`，不会启动进程或执行 recommended command。
