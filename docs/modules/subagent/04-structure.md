...
- `agent_py_agent/agent/subagents/result_structured.py` / `result_structured_evidence.py`：解析 runner structured output 中的 capability requests、tools、artifacts、tests、blockers，并写回任务事实；evidence、evidence_packets 和 findings 的解析写入由 `result_structured_evidence.py` 承接。`result_artifact_evidence.py` 承接 artifact refs 合并和 refs-only evidence packet 合成；若真实模型只给 artifacts 而漏写 `evidence_packets`，会从 artifact refs 合成证据包，避免正确产物因为缺证据包被父级误拒。
