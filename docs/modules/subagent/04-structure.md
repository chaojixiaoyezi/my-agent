## 2026-05-25 agent tree structure and inspect boundary

- `agent_py_agent/agent/subagents/kernel.py` 的 `SubagentKernelRun` 现在显式暴露 `task_id/run_id/parent_task_id/parent_run_id/root_run_id/depth/agent_kind`，让主、子、孙代理关系可由机器字段还原，不靠自然语言描述。
- `agent_py_agent/agent/agent_core/agent_tree_status.py` 是新的只读状态树渲染层：它从 `SubAgentManager.kernel_snapshot()` 读取 refs-first 快照，再补一个合成的 `main_agent` 根节点。
- `agent_py_agent/agent/agent_core/orchestration_tools.py` 注册 `inspect_agent_tree` 工具；它和 `subagent_board` 同属 read-only 观测入口，和 `dispatch_subagents` 的 mutating 推进入口分离。
- coordinator / leaf 默认工具集都加入 `inspect_agent_tree`，这样中间代理可以查看下级状态，但查看不等于调度。
- 结构原则：状态查看路径不能创建子任务、不能恢复、不能验收、不能改变 pending 标记；推进动作只能走 dispatch/schedule/create 这些 mutating 工具。
...
- `agent_py_agent/agent/subagents/static_flow_parent_oracle.py` 是父级静态流程验收 oracle 生成器。必需区块、按钮 action、本地资源和响应式布局都来自结构化请求，不在生产代码里内置某个真实任务样本。
...
- Typed Action Protocol 第一片已接入：`agent/action_protocol.py` 是兼容 facade，具体类型拆在 `action_protocol_core.py` / `action_protocol_tooling.py` / `action_protocol_subagents.py` / `action_protocol_subagent_dispatch.py` / `action_protocol_compact.py`；旧文本协议只作为兼容输入，后续执行/恢复/验收优先读 envelope 字段。
...
- `create_subagents` 与 runner 内 `schedule_child_subagents` 会在 JSON 响应里附带 `typed_envelope.kind=subagent_schedule`，多层派工通过 parent/root/created/reused/dispatch refs 串联；envelope 同时携带 `next_action` 与 `current_turn_run_state`，这只是 refs-first 调度事实，不代表 child 已完成。
...
- `dispatch_subagents` 现在也会附带 `typed_envelope.kind=subagent_dispatch`，把 `actionable_run_ids`、`recovery_run_ids`、blocking/unfinished ids、pending/deliverable artifact/evidence refs、`completion_status`、`must_not_report_done`、dispatch 报告 refs 和当前轮状态摘要作为稳定控制字段；父级推进和恢复优先读这些字段，不从自然语言 `message` 里猜 run id。
- `dispatch_subagents` 的 `result_refs_by_run` 是父级最终汇总的 refs-first 交接入口：每个直接 child 一行，包含 run id、状态、主产物路径、`output.json` 路径和从 `output.json.artifacts[]` 提取的 `primary_artifact_summaries`。这吸收 长期助手 “parent 拿 summary/refs、不吞 child 原文”和 通道运行时 “控制面先看 session/run 状态”的做法；父级要先读这些机器字段，只有需要核对细节时才按需读取 artifact 正文。
...
- runner artifact refs 现在会在 `result_structured.py` / `result_structured_evidence.py` / `result_artifact_evidence.py` / `result_artifact_integrity.py` 中归一化和校验：模型写短路径时，系统会优先在当前 task/output/report/allowed_write_roots 内解析成真实路径；父级 coordinator 猜错 child 产物目录时，会有界读取直接 child 的 `task.json`，按 child `artifact_refs` 和文件名把 ref 规范到真实产物路径。`workspace_roots.py` 负责从 `.my_agent/subagents/<run>`、`.my-agent/subagents/<run>`、`data/subagents/<run>` 推导项目工作区根，供 runner input gate 和 artifact integrity 共用。证据包里声明的本地产物若仍找不到，runner 会直接转 `BLOCKED / missing_artifact_refs`，不让父级把不存在的文件当作真实交付。这里仍然只检查路径和小型 task 元数据，不读取正文、不扫描整机。
- `create_subagents.allowed_tools` 现在是工具偏好提示，不是能力删除列表；模型少填 `write_file/apply_patch/run_command` 时，系统会补齐基础读写和命令工具包，防止 worker/researcher/coordinator 因提示词少写一个工具名而无法产出报告。
...
- 子代理硬行为只能读机器字段、模板 id、任务状态和 refs，不读普通自然语言关键词。`required_files` / `forbidden_files` / `required_content_lines` / `required_qa_roles` / `qa_roles` / `output_files` / `workflow_task_type` / `workflow_template_id` / `delegate_only=true` / `refs_only=true` 是合同；`required_read_paths` 只是可读线索/授权范围，不是启动依赖门；`dependencies` / `workflow_depends_on` 不再生成 sibling 等待关系。“必须生成”“不要创建”“需要测试”“小傻妞再找小小傻妞”等普通句子只交给 LLM 理解，代码层不得把它当机器事实。
- 旧文档里关于顶层 `create_subagents` 因 `grandchild` / `小小傻妞` 展示名直接拒绝的描述已废弃。现在真实层级只看 parent/root/depth 机器字段；名字只是展示文本，不作为启动硬门。若用户真的要孙代理，父级 LLM 应按 prompt 和 tree 状态决定是否让对应子代理调用 `schedule_child_subagents`。
...
- `agent_py_agent/agent/subagents/services/base.py`：子代理基础创建服务；不再从自然语言目标里推断本地写入根。额外写入根必须来自 `extra_write_roots`、workspace/product root 或上游结构化 refs，避免 URL、图片地址、报告正文被误当成本地授权目录。
- `agent_py_agent/agent/subagents/services/output_ref_rebinding.py`：run 创建后的自写产物路径绑定层。模型有时会把旧的 `data/subagents/subagent-.../file.md` 写进新 child goal；该模块只重写结构化 output 字段里的 refs，把旧 run id 换成真实新 run id，并把审计记录写入 `task.attributes.output_ref_rebindings`。输入 refs 必须走 `required_read_paths` / `source_refs` / `context_manifest`，不从普通句子里猜“读取/基于/参考”。
...
- `agent_py_agent/agent/subagents/services/hierarchy_write_policy.py`：集中维护层级写入根策略，把“拥有产物根覆盖权限”和“写入路径边界”拆开；写入根只来自结构化 `extra_write_roots`、父级 `allowed_write_roots`、workspace/product roots 和 refs。coordinator/tester/reviewer/worker 都能在授权目录内读写必要产物、报告和修复文件；角色模板只影响职责重点，不再默认剥夺读写能力。真正的硬边界仍是 workspace/write roots、系统自毁红线和显式 `product_write_policy=delegate` 这类任务级严格覆盖。
...
- `agent_py_agent/agent/subagents/services/hierarchy_scope_guards.py`：承接层级调度的空计划、显式深度/数量、结构化禁止 sibling 领域、child 写入根漂移和 domain mismatch 检查；scheduler 只消费红线阻断原因，避免调度主文件继续膨胀。`forbidden_child_scopes` / `domain_scopes` 是领域约束事实来源；普通句子里的“不要创建 depth>=4”不会触发机器阻断。`mixed_coordinator_leaf_children` 不再硬阻断，只返回 `scheduling_warnings`；父级 LLM 可以在同一次计划里混合派 coordinator、worker、tester 或 acceptor，只要任务说明和 ownership 讲清楚。4 层链路不再靠调度阶段硬性要求每层都是 coordinator；root/parent 可以按任务复杂度直接派 coordinator、worker 或 writer，最终是否满足“至少一条 4 层链路”交给验收和真实 `task.json` 事实判断。`duplicate_child_domain:<domain>` 和 `duplicate_leaf_target:<file>` 都不再作为调度硬阻断；重复 checkout/quality coordinator、多个 QA 角色看同一个 build 目录、修复 worker 重写同一文件时，scheduler 返回 `scheduling_warnings`，提醒父级用看板、消息或任务说明协调 ownership。真正红线仍是写入根漂移、真实领域越界、显式深度/数量上限和空产物 QA；同域 repair、共享资产补丁或后续协作由父级 LLM/模板/验收事实决定。`child_write_root_drift` 仍用于阻断模型把父级权威产物根 invent 成 sibling 目录后继续落盘。
- `agent_py_agent/agent/subagents/services/hierarchy_scope_guards.py` 还会提示 QA 顺序风险：有产物根的父任务如果直接请求 tester/bug_finder/acceptor，但整个子树里还没有任何 worker/writer/leaf_worker 进入 `AWAITING_ACCEPTANCE`、`NEEDS_ACCEPTANCE` 或 `DONE/VERIFIED`，会在 `scheduling_warnings` 返回 `qa_before_implementation_ready`。它不再硬阻断创建；父级可以根据任务 prompt 选择先实现、先派 QA 旁路观察，或把限制上报给用户，最终质量仍由 closeout/acceptance 统一判断。
...
- 当前轮 run id 记账只作为状态事实，不再用 `pending_dispatch_redirect` 阻止后续 `create_subagents`。父代理可以看 tree/board 决定先推进旧 run 还是临时加派新 run，系统不在创建阶段替它卡流程。
- `agent_py_agent/agent/agent_core/orchestration_create_items.py`：`create_subagents(items=...)` 的批量参数解析层；只负责把每个 item 解析成独立子任务，不再有 `orchestration_item_dependencies.py` 批次依赖推断层。`dependencies` / `depends_on` / `workflow_depends_on` 不再在 create 阶段生成 sibling 等待关系。
- `agent_py_agent/agent/agent_core/dispatch_runner_candidates.py`：runner 候选策略层；只处理显式 `run_ids`、packet/checkpoint 恢复候选和普通 runner 状态，不再用输入文件等待或 `workflow_depends_on` sibling 等待挡住 runner。真要流水线，由父代理在 A 完成后再派 B。
- 旧 `agent_py_agent/agent/subagents/dependency_artifact_refs.py` 已删除。系统不再根据 sibling workflow 关系自动把上游产物补进下游 read refs；如果下游需要上游结果，父代理应在派工时把 refs 放进 prompt/结构化参数，或等上游完成后再派下游。
...
- 已删除旧派工前正文读取门：root/父级不再因为“还没创建子代理”而被禁止读某些目录或正文。是否先看资料、再派子代理，交给用户 prompt 和模型判断。
...
- `agent_py_agent/agent/agent_core/orchestration_quality_advice_payload.py`：schedule 和 dispatch 共用的 QA advice JSON 渲染层；只输出 `ready_work_refs`、`source_run_ids`、`source_artifact_refs`、`source_output_refs` 等小字段，不读取或展开业务产物正文。
...
- `agent_py_agent/agent/agent_core/runner_dispatch.py`：runner 候选只过滤真实不可执行状态、通道、能力请求和重试预算，不再按角色阶段排序或只放行某一阶段。先做、后测、再验收由父代理显式调度和 closeout 验收控制。
...
- 工具回执的长期架构原则：模型文本只能“申请工具调用”，不能“证明工具已执行”。这个原则参考了 长期助手 的 `assistant(tool_calls) -> tool(tool_call_id)` 消息链、通道运行时 对泄漏 tool tags / downgraded tool-result 文本的剥离，以及两者对大工具输出外置化的处理。当前 my-agent 仍保留文本协议 `[TOOL_CALL]`，但真实状态只信工具循环写入的 records、task refs、filesystem refs 和外置 artifact refs；模型自己写出的 tool-looking 回执全部按不可信文本处理。
- 已删除旧委托期正文读取门：父级已有 child 时，也不再被 runtime 禁止 `read_file/read_artifact/run_command` 读取业务正文。父级可以按 prompt、tree/board 状态和子代理回报自行判断是否需要查看文件。
...
- `agent_py_agent/agent/subagents/required_file_terms.py`：层级 handoff、context bundle 和静态站父级验收共用的文件契约提取器。它只读取 `required_files` / `forbidden_files` / `required_file_refs` 这类机器字段，以及父级系统生成的精确中文合同标签；普通“必须生成”“不要创建”“读取/参考某文件”等句子不会进入机器合同。多行列表也必须挂在这些字段或标签下面，不能靠散文上下文延续。
...
- `agent_py_agent/agent/agent_core/tool_round_execution.py` / `tool_loop_completion.py`：承接一轮工具调用执行、live context 回写、子代理 `output.json` 收口和顶层 dispatch 完成收口。当前 runner 成功写入自己的 `output.json` 后，会从该 JSON 合成 `SUBAGENT_RESULT` 并停止工具循环，避免完成态子代理继续请求模型；顶层 `dispatch_subagents` 后若 scoped tasks 全部 `DONE/VERIFIED` 且没有缺失的结构化 `required_qa_roles` / `qa_roles`，会直接用本地 task 状态、artifact refs 和 output refs 生成确定性收口，不再额外发起自由模型轮。收口检测同时支持 `path` 和 `filesystem.path` 两种 write_file 参数形态，适配真实模型常用的 bundle 写法；任务本地 progress 收口不再只认识 HTML，任意工具只要返回明确产物引用，并且该引用命中机器声明的 `output_refs` / `output_files`，即可生成 refs-only `SUBAGENT_RESULT` 交父级验收。HTML 仍走额外结构完整性 fast path，其它格式不靠闭合枚举硬拒。同一轮如果先执行了 `create_subagents` / `schedule_child_subagents`，后续依赖真实 run id 的编排工具会被延后到下一轮，防止模型用脑补 id 直接 dispatch。
...
- `agent_py_agent/agent/tooling/_filesystem_read.py` / `agent_py_agent/agent/tooling/_filesystem_find.py` / `agent_py_agent/agent/tooling/_filesystem_search.py` / `agent_py_agent/agent/tooling/filesystem_read_file.py` / `agent_py_agent/agent/tooling/filesystem_artifact_guard.py` / `agent_py_agent/agent/tooling/artifact.py` / `agent_py_agent/agent/tooling/artifact_read_budget.py` / `agent_py_agent/agent/memory_archive/artifact_reader.py` / `artifact_read_modes.py`：普通文件、明确路径的大工具输出和历史产物优先走 `read_file`；`list_files` 保持目录查看职责，输出稳定排序并默认跳过 `.git`、`node_modules`、缓存目录等项目噪声；`find_files` 专门按 glob 查找文件路径，避免继续把“列目录”和“找文件”混成一个模型选择点；`search_text` 保持默认普通文本搜索，同时支持 `literal=false` 正则、`ignore_case`、`content/files_with_matches/count` 输出模式，优先用 `rg --json` 做快速后端，`rg` 不存在或异常时回退 Python 搜索，并同样默认跳过噪声目录；`filesystem_read_file.py` 承接读取、行号分页、结构化摘要和截断提示，避免工具声明文件继续变厚；读/列/find/search 同时接受顶层 `path/query/range` 和 `filesystem.path/query/range` bundle 参数，避免模型按 bundle 规范调用时退回全工作区；遇到疑似工作区路径拼写错误时会给 `suggested_target`；`read_file` 现在会识别 `memory_archive/artifacts/tool_outputs/*.json` 外置工具输出包装，读取其中 `content` 正文并按普通文件一样分页，不再强制模型换到 `read_artifact`；如果模型把外置 artifact 包装路径前缀抄错，纠错提示也保持在 `read_file suggested_target` 路径。`read_artifact` 仍保留为严格恢复/审计入口：通过已登记 `artifact_ref` / `scoped_call_id` / `call_id`、当前 run/task/request scope 和 `mode=slice/head/tail/search` 窄读，避免短 call id 跨 run 读到旧 artifact；单 run artifact 正文读取预算按 `run_id` 限制读取字符，防止下级反复展开大 artifact。
- `agent_py_agent/agent/tooling/registry_execution.py` / `agent_py_agent/agent/tooling/registry_payload_normalize.py` / `agent_py_agent/agent/tooling/parse_error_hint.py`：工具调用解析失败时返回固定重试格式提示，要求下一轮使用 `[TOOL_CALL]` + 单个 JSON 对象 + `[/TOOL_CALL]`，不回显原始坏工具正文；`registry_payload_normalize.py` 专门负责 JSON payload 校验、工具名别名和路径参数别名归一，保持执行/授权主流程薄；如果坏块像 `write_file` 的长 `content`，提示会转向较小 `write_file` 文本、`apply_patch` 小 diff，或用 `run_command` 在当前 `access_mode` 内生成文件，而不是让模型重复输出完整正文。
- `agent_py_agent/agent/tooling/content_transport_policy.py`：统一管理长内容传输规则。`write_file.content` 不能把大文件正文一次塞进工具 JSON；超过 inline 上限会提示使用短骨架、较小片段、`apply_patch` 小 diff，或通过 `run_command` 在当前 `access_mode` 允许的目录内运行脚本生成文件并只回传路径/摘要。普通任务不再依赖 `controlled_exec` grant 作为长内容写入绕路。
- `agent_py_agent/agent/tooling/content_recovery_mode.py`：当上一轮因为长 `write_file` 正文导致 parse error 或 inline 上限拒绝时，`_tool_loop_service.py` 会给下一轮 live prompt 追加 `long_content_recovery_mode`。它要求模型避免再次复制同一段大 JSON，改用较小 `write_file`、`apply_patch` 或 `run_command` 生成文件。这个做法吸收 长期助手/通道运行时/会话运行时 一类“输出外置、摘要回传、小 diff 优先”的方向，但实现上只落成一个独立策略模块，后续扩展流式写入或 artifact-backed 写入时不用改散落提示词。
- `spawn-subagents --role coordinator --agent-name <name>` / 顶层 `create_subagents(role=coordinator)` 是真实层级 E2E 的 seed 入口：外层只创建一个 root/coordinator，自动授予 `schedule_child_subagents` / `dispatch_subagents` / `subagent_board`、基础读写、web 取证和报告写入工具；shell/exec 仍走后续受控网关，不靠角色模板暗开。root/coordinator seed 只继承结构化 required/forbidden 文件合同、refs、context packs 和写入根；不再从普通 prompt 句子或 goal 文本里抽产物路径。最终业务代码/页面/文档产物仍应派给 worker/writer，角色职责和父级验收负责约束“有权限但不乱写”。后续子、孙、孙孙任务必须由上一层 runner 通过 orchestration tools 创建，外层不得直接替下层创建。
...
- `agent_py_agent/agent/agent_core/runner_dispatch.py`：runner 候选会先过滤可执行状态、通道、能力请求和重试预算，然后按创建/显式 run_ids 顺序放行到 `max_runners`。它不再做角色阶段排序，也不再检查 workflow refs 依赖。runner dispatch record 会保存内部创建的 child ids/roles/status counts；runner 超时但已创建 child 时标记 `runner_partial_success` 和未完成 child ids。
...
- `agent_py_agent/agent/subagents/execution_inferred_content_items.py`：根据普通文件 artifact 和显式内容合同生成逐行 `content_check`；单个普通文件 artifact 可吃全局 `required_content_lines`，多个普通文件 artifact 只吃 `required_content_lines[文件名]` / 自然语言文件块这类 per-file 映射，避免把 CSV 行套到错误文件上。
- `agent_py_agent/agent/subagents/execution_test_checklists.py`：过滤缺 `command/file_path/content_pattern/site_root` 的模型空壳测试清单；只有静态站点或内容行机器验收已经生成时才过滤，避免没有兜底时吞掉失败线索。
...
- `agent_py_agent/agent/subagents/required_content_lines.py`：从 task goal/thought/description/acceptance_checks 里的结构化内容合同提取普通文件验收要求；支持 `required_content_lines` / `required_content_texts`、`required_content_lines[file.md]`、字段后的 bullet 和 fenced block。它只吃机器字段和字段续行，不从普通用户话术或散文里猜内容要求。
...
- `agent_py_agent/agent/subagents/parent_acceptance_content_contracts.py`：父级验收普通文件内容合同 bundle helper；preflight、confirmed execution 和 findings 都通过它拿 `required_content_lines` / `required_content_files`，避免三条入口散装参数漂移。
- `agent_py_agent/agent/subagents/static_required_files.py`：从 task goal / thought / description / acceptance_checks 里的结构化 `required_files` 和内部必需文件标签提取 `index.html`、`style.css`、`app.js` 等静态 Web 必需文件名；不从普通正文猜业务文件，也不读取产物正文。
- `agent_py_agent/agent/subagents/execution_test_items.py`：父级验收 tests 的预处理层。除安全 cwd 推断外，还会把真实模型常写的 `cat <workspace内文件>` 内容验收归一成受控 `content_check`，只有能从字段或窄文本中拿到明确期望内容时才转换；系统仍不把 `cat` 加进命令 allowlist。静态 Web 自动测试只在 artifacts 足够明确时补 `static_site_check`，普通文件自动测试只在 `required_content_lines + 单文件 artifact` 足够明确时补逐行 `content_check`，已有同类测试不会重复追加。
...
- `agent_py_agent/agent/contracts/*`：主代理和子代理共用的四块硬合同层已经落成：`action_protocol_*` / `tooling/registry_envelopes.py` 负责结构化工具协议（call_id、operation_id、RunScope、ToolCallResultEnvelope），`contracts/state_machine.py` 负责统一运行状态快照和恢复建议，`contracts/artifact_acceptance.py` 负责产物引用和格式/质量验收（ArtifactRef、hash、size、finding），`contracts/acceptance_contract.py` 负责把状态、真实测试和产物验收合成最终完成结论。普通父级验收现在会对已解析到的 artifact 走同一套 artifact acceptance，不再只看“路径存在”。
...
- 已删除旧 refs-only body-read 保护：读取权限不再由“父级是否已有 child/acceptor 是否完成”决定。真实文件读写边界仍由通用工具权限、workspace 边界和 closeout 验收负责。
...
- gateway / daemon 的 dispatch watch 现在区分 heartbeat tick 和 durable audit record：没有任何 subagent run 时进入 `idle`，不调用 `dispatch_subagents`，只保留一条 coalesced idle 记录；重复 due-check/inspect 这类 no-progress 轮次也会复用 `dispatch_loop` 的进展判断，避免空闲后台把 local_store 和 watch report 刷爆。连续空转到阈值后状态为 `idle_by_limit` 并进入空闲睡眠，不再 tight-loop。
...
- `hierarchy_scope_guards.py` 的同父级重复领域去重只使用结构化 domain scope、role/template identity 和已登记 refs；不会再从 goal 自然语言里兜底提取领域词。这样恢复树里的编号 checker siblings、自动 QA children 或共享同一 workspace 路径的不同 child 不会被自然语言片段误判同域。
...
- `agent_core/orchestration_repair_contract.py` 是 repair/execute/verify 的共享合同层。parent-acceptance repair 和 artifact-integrity repair 都通过它给 suggested tool call 附加 `repair_contract`、`required_read_paths`、`context_manifest` 和 `context_packs`，要求同一个 repair run 完成读取失败 refs、修复、必要执行、产物验证和 refs 汇报；合同同时携带 `full_success_checks`，避免 repair worker 只修最新报错而丢掉原始验收条件。这条边界学习 会话运行时 的 schema 化工具返回、通道运行时 的 completion handoff 状态、长期助手 的 refs-only delegate handoff，避免 repair 链继续拆成“只修”“只执行”的漂移 child。
- `agent_core/orchestration_repair_contract.py` 同时要求 repair worker 输出可执行父级验收项：`output_tests_required=true`，推荐 `file_check/content_check/static_site_check/command`。这让 repair 子代理不只“修完口头汇报”，还要把如何验收修复结果写回机器字段，父级后续按 tests/refs 检查，不靠自然语言自证。
- `subagents/services/repair_goal_identity.py` 只保留旧 API 兼容；repair owner 身份必须来自 `repair_contract`、`context_packs[*].contract`、target artifact refs 或 required read refs，不再从“修复/repair/fix 某个文件”这类自然语言目标里猜。`agent_core/orchestration_create_constraints.py` 的产品写入判断只读结构化 output refs，`agent_core/orchestration_create_target_roots.py` 优先从 target/read refs 推导产品写入根；目标 ref 是文件时会归一为父目录，避免修复同一个产物时写到 sibling 目录。
...
- `agent_core/tool_context_orchestration_summary.py` 在 dispatch 大输出被外置后会保留 `recovery_action_counts` 和 `recovery_strategy_preview`，让 root 不需要读取完整 artifact 就能看到 packet-first 恢复动作；`tool_context_recovery_summary.py` 承载 recovery strategy 预览，避免 live prompt reducer 再次接近 code-size 风险线。
...
- `agent_core/orchestration_recovery_batches.py` 是恢复批次计划层：从 `recovery_strategies` 按 `recommended_action` 分组，把 packet 续跑、takeover 写回、leader recovery 和 no-progress 熔断分成不同批次。批次内只在单 run packet 续跑时附带 `runner_instruction`，多 run 不共享自然语言续跑指令；takeover 批次先 `execute_runners=false/max_runners=0` 写回接管状态，不继续启动挂死 runner。
...
- 已删除旧委托读/写硬门：不再用“当前 root 已 create/dispatch 子代理”来禁止父级读取正文或直接写业务产物。父级是否亲自写、是否继续派工，靠 prompt、任务树事实和最终 closeout 验收，而不是额外 runtime 卡点。
...
- Runner contract 和文件工具说明明确 `write_file` 会在授权 `allowed_write_roots` 内自动创建父目录；叶子节点不应因为目标产品目录尚不存在就上抛 shell/mkdir capability request。长 CSS/JS/HTML 或大段代码应优先走 `write_file` 的短骨架/安全落盘、`apply_patch` 小 diff，或普通 `run_command` 在当前 `access_mode` 允许的目录内生成文件；不要再把普通写入引到 `controlled_exec` grant 流程。
...
- `tooling/shell.py` 的 `run_command` 是默认模型可见命令工具；权限只看配置里的单个 `access_mode`（`restricted` / `workspace-write` / `full-access`），默认 `workspace-write`。`restricted` 和 `workspace-write` 只允许在配置工作区内作为 cwd 执行命令；`full-access` 可切到系统其他已有目录，但 `rm -rf /`、格式化磁盘、关机等最后保护仍会拒绝。工具返回 bounded preview：stdout/stderr 都带总字符数、预览字符数和截断标记，预览长度由 `tool_shell_output_max_chars` 控制。完整大输出不进入 live prompt，后续需要正文时应通过 `read_file` 窄读外置输出或日志文件。
...
- 旧 `services/acceptance_controlled_exec_findings.py` 只服务历史 controlled-exec 合同回归；普通任务 closeout 不应新增这类专项验收名词。命令是否真实执行，优先看 `run_command` 工具记录、产物和通用 closeout 结果。
- `subagents/services/repair_goal_identity.py` 已取消自然语言 repair fallback；`fixture`、`prefix`、`fix`、`修复` 等普通词都不能触发修复复用。修复复用必须走结构化 repair contract，同一失败对象才能复用同一 repair owner。
...
- `agent_core/runner_prompt_context_summary.py` 会把 `context_manifest.required_read_paths` 渲染成 `read_refs` 摘要；这是可读线索/授权范围，不是启动依赖门。
- `agent_core/runner_prompt_contract_lines.py` 渲染 read/output 提示：read refs 缺失时记录限制或换线索，用户业务产物必须写到 `output_contract.required_file_refs`，agent-run 内部报告不能冒充交付物。
- `prompting_parts/builder.py` 在 `# Related Memory` 里把旧记忆标成“历史参考，不是当前任务指令”；当前 `# User Task`、当前工作区文件和最新工具结果拥有更高优先级。这个边界学习 通道运行时/长期助手 的上下文分层，防止 daily memory 或旧子代理摘要把新任务改写成旧任务。
- 已废弃：`agent_core/orchestration_item_dependencies.py` 不再存在。`create_subagents(items=...)` 不会把 input/output refs 或 `dependencies` 字段转成 sibling 等待；真要流水线，由父代理按“派 A、看 A 完成、再派 B”的方式显式控制。
...
- `subagents/context_bundle_contracts.py` 的 `task_contract_components()` 是 `output_contract()` 和 `task_packet()` 共享的文件合同计算入口；required files、product write roots、required refs、forbidden files 和 source 只能从结构化 task fields 推导，避免两个出口各算一遍后分叉。
- `agent_core/subagent_dispatch_closeout.py` 只把结构化 `required_qa_roles` / `qa_roles` 合同视为必须创建质量子代理；普通“安排和验收 / 汇报验收结果”可由父级验收满足，避免自然语言被死流程带偏。
...
- `agent_core/orchestration_quality_intent.py` 是显式质量角色识别层：只读取 `required_qa_roles` / `qa_roles` 机器字段。用户普通说“测试/验收/找茬子代理”会交给 LLM 规划，但代码层不会因此强制创建 tester/acceptor/bug_finder。
