...
- `agent_py_agent/agent/subagents/manager_memory_gate.py`：给 manager 增加候选列表和 review decision 写回方法；写回只更新 gate 文件和 checkpoint refs，不导出 memory/skill。
- `agent_py_agent/cli/_memory_gate.py`：实现 `subagents-memory-gate` CLI，显式列出候选或写回 approve/reject/needs_evidence。
...
17. 只有后续显式 review/gate 通过且再触发独立导出流程后，候选才允许进入长期 memory 或正式 skill 流程；当前 approve 不会自动导出。
18. `memory-resume` 在跨天恢复时用 archive/LocalStore 作为线索，最终推荐读取任务目录里的事实源和 checkpoint artifacts，再由父级决定是否验收；这只是恢复入口推荐，不代表子代理写入主代理长期 memory。
19. workflow preview 仍可通过 CLI dry-run 展示；真实路径已接入 `create_run(... workflow_mode="plan|auto")` 和 `subagents-dispatch --apply --workflow-mode auto`，可把父任务上的 `workflow_plan` 物化为 worker 子工单。LOG 专项 apply path 和 runner 恢复 scenario 继续作为真实任务记录的先行验证样本。
