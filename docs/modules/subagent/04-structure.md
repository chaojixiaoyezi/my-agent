...
9. Parent Acceptance Auto Execution facade v1 当前只消费 auto-policy 输出并写 `parent_acceptance_auto_execution.json`；它把 future executor 需要的 request/result bundle 固定下来，但第一版 `execution_allowed=false`、`guard_status=blocked`、`executed=false`、`mutates_task_state=false`，不会启动进程或执行 recommended command。
