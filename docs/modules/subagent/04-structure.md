...
- `agent_py_agent/agent/subagents/required_content_lines.py`：从 task goal/thought/description/acceptance_checks 里提取明确内容合同；支持结构化 `required_content_lines` / `required_content_texts`、`required_content_lines[file.md]`、锚定代码块，以及“下面 N 行要一字不差”这种普通用户话术。它只吃带数量、文件名或内容锚点的明确块，不从散文里泛猜，避免把说明文字误当必须匹配的文件内容。
...
- `hierarchy_scope_guards.py` 的同父级重复领域去重只使用稳定业务域词；会过滤 generated run-id 片段、数字编号和 `grand/one/two` 泛词，避免把恢复树里的编号 checker siblings 当成重复业务 coordinator。goal 兜底提取领域词前会剥离绝对路径、文件名和 inherited parent blocks，并过滤 `users/模型助手/code/shop/tests/css/js` 等路径或脚手架词，避免多个不同 child 因共享 `/Users/.../my-终端应用/...` 被误判同域。
...
- `subagents/services/repair_goal_identity.py` 是 repair contract 的自然语言兜底：当模型忘记把 `repair_contract` 放进工具参数、但目标明显是“修复/repair/fix 某个文件”时，create/schedule 会按同父级、同 root、同目标文件复用已有 repair owner。`agent_core/orchestration_create_constraints.py` 识别产品写入意图，`agent_core/orchestration_create_target_roots.py` 优先从 target/read refs 推导产品写入根；目标 ref 是文件时会归一为父目录，避免修复同一个产物时写到 sibling 目录。
