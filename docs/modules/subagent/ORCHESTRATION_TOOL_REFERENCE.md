...
- 创建一个子代理时传顶层非空 `goal`，它就是该 child 的完整目标；使用 `items` 批量创建时，每个 item
  必须有自己的非空 `goal`，顶层 `goal` 可省略，也可作为不参与机器裁决的批次说明。
  这些 `goal` 都只是内部派工说明，与用户命令 `/goal` 无关；普通聊天中的实际任务同样可以自主派工。
  两种形态都缺失、空批次且没有单目标、或任一 item 缺目标时，会在创建任何 run 前返回 typed 参数错误，
  不能静默退回主代理独自完成，也不能从用户正文猜目标。
