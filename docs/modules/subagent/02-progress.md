...
## 已完成
- 2026-05-17 非网页 repair-wave 内容合同已落地：父级验收现在支持从任务验收文本里的 `required_content_lines` 提取普通文件必须包含的字面行，并在只有一个普通 artifact 时自动生成 `content_check`。这吸收 会话运行时/长期助手/通道运行时 的结构化 refs-first 思路，把“模型说内容齐了”降级为备注，把真实文件内容检查变成机器事实。
...
- 2026-05-16 普通中文流水线第七个真实问题已修复：顶层 `dispatch_subagents` 后如果本轮 scoped 子代理全部 `DONE/VERIFIED`，工具循环直接用持久 task 状态和 artifact/output refs 收口，不再额外发起自由模型轮；如果用户明确要求 tester/acceptor 而还没创建，仍交回 root 继续派质量角色。这吸收 通道运行时 completion event / 长期助手 terminal task state 的做法，避免完成后被旧上下文带偏。
- 2026-05-16 普通中文流水线第六个真实问题已修复：`allowed_tools` 不再参与 create role 纠偏；`create_subagents(items=...)` 里的 worker 即便继承 `create_subagents` / `dispatch_subagents` 等宽工具授权，也不会被误升成 coordinator。对齐 通道运行时/长期助手 的“工具能力”和“任务角色/依赖状态”分离原则。
...
- 2026-05-16 真实普通中文流水线复测暴露的“依赖视野太窄”已修复：第二轮只调度下游 `run_ids` 时，runner 候选仍会用完整可见任务表查 `workflow_depends_on` 的上游状态；这对齐 长期助手 kanban/通道运行时 session 控制面的做法，避免上游已完成但下游看不到依赖事实，进而诱导 root 亲自补写文件。
- 2026-05-16 Task18 短片复测暴露的 sibling pipeline 抢跑已收口：`读取上游输出 data/...` 不再被误判成当前任务自己的输出；`create_subagents(items=...)` 会把“基于小傻妞-数据收集/内容编写的结果”这类自然引用固化成 run 级 `workflow_depends_on`，显式 `run_ids` 调度也必须等上游完成。对照 长期助手 的 kanban parents 和 通道运行时 的 session/run 控制面，这次修复把依赖关系放进机器字段，不再靠提示词赌模型按顺序跑。
...
- 真实测试目录：`/Users/example/my-终端应用`。临时 E2E 配置把 workspace、subagents、gateway、LocalStore 都指向测试目录下的 `_agent_runtime/`，避免再把测试文件写回源码目录。
- 产物目录：`/Users/example/my-终端应用/deliverables/e2e_root_reply_20260514_003941/build`，包含 `index.html`、`styles.css`、`app.js`、`README.md`。
...
- 真实验证目录：`/Users/example/my-终端应用/recovery_cases/case02_takeover_run`。外层只运行 root，root 通过 `schedule_child_subagents` 和 `dispatch_subagents` 推进。
- 已修正：
...
- 真实产物：`/Users/example/my-终端应用/recovery_cases/case09_shopping_recovery_e2e/deliverables/shopping_site/build`，包含 `index.html`、`product.html`、`cart.html`、`checkout.html`、`register.html`、`login.html`、`styles.css`。
...
- 真实验证目录：`/Users/example/my-终端应用/recovery_cases/case11_main_multi_compact_clean4`。
- 真实 apply ids：
...
- 真实验证：Case 12 root-only 真实运行通过，root 创建 1 个 worker，worker 写出 `/Users/example/my-终端应用/recovery_cases/case12_subagent_multi_compact/算法测试方案.md`，最终 `total_runs=1`、`done_verified=1`，耗时约 `90.6s`。
...
- 真实验证目录：`/Users/example/my-终端应用/phase3_hierarchy_e2e_user_style_fixed5`。
- 结果：
...
- 中文说明：这轮开始按 会话运行时/通道运行时/长期助手 的成熟思路收敛子代理：结构化任务包优先，角色只追加职责，不再用一堆用户配置和流程限制把子代理变成脆弱的小工具。
...
- 对照结论：通道运行时 会把 active subagent context 标成运行态事实，不是用户指令；长期助手 worker context 把当前 task body 放在第一事实源，历史尝试/父任务 handoff 只是上下文。
...
- 对照结论：通道运行时/长期助手 都用 run/task handle、parent links 和 handoff refs 表达依赖，不让父级从自然语言路径里重新猜谁等谁。
...
- 对照结论：长期助手 同题直接生成 `data_collection.json` / `content_writeup.json` / `final_report.md`，下游输入是结构化文件；通道运行时 同题主会话先写 `data.json` 并 spawn 后续 session，使用 workspace/session refs，而不是把口语路径当唯一事实。
...
- 真实复测：`/Users/example/my-终端应用/third-party-eval/runs/my-agent/dependency_pipeline/20260516-222638/pipeline_dependency_check` 完成，最终 `final_report.md` 在任务根目录，报告 worker `DONE / VERIFIED`，未再创建修复 worker 兜底。
...
- 下一步：重跑全量门并推远端；后续继续用 通道运行时/长期助手 的 refs-first 思路对照更大任务。
...
- 对照结论：通道运行时/长期助手 更偏向 session/task handle 和 task-local workspace；它们不会让模型预先猜一个未来 run id 再把这个猜测当事实源。
...
- 下一步：重新跑 Task18 short 的 my-agent 真实链路，并同步跑/检查 通道运行时、长期助手 对照；如果仍有循环，继续优先修结构化 closeout/输入输出合同，而不是加提示词补丁。
...
- 对照结论：通道运行时/长期助手 都更强调 task/session workspace 是事实边界，产物 refs 相对这个工作区解析；不会只在某个内部 run 目录里找公共任务产物。
...
- 对照结论：通道运行时/长期助手 更偏向 session/task handle 和控制面状态事实；重复派工时应该复用已有 run 或返回已有 run ids，而不是依赖模型自己记住“刚刚已经创建过”。
...
- 真实复测：`/Users/example/my-终端应用/third-party-eval/runs/my-agent/task_18_short/20260517-003526/task_18_short_github_star_growth_xlsx` 一开始只创建 3 个 child，未再出现同名第二批；数据收集在授权网络能力后重跑到 `DONE/VERIFIED`，内容编写也 `DONE/VERIFIED`。
...
- 对照结论：会话运行时 的 spawn/wait/list 用工具 schema 固定任务状态；通道运行时 completion handoff 返回稳定状态和结果；长期助手 delegate 父级只拿子任务 summary/result。我们这里不再只给一段自然语言 repair goal，而是同时返回 `repair_contract`、`required_read_paths`、`context_manifest` 和 `context_packs`。
...
- 对照结论：会话运行时 的 spawn/wait/list 走结构化工具 schema；通道运行时 的 subagent 是独立 session/run，但 completion/status 由控制面传回；长期助手 delegate 让子任务拿到干净任务空间和可用工具集，父级只接 summary/refs。三者都不要求模型靠 prompt 猜工具名、路径开头或“谁是 root”。
...
- 对照结论：会话运行时 用 thread/run id 推进，通道运行时 用 session/run 控制面看状态，长期助手 用 delegate task/session refs 做隔离。共同点是“已经创建过谁”来自机器字段，而不是让模型靠自然语言记忆。
...
- 对照结论：会话运行时 的 tool/collab item 会用结构化 status/result 字段承载工具事实；通道运行时 的 parent-owned background session 会把 parent/session/run 状态进任务注册表；长期助手 delegate 明确区分 child summary 和 task refs。共同点是“控制事实要在协议对象里”，不是让模型从 message 里再抄。
...
- 对照结论：长期助手 delegate 父级拿 summary/refs，通道运行时 控制面知道每个 run 的状态，会话运行时 工具结果用结构字段承载事实。我们这里让 QA 波次读 `ready_work_refs` / `source_run_ids` / `source_artifact_refs` / `source_output_refs`，不靠自然语言猜产物。
