...
旧信封。`conversation/runtime.py` 在预扫前后按原 `wakes.pending_one` 读回精确编号与 thread/task/reason；
缺失或已处理的信封不能再启动工作片。`background_claim.py` 取得原 claim 后再调用窄来源查询，封住领取
间隙的变化；仍先执行原停止、终态和恢复裁决。信封内容变化时释放本次 claim，由原队列重新选取，不能
把旧快照当成新的执行权；冻结交付仍只重投原模型回复，不再运行模型。

批次中任一成员已被消费时，不确认其余未采样成员；它们留在原队列等待重选。不新增通知状态副本、
定时器或文本去重，不因旧通知失效而确认另一个新 DONE 通知。已开始的能力工作片仍按上一节的当前
父任务状态交付，孩子保留 BLOCKED 不会吞掉父级已经完成的自然回复。

`subagent_runner_finished` 的 BLOCKED/FAILED 等状态属于原工作片历史。投递裁决读取同一 canonical 根
查询，并覆盖 `metadata.events` 中每个批次成员及 observation 后备的全部来源。当前仍有真实失败时允许
及时通知；源 run 已接续或完成时，旧失败不再绕过当前子树及未读生命周期邮箱的收口检查。当前状态
读取错误或来源缺失保持未知，不能回退用历史失败放行；仅已 completed 的父任务保留原读取错误下的
交付规则。控制命令、活跃 Goal、普通新消息和独立新事件的既有语义不变，历史结果不被改写。

参考仅窄读 Codex `578c1b22` 的 `tasks/mod.rs` 中按原 turn 身份释放后重查 pending 工作，以及 Hermes
`0a62610f1` 的 `gateway/run.py` 中结束后处理当前队列、核对原投递事实的局部。借鉴精确身份和当前事实，
不移植其队列或正文判据。验证使用真实隔离 wake store、claim 与 fake model：覆盖预扫内旧通知被确认、
当前 DONE 新通知保留、领取间隙变化、批次真实失败、未知状态、停止、冻结重投及能力片完成；真实 TUI
由发布主线复验，旧失败原账保留。

候选验证与交接：两个独立因果用例在旧代码上失败、修复后通过；后台 runtime、claim、conversation wake、
直属父生命周期、唤醒发布恢复和资源停止六文件组合304项通过。Ruff、文档同步、严格尺寸、diff、clean-package
均通过。资源停止夹具显式迁移新增来源查询，生产依赖不添加默认旁路。发布主线负责组合授权接续验证、
同包部署和新版原生TUI；与Compact／历史范围读取开发保持文件归属隔离，未完成真实验收前不关闭本步。
