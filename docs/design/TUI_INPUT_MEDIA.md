...
## 决策模型集成边界（2026-09-23，本地）

本文件原真实M3证据对应媒体提交 `3adb61904`，不代表其与决策模型/Compact整合后的候选已实机通过。普通选定模型仍可发送媒体；主/子自动选模遇到原始媒体或未知块保留原模型，不用附件字节数证明候选窗口足够。

自动Compact在计量未知时保留请求；实际overflow后的强制恢复在摘要与CAS之前拒绝未知容量，完整媒体refs及工具往返继续保存在原历史。手动transcript Compact仅覆盖完整文字前缀，从首个媒体所属回合起保留全部后缀，分段摘要不把媒体引用JSON冒充已读附件。当前后端文字思考可保留，跨模型推理签名不能搬运。完整证据与剩余项见[容量审计](../tasks/DECISION_MODEL_CONTEXT_AUDIT.md)。
