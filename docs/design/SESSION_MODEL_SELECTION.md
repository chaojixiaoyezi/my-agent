...
真实单 Gateway TUI 的多窗口 A/B、不同 owner C/D、重开、切小窗口与后续任务由集成线验收。
管理员显式共享模型目录是独立切片；引用解析可在既有 `selected_model_config` 和 child 显式解析入口扩展，
不能将共享密钥复制进用户会话或 TUI 回执。

建议下一步：集成菜单与共享目录，执行真实 TUI 验收；独立 Compact 修复仅在入口配置一致性上联合验证，
