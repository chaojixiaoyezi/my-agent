...
状态：第 4 步内部环境准备器已有本地实现，尚未接启用命令、发布或完成真实 TUI 验收。安装表仍是插件状态唯一权威；环境目录存在不等于启用。
...
接入原启用工具前，须先在原操作的固定输入/资源声明中冻结候选引用与解释器指纹，再开始准备，
确保中途崩溃也能按原操作定位候选；不能只依赖函数成功返回来登记归属。
...
输入为空、输出有界或不采集，等待有期限且响应原取消 token。超时/取消使用原进程出生标识和终止回执，不能按名字批量杀进程。
