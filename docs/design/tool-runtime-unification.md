...
渠道不可用软提示读取既有错误分类，不以全部 `ok=false` 推断“系统失败”。
当前仅工具不可用与可重试网络故障参与累计；测试/编译返回非零、参数、文件状态、权限、
取消、超时原因未知与未知错误沿各自原回执恢复，不额外引导模型枚举其它渠道。
同一 tool/call_id 只消费最新宿主回执；缺身份不计数，重复归档不冒充多个故障。
提示仍每工具一次、阈值 0 关闭，不发额外请求，不自动重试、不允许通过换工具绕过拒绝。
这修正的是诊断分类，不删除历史或更改原错误码，也不保证模型一定停止语义重复。

归档预览与模型正文分别投影：`output_preview` 仅供日志/外置大输出摘要；归档层裁定内联的结果
必须保留 handler 已限制大小的整页正文。否则读取结果会被二次裁到预览长度，丢失尾部、
`file_version`、`next_read`/下一页游标，模型既读不全，也无法按原分页合同继续。
`read_file`、`read_artifact` 和显式 `preserve_prompt_output` 共用生产归档入口修复；
真正外置的大输出继续返回安全预览和 owner-scoped 恢复引用，脱敏/外部数据边界不变。
这修复了一个可能放大重复读取的确定性信息丢失问题，不保证消除模型自身生成退化；
慢模型必须用原失败任务进行真实 TUI 复验，不能只以回归通过宣称不再复读。

Shell 命令取消旧的 2,000 字符人工上限：Schema 和 handler 同时移除，不要求模型为了通过
长度门反复拆改合法脚本。输入非空、结构解析、危险命令、owner 路径、审批、沙箱、超时及
输出预算各守原边界；没有截断、自动写临时脚本或绕开平台真实进程启动错误。
`COMMAND_TOO_LONG` 仅保留旧归档诊断识别，不是当前工具仍会产生的新拒绝。

原生模型输入的执行事实只补最近完成批次，不把近期调用按成功/失败/最近明细重复复制到每轮。
批次读取宿主归档的精确 turn_id/tool_round，调用状态沿用原规范化入口，批准、失败、未知和 refs 不丢。
展示缺省不代替最终操作核验；后者仍读取全轮账本。原始结果、已有会话和缓存前缀不作追溯裁剪。
...
代理树查询复用同一稳定进展元数据：只比较授权子树身份、状态、实际工具进展时间和结果引用；
排除查询自身活动、心跳与距上次进展秒数。时钟增长不再绕过成功软观察，子代理正常慢思考不因此被判死。
