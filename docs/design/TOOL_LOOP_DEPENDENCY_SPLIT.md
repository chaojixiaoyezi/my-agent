...
> 状态更新（2026-09-23）：决策分支合并版已改用 v3 四元身份，本节及"来源引用修复的兼容与回滚边界"中的三元方案由[合并节](#两线合并后的来源身份与模型轮结果决策分支吸收-main2026-09-23)取代。问题背景和验证场景仍然有效。
...
> 状态更新（2026-09-23）：合并版的数据兼容与回滚以[合并节](#两线合并后的来源身份与模型轮结果决策分支吸收-main2026-09-23)为准。
...
## 两线合并后的来源身份与模型轮结果（决策分支吸收 main，2026-09-23）

状态：已在决策分支本地合并 main `66a598cf3`，完成冲突解决及测试；等主线 owner 验收后再合入 main，尚未部署。上文"逐调用来源"两节描述的是主线当时的三元设计，已被本节取代。

**唯一来源身份**：工具覆盖一律采用 `conversation/compact_tool_identity.py` 的四元 refs（run/attempt/turn/call），写入 `conversation_compact_checkpoint.v3`。v3 同时携带 scope、摘要基础和 transcript/工具混合来源，候选编号对 scope、base、来源与保留 refs 及创建时间做内容寻址。主线原 `tooling/call_ref.py` 的三元 ToolCallRef 已删除，不保留转发。四元身份多出 turn，因此同一 attempt 内不同模型轮复用厂商编号时，也不会互相覆盖。

**保留主线行为**：
- 缺完整身份的记录始终保持可见，只压缩身份已知的来源；全部未知时返回 compacted=false，并附结构化的 `source_resolution=uncertain` 和 `uncertain_call_count`。逐条记录上的标注在生产代码中没有消费者，不再写入。
...
- `tool_loop/model_turn.py::ModelTurnRequest(prompt, response, params)` 把实际产出响应的那份参数交回。计量、恢复、确认三个回调显式接收这份参数，失败尝试的参数不会被计量或确认。
- `_model_turn_or_retry` 返回具名结果，`execute_tool_loop` 显式写 `params = turn.params` 并返回 `ToolLoopRunResult`。选模前后都不使用回调或闭包暗改参数。
- 发送前被宿主拒绝时，退回同一次尝试的原参数和原 prompt；HTTP 之后不再切回。
...
**共享 tool_context（2b 起为显式合同）**：`replace_recovery_history` 用 `dataclasses.replace` 生成候选参数，只替换了历史种子、Compact 上下文、`runtime_injections`、`live_archive_state`（深拷贝）和 `tool_ir_history`，`tool_context` 仍与原参数共享同一个列表。所以恢复提交后，写进任一份参数 `tool_context` 的内容（插话、空响应修复提示）在另一份里也看得到。目前这恰好保证插话只出现一次，原先它是隐式别名。2b 选择写成显式合同：`replace_recovery_history` 注释写明共享，`test_compact_recovery_release.py` 断言候选与原参数的 `tool_context` 是同一个对象，行为不变。以后若要改为复制，须先迁移这条合同，并按结构化记录转移尚未发送的条目。
