...
**已确认的相邻缺陷（未修，待主线 owner 决定）**：`_model_turn_or_retry` 有两个同轮重跑分支：空响应修复，以及插话取代不可用响应。它们在提交后仍用压缩前的原参数调用 `build_tool_loop_prompt`，共享预算回收在原参数上触发运行中压缩，同样抛上面的 `ValueError`。
- 后台原生 IR 探针：候选返回空响应后，修复分支在原参数上重建并崩溃；有没有本修复，结果相同。
- 建议修法：异常分支里若有已提交记录，改用候选参数构造修复提示和后续轮次，并在 `_model_turn_or_retry` 结束时清除记录。
