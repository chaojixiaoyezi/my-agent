...
2026-06-01 补充：写入类工具把显式声明的交付产物全部落盘后，运行循环会给模型插入一次
`[delivery-completion-soft-hint]`。这只是中文软提醒：如果这些文件已经是最终交付物，
请做必要查漏补缺后尽快调用 `submit_for_acceptance`，或在无需落盘验收的任务里直接汇报。
它不触发 closeout、不改变任务状态、不阻断继续工作，也不会因为只读、搜索或失败工具输出而出现。
