...
本地修正（待发布验收）：只有未激活版本生成环境计划并声明该计划资源；已激活或缺失版本不构造空的 declared 资源域。
它们沿原操作链核对原激活并返回 unchanged，或明确拒绝缺失／未清理激活；不创建新环境、不换激活代次，也不降低审批与权限。

状态：第 4 步本地源码已串通启用、普通工具组合、停用后释放、重新启用及卸载，显式业务命令仍待接通。安装表继续是唯一当前状态，
...
当前管理 disable、enable/普通调用、释放消费及 remove 见下文；显式调用须接通后一起做真实验收。
...
## 卸载与重新安装边界（本地实现，未发布验收）

卸载沿原 HostCommand 和 ToolExecutor，先停用、确认原执行器及资源退出并释放旧环境，
再用释放返回的完整安装记录在原锁内 CAS 删除安装行。不能重新按插件名挑选目标；原准备仍在执行时保留安装。
原管理结果成功持久化并严格读回之后，才消费固定退出证明，并在同一插件锁内回收无人引用的原包。
同摘要重新安装已经发生时保留包；清理失败保留原成功结果和待收尾状态，原请求重送只补清理，查询不写。
UNKNOWN 不因当前插件缺失而变成功，也不重跑卸载。用户工作目录、产物及原操作历史不删除。

安装表不新增墓碑或第二套代次计数。公开目录 v3 从原最后提交的操作身份、动作、插件、包及版本
派生 `installation_ref`；不包含私有配置及其摘要。卸载后同包重装即使版本重回 1，旧目录仍必须失效。
删除回执只保存在原操作结果，不能写回一条已安装记录；整表原子替换及提交异常读回继续使用原 Store。
替换后刷盘或锁清理出错，但原锁已严格读回确定删除且回执完整匹配时，沿原操作保存成功并附存储警告；
未提交与 UNKNOWN 分别保留，不用当前缺失推断删除成功。管理工具选择与执行器组装分开，不复制执行链。
...
实际启用、普通贡献、重新启用及卸载已有本地组件链路；先前假启用组件不算实际包 enable 验收。
