...
- 失败原因给用户看得懂的说明：启用失败（平台不符、找不到或读不了解释器、确认后解释器又变了）按结构化原因码
  （`plugin_runtime_facts.runtime_reason_message`）给出具体中文提示；显式调用 `/plugins@<插件>` 前用同一个复核函数先查一次，
  解释器被换或平台不符时以 `PLUGIN_RUNTIME_UNAVAILABLE` + `details.reason` 结构化拒绝，不建运行、不启动进程，
  提示用户停用后重新启用并确认。模型侧同样不接入该插件的工具（原注册链路的既有行为）。
...
构建自提交 `e895dadbf` 的 wheel，隔离 `MY_AGENT_HOME`、Gateway 只绑 `127.0.0.1:8431`，本机 main owner 身份，
用产品 TUI 逐条输入命令（确认码由测试者从屏幕上的回执读出后输入，和真实用户一样）。证据在仓库外的验收目录。

- 本机 macOS（darwin-arm64，node 25.8.2，go 1.26.1）：hello-go 安装 → 回执（入口、大小、摘要、本机平台）→ 带码启用 →
  `/plugins@hello-go hello` 返回“来自 Go 程序（darwin-arm64）”；hello-node 回执列出真实 node 路径与摘要，错码被拒并重新给出回执，
  带码启用后 `hello` 返回“来自 Node.js v25.8.2”；真实模型用插件工具 `read_text` 读到工作区笔记里的暗号。
  Gateway 换成 PATH 最前面放一个 node 包装器重启后：已启用插件仍用原先固定的真实 node；停用再启用时回执变成包装器路径与新摘要、
  新确认码；测试者改动包装器内容后，显式调用被拒绝，停用再启用给出第三个确认码，确认后恢复。
  装 Linux 版 hello-go 在本机启用被拒（平台不符）。
- 测试机 Linux（linux-x86_64，node 24.19.0）：交叉编译的 hello-go 返回“来自 Go 程序（linux-amd64）”；hello-node 回执固定到
  `/usr/local/bin/node` 解析后的真实文件，调用返回“来自 Node.js v24.19.0（linux-x64）”；换装 macOS 版 hello-go 启用被拒；
  停用、卸载、列表均正常。
- 验收中发现并已修：平台不符与解释器被换两种情况，TUI 原先只显示通用的“请求失败/被拒绝”，看不出原因；现在按原因码给出具体说明，
  显式调用改为结构化拒绝（见上文“运行时事实与解释器固定”），修后用新构建在 TUI 复验。
