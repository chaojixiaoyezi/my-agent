...
管理授权、HTTP/direct 安装/配置/启停及原请求查询已有本地接线；实际启用验证完整目录并确认候选退出，普通工具按新运行接入。
释放/消费、重新启用、卸载及显式业务命令也已本地接通，完整多 TUI 仍待完成。已部署的第 3 步版本不因此获得装卸能力。
逐次工作区读取元数据见[上下文合同](PLUGIN_WORKSPACE_CONTEXT.md)；它与安装设置、业务参数分离，SDK 和首个样本仍待实现。
...
完整装卸与实际多 TUI 仍待验收，配置写入本身不授予插件执行权。
...
- 工具和设置 schema 用规范 JSON 冻结，读取返回独立副本；设置及运行时参数均沿原校验器，显式业务调用复用原工具执行链。
...
- 同插件不同包摘要拒绝原地替换，包括版本不变而字节改变。完整卸载后可重新安装，旧请求不能跟随新安装；在线升级/回退尚未实现。
- 卸载先经原停用链释放环境，再 CAS 删除准确安装，删除回执只留原操作结果；用户产物与操作历史保留。
  原成功结果严格读回后在同一插件锁内检查包引用并回收，重装已引用时保留包；UNKNOWN 与坏账不触发回收。
