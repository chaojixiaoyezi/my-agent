...
| DNS 解析失败 | deadline（传输层在期限内有界重试） | 冷却 30 秒；故障仍在则下次冷却 60 秒 |
| 超时 | deadline | 冷却 30 秒，连续超时逐次翻倍 |
