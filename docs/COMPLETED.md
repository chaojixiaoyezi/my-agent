...
## 2026-09-12 会话模型与完整显示（实现与真实 TUI 验收）

会话模型与用户默认分离，显式管理员共享保留单份凭证；完整原文分页和回到最新已接入。
修正工具边界思考串块、活动思考全量排版，以及输出截断/工具参数错误被上层隐藏的终态投影。
不改变 Compact 摘要算法、不新增自动续跑。五路单 Gateway 验证已覆盖模型独立选择、
主子浏览、长 Bash 中段、快速按键和会话恢复，详见 [本轮验收记录](audits/SESSION_UI_ACCEPTANCE.md)。
模型原始流异常、累计用量口径和后台空闲扫描不计入本轮已解决。

## R230 同轮修复（实现验证，真实安装结果见台账）

解决成功 Shell 重复未提示、流式字符误裁剪、OpenAI 长工具准备不显示、连续复制覆盖和主代理派工后被软禁写。
复用既有结构化工具记录与进度合同，复制按代次串行，职责提示对齐用户授权与不重叠分工；不新增机器完成硬门。
先红后绿的定向结果及原会话未关闭项见 [R230 台账](audits/R230_CONSOLIDATED_FIXES.md)，不以单元结果冒充真实任务通过。

## R230 已实现采样切片（未独立部署，不代表原故障关闭）

增加可选 top_p 配置与 /model 模型级字段；有限范围校验、保存/清空、工作片冻结、缓存与子代理继承贯通。
修正扁平新增/旧 v1 迁移丢失显式采样。已核对官方/Go/Zen 的精确 V4 Flash 默认 0.95、遵守协议下限；
未知端点默认省略、显式值传递。仅已知 Flash 省略未显式配置的温度，其他模型保持原温度行为。
9 文件联合 273 项定向通过；工程验收和真实 TUI 分开记录，旧会话 400 和复读仍由 R229/R230 主线继续诊断。
...
编译/启动/业务与观察/假设。分段摘要禁用执行工具并移除任务执行 system，保留全部源字符和摘要要求；
原本机 TUI 已压缩并续跑。辅助模型成功指标修复异常分支局部导入错误；MCP 取消测试改为原子发布观测文件。
开发指南删除旧累计规则、Runbook 修正目录/审批冲突、失效索引清理；第三方源码/许可随包检查补齐。
最终发布验收与尚未关闭的问题以 [R228 台账](audits/R228_RELEASE_CANDIDATE.md) 为准。

## R227 已落地的当前会话修复（原会话持续验收，非所有遗留问题结案）

补齐 child 默认 PTY，正文与思考分色，修正 工具运行时 冷探针 tool_choice；跨窗口摘要按真实预算覆盖完整原文，
失败保留原 checkpoint 与类型，成功保存有界工具路径线索。Read 可读自己上下文材料，不放开内部状态或跨 owner。
普通 Shell 不再被无关旧引用拖死；无效 PTY 句柄明确零副作用，真正部分输入仍阻止盲重放。
提示词删除过时自动后台承诺和互相打架的工作顺序，授权只在 system 一份，不删工具字段或权限硬门。
23 文件 806 项通过，本机及 `.10` 单 Gateway 已部署；原长会话两次真实跨窗口 Compact、child 读自身材料、
19 次交互终端和普通命令均有实录。第一次业务续接失败与后续新错误保留，不把模型的完成声明当完整验收。
当前差距与每项影响见 [R227 台账](audits/R227_MODEL_COMPACT_TUI_REPORT.md)。

## R226 当前模型事实与自主权限实现（.10 真 TUI 通过，本机已部署验收）

修正 gateway_status 当前模型读取源，并为并行工具传递冻结的模型/权限上下文。
/permissions、F4、三档说明、管理员限制、owner 原子保存、待确认主/子调用原地续跑已实现；
可选确认和权限硬门分开，不把自主模式等同于全盘权限，不新增自动审批模型调用。
320 定向通过，包含伪造管理员、坏配置、owner 隔离、权限视图与线程快照、菜单保存/取消。
`.10` 单 Gateway 两个 owner 的真 TUI 通过模式选择、等待时切自主、当前 MiniMax/DeepSeek、三子代理整合任务；
追加 child 7 次 PTY 全成功，主代理同期改文档，无用户批准点击。错误工具分配样本仍保留，见
[R226 台账](audits/R226_PERMISSIONS_MODEL_REPORT.md)。用户确认后，本机全局入口已切换到 runtime-r226 并重启唯一 Gateway；
旧审批等待精确停止，真 TUI 短问答 done/ok，当前 DeepSeek 身份正确。旧 runtime 可回滚，配置、记忆、任务文件保留。
...
新增/编辑/删除/选择/启停、密钥留空保留及明确清空、自定义头、稳定会话头、独立温度、目录发现已实现。
OpenAI Chat、Anthropic Messages、OpenAI Responses 共用现有传输主链；旧 v1 按 schema 显式迁移保留 UUID。
真 TUI 逐个测试 35 模型：26 正常、7 上游不可用、2 需同意训练条款；Kimi 温度 1 后通过，后 9 项停用保留。
291 focused 通过；同 TUI 的自定义表单保存/删除、目录、编辑保密钥、模型切换及普通问候通过。
Auth 保留未实现，Embedding 仅目录配置；没有工具/任务/child 验收，不声称完全复制其它 Agent 产品行为。
新模型价格回落估算、旧 P0/P1 和跨模型 child 真实组合仍待处理，详见 [R222 报告](audits/R222_MODEL_PROVIDER_REPORT.md)。

## 正式 my-agent 合流提交已推送

2026-09-09 正常非强推到正式 main f47f2002，包含 gate 修正 c3efc934，原 main 63422888 历史保留。
本地严格 gate 已通过；未创建 Release/改版本号，线上 CI 没有本次新提交运行作为验收来源。

## 正式库提交前失败修复（本地定向完成）

修正runs归档终态遗漏和scale绕过Gateway claim两条主链；保留owner/逐级链接/精确执行身份检查。
工具发现恢复移到共享tooling层，接口改为明确参数，3项错误码明确不可自动重放/补授权。
旧测试改按家目录、独立运行归档、显式cwd和canonical历史分页验收，Adapter进度查询不再误连真实服务。
23文件514项零失败，29项已有预期失败；未重复全仓。真实TUI/IM与旧P0/P1问题没有据此关闭。

## R221 公开过程逐块耐久化（本地实现，真实 TUI 待验）

解决问题：已显示过程等final才保存，中途崩溃恢复缺工具/思考；child有界增量流也不能充当完整历史。
同一canonical消息JSONL新增typed display记录，主/子mapper先保存完整块再投递；不逐token写、不保存控制事件。
默认模型历史、Compact和Memory读取排除display，尾读不会让过程块挤掉真实对话；thread活动时间不变。
恢复去重按block及工作片，真实结果原位替换typed未知占位，已知终态不覆盖；实时新类型显式协商，冷owner不初始化。
13个相关文件288 focused通过；没有真实模型调用，测试机仍不可达，尚未部署或宣称BUG-146完整关闭。
...
移除 live adapter 的 started 特判副本。不改旧历史，不回读原始参数或补跑工具；显式标题优先。
三个失败定位用例后140 focused通过，包含失败工具、缺 started、显式字段优先与非工具过滤。

## R217 完整前台过程真实分项

唯一Gateway3593187，ma-r217-110-live/-mirror实时思考、工具、diff一致；-resume恢复有完整18个思考和相同final。
三页user/final各一次、无Working，恢复无新增模型调用或canonical重写。18次MiniMax/0重试/220.815秒。
发现BUG-151命令预览丢失和空闲Context缺失，分别修复/留待查；这不是完整P0/P1封板。

## R217 前台实时过程与完整快照（本地实现，真 TUI 待验）

解决同会话观察页看不到思考/工具、最终重复半句的问题。复用原公开 mapper，不新增持久正文或模型调用；
显示快照进入同一 final/repair，默认 child writer 不改。用户消息先于增量，最终按精确 ID 接替候选。
取消/异常只收起本片动画，旧客户端未声明 foreground_transcript 不接收新流。扩大回归349 focused通过；
真实前台/观察/恢复、多轮长任务和关闭组合尚未验收，BUG-146 保持开放。
...
同一 chunk 投影按未决 permission ID 保持等待；无关/缓存回执不清当前审批，并发工具不覆盖等待。
上下文数字持续更新，最后对应回执后恢复，关闭流不继续留审批显示；观察页没有新增授权入口。
87 focused 通过，真实 TUI 等待/解除尚未验收，完整正文同步另列 BUG-146。

## R214 子代理可显式选择已有模型（本地实现，后端未部署）

解决问题：主模型 A 需要把独立工作交给模型 B，原来只能继承 A。
create_subagents 单项/批项的可选 model 解析当前 owner 的已新增配置；不指定仍继承父级。
宿主绑定已有私有 profile ID，子孙和恢复继续使用原引用；未知/重名/跨 owner 配置创建前拒绝，
密钥不进入工具参数/回执；主菜单选择不变。不同显式模型参与派工去重，旧无 model 的键保持不变。
配置、创建、递归与幂等共 117 focused 通过，严格本地 gate 通过，未跑全仓 pytest。
按用户要求本地 Qwen 没派子代理，真实跨模型 child 仍待后续 TUI 验收。

## R214 真 TUI 基础调用与模型标签（已验）

新客户端 ma-r214-110-model-label 读取已选 Qwen，普通问候一次完整回复；结构化 run 为 done，child=0，
...
解决问题：/model 已切换，头像下方仍显示部署默认值。模型选择成功回执更新唯一 typed 显示名，
新开/恢复 TUI 读取已保存选择；取消、失败、仅新增不启用不更换当前标签。执行配置和聊天历史不改。
117 focused 通过，包括真实 prompt_toolkit 菜单、58/120 列冻结视图、重新读取、失败与迟到结果。
本地 Qwen 长任务已按用户缩小测试范围在 TUI 停止，不判为大型任务通过；后续只做基础调用。

## R212 模型传输线程与冷用户配置（本地修复，尚未部署验收）

解决问题：菜单选择的模型未进入真正发请求的保护线程；纯配置保存触发完整Agent初始化并回执超时。
保护线程复制当前工作片Context，配置接口改用可信owner与canonical配置路径，不启动运行时。
真实Thread成功/异常与冷owner先红测试均复现，修后模型/超时/菜单相关58 focused通过。
真实本地Qwen任务之前失败且混用默认模型，不能算验收通过；下一步部署后重新在TUI核对provider。

## 2026-09-08 正式产品名称统一

解决问题：实验后缀留在当前产品标题、新 VM 初始化和发布来源中。统一名称、源码入口、包元数据为 my-agent；
CLI 与用户数据目录本来就是正式名称，保持不变。历史证据路径和 DeepSeek Harness 来源说明保留。
本项只表示命名及后续发布目标准备完成，不表示代码已推到正式仓库或已发布新版本。


## R211 /model 配置菜单（本地实现，真实 TUI 待验）

解决问题：用户无需改 YAML 就能添加/选择模型并设置总上下文窗口。复用现有 owner、后端与工作片入口，
新增私有配置、两种接口表单、Auth 预留和取消路径；密钥不进入公开列表、输入历史或聊天任务。
当前工作片不被切换，child 恢复创建时配置；用户设置窗口实际用于 Compact 压力计算。
105 focused 通过，另相关回归 254 passed / 2 既有 xfailed，两组有重叠；本地严格 gate 通过。
下一步是单 Gateway 真 TUI 菜单、真实 MiniMax 用量和上下文核验，暂不作为已部署能力宣告。


## R209 展开底栏（已部署，真 PTY 分项通过）

解决问题：展开并上翻时看不到状态刷新失败。102 focused 与严格本地检查通过，R209 wheel 已安装；真实
child Ctrl+O/上翻、503 故障、恢复均保持底栏健康及阅读位置。专用 8428 读取代理已退出，原配置直连恢复；
单 Gateway 3336610 / MiniMax-M2.7 保持。BUG-143 此显示切片闭环，不代表所有故障/取消/重启场景通过。

## R207/R208 真 TUI 分项与 R209 漏项

解决问题：当前清单被恢复历史覆盖、失联旧 Working 假装实时。R208 新版同 Go 会话恢复/上翻后保持 1/6，
主/子普通视图故障警告和恢复追上通过；Gateway 与原 child/attempt 未变。Ctrl+O 仍被 decorator 覆盖底栏，
已单列 R209 修复，不能宣称全部视图已通过。

## R208 后台刷新健康（本地实现）

解决问题：旧 Working 无法说明快照已经失联。客户端独立 bool 健康、静态上次状态、全视图重试提示已实现；
既有任务、权限、队列与模型活动计时不改，166 focused 通过。真实 TUI 验证前 BUG-143 保持开放。

## R207 历史卡片与当前 Todo（本地实现）

解决问题：完整历史恢复后当前计划被旧计划顶住。历史导入共用一次字段过滤，不改变原 metadata 或模型缓存，
向前补页也遵守同一边界。两个用例先红后绿、89 focused 通过；新版真实 TUI 未验，不关闭 BUG-144。

## R206 空选区复制（已部署，真实 PTY 分项通过）

- 解决问题：真实右键在没有选区时访问 `None.anchor`，中断 TUI 交互。
- `_selected_text` 接受空选区并返回空串；调用方已有的非空判断避免清空剪贴板，中文索引行为不变。
  对照 终端交互 `getSelectedText`，38 focused 通过。此片不改模型、权限、运行状态或上下文预算。
- 独立客户端 ma-r206-110-copy / 3362381 恢复原截断会话；无选区右键无异常且 tmux buffer 哈希不变，
  中文整句拖选/右键完整复制，历史长度提示仍显示。只证明真实 PTY/tmux，不等同于本机 OS 剪贴板已验。
- 断线状态、恢复旧 Todo 和上下文差异尚未一并解决，不能据此宣布 Go 长任务或整个 TUI 验收通过。

## R205 截断原因贯穿显示与历史（本地实现，真 TUI 待验）

- 解决问题：宿主已记录 max-tokens，前端却静默停在半句，退出重进后连技术原因也丢失。
- live local/Gateway 保留正文并明确提示；canonical final/repair、后台结果和 child 共用 typed 结束原因。
  提示独立于模型正文，按原消息/工作片身份重放；不新增模型调用、不更改权限/任务状态/缓存前缀。
- 3 个历史定位用例先红；11 个定向文件 429 passed / 2 原有 xfailed，Ruff/doc sync/strict code-size 通过。
- 本地实现不等于产品封板；真实 provider/TUI 截断和有界同轮恢复仍未完成，BUG-140 保持开放。

## R204 后台精确回合目标（本地完成，真 TUI 待验）

- 解决问题：旧任务未结束时再派新任务，子代理完成后主代理恢复旧登记目标。
- lifecycle 目标改从精确请求的 canonical 用户消息读取，保持旧任务树和路径；分页读，不改历史/缓存。
- 179 focused passed / 2 原有 xfailed；真实长链失败证据保留，部署和真实 TUI 续作另行验收。

## 2026-09-07 R203 Todo 动画分离（本地，待 TUI）

- 解决 F final 后 Todo 仍持续闪动：清单事实不替代执行活动，空闲时显示待继续并停止刷新。
- 两个定位红测后 144 focused 通过；没有改任务状态、模型或权限。真实客户端复验继续，不算产品封板通过。

## 2026-09-07 R202 非交互命令不继承宿主 stdin（本地，待 TUI）

- 普通 shell、受控执行和 attempt.run 显式关闭宿主输入，不变更独立 PTY/MCP 或命令里的管道。
- 6 个定位红测后，80 focused 通过 / 9 Linux 环境 skipped。尚未宣布生产验收通过。
- 原 C 同一真实 attempt 连续两次 240 秒超时后仍有 11 次操作并最终 done，为此前 R196 的继续能力补齐一份证据。

## 2026-09-07 R201 详细正文随代理切换（已部署，原 TUI 组合通过）

- 解决问题：Ctrl+O 冻结正文不会随 Ctrl+G 的代理导航换来源。
- 在同一个 frame-provider 切换点更新冻结 snapshot 和搜索状态，不改变模型上下文、控制目标或后到消息冻结边界。
- 定位红测后 44 focused 与严格 gate 通过；a447db1 只升级客户端，不重启 R200 Gateway。
- 原 child 展开返回 root、grandchild 全展开逐层返回父级、D 上翻首条输入均通过；两会话消息/用量
  四份文件 SHA 不变。系统剪贴板仍待独立验证，不合并宣称全过。

## 2026-09-07 R200 正常代理接续不按累计工作片封顶（已部署，两份原现场恢复）

- 解决问题：协调员完成四个正常工作片后无法接收最后一名孩子结果，主界面永久 Working。
- 删除孤儿监督的重复次数门，统一走 runner candidate；错误重试、能力、活跃执行、owner 和 UNKNOWN 不变。
- 3 个定位红测后，88 focused 通过 / 1 Linux 专属 skipped，另 37 runner-dispatch 通过。原 F 和旧 R171
  coordinator 均无插话地进入第 5 工作片并 DONE，root 依次继续；R171 root final 已可见/Working 收起。

## 2026-09-07 R199 历史分页实现（已部署，原 D 分页通过）

- 解决问题：正文仍在 canonical 文件但 TUI 无法读取更早页。接入只读分页、独立游标和异步滚动加载，
  保留当前块内阅读位置及 Ctrl+O 冻结边界；不改执行/模型上下文/Compact。
- 修复旧显示重排被 renderer 到达序抵消；测试检查实际渲染行序。原 D 精确恢复后上翻首条输入、
  Ctrl+O 回看通过；canonical 121 条消息/8 条用量 SHA 不变、Compact 5。主子切换/复制组合仍待验。

## 2026-09-07 R198 前后台恢复执行归属（本地完成，真实 TUI 待验）

- 解决问题：子代理唤醒抢先续完原任务，旧前台再恢复旧 attempt 而报失败。
- 原 claim 增加宿主声明的恢复归属；write-ahead 绑定、领取与停止串行、终态和启动补交 exact-task 清理。
  无新队列、目录锁或质量判定，不增加模型调用；普通后台/其他 owner 维持原语义。
- 318 focused / 2 原有 xfailed；严格检查和真实部署另记录，不把本地通过写成封板通过。


## 2026-09-07 R197 HTTP 断流分类（已部署，真实重连切片通过）

- `.10` 唯一 Gateway R197；新 J 家庭资料归档长任务，真实 MiniMax 响应截断一次，原 TUI 显示退避并继续。
- 失败与成功重试请求 SHA 相同，同 root/attempt，模型修复派工参数后只一次创建三个 child。
  这不是完整任务质量或全网络矩阵通过；新增前后台恢复竞争 R198 仍开放。
