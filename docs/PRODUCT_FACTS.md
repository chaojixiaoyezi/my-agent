...
| 本地文件工具、结构化 Tool Gateway、错误分类 | 稳定 | 正式工具调用统一经过注册、授权、参数、路径、限流、effect 和幂等门；不得通过直接新增旁路执行器绕开。参数层已收敛为一份完整 ToolSpec JSON Schema：provider、text/native、MCP、恢复门与最终执行共用；只做无歧义强类型纠正，随后在副作用前检查必填、类型、枚举、嵌套、额外字段、长度/范围、组合规则和本地引用。typed 外层协议与工具参数分离，Schema 声明的 `kind/run_id/status/metadata/artifact_refs` 不再被同名协议字段误删；纠正/错误审计只含路径和类型/约束，不含原始值。当前待发布切片把缺失参数进一步限制为 ToolSpec 逐字段明示的安全默认值或 Registry typed 上下文绑定；显式模型字段永不覆盖，Schema `default` 注解本身没有执行权，其余必填字段继续失败。每个有效参数只记录不含原值的 `source/source_ref`，旧 artifact scope 与 process cwd 专项补参已删除。POSIX shell 使用 `pipefail`，管道末端成功不能掩盖前段失败；前台超时、用户中断、后台 kill 与日志上限共用完整后代进程树终止，覆盖 bwrap 内层新 session，随后只做有界 pipe drain。控制流使用归一错误类别，同时保留提供方原始错误码和脱敏输入形状供审计；供应商额度耗尽有正式 `PROVIDER_QUOTA_EXHAUSTED` 合同并引导切换后端，不再退化为 `UNKNOWN_ERROR`。上一参数主链已随提交 `5d082241` 进入远程 `main`，精确提交 wheel（SHA-256 `a0c0c72d…a86460`）已部署 1.10；当前有限补全切片仍需完整门禁、真实模型、推送、部署与正式双 owner 复验后才能升级为已发布事实。 |
