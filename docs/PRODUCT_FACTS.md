...
| 用户级 Scheduler 定时与提醒 | 部分可用 | 当前工作树只有 owner `data/scheduler/` 中一份 job/run 事实源和一个 typed `schedule` action tool，支持 at/every/cron、时区、CRUD、暂停/恢复、立即运行、历史与状态。到期 claim、heartbeat、CAS、misfire、重启发现和同 owner/thread 续接已通过聚焦回归及完整本地 CI；`wait` 仍只服务当前 active task，不代替持久 Scheduler。1.10 重启/到期和双 owner 飞书真测未完成。 |
