...
更新时间：2026-07-23。本文是 `my-agent` 当前能力状态的唯一权威页；README、路线图和历史审计
...
| MCP stdio 工具 | 实验性 | 未声明工具默认 `dangerous` 并进入统一 effect/幂等/审批门；只有部署配置可逐工具声明更低 effect。当前 wheel 已由本地 Qwen 驱动 `@modelcontextprotocol/server-filesystem` 完成 bwrap/stdio 握手、14 工具发现和 `list_allowed_directories → read_text_file`；写工具仍在 client call 前被审批门阻断。当前工作树把已握手 stdio 子进程状态纳入通用 availability：进程退出后 proxy 不再进入下一 run 的目录/搜索/Schema，当前 run 的调用也会在真实 MCP call 前复检；检查不会重启 server。主流 server 生态仍需扩大验证。 |
| 工具检索、权限范围与运行可用性 | 部分可用 | 关键词与真实 embedding 语义通道共用一个混合检索器；默认未配置 embedding model 时不会伪装成语义可用。当前工作树按 会话运行时 `StepContext/spec plan` 与 长期助手 `check_fn/session toolset` 的共同窄腰，把每个 run 的工具面固定为 `注册工具 ∩ owner policy ∩ allowed_tools ∩ availability`；文本目录、推荐区、原生 Schema、`list_tools`、`tool_search` 和最终执行共用同一不可变快照，搜索不再泄露或加载子代理未授权工具。执行前实时复检，工具在快照后掉线会以 `TOOL_UNAVAILABLE` 停在实现之前。视觉/LSP 未配置时不进入工具面；Playwright 只检查依赖/既有连接，MCP 只检查已握手子进程，所有 availability 检查均不得启动进程、发网络请求或写业务数据。旧的 `granted_capabilities` 字符串、manifest 字段和整条无消费者运行管道已删除，工具扩权只认结构化 allowlist/policy/grant。1.10 候选 wheel 已分别用本地 8899 与 MiniMax-M2.7 完成普通 CLI 真实工具调用，并在唯一正式 8420/Feishu 运行面复用 Chi/Chalk 两个合成 owner 并发复验：两端只读到自己、跨 owner 文件读取被拒、跨 owner HTTP result 为 403、不可用的视觉/LSP 不在 45 项目录、保护的 USER/SOUL/Memory/Skill/Scheduler 哈希不变，真实记录没有额外工具或内部协议泄露。该证据仍不等于所有 MCP/浏览器/LSP 组合和十万用户长稳，因此保持部分可用。 |
