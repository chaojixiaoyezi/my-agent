...
更新时间：2026-09-04（`my-agent` 测试机）。本文是当前工作树能力状态的唯一权威页；README、路线图和历史审计
...
## 2026-09-04 长期会话的任务目录续作

- **状态：稳定（本地 focused 与 `.10` 单 Gateway + MiniMax-M2.7 真 TUI 通过）**。普通任务完成后不把 cwd
  黏给下一回合；新工作仍从 owner home 起步并在首个工作动作建立新 task。为理解“回到前面的项目”，每轮
  动态尾部只列同 owner/thread 最近四个已完成且目录仍存在的 canonical task-path 候选，更旧历史走
  `session_search`。候选由模型判断相关性，不自动选择 cwd，也不授予任何读写权限。
- 模型明确读写某个唯一历史 task root 后，现有 exact rebind 建立 continuation successor 并迁移本代 Todo。
  本轮懒建的占位目录只有在身份完全匹配、且整棵树仍是系统原始脚手架时才清除；任何用户文件、陌生目录、
  symlink 或身份冲突都会保留。superseded task link 继续作为审计事实存在。
- R165 同一真 TUI 完成 A、无关 B、再自然续作 A，三轮定向测试 23/17/26 项通过，最终物理目录只有 A/B；
  主 Compact 计数 1→2→4，最终回复均直接显示。该结论不覆盖更旧任务的模糊语义命中率，后者仍依赖搜索与模型。
...
- 本轮只有 B 的这次续接来自真实 Feishu 客户端；A 的纠正和双向越界反证是可信 localhost
  ...
  仍未取得新证据。当时另发现约 4.4 GB、100 万文件 owner 的应用层精确配额扫描会让单次写入准入约
  60 秒；该历史样本已于 2026-08-28 通过 `quota.v2.max_disk_mb=0` 退出默认热路径。管理员显式非零上限仍可
  开启应用门，规模部署的硬容量隔离继续依赖 filesystem/project/container quota。
...
| 长期 Memory 可维护性 | 部分可用 | 当前工作树保留每个 owner 的 `memory/long_term/memory.jsonl` 为唯一 active 事实源，`remember` 单工具支持稳定 ID 的 add/list/replace/remove/batch、版本、来源、typed kind 与过期；锁内重读和原子整批提交避免并发丢写，daily mirror 记录同一 operation。Candidate/Curator/Promotion 是唯一整理链：用户明确事实与工具证据按结构化引用自主晋升，lesson/HOT 按独立证据、阈值、冲突和精确目标自主治理；模型正文不能取得状态或写入权威。SQLite/FTS 与向量索引返回前按 JSONL active state 过滤，删除/旧版本不能从陈旧索引复活。40 writer 并发、batch 回滚、expiry、route 冲突/缺文件、损坏恢复包、Gateway/subagent/doctor 联合恢复、600 条 MiniMax 长链、连续 8 次真实 compact 和双飞书 owner 隔离均已验证。写入是同步 write-through，没有待 flush 的外部 provider 队列，因此不复制 长期助手 的 pre-compact flush；绕过统一链的 HOT/lesson 直写 helper 已删除。保持部分可用只因为自主策略这轮仍待真 TUI、多 owner 重启和长期召回质量验收，且十万 owner 规模尚无持续运营证明。 |
| Persona 人格与用户画像 | 部分可用 | 当前工作树由唯一 `PersonaRepository` 统一 SOUL/USER/AGENTS 的逐轮加载与变更；owner 边界、regular-file/symlink、UTF-8、2 MiB、逐行威胁扫描和 prompt budget 形成结构化诊断。USER 与 AGENTS 由所属 owner 的 Agent 通过专用工具自主维护；只有 SOUL 变更需要所属用户确认。版本 snapshot、history、CAS、rollback 和 SOUL 确认期间 SHA 冲突保护已接主链。基础文件、patch、shell 和 admin full-access 都不能绕过专用入口。32 writer 并发、恶意行不进 prompt/list、确认幂等/CAS、user A/B/group 路径反证与自主 owner 隔离 focused 回归已通过；保持部分可用只因为新自主边界尚待 MiniMax 多 TUI 真测，群组长期使用和十万 owner 规模尚未证明。 |
| Owner 配额、隐私与自动清理 | 部分可用 | 当前工作树在创建 child 前把 `max_active_agents` 与子代理各级容量取严格交集；权威状态不可读时整批 fail-closed。用户 owner home 默认 `quota.v2.max_disk_mb=0`，普通写入不扫描全树；管理员显式非零时，文件 write/edit/patch、Memory、Persona、Scheduler 与 Skill draft 才共用 `quota -> repository/file lock -> mutation` 的精确准入门，坏 policy 继续 fail-closed。Gateway 以 cursor 有界扫描 owner，按结构化 terminal status/timestamp 清 task 与 child scratch，执行前二次校验，先写 trash tombstone 再按期限删除，并支持 owner/task legal hold 与 audit。应用门无法绝对拦截 Shell/PTY/LSP 任意进程写盘或 SQLite/向量派生页增长，规模部署仍须 filesystem/project/container quota；最新默认不限与 TUI `/remember` 非阻塞改动已 focused 通过，`.7` 真 TUI 正在复验。 |
...
- 默认规则改为随 wheel 发布的 `builtin:prompts/default.md`，不再依赖 systemd WorkingDirectory。
  ...
  可选 `source_quote` 仅进入版本审计，不用自然语言子串匹配决定授权。AGENTS 也由当前 owner 的 Agent
  通过 `update_persona` 自主维护；只有 SOUL 在飞书必须由发起人点击确认卡片后才写。基础文件工具、patch 和 owner-scoped bwrap
...
- persona 工具使用稳定 entry ID 做 list/add/replace/remove；USER/AGENTS 可由所属 Agent 自主维护，只有
  SOUL 的 add/replace/remove/rollback 须所属用户确认。密码、OTP 和临时验证码在写 owner durable memory 前被结构化拒绝。
