...
- 官方 Jev 按尝试累计 **98 次**，全部在隔离目录与唯一测试 Gateway 8431 上完成。日常 8420 与用户日常模型从未改动，每次测试后都经原 CAS 恢复决策设置。
- 凭据方面：
  - Jev 凭据只在隔离私有模型目录。
...
| 分支 `claude/subagent-lesson-ledger`（待合入） | 子代理结构化 lesson 通道 `record_lesson` | 端到端真实验收通过 |
...
1. **动作候选**：先与插件线确定 SDK 层的观察候选结构（插件声明、宿主校验、带内容哈希与代次的 ID），再接决策点。这是第 15 项最后的剩余。
