...
3. **动作候选**：设计稿见[插件观察候选结构](../design/PLUGIN_OBSERVATION_CANDIDATES.md)，待插件线评审；评审后插件线做声明、校验与复核，决策线再接 `action_candidate` 点。
...
1. **动作候选**：插件线评审[设计稿](../design/PLUGIN_OBSERVATION_CANDIDATES.md)并回答第 5 节的五个问题，然后按第 4 节分工实施：插件线先做，决策线接点。这是第 15 项最后的剩余。
