...
**观察**：
- 能力推荐的结论没有落盘。`recommend_capabilities` 算出的 `finding`（如 `skill_tool_decision:apply:cooldown`）和展示短名单、延迟名单只存在于本次运行的内存里。已落盘的只有调用层终态：线程 model_usage 的 decision 分项逐请求记 failed/timed_out/finished 和输入用量（见下一节）；没有发出调用的结果（冷却、关闭、过期）和采用结论，在请求记录、线程指标和归档上下文快照里都没有。真实样本因此看不到结论是否被采用、为什么保留原样。已记为设计台账提议。
...
## 13 决策端挂起的真实样本（2026-09-24，main `4163e66c0`）

**安装与配置**：
- 同一 wheel（SHA256 `383b000fa974c01e1bac48b64970b6b5f0aacb09d67bae44b5f2ca466e538dbe`）装在测试机新建的隔离目录和 venv。唯一 Gateway `127.0.0.1:8431`，两个原生 TUI 各连一个 owner。
- 双 owner 配置同上一节，只把 B 的 Jev 连接改指向测试机本地一个只接受连接、从不回复的监听端口，模拟挂起的供应商；A 的目录未改。两个 owner 都只开 `points.skill_tool=apply`，期限 8 秒，阶段期限 10 秒；owner revision 26。
- 监听端口记录每次接入和对端关闭的时刻，用来核对客户端何时放弃。

**过程**（两个 TUI 每轮同时提交同一句话，每轮只给一次 prompt；Gateway 新启动，R1 两边都先做一次工具能力探针）：

| 轮 | 距首轮 | 需求 | A 的 Jev | B 的 Jev | 首个业务请求距提交 |
| --- | --- | --- | --- | --- | --- |
| R1 | 0 秒 | 读当前目录的 readme.md，一句话说写了什么 | URLError，0.079 秒（真实端点瞬时故障） | 挂起，7.97 秒超时 | A 5.0 秒，B 11.7 秒 |
| R2 | 约 21 秒 | 只回复一个字 | 未发出（冷却中） | 未发出（冷却中） | A 1.7 秒，B 1.5 秒 |
| R3 | 约 46 秒 | 只回复两个字 | HTTP 200，1.71 秒 | 挂起，7.98 秒超时（冷却已过，重试一次） | A 2.9 秒，B 8.8 秒 |

六个请求全部 done，两边都只读到自己的 readme，回答正确。

**证据**：
- 出站共 16 次：
  - M2.7 12 次：业务 8 次，全部 HTTP 200；工具能力探针 2 次（R1），HTTP 200；R3 之后 2 次约 28KB 的后台结构化输出请求，第一次 HTTP 200（39 秒），第二次在停止 Gateway 时仍在途，没有终态记录。
  - Jev 官方端点 2 次，都来自 A：R1 URLError（0.079 秒，观察器不记录具体原因），R3 HTTP 200。
  - B 的挂起端点 2 次：R1 为 ProviderTimeoutError（7.966 秒）；R3 在传输层记为 InterruptedError（7.981 秒），即调用方期限先到、中断了 worker。
- 监听端口两次都在客户端超时的同一时刻看到对端关闭（接入后 7.96 秒、7.98 秒），没有残留连接。
- 调用账（线程 model_usage 的 decision 分项，逐请求）：A 为 R1 failed、R2 无调用、R3 finished（输入 17,352）；B 为 R1 timed_out、R2 无调用、R3 timed_out。B 的 R3 在账本里仍记为超时，没有误记成用户取消。
- 冷却：A 在 R1 失败后、B 在 R1 超时后各记 30 秒冷却。R2 两边都在冷却内，没有发出决策请求；R3 两边的冷却都已过（A 距失败约 42 秒，B 约 35 秒），各重试一次。
- TUI 用量行：A `决策入 17.4k+? · 缺报 1`，B `决策入 未知 · 缺报 2`，失败的调用都没有被记成 0。
- 采用痕迹：按系统提示摘要区分两个 owner 的业务请求。B 全程一个摘要；A 在 R1、R2 是同一个摘要，只有决策成功的 R3 变了；8 次业务请求的工具 schema 摘要全部相同。采用只改写了提示里的名卡和推荐段：默认可收起的工具类别只有插件（`decision_skill_tool_optional_categories: ["plugins"]`），而隔离 owner 的 29 个工具都是内置核心工具。

**观察**：
- 挂起的决策端让受影响的那一轮多等满一个期限：B 的首个业务请求在 R1 比 A 晚约 6.7 秒，在 R3 晚约 5.9 秒（期限 8 秒）。冷却 30 秒只能挡住窗口内的轮次；挂起持续时，冷却一过的第一轮又要付一次完整期限。用户每隔半分钟以上说一句话时，几乎每轮都要多等这么久。之后已改为连续失败逐步加长冷却（30 秒起翻倍，封顶 300 秒，成功即复位；已合入 main `3933b1db1`），这台安装版还是旧行为；新行为的真实复测见下文"冷却退避的真实复测"。
- A 的真实 Jev 端点在 R1 出了一次瞬时故障，同一机制下快速失败，冷却内不重试，过期后恢复 200。这是真实供应商侧的瞬时故障样本；额度（429/402）仍没有真实样本。
- 停止 Gateway 时有一个后台请求在途，没有终态记录。pending/processing 都是 0，6 个业务请求全部 done，不影响本样本的结论。

**清理**：
- 两个 TUI 用 `/exit` 退出。Gateway 经原 CLI `gateway stop` 停止，PID 不存活，8431 无监听，pending/processing 都是 0，done 6 条；挂起监听已停止，端口关闭。
- 原 CAS 恢复两个 owner 的设置：enabled=false，skill_tool 关闭，owner revision 27。
- 证据（测试机隔离目录）：`artifacts/staging.json`（SHA256 `d4540db9338c709252f3417c2e0535b92cb1c6b0ca2e5e0206066d9200f7789e`）、`artifacts/cleanup-current.json`（SHA256 `231c5f58d75f352e0a770acc73e1d9dd378ffa6774df7bdab23ce6488b27e6a9`，含逐条出站、监听事件、调用账和轮次时刻）。本机副本和脚本在 `~/.my-agent/decision-evidence/slow-4163e66c0-20260924/`（仓库外）。

**结论**：
- 决策端挂起时，宿主按期限放弃、不留连接，按连接冷却，冷却内不再尝试，过期后重试；另一个 owner 不受影响，业务全部完成。代价是受影响的轮次多等一个期限，挂起持续时会反复出现。
- 第 13 项现有断网（连接被拒）、挂起（慢响应）和真实端点瞬时故障三类真实样本。
- 本轮官方 Jev 端点尝试 2 次（1 次 HTTP 200，1 次 URLError 未得到 HTTP 响应），按尝试累计 71 次。

## 13 冷却退避的真实复测（2026-09-24，main `3933b1db1`）

**安装与配置**：
- main `3933b1db1`（tree `f460a69dbb69586270269315ba232a7b19a7d9f5`，git archive SHA256 `00d8b234e8f7c1db289ada3c26ab1f0e8dc33e05be9da990e8114e76f9de9eea`）。wheel SHA256 `82a3c65890fea7906ba96d0153c025b217506435ed43d424b0adec3642ca661b`，5,600,898 字节，源码与 wheel 过 clean-package。
- 测试机新建隔离目录和 venv；配置同上一节挂起样本：唯一 Gateway 8431，两个 owner，B 的 Jev 指向只接受连接、从不回复的本地监听，两个 owner 都只开 `points.skill_tool=apply`（期限 8 秒）。
- 轮次按绝对时刻安排：R3 放在 R2 超时后约 41 秒，原先固定 30 秒的冷却在这里会放行重试，新行为的 60 秒冷却应挡住。

**过程与结果**（两个 TUI 每轮同时提交同一句话）：

| 轮 | 距首轮 | B 的 Jev | B 距上次失败 | B 首个业务请求距提交 | A 的 Jev |
| --- | --- | --- | --- | --- | --- |
| R1 | 0 秒 | 挂起，7.99 秒放弃 | — | 12.2 秒 | HTTP 200，1.16 秒 |
| R2 | 约 45 秒 | 挂起，7.98 秒放弃（30 秒冷却已过，重试） | 约 34 秒 | 9.3 秒 | HTTP 200，1.47 秒 |
| R3 | 约 94 秒 | 未发出（仍在 60 秒冷却内） | 约 42 秒 | 1.2 秒 | HTTP 200，2.18 秒 |
| R4 | 约 124 秒 | 挂起，7.97 秒放弃（60 秒冷却已过，重试） | 约 72 秒 | 9.5 秒 | HTTP 200，1.61 秒 |

**证据**：
- 挂起监听只看到 3 次接入（R1、R2、R4），每次都在客户端放弃的同一时刻关闭；R3 没有接入。
- 调用账（线程 model_usage 的 decision 分项）：B 为 R1、R2、R4 各 timed_out 1 次，R3 无调用；A 四轮都 finished（输入 17,353–17,383）。
- TUI 用量行：A `决策入 69.4k`，没有缺报；B `决策入 未知 · 缺报 3`。
- 出站共 21 次，全部有终态：M2.7 14 次（业务 10 次、工具能力探针 2 次、后台结构化输出 2 次），全部 HTTP 200；Jev 官方端点 4 次，都来自 A，全部 HTTP 200；挂起端点 3 次（传输层 2 次记 InterruptedError、1 次记 ProviderTimeoutError，账本都是 timed_out）。
- 8 个请求全部 done，两个 owner 的回答都正确。

**清理**：TUI `/exit`，Gateway 经原 CLI 停止，PID 不存活，8431 无监听，pending/processing 都是 0；挂起监听已停止。原 CAS 恢复两个 owner 的设置（enabled=false，owner revision 27）。证据：测试机隔离目录 `artifacts/staging.json`（SHA256 `562aa53ee41ae3daf3543139e8162215deb48fcce01ef94063f1e0479b755b04`）、`artifacts/cleanup-current.json`（SHA256 `4f259b3aeee3b8251db7f87b9a4fb0db9697db7bee401d2348a3f2bfcf464e5a`）；本机副本在 `~/.my-agent/decision-evidence/backoff-3933b1db1-20260924/`（仓库外）。
...
## P5-D 主会话第一次真实跨模型采用（2026-09-24，main `4ec0e11f3` 与分支 `claude/decision-selection-question` `10050d927`）

同一 325k 字长资料任务，候选带用户用途标签（M3、DeepSeek 为 `long_document, large_context`，M2.7 为 `general`）。修正前 Jev 仍选当前 M2.7，任务由 8 个子代理分摊，结论算错；把采用模式的问题说明改为照实描述宿主核对流程后，Jev 选 M3，宿主按完整请求估算与工具探针核对后经线程 CAS 自动采用，8 次业务请求都走 M3，答案与事实完全一致（ledger-08 最多，四类原因各 1,340 次并列）。两轮 Jev 共 2 次，累计 77 次。详见[主会话真实交接](DECISION_MODEL_MAIN_MODEL_LIVE_HANDOFF.md)。

## 14 P5-A 召回前补充查询：语义召回下的真实收益（2026-09-25，main `ab23a2666`）

**环境**：
- 测试机新隔离目录，唯一 Gateway 8431，原生 TUI，wheel SHA256 `b8a8879128528d182479fcdb047497049cd2e9d571c7230c3b77233c36d10f70`。
- Gateway 配置开语义召回：MiniMax `embo-01`，嵌入密钥只由启动器注入 Gateway 进程环境，YAML 里只写环境变量名。另外 `memory_top_k=5`、工具向量检索关闭、后台 Curator 关闭。
- 决策只开 `points.pre_recall`：off 与 apply 两档各跑一轮，期限 8 秒单次、10 秒阶段；其余接入点全部关闭。
- 20 条事实由测试者经 TUI `/remember` 写入，走原 Candidate→Promotion 链，来源为 `reviewed`、个人范围。之后测试者只输入问题并观察。

**已知漏召回样本的离线标定**：
- 方法：测试者先用产品原函数复算 `_search_scoped` 的混合检索臂（BM25 顺序 + 均值中心化余弦 ≥0.30 + RRF），嵌入端和嵌入类与产品相同。
- 选出三条多诉求问题（K1a 猫的疫苗、K2a 信用卡还款日、K3a 母亲的降压药）。目标事实与整句问题的余弦只有 -0.07—0.06，低于阈值，基线召回不到。与原片段生成函数给出的对应片段余弦为 0.64—0.95，补充查询能召回。
- 这三条的基线都只召回 1—2 条，留有 3—4 个空槽。
- 对照 N1：问题直接点名，目标已在基线里。

**结果**（apply 跑了两批，第二批只加了一个测试者旁观器，只记录决策点状态和所选片段 ID）：

| 样本 | off：召回 / 答复 | apply：Jev 所选片段 / 召回 / 答复 |
| --- | --- | --- |
| K1a 猫的疫苗 | 基线 2 条，缺目标；主模型调 `session_search("猫 疫苗 针")` 没找到，答"记忆里没有你家猫的信息" | 两批都选 `query_3`；目标由 `supplement` 追加；答复正确给出"周四上午十点、青禾宠物医院、狂犬疫苗" |
| K2a 信用卡还款日 | 基线 2 条，缺目标；主模型调 `find_files` 没找到，答"没有找到信用卡相关记录" | 两批都选 `query_3`；目标追加；答复给出"账单日每月 5 号、到期还款日每月 23 号" |
| K3a 母亲的降压药 | 基线 1 条，缺目标；答"没有检查报告，帮不了" | 两批都选 `query_1`（有声书片段，其主题基线已覆盖）；`success:no_addition`，答复仍缺药名 |
| N1 对照 | 目标在基线，答复正确 | 所选片段不新增任何记录（`no_addition`），答复不变 |

**结论**：
- 在语义召回下，补充查询有真实收益。3 条已知漏召回样本中 2 条稳定补回（apply 两批 4/4 次），答复从"查不到"变成准确事实。
- 另 1 条 Jev 两批都选了主题已被基线覆盖的片段，属于稳定的选择失误，不是偶发。
- 对照样本没有副作用。
- 主模型的工具里没有长期事实检索工具，只有 `remember`、`session_search`、`search_text` 等 30 个。off 轮它自己用 `session_search`/`find_files` 查也查不到，所以正式召回漏掉的事实对主模型是不可达的。

**代价**：
- 每轮多 1 次 Jev：HTTP 200，0.73—1.21 秒，请求 1.9—2.4 KB。
- 多 1 次嵌入：1 条查询加 20 条事实。
- 回合总时长差异在噪声内。
- 本实验官方 Jev 尝试 7 次，累计 84 次。嵌入调用 56 次，全部成功。

**观察**：
- 限定范围的语义检索每次都对全部 active 事实重新嵌入，补充查询又使嵌入量翻倍。事实多时需要按正文哈希缓存向量。
- K3a 的失误说明：Jev 只看到基线摘要，缺少"某片段能否新增记录"的结构化事实。

私有证据位于测试机隔离目录的 `artifacts/`（标定表、逐轮记录、`recall-evidence.json`）与 `wire/`（HTTP 结构、嵌入计数、所选 ID），不含密钥或请求头。测试结束已停止 Gateway、释放 8431，并经原 CAS 恢复决策设置。

## 14 P5-B 关系提示的真实 Curator 对照（2026-09-25，main `ab23a2666`）

**环境**：
- 同一 wheel 的另一隔离目录。Gateway 的后台 Curator 关闭，只在测试者执行原管理入口 `my-agent memory curator run --force` 时整理。
- 决策只开 `points.curator_relation`（期限 20 秒）；提取模型为原选中的 MiniMax-M2.7。

**输入**：
- 同样 20 条事实经 `/remember` 写入。
- 1 轮真实 TUI 对话，用户一句话里同时说了三件事：换车（对应 F05，更新）、再次说明芒果过敏（F03，重复）、更正老家（F02，冲突），并请主模型推荐周末出游。主模型没有自行写记忆。

**做法**：
- 对话后停 Gateway，保存快照；off 与 apply 各从同一快照在原路径上跑一次，共两对。
- 复制到别的目录跑是无效的：会话工作区按绝对路径建键，副本读不到这段对话。那次已作废并单独留证。

**结果**：
- Jev（每次 26 对：2 条消息 × 13 条有界正式条目；HTTP 200，2.85/3.13 秒，请求 51 KB）：
  - 用户消息这一侧，两次都判 F03 可能重复、F02 可能冲突。
  - F05 一次判可能更新、一次判可能冲突。
  - 其余 20 对都判 no_match。
- 下游提取（真实 M2.7，同一输入字节）：
  - 换车（F05）：off 两次都产出 `user_profile replace`，类型与原正式条目不一致；apply 两次都产出 `long_term_fact replace`，目标为 F05。
  - 老家（F02）：四次都是 `long_term_fact replace F02`。
  - 芒果（F03）：off 为 `user_profile none` 与未产出各一次；apply 为 `discard none`（并写明"已存在且一致"）与 `long_term_fact none` 各一次。
  - 所有 replace 都停在原晋升链的 `blocked_conflict`，四次运行都没有改动正式长期事实（20 条不变）。
- 代价：
  - 每次多 1 次 Jev；本对照共 2 次（作废的那次没有发出），累计 86 次。
  - 提取请求从 31.9 KB 增到 49.9 KB（+56%）。
  - 提取耗时 off 为 22.0/26.8 秒，apply 为 52.4/29.9 秒，噪声大。

**结论**：
- 关系提示在真实提取里有稳定、可观察的作用：更新类事实被归入与原条目一致的类型，并指向原条目。
- 重复与冲突两类在本样本里 off 也基本能处理，提示没有稳定增益。
- 所有建议仍经原提取、校验与晋升链，没有绕过权限。
- 默认保持关闭。

**发现**（均与本点无关或属于后续增强，已登记设计台账）：
- 关系对按消息顺序取笛卡尔积的前 32 对，不按相关度挑选；消息一多，后面的消息就比不到了。
- 新 owner 首次整理时，Memory v2 迁移把 memory.md/HOT 模板的标题行（`# Memory`、`# Memory HOT`）当作旧数据生成两条 `migrated_legacy` 待审候选，四次运行都有，off 同样出现。

私有证据见该隔离目录的 `artifacts/curator-*.json` 与 `curator-evidence.json`，以及 `wire-*`（每对关系只记 ID 与标签）。结束已恢复决策设置，8431 已释放。

## 15 交付复核焦点 `delivery_quality` 的真实 TUI 样本（2026-09-25，分支 `claude/verification-command-shapes` `e7178a22c`，现已合入 main `e489ffff0`）

**环境**：
- 测试机隔离目录，唯一 Gateway 8431，原生 TUI，主模型 MiniMax-M2.7。
- 决策只开 `points.delivery_quality`（8 秒）。
- 测试者只给一次普通中文要求，每轮开跑前把示例小项目恢复为同一初始字节。
- 每个样本都跑 off 与 apply 两档。

**第一个 wheel（`28d8d8989`，未含验证分类修复）的 off 轮发现**：
- 模型三次测试都写成 `cd <项目绝对路径> && python3 -m pytest tests/ -v`。
- 验证账 `verification_events` 一条都没记：原分类把 cd 前缀当作两段链式命令整体拒绝。
- 这类写法在模型里最常见，本点和交付前核对因此都看不到真实测试。已按"一次返回码只能证明一条命令"修复并合入（放行开头的 cd 前缀，管道与后台不算证据）；随后换用含修复的 `e7178a22c` 重跑。

**D1：同一 pytest 命令反复执行**（要求"先跑失败的测试文件，修好后跑全部测试"）：
- 修复后验证账如实记下两档共 6 次运行（修复前为 0 次）。
- 模型每次都跑同一条 `pytest tests/`，焦点按"项目根 + 种类 + 范围"只保留最新一次，只有 1 个焦点，达不到 2 个的最低门槛。apply 档 Jev 零请求，零开销，属于设计内的静默。
- 两档都修好 bug、全部测试通过。

**D2：`make test` 与 `make lint` 两类验证**：
- 示例在测试之外还有一个未使用的 import。
- apply 档在 `make lint` 失败时真实触发一次：焦点为测试通过、lint 失败（当前事件）。
  - Jev 1 次：HTTP 200，0.74 秒，请求 2.8 KB。
  - Jev 选当前的 lint 失败。按设计，选中当前事件不追加提示。
  - 所有主模型请求里都没有出现提示标记。
- 过程中 lint 首次失败，原因是工具环境里没有 pyflakes：
  - apply 档的模型自行 `pip install pyflakes`，装进了 owner 自己的用户 site（`<owner_home>/.local/...`，产品的 owner 级 pip 环境，没有改测试机系统 Python）。随后查出并删掉未使用的 import，最后用 `make test && make lint` 一并复核。这条链式命令按规则不记证据，但返回码 0 确实说明两者都通过。
  - off 档的模型改用 `py_compile` 替代并如实说明，没发现未使用的 import。
- 两档的差异来自主模型自身的取舍：apply 档从未出现提示，不能记为本点的收益。

**结论**：
- 触发 → Jev → 采用复核 → 按设计不提示的真实链路成立，关闭时零请求。
- 本批 4 轮没有出现一次实际追加的提示，尚无业务收益证据；默认保持关闭。
- 本批官方 Jev 尝试 1 次，累计 87 次。

**发现**（已登记设计台账，均未实施）：
1. 最有价值的场景——最后一次验证之后又改了文件、未复核就交付——本点无法触发，因为只在新的验证事件上触发。D2 里测试在 lint 修改之后没有单独复跑，就属于这种情况。
2. `&&` 串联的多条验证命令返回码为 0 时，其实证明了每条都通过，现在整体不记。
3. 返回码 127（命令不存在，例如 `python` 不在工具环境里）被记成测试失败。
4. `pytest tests/` 这类把整个测试目录作参数的写法被记成 targeted。

私有证据位于测试机该隔离目录的 `wire-*`（HTTP 结构、提示标记、本点所选 ID）、验证账 sqlite 与各轮 TUI 记录。结束时已恢复决策设置，8431 已释放。

## 17 决策实验的首次真实授权发送（2026-09-25，分支 `claude/decision-experiment-send-gate` `2b28c8c78`，现已合入 main `dfa8e498b`）

**环境**：
- 测试机隔离目录，唯一 Gateway 8431，原生 TUI，wheel SHA256 前缀 `cf146c9b`。
- 决策总开关与"实验能力"开关打开，所有接入点的普通模式关闭；实验路径要求 skill_tool 的普通模式为关闭。
- 测试者只输入一条产品自带的命令：`/experiment observe skill_tool 10m 1 50000 <任务>`。任务是只讲"把销售 CSV 按地区汇总画柱状图"的步骤，不读写文件。
- 跑了两次：第二次加装一个测试者旁观器，只记录 `settle_input_budget` 返回的结构化快照。

**结果**（两次一致）：
- **授权**：请求记录写入 `experiment_grant=granted`，并绑定本请求的 request/run/attempt。授权信封为 v2，经验上界策略 `empirical:jev_wire_bytes.v1`，最多 1 次 HTTP、50,000 输入 token、600 秒。
- **发送**：Jev 恰好 1 次，65,070 字节、27 题，HTTP 200，1.16 秒。供应商计费输入 17,375 / 17,376，输出 2,261。
- **经验上界**：C = ⌈65,070/2⌉ + 256×27 + 1,024 = 40,471，实际比例 0.4293，与本地标定的约 0.43 一致。
- **结算**（第二次的快照）：`charged_input_tokens = provider_input_tokens = 17,376`；预留 HTTP 1/1，未知用量 0；`input_bound_kind=empirical`，无比例告警。
- **推荐结果**：能力推荐观测为 `mode=observe`、`status=success`、`adopted=false`，共 27 题；只观察，不改变本轮展示。
- **主模型**：MiniMax-M2.7 正常作答（1 次，输入约 21.8k）。
- 结束后经原 CAS 恢复决策设置，8431 已释放。本批官方 Jev 尝试 2 次，累计 89 次。

**结论**：
- 授权入口、经验上界、发送硬门与按实际用量结算都在真实链路中成立，结算额等于供应商计费。
- 实际比例 0.43，说明经验上界在已标定范围内留有约 2.3 倍余量。
- 两次都在同一类请求（27 题、约 65 KB）上，不能外推到其它题量。

**发现**：结算快照只存在于进程内账本，回合结束后不可查，请求记录里也没有。P5-E 要求的"基线、实际结果、来源与配置版本"还没有持久记录，属于下一片 E2 的范围。本次靠测试者旁观器取得快照。

## 16 主会话选模：普通短任务不换模型（2026-09-25，main `dfa8e498b`）
...
## P2-F 整理标签 `curator` 的真实 Curator 样本（2026-09-25，main `e23bae7f6`）

**环境**：
- 测试机新隔离目录，wheel SHA256 前缀 `776d0448`。唯一 Gateway 8431 只用来生成素材，Gateway 自身的 Curator 关闭。
- 素材：测试者在 TUI 输入与 P5-B 样本相同的一段近况陈述，生成一轮对话后给 home 原地快照。每次运行前原地还原快照，经原设置 CAS 改 `points.curator.mode`，再用产品自己的 `memory curator run --json` 显式整理一次。
- 旁观器只记 HTTP 结构、出站请求里是否出现 `decision_annotations`、调用账终态和 curator 点每题所选的值，不保存来源正文。

**结果**（同一快照上的五次运行）：

| 运行 | Jev | 标签点结果 | 提取请求 | 整理结果 |
| --- | --- | --- | --- | --- |
| off | 0 | — | 1 次，28,766 字节，无标记 | 成功，游标推进 |
| apply | 1（16 题，1.02 秒） | success，可采用 | 2 次（首次 99 秒后重试），33,015 / 33,166 字节，都带 `decision_annotations` | 成功 |
| observe | 1（1.00 秒） | success，不采用 | 1 次，28,766 字节，无标记 | 失败：`CURATOR_SCHEMA_INVALID`，提取模型输出没通过格式校验；不产生候选、不推进游标 |
| observe（重跑） | 1 | success，不采用 | 1 次，28,766 字节，无标记 | 成功 |
| 超时回退（单次期限 0.3 秒） | 1（0.44 秒被期限中断） | deadline/provider_failed，不采用 | 1 次，28,766 字节，无标记 | 成功 |

- 16 题 = 8 个来源 × 标签、优先级两题。apply 的回答包括 high、preference、low、fact、event，也有 need_data、not_needed，都按题独立记录。
- off、observe、超时回退三种情况下，交给提取的批次逐字节相同（28,766 字节），原批次完整；只有 apply 把临时注释附进提取请求（多约 4.2 KB）。
- 调用账：两次成功的 Jev 调用都记在本次整理的 lease run（`memory-curator-run-…`）下，purpose=decision，供应商报告输入 9,839、输出约 1,222 token；超时那次没有成功结算，按原账不补零。
- 改设置本身会留下一条消息和一条审计记录，所以改过设置的几组比 off 多处理 1 条消息、1 条审计事件。它们不进提取请求（字节数不变），只是游标跨过。
- 候选数 off 为 4，其余为 3。批次相同的几组之间也有差异，属于提取模型的波动，不能据此判断标签的效果。
- 本批官方 Jev 4 次，累计 107 次。结束后经原 CAS 恢复决策设置（关闭），8431 空闲，无残留进程。

**结论与边界**：
- 第二个 P2 业务入口（Curator 前置标签与优先级）在真实链路中可配置、可观察、可关闭，失败沿原路径执行，P2-F 缺的实际场景证据已补。
- 限制：单一批次、单个 owner；没有证据表明标签与优先级提升了提取质量；observe 那次失败是提取模型的输出问题，与决策点无关。

## 15 动作候选 `action_candidate` 的 browser-lite 真实验收（2026-09-25）
...
- 每轮一个新会话、一次中文请求。旁观器只记 HTTP 结构与标记、本点状态与所选别名、提示是否生成及其候选 ID、插件调用的参数键、结果码与候选 ID，不保存请求正文、label 或插件 key。

**先暴露的真实链路缺口**（第一次运行，`668544176`，基于 main `c577ed185`）：
- 读取返回 3 个候选、资格全部满足，但新鲜度权威恒为 False，本点零请求。
- 原因：只读工具不经审批，归档没有 `runtime_gate`；`persist_tool_runtime_ledger` 在门台账记录为空时提前返回，`tool_completed` 事件一条都没写（owner 库零条），观察新鲜度没有事实源。插件线的合同测试直接调用事件写函数、带手造的 gate，没走到这个入口。
- 插件线已按通用方式修复（main `6a50d84aa`：完成事件总写，门台账仍只在有门时写；事件窗口改为保留最新行）。决策侧集成测试改走这个真实写入口，修复前的代码会让它失败。
- 修复合入前，先用只改这一处的临时构建（未提交）跑了 off/observe/apply/过期四档，全部成立，Jev 3 次。

**合并后代码的结果**：

| 轮 | 模式 | Jev | 本点结果 | 提示 | 主模型怎么操作 | 插件动作 |
| --- | --- | --- | --- | --- | --- | --- |
| off2 | off | 0 | — | 无 | 选择器 | 打开、读取（3 个候选）、填写、提交、读取确认页（1 个候选）都成功 |
| observe2 | observe | 1（1.38 秒） | success，选 c1（输入框），不采用 | 无；主模型请求里没有提示标记 | 选择器 | 同上，都成功 |
| apply2 | apply | 1（2.19 秒） | success，选 c1，可采用 | 追加，候选为该次观察的输入框；之后 4 个主模型请求带提示标记 | 用提示的候选 ID 填写，用候选 ID 点提交 | 都成功 |
| stale | apply | 1（4.35 秒） | success，选 c1 | 追加（输入框） | 用提示的候选 ID 填写、用候选 ID 提交；再按请求不重读，用旧的"保存订单草稿"候选点击 | 前两步成功；最后一步插件因页面代次已变返回 `OBSERVATION_STALE`，无副作用，模型如实转述 |

- 确认页只有 1 个候选，低于下限 2，各轮都没有为它发请求。
- 每次 Jev 请求约 3.5 KB，供应商报告输入 1,023—1,024、输出 76 token，记在对应请求的调用账 purpose=decision 下。
- 外发核对：所有 Jev 请求里都没有 `cand-`、`obs-`、插件工具注册名或插件 key 前缀。PB 三轮（off2/observe2/apply2）请求里出现 `file://`，来自用户请求原文"不要加 file:// 前缀"（按设计外发脱敏后的当前请求）；原文没有这几个字的轮次一律无标记。
- 同一合并后代码上还有 off/observe/apply 一组，请求只给了绝对路径。模型两次都先写 `file://`，再试本机 http，分别被宿主 URL 参数门 `NETWORK_FILE_URL_BLOCKED`、`NETWORK_PRIVATE_HOST_BLOCKED` 拦下；页面没打开，本点无从触发。工具描述仍写可用 `file://`，已作为插件线后续项登记。PB 组在请求里写明"url 直接填绝对路径"后通过。
- 被拦后模型曾用后台进程自行起 `python3 -m http.server`（监听所有网卡），Gateway 停止后仍在运行。测试者已结束这些进程，并已作为后台进程生命周期问题登记台账、告知主线 owner。

**结论与边界**：
- 动作候选在真实链路中可配置、可观察、可关闭。off 零请求；observe 只记录；apply 只追加所选候选的 ID 与 role，是否操作仍由主模型决定；过期候选由插件按页面代次拒绝。
- 同一请求下对照（PB 组）：off 与 observe 用选择器，apply 用了提示的候选 ID；另一组同请求对照（临时构建的 PA 组）三档都用选择器。过期样本的请求本身提到"候选"，不计入。样本少、页面简单，不能据此说明质量提升，默认保持关闭。
- Jev 6 次（临时构建 3、合并后代码 3），累计 113 次。结束后隔离 Gateway 已停，8431 空闲，插件浏览器与模型起的后台服务都已结束；隔离 home 与私有模型目录副本已删除。结构化证据在仓外证据目录，只含结构、标记、编号与结果码。
