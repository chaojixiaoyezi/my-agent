...
**观察**：
- 挂起的决策端让受影响的那一轮多等满一个期限：B 的首个业务请求在 R1 比 A 晚约 6.7 秒，在 R3 晚约 5.9 秒（期限 8 秒）。冷却 30 秒只能挡住窗口内的轮次；挂起持续时，冷却一过的第一轮又要付一次完整期限。用户每隔半分钟以上说一句话时，几乎每轮都要多等这么久。已在同一分支改为连续失败逐步加长冷却（30 秒起翻倍，封顶 300 秒，成功即复位；见设计台账），这台安装版还是旧行为，改后尚未做真实复测。
