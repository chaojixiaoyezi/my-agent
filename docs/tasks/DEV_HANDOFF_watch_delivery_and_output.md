...
**方向**:①「样本无目标」这一情形更强制地收敛到"只配 `normal_*`、不配 `target_*`"——结构化判定 sample 是否可能全是常态(目标取值频次为 0 / 取值基数与分布),据此在 `configure` 侧拒配 target、只留常态判据。②候选入账前经**二次逐条研判**(§A-1 证明逐条研判能力强、误判≈0),把"配错的 target"在研判环挡掉再落 findings。

### 7.2 主代理有时【绕过内置盯守工具、自写轮询脚本】(判据还错)

**现象(真机实锤)**:同一个盯守任务,主代理有时**不用内置 `watch_stream`,自己写脚本轮询**,且脚本自定判据错(把常态当命中、海量误报——实录 5 万+假命中)。提示里加一句"必须用 watch_stream"后才回到内置机制。

**后果**:一旦绕过,§1–§4 的机制修复**全用不上**;自写判据又不可靠。

**方向**:盯守任务的提示词/工具装配更强地把主代理**导向内置盯守机制**(而非放任自写脚本);排查 §2 改动是否无意间弱化了走 `watch_stream` 的引导。

**验收(7.1+7.2 合并)**:真机长盯守(不额外提示"用 watch_stream"),主代理走内置机制、逐条报出的 `event_id` **命中真目标**(funnel B 真目标 > 0、常态误报接近 0),而非报一堆常态。
