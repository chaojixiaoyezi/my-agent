...
### 9.2 全量测试暴露的两个【还没治】——建议下一棒一把抓
**共同根因**:endurance/coverage 的兜底(A2 自动清单)**只认行首列表记号**(`- 1. ①`),**认不出"内联枚举"**(顿号/逗号把若干项写在一句话里)。真实需求大量是内联枚举,于是兜底盖不到 → 提前收工/覆盖不全复现:

- **③ 代码建站:A2 没触发**。建站 prompt 把 11 个功能写成内联顿号("用户注册登录、笔记增删改查、富文本编辑、…"),不是行首 bullet → `requirement_coverage_seed` 正确地按现规则不种(coverage 0/0)→ 无清单兜底。两用户都交付但**行数方差 3.6×**(959 vs 3502)。
- **④ 深度分析:做 1/5 就收工**。"逐个分析 5 个项目"里 5 个项目名也是内联枚举 → 没种清单 → 无兜底。**两个用户都只深入分析了 1 个项目**(u-f5a2 甚至把产物命名 `01-分析-…` 却没做 02-05、0 子任务),另 4 个没碰;报告浅(66 / 251 行)。这是老的"分析浅+覆盖不全+提前收工"。

**底座修法(通用/非限制,一处修惠及两处)**:把 §2 的 `requirement_coverage_seed` **扩到认内联枚举**——一句话里用顿号/逗号/分号并列 ≥3 个动宾功能项(如"要有 A、B、C、D")或 ≥3 个专有名并列(如"分析 X、Y、Z 三个项目"),同样结构化拆成 coverage items。仍纯字面分隔符解析(不做语义)、仍 ≥3 才种、仍幂等、仍不发明需求。这样建站的 11 功能、分析的 5 项目**都拿到清单+逐项兜底**,coverage-incomplete-rework 就能push"做完 5 个再收工"。**③和④两个弱点同一把钥匙。**
