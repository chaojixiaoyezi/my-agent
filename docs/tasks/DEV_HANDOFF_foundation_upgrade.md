...
- 贴线阈值型存量测试(compact 20k 窗口)对"模型可见文本合法增长"很吃紧:修比率不修语义。

---

# 测试方独立复验(四类原样复测,round2 vs round1)

> 拿当初暴露问题的四类原样复测(飞书多用户 + 终端应用 同压 M2.7)。修正上轮测量:读**真交付流(chunks)+ output/**,不读投递后清空的记录。**结论:好坏参半,含一个净负——B 召回真机端到端反而退步(正是上文"残留②真源 3h 盯守复测"这一项)。**

## 复验总表

| 类别 | 修复 | round1 | round2 | 判定 |
|---|---|---|---|---|
| 1 盯守 | B 召回 | 逐条报出 156-216、0误报 | funnel A 更高(252>175)但**逐条报出崩到 18-29**、冒 1 误报 | **❌ 净负** |
| 2 建站 | A2 耐力 | 4692/5456 行 | 1748/2279 行、cc 反超 5048 | **❌ 没帮**(A2 没触发) |
| 3 分析 | A1交付+A3方差 | 159/65 行、work 内部、一个只覆盖 1/5 | 282/388 行、**进 output/**、都 5/5 | **✅ 改善** |
| 4 大文件 | A1交付 | 一个 4/4 全对 | 一个失败 / 一个 3/4 | **⚠️ 方差仍在** |

## 逐条

### ✅ A1 交付归集(Test3):真修好了
round1 分析报告落 `work/` 内部、用户在 `output/` 拿不到;round2 **两个用户报告都进了 `output/`**、覆盖都 5/5(round1 有个只做 1/5)、深度翻倍、双用户方差明显变小。**A1 的"声明交付物归集到 output/"这一半有效。** (注:上轮我报的"最终响应空"多为**读了清空后的记录**的测量假象——真交付流(chunks)有内容;A1 归集 `output/` 才是真价值。)

### ❌ B 召回(Test1):抬升有效、端到端却退步(净负,头号)
- **筛这层 B 生效**:funnel A 252 > 175(round1)、反馈车道真活(confirmed_target_similar 300+ 候选、0 误报)。**离线 0%→68% 的机制没错。**
- **但端到端崩**:真机上 audit 抽检每分钟抬出上千候选,**把 agent 判读吞吐淹了**——逐条报出的真目标从 round1 的 156 崩到 **18-29**,还冒 1 误报。**离线台没建模"agent 判读吞吐上限",真机上"多抬 ≠ 多报"。**
- **修法(下一棒头号)**:抽检率必须**按 agent 实际判读吞吐封顶**(抬的别超过它判得完的);或**反馈车道(高价值)优先于随机 audit 车道**,把有限判力先给"像已确认真目标的",别被随机抽检噪音占满。目标:funnel A 的抬升要真转成 funnel B 的上升。

### ❌ A2 耐力(Test2):机制在、但从没触发
两个用户**都没声明功能清单(`coverage.targets`=0)**→ coverage-rework 从没 fire → A2 白搭;代码量反而掉一半(1748/2279 vs 4692/5456)、终端应用 反超(5048)。**A2 靠"模型自觉先立清单"才生效,而模型不稳定地立。** 修法:大体量任务**自动派生 / 强提示立清单**(仍非硬性行数限制),让 A2 有抓手。

### ⚠️ 方差(A3):分析类压住了、代码/数据类没有
Test3 双用户齐了(都 5/5、深度接近);但 Test2 两轮差 2-3 倍、Test4 一个失败一个 3/4。**"同任务时好时坏"在代码/数据类仍在。**

## 下一棒方向(按轻重)
1. **【头号】修 B 的洪泛**:audit 抽检按判读吞吐封顶 + 反馈车道优先,别让随机抽检淹了判读。
2. **A2 真触发**:大体量任务自动立功能清单(别靠模型自觉)。
3. **A3 方差**:代码/数据类的一致性(自检+重试要对这些类也生效)。

## 复验证据
Mac `mon_range_answers/{r1,r2,r3,r4}_result.txt` + `evidence/`;真机 findings/report/chunks 坐标见各 result 文件。
