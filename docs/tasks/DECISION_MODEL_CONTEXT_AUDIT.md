...
### Gateway 21.3MB 峰值来自整块读取消息文件（2026-09-24，只读分析）
...
**方案**（通用底座，不针对样例；改的是共享模块，须经主线 owner 同意）：
1. 在 `store_io` 增加按字节块倒读、逐行惰性解析的行迭代器，沿用只按 LF 切分、坏行结构化报告的原规则。`recent_report` 改为它的消费者，行为不变。
2. `gateway_recent_artifacts` 与追加去重改为倒序流式扫描：每行只保留需要的元数据，凑够即停（产物 20 个或 80 行，去重 100 行），不再物化正文。
3. `ensure_thread_history_indexed` 改为顺序流式逐行建索引，不再一次物化全量行。
- 预期：Gateway 准备期峰值从约 21.3MB 降到与 child、后台相当的约 10MB（剩下的是请求本身必需的一份原生历史）；发送后峰值从 22.9MB 降到驻留加一行；4.2M 字符下每次请求少约 160ms。
- 验证计划：新增字节有界倒读的单测（跨块边界、坏行、NEL/U+2028、CRLF）；三处调用的等价测试（结果与原实现逐项相同）；沿用本探针的三宿主峰值对照。
