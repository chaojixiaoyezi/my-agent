...
证据边界：纯冻结 IR 的下层安全点验收与外层溢出自动接续分开。当前外层重跑 agent.run 会从归档重建交接，未保留前轮原生 ToolCall/ToolResult；本切片不能宣称外层原生 IR 接续完成。媒体线、真实供应商缓存、超大历史有界读取及旧全仓八项失败仍开放，12.4不勾选。决策线测试机可用于后续已协调的隔离验收，本片尚未部署或重启。
...
- 后台非预选入口保留原范围算法和具体种子。
- 来源同时冻结投影时刻 `projected_at`：终态工具折叠按这个时刻选热尾或冷折叠，延后解析、多次解析都与准备时一致。
- 后台来源保留空正文行（`keep_empty_text`），与原后台 messages 规则一致；Gateway/child 仍过滤空正文。
- Gateway `_conversation_prompt_section` 删掉生产不可达的 `include_transcript` 正文/摘要分支，已结束历史只经种子边界提供。
- 源文件被改写、截短或替换时解析抛 `DataCorruptionError`，被删除时抛 `FileNotFoundError`，不会变成空历史。
...
**验证**：
- `test_conversation_history_seed.py` 16 项：
  - 两个边界上具体种子与来源逐项相等，下游孤儿清扫结果、媒体、匿名信封一致，磁盘/内存两种行、三种规则、三宿主入口都覆盖；
  - 热尾窗口过后再解析仍与准备时相同；冻结后合法追加不改变已选历史；
  - 互斥校验；改写、截短、原子替换、删除后都明确失败。
- `test_host_history_seed_lifetime.py` 3 项：准备驻留有界，解析后完整 JSON hash 与行数不变。
- 相邻 11 个测试文件的断言改为经解析入口核对完整内容，没有删除内容断言。
- 相关 114 个测试文件：2,932 passed、24 xfailed、1 xpassed（评审修复前）。
- 评审修复后，接上 main 的同一 118 个相关文件：2,985 passed、24 xfailed、1 xpassed；Ruff、doc sync、strict code-size（高风险项数与基线相同）、diff 检查通过。

**独立评审**（F1–F4，已处理）：
- F1（中）：终态折叠原本在解析时按当前时刻选热尾/冷折叠，热尾窗口（300 秒）过期后来源与原具体种子不等，同一种子在一次运行内也可能前后不同。已改为冻结投影时刻，补时钟推进用例；去掉冻结时刻的变异会让 3 项失败。
- F2（低）：后台来源曾过滤空正文行，原后台规则保留。已对齐，三宿主用例逐项核对 messages；改回过滤的变异会失败。
- F3（低）：见下方"解析成本与来源生命周期"，只记录，不在 2a 改。
- F4（提示）：Gateway 正文分支生产不可达，且绕过边界解析入口，已删除。
- 测试意见：deferred source 用例改为断言缩窄的展示投影从未被调用；Gateway 测试辅助统一到 `_gateway_history_helpers.py`；补齐追加、截短、替换、删除用例；TESTS 计数更正为 11 个文件。
- 删除 F4 分支连带：两个 Gateway Compact 用例原先断言该不可达分支（摘要在 Gateway 段内、位于证据之前），改为断言生产路径（摘要只在种子历史段，证据只在上下文段）；一处种子对象相等比较改为比较解析结果，因为两次准备各自冻结投影时刻。

**解析成本与来源生命周期**（F3）：同一 4.2M fixture，`perf_counter` 取中位数，测量时同机还在跑相关测试；评审者独立测得冻结 5.6ms、文本解析 32ms、原生解析 77ms，量级一致。

| 阶段 | 旧具体种子 | 2a 只读来源 | 发生次数 |
| --- | ---: | ---: | --- |
| 准备 | 35.9ms | 6.0ms | 每次请求 1 次 |
| 文本边界解析 | ≈0ms | 34ms | 文本协议每轮 1 次 |
| 原生边界解析 | 0.19ms | 65ms | 循环建立时 1 次（文本协议也执行）；运行内原生 Compact、自动续跑、恢复候选时再次 |

- 旧原生边界几乎不耗时，是因为深拷贝共享不可变字符串，只复制外层结构。2a 把这部分成本换成每次按地址重读并校验。文本协议每轮多约 34ms，相对秒级的模型轮次约为 1–3%。
- 生命周期变化：2a 之前 canonical 文件只需在准备期间稳定；现在整个运行期间都不能改写（追加合法），因为每次解析都会重新校验 inode 和逐行 hash。现有写入方只有 `append_jsonl` 追加；Compact CAS 只写线程 JSON。唯一的非追加操作是记忆保留任务把不活跃会话移入回收站，它要求没有 `active_task_ids`、`updated_at` 早于截止时间、在文件锁下指纹不变，正在使用的线程不满足这些条件；极端竞态下下一次解析会以 `FileNotFoundError` 明确失败，不会变成空历史。
- 可选优化（都涉及共享工具循环，需要主线 owner 决定，放到 2b 一起评估）：
  - 文本协议每次运行只解析一次：每轮省约 34ms，但运行期间会重新持有一份完整文本；
  - 文本协议跳过原生解析：`provider_history_messages` 在文本路径上仍被 `text_messages_supported`、上下文压力估算和自然回复读取，跳过会改变行为，不能直接删。
...
- 小项 (a) 的前提（主线 owner 审阅后要求补测）：对只读来源，native 为空时旧文本回退的输出也必为空。
  - 理由：native 只会跳过 display 行、非 user/assistant 行，以及没有合法信封的空正文行；有合法信封至少产出一条，两条路径投影的是同一批行。
  - text 能留下而 native 跳过的只有后台规则下的空正文行，旧回退本来就跳过空正文。
  - 新增参数化用例覆盖两种情况：后台只有空正文行；child 规则只有 display 行。断言跳过回退前后的原生输出都是空。
  - 这是前提钉子，不是行为守卫，没有做变异验证。
...
分支 `claude/decision-background-context-budget`，基于 main `a2e26178b`，方案经主线 owner 同意，已合入 main `5dcdfd463`，并随测试机媒体验收的安装版安装（见[真实验收](DECISION_MODEL_REAL_VALIDATION.md)）。纯 bug 修复，不加开关。
...
- child 和后台的峰值约 10MB，等于一份完整原生历史。
- Gateway 在同一测量窗口里另有约 12.6MB 的瞬时峰值（驻留 8.68MB，峰值 21.28MB）。表里"原生解析"只是探针的阶段点，窗口从请求开始算起；后续逐函数追踪表明，这个峰值来自 Gateway 专有的消息文件读取，与原生历史是否按需物化无关，见下一节。
...
**剩余**：Gateway 的 21.3MB 峰值，已定位，见下一节；改代码前须经主线 owner 同意。

### Gateway 21.3MB 峰值来自整块读取消息文件（2026-09-24，已实施待审）

**测量**：
- 仓外探针 `decision_gateway_peak_trace.py`，放在 pytest 隔离 HOME 下运行，基于 main `41c66872e`，与 2b 同一 Gateway 夹具（128 行、4.2M 字符）。
- 用 `sys.setprofile` 在每次函数进出时读 tracemalloc，记录"执行期间把全局峰值抬高超过 1MB"的调用；只观测，不改产品。

**结果**：峰值全部出在 `MessageStore.recent_report` 的三类调用上，都是 Gateway 专有，child 和后台都不走。

| 调用方 | limit | 本次读到 | 耗时 | 峰值 |
| --- | ---: | --- | ---: | --- |
| `ensure_gateway_conversation_index` → `ensure_thread_history_indexed`（每进程每线程一次） | 0（全量） | 128 行、4,195,620 字符 | 8.5 ms | 0.19 → 17.05MB |
| `gateway_recent_artifacts`（每次请求） | 80 | 80 行、2,622,296 字符 | 49.1 ms | 17.05 → 21.33MB，即准备期峰值 |
| `append_gateway_conversation_message`（每次请求，用户行与助手行各一次） | 100 | 100 行、约 3.25M 字符 | 49.2 / 63.1 ms | 发送后 21.48 → 22.90MB，全程最高 |

**原因**：
- 这三处只需要元数据：索引逐行投影，近期产物只看助手行 metadata 里的产物引用，追加只为幂等去重。但 `recent_report` 按行数限量、不按字节限量，行很大时（夹具每行 32KB；测试机真实会话里一条带原生信封的助手行就有 75,652 字符），"最近 80/100 行"就等于整份文件。
- 尾读把整块尾部依次物化为 bytes、解码后的 str（含中文时每字符 2 字节）、按行切分的列表、解析后的 dict 和 `MessageLogEntry`，四五份拷贝同时驻留，峰值约为文件大小的 4–5 倍。

**实施**（分支 `claude/decision-gateway-message-reads`，主线 owner 有条件同意：公共接口不变、按 LF 字节切行、停止边界与原实现逐项等价、不加开关、不动 child/后台路径）：
- `store_io.iter_jsonl_tail_lines`：与 `read_jsonl_tail_report` 同窗的倒读迭代器。同样 64KiB 块从文件尾对齐倒读，同样"已读换行数超过 limit 或到文件头"才停，窗口首个不完整行同样丢弃；只按 LF 字节切行后逐行解码。内存为一个块加一行。两者共用块大小常量；`read_jsonl_tail_report`、`recent_report` 本身不改，child 与后台仍走原实现。
- `MessageStore.recent_projection_report(thread_id, limit, project)`：复刻 `recent_report(limit)` 的起始窗口、翻倍重读、坏行顺序与 `row_index`，只把最新 limit 条非 display 条目交给纯投影，不驻留正文。
  - 近期产物投影为产物引用。
  - 追加去重与补写去重投影为 `_DedupeFacts`（只含判定会读的字段）；读取、查错、计算入参、判定的顺序与原实现相同。
- `MessageStore.visit_all_report(thread_id, visitor)`：给搜索索引用的两遍正向扫描，沿用全量读取的文本模式通用换行与严格 UTF-8。第一遍有坏行就一条都不访问（原实现先读全量再判断）；第二遍核对仍是同一文件，只读到第一遍的末行。
- 顺带改了补写路径 `repair_gateway_conversation_messages`：每个待补写文件都要读最近 200 行，最多 100 个文件，结构与追加去重相同。

**效果**（同一 4.2M 字符夹具，同一基点 main `3d2424786`，tracemalloc，只算 Python 堆）：

| 指标 | main `3d2424786` | 本分支 |
| --- | ---: | ---: |
| Gateway 准备期峰值 | 21.33MB | 0.82MB |
| Gateway 全程峰值 | 22.65MB（发送后追加去重） | 10.03MB（原生历史物化，与 child、后台持平） |
| 三宿主阶段探针：Gateway 首段 | 21.28MB | 9.15MB；child、后台各阶段不变 |
| 每次请求的三次尾读 | 41.4 / 40.3 / 39.9 ms | 6.4 / 6.4 / 6.1 ms |
| 建索引（每进程每线程一次） | 峰值 16.96MB，669–719 ms | 峰值 0.72MB，676–697 ms |

建索引的总耗时基本不变：两遍扫描多出的解析时间，被逐条写索引的时间盖过了。

**验证**：
- 新文件 `test_message_tail_streaming.py` 124 项，以不改的原实现作参照逐项比对：
  - 24 个种子，轮换三种模式（混合故障、完全干净、大量 display 行），分别比对尾部行、窗口投影、全量访问、近期产物和去重判定；覆盖多块大文件、翻倍重读、坏行、解析失败、CRLF、NEL/U+2028、无换行或被截断的末行、非法 UTF-8。
  - 3 种块边界：多字节字符、LF、CRLF 恰好跨越 64KiB 边界。
  - 1 个原实现的边角：窗口内有空行时返回的行数少于 limit，新实现必须一致。
...
来源：测试机的 M3 媒体验收（main `5dcdfd463`，见[真实验收](DECISION_MODEL_REAL_VALIDATION.md)）之后，本地复现"图片之后还能走多远"。
...
**修复**：分支 `claude/decision-media-preflight`，主线 owner 已同意，测试机真实验收已通过（见[真实验收](DECISION_MODEL_REAL_VALIDATION.md#124-媒体会话越过压缩点修复的真实验收2026-09-24分支-claudedecision-media-preflight)）。纯 bug 修复，不加开关，纯文字会话行为不变。
...
- 客户端文案：`gateway_client_error_message` 对 `COMPACT_REQUEST_NON_TEXT` 单独说明原因（会话含图片等非文本内容，无法压缩且超出可用窗口；可换更大上下文的模型或新开会话），先于通用 `COMPACT_` 前缀匹配。测试机第一次验收时结构化码已进失败记录，但 TUI 仍显示通用的压缩失败文案，由此补上。
...
3. 越过压缩点后仍按压缩点收缩的软行为：大工具结果按"距压缩点余量"缩成引用视图（`safe_inline_tool_result_tokens`）；非会话运行的内容工具会延后（`_should_defer_for_compact`，Gateway 会话不走这条）。在媒体会话里，它们等不到压缩。测试机真实验收没能观察到前者（越点后的读取所在回合随即越过上限被拒），仍待观察。
4. 失败回合之后，TUI 的 Context 读数偏低：没有计入失败回合留下、会被下一请求重放的工具往返（测试机上读数 67.1k，下一请求已超过约 103.7k 的上限）。属于 TUI 显示问题，已告知主线 owner。
