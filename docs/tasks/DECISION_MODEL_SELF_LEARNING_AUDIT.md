...
2026-09-25 补充（已合入 main `52e0190e1`；已端到端真实验收）：真实子代理只做自然回复、不出结构化输出，上面的来源在产品里实际走不到。现在增加第二个结构化来源：子代理用 `record_lesson` 写本 run 的 `lessons.jsonl`，同一出口读回账本，自然回复也会把账本经验记成带账本引用的 `subagent_lesson` 候选。宿主仍不从回复正文提取经验。
