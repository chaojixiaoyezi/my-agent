...
- 并发段0c19b2e3c精选8468998b6，无生产冲突；新segment_planning只做范围／批上限判定，两个按需查询保持原次数与时序。
- 主代理后续将no-action两项常量迁到唯一使用者round_execution并迁唯一测试导入，不改变阈值／计数／handler行为。
- 主代理审阅段片无阻断，独立代理只读审1c3a2516a收口顺序无阻断；最终11文件283 passed／20既有xfail。各历史分组有重叠不累加；中央树、设计和TESTS已更新。

建议下一步：与逐调用来源引用修复组合再发布，源引用切片保留独立写入；新schema来源缺失与旧reader降级风险必须单列，真实TUI尚未开始。
