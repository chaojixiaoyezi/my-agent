...
### 5.2 完整档(5 源)—— 跑到一半 MiniMax 额度耗尽(2026-07-08),待补跑

完整档从 A 顺序跑,跑到 C/D 时**真模型额度用尽**(判读调用降级/空返)。战况:

| 场景 | 结果 | 说明 |
|---|---|---|
| A 无空壳 | **PASS** | 入队60·判60·待判0·丢弃0;空壳工签收 0 |
| B 不静默吃 | **PASS** | 2/2 目标记录全被判到,丢弃 0 |
| C 重启 5/5 | FAIL(3/5) | 落后源[3,4]恢复,但另 2 源未在时限内判完=入队 |
| D 回执+召回 | FAIL(召回0) | judged=enqueued=80·**丢弃0·覆盖闭合**;但召回 0/9 |
| E 月级缩比 | **PASS** | 归档生成、活跃 spool 峰值 52KB(总入队240)、丢弃恒 0 |
| F/G | 未跑到 | 额度耗尽,进程停 |

**关键读法(这半程恰好反向印证定性锁死):C/D 的失败全部落在【模型质量】维度
(C 的判读完成时限、D 的召回准确率),而【机制】维度自始至终成立——D 即便召回 0,
`judged=enqueued=80·dropped=0·覆盖闭合` 仍然为真;只依赖队列机制不依赖模型质量的
A/B/E 全 PASS。** 这与"缺口在模型/额度、不在判读工机制;quick 档新鲜额度下 7/7 全过"
完全一致。C 的 2 源未判完 + D 召回 0,是额度耗尽后模型空返/降级的直接后果,不是机制回归。

**待补跑(额度刷新后,~1.5h):** 完整档 A–G 重跑,回填 C 5/5(含 3 落后源)、
D/F 的真召回/精度权威数字。复跑命令见 §7。quick 档已证 7/7 机制全对,补跑取权威数字。
