...
**从 memory 目录移出**（在 `~/.模型助手/projects/` 下）：
- `TASK_TASK_MANAGE.md`
- `TASK_MEMORY_PUSH.md`
...
- [x] 确认需求：长期助手 风格 PID tracking、supervisor、adapter daemon、系统服务
...
- TASK_FAILURE_INTROSPECTION.md - 失败自省+自适应派工
...
1. **创建 `docs/tasks/` 目录**
2. **移动文件**：
   - `~/.模型助手/projects/memory/TASK_*.md` → `docs/tasks/`
