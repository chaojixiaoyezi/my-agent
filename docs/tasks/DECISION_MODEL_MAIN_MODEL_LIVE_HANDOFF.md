...
## 第七、八个样本：普通短任务不应换模型（2026-09-25，main `dfa8e498b`）

**环境**：
- 测试机新隔离目录，唯一 Gateway 8431，原生 TUI，wheel SHA256 前缀 `3204ab7c`。
- 候选与用途标签和第六个样本完全相同：M2.7 为 `general`（当前选中，声明 200k），M3 与 DeepSeek 为 `long_document, large_context`（声明 1M）。
- 决策只开 `points.model_selection.mode=apply`（8 秒单次、10 秒阶段）。
- 测试者各开一个新会话，只输入一条普通中文短需求：
  - 第七个：用两三句话解释二分查找及其为何要求有序；
  - 第八个：写一个判断回文的 Python 函数并给两个调用示例，不建文件、不运行。

**结果**：
- 两次 Jev 各 1 次（HTTP 200，0.90 / 0.78 秒，请求约 3.3 KB）。都选 `retain_original`，观察记录 `adopted=false`、`not_evaluated`，都没有换模型。
- 两个需求都由 M2.7 正常答完，答复正确。

**结论**：
- 在这组候选和标签下，普通短任务 Jev 保持当前通用模型，没有出现"该留却换"的误采用。
- 与第六个样本（超出窗口的长资料任务自动采用 M3 并答对）合起来：只有语义和容量确有需要时才换，平时不换。
- 样本仍然很少，不能外推到视觉、推理密集等其它任务类型；那些任务需要候选带对应的用途或模态事实。
- 本两轮 Jev 共 2 次，累计 91 次。
