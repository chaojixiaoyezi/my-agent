...
## 真实收益实验的前提（2026-09-24）

- 只用词面检索时，补充查询在生产接线里补不回任何事实：片段取自原问题，中文二字组不跨标点，所以片段里的词一定也在整句里；BM25 已返回全部正分文档，空槽出现时基线已包含所有正分文档。此前两轮真实样本都是 `no_addition` 即源于此。只有开启语义召回（向量相似度）时，聚焦片段才可能召回被整句稀释的事实。
- 证据：上下文包 `memory_refs` 新增 `recalled_refs`（来源为 baseline 或 supplement）与 `recall_findings`（分支 `claude/decision-recall-evidence`），真实实验据此判断补充是否带来新事实。

## 语义召回下的真实收益（2026-09-25，main `ab23a2666`）

- **实验**：Gateway 开 MiniMax `embo-01` 语义召回，20 条事实，三条经离线标定的已知漏召回问题（整句余弦低于阈值，对应片段余弦 0.64—0.95）加一条对照，每条问题各跑 off/apply。
- **结果**：
  - 3 条已知漏召回样本中 2 条稳定补回（apply 两批 4/4 次），答复从"查不到"变成准确事实。
  - 第 3 条 Jev 两批都选了主题已被基线覆盖的片段，`no_addition`。
  - 对照样本无副作用。
  - 主模型没有长期事实检索工具，off 轮自查也查不到。
- **代价**：每轮多 1 次 Jev（约 0.7—1.2 秒）和 1 次嵌入。
