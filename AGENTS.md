...
- 并发代理身份必须显式。子代理、孙代理、工具调用、产物登记、状态事件都要优先携带 `run_id/task_id/parent_run_id/root_run_id` 等结构字段；`inspect_agent_tree` 是展示投影，不是身份来源；线程级上下文只能做权限上界和兼容兜底。显式身份参数和当前 runner 上下文冲突时，不能静默猜，工具响应必须暴露 `scope_resolution/scope_warnings` 这类结构化裁决事实。消息、协作 case/request、证据提交、能力申请这类会写账的入口，也必须按这个规则处理：当前 runner 内只能代表自己，冲突参数只能成为 warning，不能冒充其它 run。
...
- 发现问题时，先做“对照组阅读”，再改代码。默认先系统查看 `/Users/example/study-agent/all-agent/` 下所有相关项目，重点优先参考 `通道运行时-main`、`长期助手-agent-main`、`会话运行时-main`，必要时再看同目录其他项目；必须确认它们是怎么处理同类问题的，再学习优化本项目，禁止闭门造车。
