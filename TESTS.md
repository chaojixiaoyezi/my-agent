...
源码尚未发布部署；LOCAL_UNMANAGED 旧排队启动仍缺少可撤销的准确接纳，继续阻断发布。
...
CLI 覆盖宿主真实 argv 构造至 parser/Options/Params 的闭环、空值仅声明无数据库模式、普通 watch、
...
以及无数据库模式完整停止/恢复仍待收口。42 个相关文件去重 757 项通过，2 项既有跳过（Linux `/proc` 与已移除 CLI）；
...
当前先定位 TUI 137 的任务资源停止缺口，第 3 步不收口；实际控制测试不得改变系统网络、Wi-Fi、网卡、路由或系统代理。
