...
- `my-agent scenario-test --case verification`、`--case gateway-restart`、`--case gateway-cross-day-resume`、`--case gateway-delayed-response`、`--case gateway-multi-worker`、`--case gateway-stale-lease`、`--case parent-subagent-cross-day-resume`、`--case structured-repair` 和 `--case runner-retry` 是坏天气/恢复场景入口，分别覆盖验收防作弊、gateway processing 请求恢复、gateway 跨天恢复、迟到 response 去重归档、多 worker 并发抢占、stale lease 重排恢复、parent/subagent runner 跨天恢复、结构化输出损坏修复和 runner 临时失败重试。
...
# 只验收日志分析产物时可直接跑这个入口。
python3 scripts/live_lab/log_analysis_replay.py --output-root /tmp/openclaw-log-replay
```
...
```bash
python scripts\live_lab\log_analysis_replay.py --output-root %TEMP%\openclaw-log-replay-codex
```
