...
- 守卫测试（`test_conversation_store.py`）：`jsonl_lines` 只按物理 LF 切分并保留字符串内
  NEL/U+2028/U+2029（对照 `str.splitlines()` 会把同一条记录切成多条）；带这些字符的消息可写可读、
...
- 真实键盘 Ctrl+O → Ctrl+E 进入 25 页完整原文，Ctrl+End 返回实时输入；运行中一次输入回显约
  71ms（含 tmux 发送/捕获开销，单样本，不代表极端慢模型 p95）。主回合结束 Working 收起。
- 显式 `/compact` 返回 generation 1，估算 63,705 → 10,121 tokens；后续追加读者报表已运行并返回最终答复。
  四子代理均返回，主代理执行整合工具后 Working 收起，但本轮最终汇报缺失：canonical messages 最后一条
  非 display 的 assistant 仍是等待期间回复插话的内容，thread.active_task_ids 已为空。此项未通过，保留现场。
  不能只归因于前端漏渲染；需核对后台工作片的模型终答、交付决策与提交路径。
- 整合工作片用量事实为 17 次真实 MiniMax 调用、0 次模型/HTTP 重试；缓存读 710,663、缓存写 83,187 tokens。
  此处只记录该片账本，未完成全部工作片/子代理/费用的逐调用交叉核算，也不把业务生成结果当作正确性证明。
- 尚未关闭：插话确认文字与用户行的相对次序、极端慢模型长期性能、系统剪贴板真实复制、
  策展真实成功与游标推进、真实超时恢复和用量账本逐调用核对。不能用本轮启动及分项通过宣称全部完成。
...
- `test_scheduler_scan_costs.py::test_scheduler_read_paths_do_not_parse_inside_the_lock`：插桩断言
  `queued_runs`（每 tick）/`list_jobs`/`get_job`/`active_runs_by_job`/`corruption_report` 在持锁期间
  **没有** JSON 全量解析、**没有**逐 run 解析、**没有**锁内权威读取，同时确实做过解析（防止空转）；
  在修复前代码上必红（已验证）。
- 同负载配对基准（N=10000 条 run、4.7MB store、交替 4 轮 × 5 次取中位）：五个只读路径的锁持有从
  38~51ms 降到 **~13µs**（2963x~4078x）；总耗时本次负载下 +10~20%（未做更精细分离）；写路径
  `_mutation_scope` 未改（锁内 ~21ms，已测量残余）。

## 2026-09-13 R273 排队等待无累计上限 + A 项成本配对基准

- 存活用例（`test_gateway_admission_wait.py`）：**夹具回拨进队时刻制造虚拟累计等待**（不是真机等待）——
  刚过 1 小时 / 半天 / 3 天 / 30 天时仍继续发新等待信号、且不产生任何"等待过期"事实；节流多轮后信号持续
  存在；网关停写→客户端空闲窗口内收口；取消→窗口内收口；权威终态→读到终态记录。原来的"超预算写过期的"
  用例已随配置一并删除。
- 成本配对基准（`_owner_fact_kind`，两 worktree 交替 5 轮 × 60 次调用取中位，**两组独立跑，比值只在组内**）：
  · 第一组（定位回退）：安静 owner 修复前 0.9918ms → 我上一版 2.5024ms = **2.52x（+152%）**；
  · 第二组（改造后复测）：安静 owner 0.6735ms → 0.7405ms = 1.10x（+10%，落在单轮波动内）；
    缓存命中 0.92x（−8%）、软事实 owner 0.95x（−5%）；"强制每次完整绑定"（busy miss）1.94x（+94%，+1.11ms，
    是正确绑定必须付的判定前后两次签名）。
  结论：**先判定、后决定签名**恢复了安静 owner 的成本，代价落在"贵且每轮都未命中"的路径上。
  （跨组把 2.5024 与 0.6735 相除得到的 ~3.72x 不是同一组配对数据，不可那样比。）

## 2026-09-13 R272 合法排队等待信号 + 插话确认终态 + 短屏真实终态

- `test_gateway_admission_wait.py`（15 例）：结构化等待字段写入且**不含** lease/status/模型字段、节流
  窗口内不重复写、**无累计等待上限**（1 小时/半天/3 天/30 天虚拟累计等待仍继续发信号、不产生过期事实）、
  恢复退避请求同样有信号、被正常认领的请求不带等待字段；客户端在只有等待信号时持续等到终态、停写后窗口内收口、取消后窗口内收口；
...
每次调用进两次锁，且**两次都在锁内**把 store.json 读+全量解析一遍。N=10000 条 run（4.2MB）时
锁内 12.3ms + 15.4ms、`load_store` 2 次/调用、逐 run 解析 10000 次/调用，并发等锁者中位 13.4ms；
N=2000 时 2.2ms+2.9ms；N=200 时 0.23ms+0.30ms。

改为锁外探针读一次字节+摘要 → 锁外全量解析 → 锁内只做一次 stat 复核（字节仍是当前文件就用
锁外结论），复核不通过/探针不可用退回锁内权威读取。配对交替实测（交替 5 轮、N=10000）：
锁内持有 **39.6ms → 0.010ms（≈3964x）**，并发等锁者中位 **33.9ms → 0.090ms（377x）**、
p95 **46.6ms → 0.273ms（171x）**，总耗时中位 **75.5ms → 49.7ms（1.52x）**。

新增 6 例（`test_scheduler_scan_costs.py` 第②组）：与旧实现参考投影逐字相等（active/paused/
deleted job + 活跃/终态/坏 run 行）、JSON 解析与逐 run 解析都不得发生在持锁期间（插桩断言，
不靠计时）、外部改写立刻可见、陈旧探针必须退回权威读取、坏账本与缺账本两态与参考实现一致。

本轮 focused：scheduler 相关 10 个套件 **85 项通过（EXIT=0）**。
**不等同真机验收**：真机 TUI 与部署未做（运行时仍为 r261）。
...
展示覆盖背景先到、前台后附着、相同文字不同 request 和观察者隔离；错误文案不否定已有执行。
先证旧实现失败，再跑定向；必须补真实 TUI，不以 mock 请求代替现场结论。
现场开放项、tmux 与安装边界见 [R229 台账](docs/audits/R229_LIVE_FAILURES.md)。
本轮用户最新选择以 工具运行时 验证现场，官网 MiniMax 对照不再发任务；旧会话与新会话分开记结果。
追加已知 DeepSeek 工具协议的旧空字段回归，联合 10 文件 347 项通过；本机旧会话仍 400，不能宣称关闭。
长选区回归检查超过 100KB 仍进入 native/tmux，OSC 不溢出，tmux 命令参数不承载全文；物理鼠标与键盘链路分列。
真 TUI 发现 Ctrl+A 未接到 view；删除仅断言渲染有行的误导测试，实际调用 view.select_all 并核对主/modal隔离。
typed 400/401/422 与 capacity/timeout/HTTP 503 冲突文案必须不重试；安全投影只展示合法 HTTP 属性，不公开私有详情。
最终 14 文件 463 项通过；`ma-r229-copy` 原会话 166215 字节键盘复制、HTTP 400 安全展示均真验，
原长会话恢复仍失败。此结果不等于物理鼠标或所有重试路径真实故障注入通过。
...
新增分段摘要测试验证每个连续源字符均进入受预算请求、固定缓存面不变、越界缩小请求、停止及空摘要失败；
旧 transcript checkpoint/CAS 测试的小触发线与 fake provider 请求容量显式隔离，不能当作实际模型容量验证。
真 TUI 在 `.10` 单 Gateway 的 `ma-r227-main` / `ma-r227-models`。用户随后明确授权本机更新与重启，
本机 `ma-r227-local-resume` 复用原长会话，通过真实 `/model` 菜单选择 工具运行时 DeepSeek，再切官网 MiniMax；
不由开发者补写被测项目产物。所有任务和记忆保留；常规测试默认仍为官网 MiniMax-M2.7。
新增真实 Shell 回归覆盖已有 ready 产物、Full Access、registry 空补充读根的组合，命令必须正常启动且保留原文件。
真实跨窗口测试已触发 594855 → 16337 的 transcript Compact、代次 0 → 1，但随后模型找错目录，
该业务续接不算通过：源码真实存在。追加匹配工具路径持久投影、排除 runs 目录候选、分段真实进度和
压缩开始刷新模型窗口；对应 focused 覆盖失败/孤立/伪造文字、跨代保留、完整路径长度和当前模型显示。

提示词去重回归：provider 系统规则只出现一次、原生工具说明/Schema 与冻结快照一致、默认 YAML 与 dataclass 同文，
不恢复“清单触发后台”、强制先写骨架或诊断必须落报告。26 工具同口径离线计量仅用于说明瘦身，不替代真 TUI。
Shell 旧引用选择测试保留范围内文件真实前像、动态收窄权限、无外部读取/备份/修改、缺失源计数；符号链接与备份失败仍拒绝。
`.10` 的 `ma-r227-main` 中 `subagent-1789039605-b40f40a2` 已真实读取自身 context_bundle.json，随后完成导入导出测试；
其中另一个缺失路径失败如实保留，不能写成全工具零失败。PTY 实际调用与工具仅可见分开计数，详见 R227 台账。
R227f 追加真实执行器与耐久操作账本回归：无效 PTY 句柄是 failed 操作/零副作用，部分输入仍是 unknown；
跨会话不可写，写完后立即退出不抹去已写事实。23 文件最终联合 806 通过，未跑全仓。
原会话第二轮 438K → 34K 后已正确定位并启动命令，204 条越界旧引用未被读取；后续 PTY 错误单列，
修复后同一 TUI、同一 session 继续，冒烟 19/19，两个子代理继续实现；不把这些局部结果算作项目全部复刻通过。

## 真实模型测试的默认来源（2026-09-10 用户约定）

后续常规真实 LLM/TUI 测试默认使用 MiniMax 官方接口的 `MiniMax-M2.7`，不使用 工具运行时 转接的 MiniMax。
启动前核对选中的 provider、实际请求端点和模型名称；不能仅凭同名模型或 TUI 标题判断供应商。
密钥只留在私有配置，证据记录来源与是否读取成功，不输出密钥内容。用户明确要求验证其它模型/协议时再做对应专项验证。
这项约定不自动改变用户日常 TUI 已选的模型；测试默认与个人会话选择分开维护。
...
本机授权切换后追加 ma-r226-local-deploy：从仓库外全局入口启动、F4 三档取消不改策略、DeepSeek 短问答
done/ok、当前 caller_model 与启动默认明确分开；旧等待通过精确控制请求到 interrupted/closed。
本次未改生产代码，未重复全仓 pytest。请求/run ID 与本机全局入口、`.10` 服务已部署、`.7` 未连通的边界见
...
R197 已按此方法在新 J 验证一次：首段 329 bytes 后断流，原字节请求重试成功（HTTP 200 / 4484 bytes）；
同 attempt 未重启，重连后只有一次成功派工。故障夹具计数用尽后透传，下一安全切换恢复真实上游直连。
同次部署的旧 G 暴露前后台恢复竞争：后台 final 已提交，旧前台恢复仍失败。该问题单列 R198，不混入断流结果。

## R196 已知超时与真实 UNKNOWN

先红测复现“8 次成功调用被灌入 5 条 UNKNOWN 放行提示”、普通 TOOL_TIMEOUT 无证明豁免和重复 UNKNOWN
追加放行；删旧分支后转绿。定向测试覆盖实际进程/独立 session 后代回收、中文与 stderr 部分输出、
枚举不完整/仍存活/身份读取失败/PID 复用的保守结果、启动前 deadline、FAILED 与 UNKNOWN 各自幂等账本、后台控制、
原会话精确恢复和安全公开文案。共 352 个唯一用例分组通过，另复验 66 项 R195 展示检查；focused 不代替真实 TUI。

真 TUI 继续长项目任务，核对真实 command timeout 的进程回执→工具 FAILED→模型读输出修复→后续请求恢复；
另外核对无终态证据仍保留 UNKNOWN。不能手工修改旧失败账本、用旁路执行项目测试或把命令已终止当任务完成。
部署新 TUI 后同步检查 R195 footer 与最终回复。磁盘不足时先核对旧环境可回收范围，保留当前运行与回滚环境。

## R195 递归协作的代次与展示

真实图书任务先复现 coordinator 正常唤醒被显示为重试；新增 renderer 回归先红后绿，覆盖等待下级、
运行、完成和失败四种状态，attempts=8 仍不得声称失败七次；ctx、Compact 和真实状态保持。
产品验收使用原递归任务，不造假树或代填状态。退出/恢复前后比较 canonical 消息、用量和压缩代数，
只读回看不得额外调用模型。最新客户端复验未完成前，不把本地定向结果写成真 TUI 通过。
...
新增源值 A→B→A、来源隔离、回滚后重新追加、不可变布局、重复来源/双正文拒绝、空注入复位和完整渲染验证。
两代 Compact、取消和 CAS 失败回归增加独立且未改变的 workspace/memory 来源，验证每类最新值保留。
当前共 260 focused 通过；只验证结构、无副作用和缓存前缀，不代替真实 provider cache-read 或价格收益。
真 TUI 继续三个原 owner/session，MiniMax-M2.7；采集部署前后原始请求、模型账、压缩代数和主/子代理状态，
跨 Compact 的记忆和后续任务继续验证，不人为降低窗口、替被测对象做任务或制造脚本结果。
...
- 模型把 task_ref 复用成 `tasks/日期/任务名/...` 时，参数规范化必须从宿主
  `canonical_owner_home_root` 还原，不能相对当前 cwd 再嵌套一层；旧内部调用可回退安全墙字段。
  Full Access 管理员没有 owner 安全墙时也必须读到自己的历史任务，而 WorkspaceOnly 的跨 owner 与
  非授权写仍由 `effective_owner_scope_root`、allowed roots 和 exact rebind 拒绝，地址规范化不是授权。
- focused 命令：
...
- 真 TUI 第一阶段：同一 MiniMax-M2.7 会话的项目 A、无关项目 B 已在 R157 分别跑通 19/15 项常规 pytest；R158 新样本
  ...
  task_ref，但又暴露 `tasks/...` 被嵌套到当前 cwd。R159 继续暴露 Full Access 无 owner wall 时地址基准缺失；R160 复验必须证明 processing request runtime、successor link
