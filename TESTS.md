...
## 第 3 步参数合同次片（真实 TUI 交互失败，修复待复验）

参数次片 `c1484f7f2` 已发布，同包部署双机，各 1,214 个包文件一致、每端一个 Gateway，配置与回滚保留。
实际 TUI 143 复现：输入 `/plugins help ins`、Tab 补全后按 Enter，输入变成 `help install -h`，没有提交原命令。
现场保留，三路普通业务需求尚未发送，不能把开发 gate 改记为真实验收通过。源码修复区分自动候选和显式 Tab：
完整命令不自动补可选旗标，保留正常 Enter 提交。新增真实输入 Buffer 的接受候选→Enter 定向回归，新安装版仍待复验。
...
最终 10 个相关文件合计 475 项通过，零失败、错误或跳过；Ruff、doc sync、strict code-size、diff、clean-package 与新增行隐私检查均通过，尺寸基线未改。
这些是开发合同与入口替身证据，不是已安装插件的业务验收。累计改动未触发全仓阈值，没有重跑无关全仓，线上 CI 未作为本轮验收来源。
接着通过两路以上实际 native TUI 验帮助/错误及普通官方 MiniMax-M2.7 任务，原始屏幕、身份和产物只留仓库外。
