...
- `my-agent scenario-test --case verification`、`--case gateway-restart`、`--case gateway-cross-day-resume`、`--case parent-subagent-cross-day-resume`、`--case structured-repair` 和 `--case runner-retry` 是坏天气/恢复场景入口，分别覆盖验收防作弊、gateway processing 请求恢复、gateway 跨天恢复、parent/subagent runner 跨天恢复、结构化输出损坏修复和 runner 临时失败重试。
