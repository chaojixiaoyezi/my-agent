...
## 第 3 步宿主目录片（源码定向验证通过，待实际验收）

源码新增目录合同、宿主接口与客户端接线，尚未部署或执行本片真实 TUI；既有 143—150 属于前一发布版本。
...
上述替身与组件测试不冒充实际 TUI。发布前仍需相关 focused tests 和全部严格 gate，再做多 TUI 的宿主帮助/参数/活动任务隔离及普通工具链。
