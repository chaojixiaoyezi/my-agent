...
- 存活用例（`test_gateway_admission_wait.py`，虚拟时间）：累计等待刚过 1 小时 / 半天 / 3 天 / 30 天时
  仍继续发新等待信号、且不产生任何"等待过期"事实；节流多轮后信号持续存在；网关停写→客户端空闲窗口内
  收口；取消→窗口内收口；权威终态→读到终态记录。原来的"超预算写过期的"用例已随配置一并删除。
- 成本配对基准（`_owner_fact_kind`，两 worktree 交替 5 轮 × 60 次调用取中位）：安静 owner 修复前
  0.6735ms / 上一版 2.5024ms（2.52x 回退）/ 现版本 0.7405ms（1.10x，噪声内）；缓存命中 0.92x；
  软事实 owner 0.95x；"强制每次完整绑定"（busy miss）1.94x（+1.1ms，为正确绑定必须付的判定前后两次签名）。
