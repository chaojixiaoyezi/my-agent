...
显式管理请求的[宿主执行身份](docs/design/HOST_COMMAND_EXECUTION.md)正在接线：原 RuntimeDB 原子绑定请求与独立 pending 运行，重送只读原结果，不另建执行器或历史账本。
唯一安装表沿 canonical owner data，版本与最后提交回执原子保存；原请求重试不覆盖新状态，清理异常不抹掉提交事实。
公共目录锁复用原后台系统锁，保留锁名/顺序并拒绝链接；没有第二套操作历史。管理授权、原操作链接线、独立环境及撤销尚未完成，装卸命令未开放。
