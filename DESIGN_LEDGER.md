...
**新增存活用例（虚拟时间，不是把 1 小时改大）**：累计等待刚过 1 小时 / 半天 / 3 天 / 30 天时仍必须继续发
...
| 场景 | 修复前(717d6b35) | 我上一版 | 现版本(先判定后签名) |
|---|---|---|---|
| 安静 owner（cheap 分支） | 0.6735ms | **2.5024ms（2.52x 回退）** | **0.7405ms（1.10x，噪声内）** |
| 缓存命中（hot） | 0.9436ms | 1.0024ms | 0.8662ms（0.92x） |
| 软事实 owner | 0.9409ms | 1.1033ms | 0.8947ms（0.95x） |
| 强制每次走完整绑定（busy miss） | 1.1813ms | — | 2.2938ms（1.94x） |

数据确认了验收的预判：**上一版让安静 owner 每调用慢 2.52x**（因为它在判定之前就付了整份签名，
而这类 owner 恰恰是 `_FACT_MIN_CACHED_SECONDS` 要保护的人群）。现改为**先探测判定、再决定是否签名**：
...
**只写等待事实**，不写 `status`/lease/模型字段，不伪装成已认领或在推进。写入节流 30s，且受
**只做节流（30s），不设累计等待上限**：健康网关持续证明"仍在合法等待"时，慢模型排队再久也不会因总时长
被判失败。**（R273 更正：第一版误加了 1 小时总预算，验收判定那是把误超时从 5 分钟搬到 1 小时并制造
"前台报失败、后台照常执行"的隐式语义，该配置与过期分支已撤除。）**客户端**零改动**（它已按 chunk/inbox/processing 文件活动
续期），因此这些写入天然成为"合法存活"证据；收口条件不变：停写→空闲窗口超时、取消→文件消失→超时、
终态→终态记录、崩溃恢复→新网关继续按同一预算写。准入判定改为结构化：`try_acquire_report()` 返回原因码
（空串=已获取），`try_acquire()` 是其布尔包装。**15 例**回归，含"真实 worker + 真实客户端"端到端用例、"关掉信号后同一场景必然超时"的负向对照，
以及刚过 1 小时/半天/3 天/30 天虚拟累计等待的存活用例（断言继续发信号、不产生任何等待过期事实）。
