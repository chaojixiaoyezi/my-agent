...
- 已实现（2026-09-24，分支 `claude/plugin-update`）：`/plugins update <插件> <新包路径>` 首片——同 ID 新包替换已停用安装，install+可选 configure 两步回执保留兼容配置，不新增持久动作；不做双版本准备切换与 rollback（需安装记录持有候选版本字段，另开一片）。见 [插件方案](docs/design/PLUGIN_LIFECYCLE.md#管理与使用分开的命令语法待实施)。
