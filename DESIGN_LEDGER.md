...
`gateway_admission_wait_budget_seconds`（默认 3600s）约束；超预算停写并只留一次 `admission_wait_expired_at`，
客户端按自己的空闲窗口如实收口，请求不被丢弃。客户端**零改动**（它已按 chunk/inbox/processing 文件活动
续期），因此这些写入天然成为"合法存活"证据；收口条件不变：停写→空闲窗口超时、取消→文件消失→超时、
终态→终态记录、崩溃恢复→新网关继续按同一预算写。准入判定改为结构化：`try_acquire_report()` 返回原因码
（空串=已获取），`try_acquire()` 是其布尔包装。**12 例**回归，含"真实 worker + 真实客户端"端到端用例与
"关掉信号后同一场景必然超时"的负向对照。
