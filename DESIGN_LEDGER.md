...
状态：用户已明确选择框架功能与模型交付质量分开记录，Goal 恢复 active；第 1 步框架基线收口，第 2 步进度纯计算、供应状态与唤醒/Goal 路由已分离并部署，相关实际链路已验；租约/恢复与稳定资源换代修复也已同版部署，相关实际验收和资源收尾完成，第 2 步本轮框架范围收口，第 3 步公共声明与命名空间首片已发布并同包部署；实际 TUI 137 的资源停止缺口待修复，整步尚未收口。命令公共声明、文件队列守门和旧控制分派的扩展边界已记入[首片接线核对](docs/design/PLUGIN_LIFECYCLE.md#第-3-步首片接线与实现边界)；首片已部署，完整参数和装卸仍待做。my-agent 两模型短题通过，Flash 核心长题通过但报告有保留项，M2.7 长题跨框架失败保留。

新增待修复边界：显式任务资源停止须覆盖主代理启动并跨工作片存活的受管后台命令，不能只取消回合或关闭 PTY。
按可信宿主身份冻结执行归属，核对启动/停止竞态与恢复轮隔离；访问范围、完成通知和执行归属不混用。
详细事实及验证要求见[受管后台进程合同](docs/design/MANAGED_BACKGROUND_PROCESS_SESSIONS.md#任务资源停止缺口待修复)。
源码首片已将访问身份及 PTY 执行身份提取到 `tooling/process_scope.py`，删除原定义与专用类型；缺失身份不从访问回退推断。
direct/local 的 `/interrupt` 已在插话清退和子代理取消前返回。该片尚未发布或实际 TUI 再验；后续源码的后台归属与交接进度见下文，整任务停止仍待接线。
v2 存储部分已实现于源码：同一 session JSON 地址，目录互斥、版本 CAS、固定停止清单和可恢复的文件提交。
v1 不推导执行身份；已发布 redo 的失败保留“已提交、待恢复”，未知资源不裁剪。源码已接 v2 预留/明确交接：
权限复查冻结原 operation 的 task/run/attempt，只读原门，不重复审批、预算或 claim；不能把回合 token 长期挂给已交接的资源。
Registry 只缓存当前 Store，丢失 host 保持未知；单 session 停止冻结原实例，清理异常保留实际回执及目标提交与其它恢复事务的区别。
原执行权限取消与 Gateway 持久主任务资源冻结已在源码接入；按用户要求直接核对 Codex `578c1b2` 的两类控制和固定集合清理。
沿原 RuntimeDB CAS 关闭精确 attempt，UNKNOWN 保留锁；后台领取前登记中断，换代前再查，Goal 恢复不复活旧片。
部分冻结错误保留已提交清单；后台资源退出确认不代表 PTY 已退出。direct/local 主链已接入原运行绑定回调，
每条消息持独立控制句柄，Compact 复用句柄但不把消息编号当执行身份；启动前中断和旧回调关闭均有明确边界。
无持久任务链接的 Gateway 主链沿请求锁关闭发布门并读取新鲜绑定，退出请求锁后按精确执行轮冻结；
已晋升、读取失败、未发布或错代次不能降级为历史 main 清理。源码尚未发布，完整子树及终态孩子遗留资源现已接通固定清单，实际验收仍未完成。
具体源码映射、锁序与未覆盖边界见[整任务控制合同](docs/design/MANAGED_BACKGROUND_PROCESS_SESSIONS.md#整任务控制接线实现中尚未发布)。
...
整树取消已拆为 creation 内关闭权限/固定清单、锁外清理，原工具模块只保留参数、授权和回执；终态资源清理不改业务结果。
runner 宿主可能派生接续进程，因此独立 PID 也只读观察退出；原心跳读取持久取消并转交精确 attempt，不新增取消账。
插话重放已在源码改为旧/新 turn 排序、先修提交与确认批次、原回执锁内复读，只有 pending 才预留准确后继。
RuntimeDB 接受原 current/候选 ID 的成对 CAS；数据库成功而回执或索引失败沿同一 pending 重试，不虚构跨介质回滚。
通用恢复的 reserved 改绑规则保持，网络重试只接续本条消息；源码验证见 TESTS，尚未发布或实际 TUI 验收。
无数据库空接纳身份已补源码：原文件启动记录预留非空 attempt、唯一 launch 和一次性消费事实；
激活沿 canonical mutation，旧整任务快照不回滚该域，取消也撤销已终态任务的新预留。相关回归已通过，完整发布与实际 TUI 仍待收口。
...
当前未发布源码已将明确取消改为固定工具资源清理和 worker 内协作中断，上述整树停止合同优先。
