...
可长大的账本只有 scheduler `store.json`（N=10000 条 run 时 4.7MB）；其余是游标/marker/guard/单请求文件/
transcript 尾部，量级在 KB 级。

**同负载测量（N=10000，配对交替 4 轮 × 5 次取中位）**：

| 路径（频率） | 修复前 总/锁(ms) | 修复后 总/锁(ms) | 锁改善 |
|---|---|---|---|
| `queued_runs`（**每个调度 tick**） | 51.69 / 46.710 | 61.77 / **0.013** | 3593x |
| `active_runs_by_job`（工具 list） | 106.24 / 44.123 | 126.35 / 0.013 | 3394x |
| `list_jobs`（工具 list） | 41.61 / 38.520 | 49.34 / 0.013 | 2963x |
| `get_job`（工具 get） | 42.22 / 38.818 | 46.34 / 0.011 | 3375x |
| `corruption_report`（诊断） | 55.58 / 50.974 | 61.67 / 0.013 | 4078x |
| `reserve_due_runs`（每 tick 写，**未改**） | 48.27 / 45.222 | 47.40 / 44.312 | 1x |

**修法**：新增 `_store_snapshot()`（锁外读+解析 → 锁内一次 stat 复核 → 失败退回锁内权威读取），六个只读路径
改用它；同时去掉单次读者白付的 blake2b（`_probe_store_bytes(digest=False)`，跨调用缓存仍 `digest=True`）。
**总耗时**本次负载下 +10~20%（未做更精细分离），收益是锁持有从几十毫秒降到 ~13µs。**写路径**
`_mutation_scope`（每调度事件一次，锁内 ~21ms）记为**已测量残余**，本轮不改（CAS 读-改-写不适用该模式）。
其余 40+ 处锁内解析均为小文件，未发现量级影响，记为结论不再扩项。

**验收状态汇总（截至 R274；"真实 TUI"一栏未部署即不做）**

| 项 | 代码 | 受控证据 | 真实 TUI | 运行版本 |
|---|---|---|---|---|
| A 事实缓存快照绑定 | `f082695b` + `79c08473`（先判定后签名） | 竞态 4 例（旧实现必红）+ 成本配对基准 | 待部署 | r261（不含） |
| B 投影命中原子化 | `f082695b` | 淘汰注入 1 例（旧实现必红）+ 6 owner 并发 | 待部署 | r261（不含） |
| C 坏探针回退 | `f082695b` | 坏探针 + 有效账本 1 例（旧实现必红） | 待部署 | r261（不含） |
| 缓存上限（监督者探针） | `c95cd6ed` | 12 名字探针 12→4、端到端 40→1、双预算+清理用例 | 待部署 | r261（不含） |
| GW-03 waiting 投影/owner 事实缓存 | `ce238ac4` | 三态一致性 + 三态缓存用例 | 待部署 | r261（不含） |
| GW-01 心跳代际/就绪判据 | `51163568` | 组合 B/E 复现 + readiness 用例 | 待部署 | r261（不含） |
| scheduler status 锁内解析 | `8c914dbb` | 与旧实现参考投影逐字相等 + 锁内无解析插桩 | 待部署 | r261（不含） |
| scheduler 只读路径锁边界 | 本轮（R274） | 锁内无解析插桩 + 配对基准 | 待部署 | r261（不含） |
| 插话三段状态 | `1bd057f1` | 12 例（提交边界不消费/入历史/慢流/失败/重连） | 待部署 | r261（不含） |
| 插话确认终态 | `7752adf5` | 6 例（2 例去守卫必红） | 待部署 | r261（不含） |
| 短屏视图真实终态 | `7752adf5` | 4 例（压缩/完整 × queued/submitted） | 待部署 | r261（不含） |
| 合法排队等待信号 | `7752adf5`+`403c85bf`+`79c08473` | 20 例（含虚拟 1h/半天/3d/30d 存活 + 停写/取消/终态反例） | 待部署 | r261（不含） |
| 慢模型活性四层合同 | `73d1b38a` | 11 例（提供方/客户端/租约心跳/长工具） | 待部署 | r261（不含） |
| 策展超时根因 | `f84a2e28`+`717d6b35` | 只读槽位时间线（单槽 total_slots=1、租约期内持续 busy） | 现场只读观测 | r261（已含诊断） |
...
**① 撤掉我上一轮发明的排队等待总预算（验收判定成立）**。R272 曾加
`gateway_admission_wait_budget_seconds`（默认 3600s）与 `admission_wait_expired_at` 分支：超过总时长就停止
等待信号。验收指出这是把原来的 5 分钟误超时搬成约 1 小时，并且制造"前台报失败、后台以后照常执行"的隐式
语义；用户要的是健康慢模型长期工作，没有授权给正常合法排队新增寿命。**已按要求全部撤除**：配置项
（YAML/dataclass/normalize）、`AdmissionLimits.admission_wait_budget_seconds`、`admission_wait_expired_at`
分支与相关文档/测试分支都删掉，只保留**一个权威事实来源**：合法排队由 worker 的节流写入证明，
收口只来自权威事实（网关停写=真失活→客户端按自己的空闲窗口收口；取消→条目消失；权威终态→终态记录；
崩溃恢复→新网关继续写，真死则停写）。**没有累计等待上限**。若将来确需"用户可选的等待截止策略"，
单独提案，不作为 bug 修复的默认行为。

**新增存活用例（探针里把进队时刻回拨，属虚拟时间，不是真机等待那么久，也不是把 1 小时改大）**：
累计等待刚过 1 小时 / 半天 / 3 天 / 30 天时仍必须继续发
新等待信号、且不得产生任何"等待过期"事实；另有节流多轮后信号持续存在、网关停写→客户端窗口内收口、
取消→窗口内收口、权威终态→返回终态三类反例。

**② A 项（`f082695b` 快照绑定）的成本回归：实测确认后再改**。验收提醒"未缓存 owner 新增完整前置签名，
旧 cheap 判定也会付成本，先测再判断"。用配对交替基准（两个 worktree、同一夹具、交替 5 轮、每轮 60 次调用、
取中位）实测`_owner_fact_kind`四类路径：

**两组独立配对跑（每组的比值只在本组内成立，跨组数字不可相除）**：

第一组（定位回退，安静 owner / cheap-forced，各 5 轮交替取中位）：

| 修复前(717d6b35) | 我上一版(先签名后判定) | 组内比值 |
|---|---|---|
| 0.9918ms | 2.5024ms | **2.52x（即 +152%）**，绝对值 +1.51ms |

第二组（改造后复测，同一夹具、同一方法）：

| 场景 | 修复前(717d6b35) | 现版本(先判定后签名) | 组内比值 |
|---|---|---|---|
| 安静 owner（cheap 分支） | 0.6735ms | 0.7405ms | 1.10x（+10%，落在单轮波动 0.54–0.88ms 内） |
| 缓存命中（hot） | 0.9436ms | 0.8662ms | 0.92x（−8%） |
| 软事实 owner | 0.9409ms | 0.8947ms | 0.95x（−5%） |
| 强制每次走完整绑定（busy miss） | 1.1813ms | 2.2938ms | 1.94x（+94%，+1.11ms） |

数据确认了验收的预判：**上一版在安静 owner 上回退 2.52x（+152%）**（它在判定之前就付了整份签名，
而这类 owner 恰恰是 `_FACT_MIN_CACHED_SECONDS` 要保护的人群；跨组把 2.5024 与 0.6735 相除得到的 ~3.72x
不是同一组配对数据，不能那样比）。现改为**先探测判定、再决定是否签名**：
安静 owner 回到"一次判定、零签名"（与修复前一致）；只有判定确实贵、值得缓存时才做
"签名 → 判定 → 签名"的快照绑定。代价是贵 owner 的**缓存未命中**路径多付一次判定（1.94x，绝对值约 +1.1ms）；
命中路径不变。这是"正确绑定必须有判定前后的两次签名"的最低代价；不为它引入按 key 记忆的自适应状态
（会反过来让变安静的 owner 重新付签名，风险更大）。测试里三处"首次判定次数"断言按新成本模型更新，
并在用例内注明"探测 + 窗口内判定"两步；A 的三条正确性守卫在旧实现上仍然必红（已逐条验证）。

## 2026-09-13 R272 合法排队的结构化等待信号 + 插话确认终态 + 短屏视图真实终态【状态：已修+受控验收，未部署】

**① 排队等车道不再被误判超时（需求"慢模型长期工作"内的底座缺口）**。R271 发现：请求因车道/配额占满
留在 inbox 期间没有任何写入者，客户端续期没有信号，超过初始窗口（默认 300s）即报等待超时。按既有准入
事实源补齐：worker 在每次扫描被限流（`global_inflight`/`user_inflight`/`conversation_busy`）或恢复退避时，
在请求文件上写结构化等待事实 `admission_wait_at`/`_reason`/`_count`（首次记 `_since`）；
**只写等待事实**，不写 `status`/lease/模型字段，不伪装成已认领或在推进。写入**只做节流（30s），不设
累计等待上限**：健康网关持续证明"仍在合法等待"时，慢模型排队再久也不会因总时长被判失败。
**（R273 更正：本条目第一版曾加"1 小时总预算 + 到点停写并记过期"，验收判定那是把误超时从 5 分钟搬到
1 小时、并制造"前台报失败、后台照常执行"的隐式语义，该配置与过期分支已撤除。）** 客户端**零改动**
（它已按 chunk/inbox/processing 文件活动续期），因此这些写入天然成为"合法存活"证据；收口只来自权威
事实：停写→空闲窗口超时、取消→文件消失→超时、终态→终态记录、崩溃恢复→新网关继续写（真死则停写）。准入判定改为结构化：`try_acquire_report()` 返回原因码
（空串=已获取），`try_acquire()` 是其布尔包装。**15 例**回归，含"真实 worker + 真实客户端"端到端用例、"关掉信号后同一场景必然超时"的负向对照，
以及刚过 1 小时/半天/3 天/30 天**虚拟**累计等待的存活用例（夹具回拨进队时刻，未真机等待；断言继续发信号、
不产生任何等待过期事实）。

**② 已确认插话不得被迟到的"已提交"降级**（验收复现：added→submitted→promoted→submitted 后 pending 变回 1）。
`TuiStateStore` 新增**终态身份集合**（只存 client message_id，有界 1024）：`steer_promoted` 与新增的
`steer_confirmed`（重连/切视角按纯身份补发，由已消费事件驱动）都写入终态；此后同 id 的 `steer_submitted`/
`steer_added`/`steer_removed` 一律忽略并记 `STEER_CONFIRMED_REPLAY` 诊断，不复活等待项、不重复建行。
**6 例**回归，其中 2 例在去掉守卫后必红（已验证）。

**③ 短屏压缩视图与完整视图同源的真实终态**（验收发现：压缩视图 submitted 分支缺 `turn_active`）。
两个视图都按"子代理终态 / 主回合仍在跑 / 主回合已终态"三分支给文案：主回合终态后不再出现
"等待模型回应"、"等待当前回合接收"或"将在下一次工具调用后送入"等假承诺，改为"已送入当前回合但未获
模型确认" / "当前回合已结束；插话未获消费确认"。**4 例**回归（压缩 submitted 终态、压缩 queued 终态、
完整视图 queued 终态、慢流中不倒退）。

**风险边界**：不改用户模型选择、权限、普通 Compact、游标；不动 lease/watchdog 取值与 crash recovery
语义；**未部署、未真机**。真机验收（慢首包/慢流/长工具/子代理等待/插话三态与重连/失败后状态 + TUI
输入滚动复制停止主子切换）仍需部署授权。

## 2026-09-13 R271 慢模型/长任务活性：四层合同先受控验证（发现一处待决策缺口：排队等车道没有续期信号）【状态：已验证+1 处待决策，未改运行时语义】

要求"慢模型也必须支持主代理/子代理长期工作，推进中的请求不得被固定总时限/lease/watchdog/空闲回收误杀"。
先盘点再验证：现有底座在四层已经按"进度驱动"设计，本轮逐层用受控测试钉住（新增
`agent_py_agent/tests/test_slow_model_liveness.py`，11 例全绿）：

- **① 提供方等待**（`_wait_for_generation_result`）：流式后端由传输层持有 **空闲** 超时
  （`stream_timeout_is_idle`），总墙钟守卫必须让位；受控用例证明"结果在总预算之后才到也不判死"，
  非流式仍给出**显式** `ProviderTimeoutError(stage="wall_clock")`，用户停止在传输收口窗口内生效，
  provider HTTP 错误立刻上抛（四类分开）。
- **② 客户端等待**（`poll_gateway_chunks`）：等待期只由机器可观测活动续期（chunk 文件 / inbox /
  processing 请求文件），续期**没有总上限**——慢持续流超过初始 deadline 仍等到终态；静默期只要
  租约心跳在推进就继续等；**完全没有活动**才在空闲窗口后收口（有界，不无限盲等）；窗口后迟到的一次
  触碰不会"复活"已收口的等待。
- **③ 服务端租约心跳**（`start_lease_heartbeat`）：没有任何显示活动时也持续刷新 processing 请求文件
  的租约时间（受控用例断言连续 ≥3 次单调递增），这正是②能区分"在推进"与"已死"的机器事实。
- **④ 长工具租约**（`_invoke_with_lease_renewal`）：handler 阻塞超过初始 lease 时滚动续租（受控用例
  断言 ≥2 次且到期时间单调递增），handler 返回后立刻停止续租，避免另一个请求按 lease 过期抢锁造成
  重复副作用。

**本轮发现但未擅自修改的缺口（需决策）**：请求在 **inbox 排队等前台车道**期间没有任何写入者
（worker 只在被准入限流时重复扫描，不重写队列文件），因此客户端的续期没有信号——超过初始窗口
（默认 `gateway_request_timeout=300s`）就会报"等待超时"，而请求其实还在队列里合法等待。慢模型下
"上一条长回合占着车道、下一条排队"是常态。两种修法各有取舍：①请求文件在 inbox 期间由客户端按
"队列条目仍存在 + Gateway 心跳健康"续期（不改服务端，但违背"进程存活≠请求健康"，网关卡死时
客户端会无限等）；②worker 在准入限流时对排队请求写结构化 `admission_wait_at`（服务端权威，
但要在扫描路径上增加写入）。**未获决策前不改等待语义**，只把事实与两种可选修法记档。

**风险边界**：本轮不动服务端语义、不动 lease/watchdog 取值、不动 crash recovery；真 TUI（慢首包/
慢流/长工具/子代理等待/插话 + 输入滚动复制停止主子切换）必须在部署后验收。
...
- **A `_owner_fact_kind`**：旧顺序"先判定、后签名"→ 判定读到旧状态（none）、签名读到新状态（新 wake 已落盘）
  时缓存出从未同时成立的组合，后续按摘要命中，最长 600s 发现不了新 wake。真实函数受控交错复现：
  第一次 none、第二次仍 none、只现读 1 次。改为"判定前签名 → 判定 → 判定后签名，两次一致才缓存"，
  不一致时有界重判一次（最多一次，不引入无界重试），仍不一致只返回不缓存；新增 `racing`/`unstable` 计数。
  修复后同一探针第一次即 hard、2 次判定、缓存条目与当前磁盘自洽。
- **B waiting 投影命中路径**：`lookup` 释放全局锁后再单独 `move_to_end`，多 owner 共享缓存（上限 4 条、
  文件锁不互斥）时被其它 owner 淘汰即 `KeyError`。改为 `_lookup_waiting_projection()` 同锁完成
  "读条目+核对 stat 键+核对摘要+提级"，并区分 guard 与 miss。纯内存探针（4 条填满 + 命中窗口内第 5 个
  owner 写入触发淘汰）修复前必现 KeyError、修复后正常返回。
- **C `runtime_snapshot`**：文档承诺"解析异常也退回锁内权威读取"，代码却没有 try（异常被外层变成
  `unavailable`）。坏/旧探针 + 有效账本的竞态会误报不可用。已按文档修：探针解析失败退回权威读取，
  只有当前账本本身不可读才 `unavailable`。

**审计**：全仓 `move_to_end` 调用点逐一核对，"lookup 后再单独 touch"的非原子形态只有 waiting 投影一处。
**测试**：新增 7 例（A 4 例：新 wake/新 policy 两种竞态 + 持续改写 + 5+ owner 并发且带"摘要相同⇒结论相同"
不变式；B 2 例：确定性淘汰注入 + 6 owner 并发；C 1 例：坏探针回退）。A 的 3 例、B 的 1 例、C 的 1 例
在对应旧实现上**必红**（已逐个验证）。**未部署、未真机**。
...
> **同日收窄（R261）**：R258 的触发条件已按独立验收 R1 收窄为"①本轮门槛5 重试成功过 **且** ②本 run
> `executed_tools` 非空"（仍零文本匹配，门槛5 四闸门未动）。收窄前"纯问答轮超时被救回 + 完整终答"会被
> 追问一次并**丢掉原终答**（验收用例实测用户看不到"答案是 2。"）；收窄后该场景原样交付、不再多采样。
> 判据权威来源：`executed_tools` 唯一写入点 `_tool_loop_service.py:2256`（执行成功才登记），compact 续跑不归零。
> 未选 `required_actions`（该字段在工具循环契约快照里恒为空，会让续跑死门）。证据：续跑三套测试 34 passed，
> 含"有工具执行仍续跑 1 次、工具副作用仍只 1 次"的真实循环端到端。
...
- `/model` 显式区分会话选择与新会话默认。管理员逐个发布共享引用，默认不共享，不复制密钥。
  共享目录损坏或撤销不锁死私有模型菜单，实际模型调用仍严格核验可用性。
- 同一 channel binding 的首次创建为短原子事务，避免同时开两个窗口分裂成两条 canonical thread。
...
（不动用户会话 ma-port-2）。每条都写清"事实证据 / 根因 / 修改位置 / 验收结果 / 尚未解决项"。

**① 出口把模型正确的绝对路径改短（诊断第 1 条，成立）**：真机审计里模型原文是
`/Users/.../httpie-go-reclone/src/`，界面却显示 `src/`。根因是 `_public_model_text` 等出口对**所有**公开
正文调用 `redact_host_absolute_paths`（本意是外部 IM 通道防泄露宿主拓扑），把绝对路径替换成 basename。
现在新增唯一判定 `channels.project_host_paths_for_channel`：本机私有通道（chat/tui/cli/gateway-cli/
terminal/local）保留可用绝对路径，外部与未知通道照旧脱敏；流式、终稿、交付投影、commentary 落账、
历史回放、投递服务共用同一判定。
**真机验收**：问"用 / 开头的绝对路径告诉我文件在哪"，界面显示 `/private/tmp/ma-eval/verify-r252/hello.txt`
（修前只会显示 `hello.txt`）。
**同族顺带修复**：`redact_executed_operation_labels` 原来会把**裸工具名**（含下划线）换成"相关操作"，
真机界面因此出现"可以用 相关操作 设置"这种读不通的句子；现在只遮蔽 `tool/action` 组合。

**② 插话导致仍在运行的计划面板消失（诊断第 2 条，成立，且比我先前的结论更准）**：客户端
`_handle_task_progress_generation_started` 在**每一条新聊天请求**上 `pop("todo:task_progress")` 并把期望
代次换成新的 request id，服务端按计划账本下发的旧代次快照随后被拒收——面板既被清空、又在那一轮里回不来。
根因是把"聊天轮次"当成"计划代次"。修法：回合开始事件不再清面板、不再改写计划代次；异代次的**空**快照
忽略，异代次**带内容**快照才算真正换计划；服务端 `agent_activity` 收面板的条件从"链接终态"改成
"链接终态**且**没有在跑的子代理"（判据是结构化行状态，不读正文）。
**真机验收**：2 个子代理运行中连插话两次（进度询问 + 任务指令），面板全程存在并实时更新
（完成 0/4 → 2/4 → 4/4）。

**③ 收尾窗口插话（诊断第 3 条）——更正我自己上一轮的错误结论**：我先前用"网关审计 0 命中"推断
"给我绝对路径"没进账本，**这个推断是错的**：审计只记 request/response 配对，不记 steer。查
`guidance/*.jsonl` 与 `guidance_delivered.json` 可见该插话 20:17:25 记录、20:17:39 投递确认，且文本确实
出现在该轮 assistant 的 `canonical_native_messages` 里——它进了模型输入，模型也回答了（带完整绝对路径），
用户看不到纯属 ① 造成的。
**但确实存在一个真洞并在本轮修掉**：`claim_guidance_once_for_turn` 把 `expected_turn_id` 当"只能由这一轮
认领"的硬约束，那一轮若在认领前结束（正是"收尾时插话"的窗口），这条用户消息就永远等不到能认领它的轮次。
现在 Gateway 回合终态（成功/失败同一出口 `_finalize_gateway_response`）调用
`ConversationStore.release_unclaimed_guidance_for_turn`：只给仍为 `pending` 的回执打
`migration.released_turn_ids` 标记（不动 entry metadata——它参与回执/投影一致性与正文指纹），
已 reserved/submitted 的不动，未释放的仍拒绝跨轮认领。
**真机验收**：插话"现在进度咋样了？"21:18:23 记录 → 21:18:32 投递确认 → 界面收到针对该插话的回复。

**⑤ 输出上限截断后的恢复原地打转（诊断第 5 条，成立且有硬数据）**：`WORK_LOG.md` 显示
client-builder 连续 **3** 次、output-system 连续 **4** 次 `max-tokens (MODEL_RESPONSE_TRUNCATED)`，
分别靠管理器重启**整轮 runner** 到第 4/5 个 attempt 才完成（约 35 分钟）。根因是恢复发生在**轮边界**：
截断 → 收口 unfinished → 上层用同样策略重启整轮，什么都没变所以必然重复。
修法：改成**轮内续跑**——截断的最终答复不再直接收口，先回灌一条"从断点接着写、不要道歉不要复述、
把剩余工作拆小"的宿主指令并 `continue`，有界（2 次）后才按原语义收口；指令**不点名任何工具**，
避免"伪造恢复指令"。分块写纠偏与截断续跑分开计数，互不吃预算。
**验收**：3 条合同单测（首次截断续跑 / 超限后 unfinished 收口 / 未截断行为不变）+ 1 条回归
（不得点名工具）。**尚未在真机复现截断**：需要单轮 >16k 输出 token，我构造的"把 1..20000 用逗号写进
正文"被模型以"刷屏无价值"拒绝并改用文件，未触发截断——如实记为未复现。

**④ 模型无法回答/修改自己的配置（诊断第 4 条，成立）**：18:31 用户问"compact 阈值能不能改"，
模型第一次没调任何工具、第二次只调 `gateway_status`，然后答"我没有任何修改 compact 阈值的工具或权限"，
并把**安装目录里随包的** `agent_config.yaml` 当成配置文件。事实是：模型侧确实没有配置入口，
用户配置 `~/.my-agent/config/desktop.yaml` 又在 owner 墙外（默认 workspace-write）。
修法：新增唯一权威 `settings/user_config_capability.py`（白名单 + 区间校验 + 生效时机 + 边界清单）与
`tooling/user_config_tool.py`（main_agent 注册）：`action=view` 给生效值/来源/白名单/边界项及原因，
`action=set` 只写白名单键，原子写用户配置并回读确认，返回"保存到哪 / 什么时候生效 / 现值"。
安全边界（access_mode、path_*、api_key、宿主控制面）**结构性拒绝**并给原因；没有用户配置路径时拒绝写入。
**真机验收**：同问在 r253 上回答"生效值 90%、来自随包默认 <正确路径>、没有用户级配置文件、
可以改（白名单、原子写）、next_session 生效、当前进程不热加载"，且正文里的工具名完整可读。

**操作层教训（本轮又踩一次）**：这个富 TUI 里 **Esc = 停止主代理**。我用"先 Escape 再输入"的旧投递
习惯做插话测试，结果 `reason=conversation_user_stop` 把两个正在跑的子代理取消了。正确做法是
**只在需要清空未提交输入时才按 Esc**；投递新消息直接发文本 + Enter。真实 TUI 验收必须报 tmux 名，
并在结论里区分"产品行为"与"我的操作造成的现象"。
