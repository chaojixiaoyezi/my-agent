...
- **合同**：JSONL 记录边界只有一个——物理 LF。任何"按行读 JSONL"的实现禁止使用 `str.splitlines()`
  （它会在 U+000B/U+000C/U+001C-U+001E/U+0085(NEL)/U+2028/U+2029 处切开），这些字符都可以是合法
  JSON 字符串内容。唯一实现 `common/json_io.py::jsonl_lines()`；读取端禁止自行 `splitlines()`。
- **不允许的"修法"**：过滤/替换这些字符、跳过解析失败的行、把 load_errors 降级为 warning。
  真正的半行/截断/非法 JSON/非对象行必须继续产生结构化 `load_errors`（因为调用方用它判"账本损坏"）。
- **重写路径同样受约束**：读时按 LF 切、写回按 `"\n".join` 的路径（审计清理、task workspace 摘要同步）
  必须用同一边界，否则读时切开、写回把 NEL 落成 LF，等于静默改写用户/任务产物。
- **真实影响**：三个子代理 child 因 `conversation transcript is unreadable` 整体 FAILED；同一批文件
  按 LF 读 0 错误。修复后原文件只读重放：175/162/154 条记录、0 错误（尾部倒读同样 0 错误）。
