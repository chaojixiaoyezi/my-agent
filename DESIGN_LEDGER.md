...
决策设置的宿主非阻塞读取改为无锁读取已提交版本（本地分支 `claude/decision-fault-matrix`，待审）：原先读取也拿排它锁，同一 owner 的并发决策互相挤成 `settings_busy` 静默回退；两份设置文件都是原子替换写、单次写事务只改一个文件，旧建议仍由调用前后版本复核与在途取消挡住。同一分支用本机 HTTP 故障矩阵钉住断网/DNS/TLS/额度/计费/5xx/慢响应的冷却与恢复。详见[接入设计](docs/design/DECISION_MODEL_INTEGRATION.md#42-已实现的可选服务边界)。
...
12.4 第二片 2b 已由主线 owner 审阅合入 main `911d0d14d`，尚未部署：恢复宿主决定摘要后解绑旧请求的完整原生历史（原参数与 frozen，只解绑这一对象，不原地清空）。4.2M 字符下摘要期峰值从 10.2–11.7MB 降到 1.7–3.3MB。建循环时的首次物化峰值仍在；实测每次请求只读一次完整历史，改为按需物化只会挪动峰值、不降峰，所以不做（方案 B 结论，主线 owner 已同意）。Gateway 的 21.3MB 峰值另有来源：索引、近期产物和追加去重三处按行数而不按字节整块读取消息文件，行大时等于整份文件；方案是字节有界的倒读流式行迭代器（只读分析完成，实施待主线 owner 同意），见[容量审计](docs/tasks/DECISION_MODEL_CONTEXT_AUDIT.md#gateway-213mb-峰值来自整块读取消息文件2026-09-24只读分析)。详见[容量审计](docs/tasks/DECISION_MODEL_CONTEXT_AUDIT.md#摘要期释放旧请求历史2b2026-09-24本地)。
...
- **待决策：媒体屏障**（2026-09-24，需用户定方向）：
  - 现状：transcript checkpoint 只覆盖连续前缀，从首个媒体回合起的后缀全部原样保留，所以首张图之后的文字轮永远进不了摘要。会话余量等于窗口减去图片之后的全部内容；越过窗口只能 typed 拒绝（`COMPACT_REQUEST_NON_TEXT`），需要新开会话。
  - 方向一：旧媒体降级为可重新附上的结构化引用。摘要写明"此处有图、按内容地址可重新附上"，原件留在 owner 内容寻址目录，需要时用工具重新附上。不增加模型能力要求，但压缩后丢失视觉细节，需要一个重附工具。
  - 方向二：用能看图的摘要模型，把媒体回合连同原图一起摘要，只有摘要请求真的带了原图，覆盖才成立。保真度高，但依赖摘要模型的视觉能力和图片 token 计量（当前没有）。
  - 附带小项：强制恢复遇媒体时，能否只压媒体之前的文字前缀（媒体后缀原样保留，压缩前后的差值可知）。
