...
- **自学习的 lesson 来源在当前产品里是死路**（2026-09-25，真实验收发现；已由 `record_lesson` 修复并做端到端真实验收，分支 `claude/subagent-lesson-ledger` 待合入）：
...
- **自学习 lesson 的结构化来源 `record_lesson`（第 15 项 P5-C，S1/S2 的上游）**（2026-09-25，已实施并真实验收：分支 `claude/subagent-lesson-ledger`，待合入；端到端真实验收见[真实验收](docs/tasks/DECISION_MODEL_REAL_VALIDATION.md)「15 自学习 S1/S2 端到端真实验收」一节）：
