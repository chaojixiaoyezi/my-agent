...
- **设计（未实施）：动作候选接入需先有插件层通用的"观察候选"结构**（2026-09-25，第 15 项剩余点，依据[动作候选审计](docs/tasks/DECISION_MODEL_ACTION_CANDIDATE_AUDIT.md)；完整设计稿见[插件观察候选结构](docs/design/PLUGIN_OBSERVATION_CANDIDATES.md)；2026-09-25 插件线评审通过，插件线先实施 manifest v5 与代理结果路径，决策线随后接 `action_candidate`）：
