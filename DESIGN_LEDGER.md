...
**可选决策模型：P1—P5 完整实施已授权，P1-A/B 本地合同已验，其余继续。**
...
原模型目录 v3 已区分 decision 与 agentic；v1/v2 显式迁移、默认/子代理/共享用途守卫已接，
原生 Jev 请求、严格 HTTP、有界调用与取消/准入已有组件验证；实际决策服务、开关及设置双入口尚未接通，不能把组件可调用当成产品已可用。
