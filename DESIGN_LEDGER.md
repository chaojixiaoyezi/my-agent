...
第8步模型／工具循环拆分首片本地实现：先分离模型响应采纳与失败恢复，原工具执行、请求运输和Compact权威保持唯一；具体调用顺序、参考阅读范围和独立候选移植边界见 [工具循环职责拆分](docs/design/TOOL_LOOP_DEPENDENCY_SPLIT.md)。模型采纳只绑定五项操作；两层重试共用guidance投递查询。十文件212项通过、24项既有xfail，尚未集成部署，不计第8步完成。
