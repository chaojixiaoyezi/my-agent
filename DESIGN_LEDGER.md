...
第 7 步发现既有 wake／去重回执半写缺口（已复现，待修复）：原信号先写、回执后写，
后者失败可使恢复再次通知。将沿原 dedupe 记录冻结本次发布身份和完整观察／信号负载，
恢复同一发布后才确认交付；通用 pending 合并与完成通知保留 handled 的策略显式区分，
不截断同键 Goal 的后续轮。设计、参考和边界见[发布恢复合同](docs/design/closeout_state_machine.md#唤醒配对发布的半写恢复第-7-步待实现)。
