...
- **Compact 分段请求改为片段在前、累计摘要与规则在后**（2026-09-26，分支 `claude/curator-budget`，隔离真机对比通过，见 TESTS）：
  ...
  - **仍开放**：checkpoint 的 `projected_tokens_before` 是未校准估算（本场景 35,915，实际约 31.3 万），待查；地标预算按新近优先，最早的用户原话可能被挤掉，暂不改。详见 [会话上下文设计](docs/design/CONVERSATION_CONTEXT_DESIGN.md#分段摘要请求的排列顺序2026-09-26集成方分支-claudecurator-budget)。
