...
- 停止、完成与续接只认会话中的持久 task link 和 `workspace_task_id`：`/stop` 中断 active turn，closeout 结束 task lifecycle，但两者都保留当前 workspace。下一轮普通聊天继承目录但不重开任务、不写任务归档；若上一 task 已终态，第一个带 `promotes_task` 的工作工具、`create_subagents` 或 `wait` 会在同一 cwd 上建立当前 request 的新 task id，并记录 `continued_from_task_id`，旧 task 始终保持终态；只有持久 `/goal` 的精确 paused/blocked task 可以原 id 恢复。`task_progress(action=select, task_id=...)` 只用于切换到同一 thread 的另一个精确 workspace/task 候选，`action=start, new_task=true` 才显式创建并切换全新目录。`read/update/select/start` 是严格动作变体，混用在产生 task link 前失败；失败选择不得落入懒晋升。禁止从“继续、接着做、重来”等自然语言推断 task id。该边界直接对照 会话运行时 持久 thread/cwd 与每轮独立 active turn；IM 只负责把结构化 conversation/task/run 关联送入同一状态机。
...
- 主代理普通正文不是完成权威：当本轮已有持久 `task_progress` 且仍有 open item 时，模型的第一版 plain final 只作为可丢弃草稿；运行时参考 终端交互 `TaskUpdateTool` 的 structural verification nudge，把结构化 `open_count` 交给同一模型做一次无工具自然表达，然后沿既有 unfinished continuation 继续。它不解析“完成”等自然语言、不扫描目录、不执行验收、不阻止 task 状态迁移，也不新增完成硬门。
