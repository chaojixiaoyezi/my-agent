...
- 设计中、待用户确认（2026-09-25）：Gateway 安全重启与代理自助重启。用户要求代理能自己重启 Gateway，且所有 TUI 和 IM 不出事。
  ...
  重启窗口里 `/stop`、`/btw` 找不到续跑回合；授权确认框续跑后可能被吞。这些列为同一方案的必修项。详见 [Gateway 安全重启](docs/design/GATEWAY_SAFE_RESTART.md)。
