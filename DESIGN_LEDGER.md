...
- **插件层"观察候选"结构已实施（2026-09-24 晚，main 待部署；manifest v5、代理结果路径、runtime_events 新鲜度、browser-lite），决策线 `action_candidate` 点待接**（原稿 2026-09-25，第 15 项剩余点，依据[动作候选审计](docs/tasks/DECISION_MODEL_ACTION_CANDIDATE_AUDIT.md)；完整设计稿见[插件观察候选结构](docs/design/PLUGIN_OBSERVATION_CANDIDATES.md)；2026-09-25 插件线评审通过，插件线先实施 manifest v5 与代理结果路径，决策线随后接 `action_candidate`）：
