...
def seed_failed_file_child(fixture_root: Path) -> FileRepairSeed:
    ...
    task = manager.create_run(
        ...
        attributes={"required_content_lines": list(_REQUIRED_ORDER_CSV_LINES)},
    )
...
def assert_file_repair_wave_created(fixture_root: Path, seed_run_id: str) -> None:
    ...
    repairs = [task for task in tasks if seed and _is_verified_file_repair(task, seed)]
    if not repairs:
...
# LLM: _is_verified_file_repair requires terminal status plus structured artifact target overlap.
# 函数用途: 判断某个 run 是否覆盖同一个失败 CSV 产物；不再从名称或 goal 里猜“修复”字样。
def _is_verified_file_repair(task: SimpleNamespace, seed: SimpleNamespace) -> bool:
