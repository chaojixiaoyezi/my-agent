...
def build_parser() -> argparse.ArgumentParser:
    ...
    # LLM: Live Lab separates per-model-call timeout from total gateway ask wait budget.
    # 函数用途: CLI 仍只暴露一个简单 timeout，session 层再派生 request/gateway/processing 三类预算。
    parser.add_argument(
        "--timeout",
        type=float,
        default=300.0,
