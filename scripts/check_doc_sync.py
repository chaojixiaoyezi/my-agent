...
class ModuleDocRule:
    """LLM: Describes one module's code paths and the docs that must move with them.
...
COMMENT_SYNC_MARKERS = ("LLM:", "新手说明:", "参数说明:", "返回说明:")
...
def evaluate_sync(changes: list[str], diffs_by_path: dict[str, str]) -> list[str]:
    """LLM: Pure evaluation helper used by tests and by the CLI wrapper.
